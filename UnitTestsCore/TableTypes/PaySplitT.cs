...
namespace UnitTestsCore
{
    public class PaySplitT
    {
        ...
        public static PaySplit CreateOne(long patNum, double splitAmt, long payNum, long provNum, DateTime procDate = default, long procNum = 0,
            ...
            PayPlanDebitTypes payPlanDebitType = PayPlanDebitTypes.Unknown, bool doInsert = true)
        {
            return CreateSplit(clinicNum, patNum, payNum, payPlanNum, procDate, procNum, provNum, splitAmt, unearnedType, adjNum, datePay, payPlanChargeNum, payPlanDebitType, doInsert);
        }
        ...
        public static PaySplit CreatePrepayment(long patNum, int amt, DateTime datePay, long provNum = 0, long clinicNum = 0, long defNumUnearnedType = 0)
        {
            Def defUnearnedType;
            if (defNumUnearnedType == 0)
            {
                defUnearnedType = Defs.GetDefByExactName(DefCat.PaySplitUnearnedType, "PrePayment") ?? DefT.CreateDefinition(DefCat.PaySplitUnearnedType, "PrePayment");
            }
            else
            {
                defUnearnedType = Defs.GetDef(DefCat.PaySplitUnearnedType, defNumUnearnedType);
            }
            ...
            split.UnearnedType = defUnearnedType.DefNum;
