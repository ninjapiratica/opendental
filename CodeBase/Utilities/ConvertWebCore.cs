...
namespace CodeBase
{
    public class ConvertWebCore
    {
        ...
        public static string BytesToString(byte[] asBytes, bool outputAsHex)
        {
            ...
            if (outputAsHex)
            { //Output to legible hex string.
                return BitConverter.ToString(asBytes).Replace("-", "");
            }
            else
            { //Output to UTF8 bytes (0-255).
        ...
        public static byte[] StringToBytes(string asString, bool inputIs7BitAscii)
        {
            ...
            byte[] bytes;
            if (inputIs7BitAscii)
            { //Inputs are ASCII encoded (0-127) and are ledgible.
            ...
            { //Input characters are UTF8 encoded (0-255) and not necessarily ledgible.
    ...
    public class Conv<TypeCur, TypeTo>
    {
        ...
        public static List<TypeTo> L(List<TypeCur> listTypeCur)
        {
            if (listTypeCur == null)
            {
                return null;//Methods need the ability to pass around a null lists.
            }
