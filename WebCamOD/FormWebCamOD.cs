    ...
    public partial class FormProximityOD : Form
    {
        ...
        private void TimerDatabase_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (isProximal == _proxDB && DateTime.Now.Subtract(_dtLastSave).TotalSeconds < 30)
                { //Nothing changed and we just saved within the last 30 seconds. No db update needed.
                    return;
                }
                if (DateTime.Now.Subtract(_dtLastMovement).TotalSeconds > 60)
                { //We can't possibly be proximal if there has been no movement in the past 60 seconds.
                    isProximal = false;
                }
                ...
                if (SetProximity(isProximal))
                { //We saved to db so update the local cache.
                ...
                { //Db comm is not working so slow the interval down to prevent mini DDOS from this program.
        ...
        private void Sensor_RangeChanged(int range)
        {
            ...
            this.Invoke((Action)(() =>
            {
                try
                {
                    ...
                    if (_proxSoftLogic && (range < _rangeOverride))
                    { //currently proximal and should be proximal
                        _isReleasing = false;
                        _releaseTimer.Stop();
                        return;
                    }
                    if (!_proxSoftLogic && (range >= _rangeOverride))
                    { //currently not proximal and should not be
                        _isAcquiring = false;
