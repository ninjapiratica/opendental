...
namespace SlowQueryTool
{
    class QueryLog
    {
        ...
        public Test AnalyzeRowsExamined()
        {
            ...
            if (percent > 20 && ListQueries.Count < 100)
            {//if the percent is high but the data is low
            ...
            {//if over 15% of queries have less than 5000 rows examined
                testResult.ResultText += "\r\nOut of the " + totalQueries + " queries analyzed, "
        ...
        public Test AnalyzeThirdPartyQueries()
        {
            ...
            if (percent > 15)
            {//if over 15% of queries are flagged as likely non-OD
        ...
        public void FillQueries()
        {
            ...
            using (var sr = new StreamReader(FilePath))
            {
                ...
                while (sr.Peek() >= 0)
                {
                    ...
                    if (Math.Ceiling(actualPercentDone) != Math.Ceiling(curPercentDone))
                    { //Throttle to prevent choking out the UI handler.
                    ...
                    if (skipEntry)
                    {
                        if (line.StartsWith("# Time:"))
                        {
                            ...
                            if (!MiscUtils.Between(timeQueryRan, dateTimeLower, dateTimeUpper))
                            { //Not in time range, keep skipping.
                                continue;
                            }
                    ...
                    if (line.StartsWith("# Time:"))
                    {
                        timeQueryRan = ExtractTimeRan(line);
                        if (!MiscUtils.Between(timeQueryRan, dateTimeLower, dateTimeUpper))
                        { //Not in time range, start skipping.
                            skipEntry = true;
                            continue;
                        }
                    ...
                    {//Filters unnecessary # line. Grabs the rest of the formatted queries.
            ...
            {//if both are blank, show last two week only.
        ...
        private void CalculateGrade()
        {
            ...
            if (executionTimeCutOff > 10)
            {//the range of supported cutoffs is between 2 and 10
        ...
        private void CalculateIsVictim()
        {
            ...
            {
                if (query.RowsExamined > 1000 || query.QueryExecutionTime < 3)
                {//not small enough or did not take long enough to be a victim
                    continue;
                }
        ...
        private static void CalculateGroupStatistics(List<QueryGroup> listQueryGroups)
        {
            foreach (QueryGroup queryGroup in listQueryGroups)
            {
                ...
                double totalSpans = 0;
                for (int i = 1; i < queryGroup.ListQueriesInGroup.Count; i++)
    ...
    public class Query
    {
        ...
        public void FormatQuery(string unformattedQuery)
        {
            ...
            if (unformattedQuery.Trim() != "Init DB;")
            {//if this is a part of another query, just remove it. It's not the important slow part.
