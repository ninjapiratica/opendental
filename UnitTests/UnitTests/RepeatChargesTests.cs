...
namespace UnitTests.RepeatCharges_Tests
{
    ...
    public class RepeatChargesTests : TestBase
    {
        ...
        public void RepeatCharges_AddZCodeFromHiddenCategory()
        {
            ...
            try
            {
                proc = RepeatCharges.AddProcForRepeatCharge(repeatCharge, dateTimeNow, dateTimeNow, new OrthoCaseProcLinkingData(repeatCharge.PatNum));
            }
            catch (ODException ex)
            {//Should not happen with Z-codes.
                ex.DoNothing();
            }
        ...
        ///<summary>Does not a D-code that is in a hidden category to a repeating charge.</summary>
        [TestMethod]
        public void RepeatCharges_ErrorAddDCodeFromHiddenCategory()
        {
            //Setup
            long defNum = Defs.GetDefByExactName(DefCat.ProcCodeCats, "Obsolete").DefNum;
            ProcedureCodeT.AddIfNotPresent("D0140", procCat: defNum);
            Patient patient = CreatePatForRepeatCharge("RepeatCharge", 15);
            RepeatCharge repeatCharge = new RepeatCharge();
            repeatCharge.ChargeAmt = 15;
            repeatCharge.PatNum = patient.PatNum;
            repeatCharge.ProcCode = "D0140";
            repeatCharge.IsEnabled = true;
            repeatCharge.DateStart = new DateTime(2021, 08, 26);
            repeatCharge.RepeatChargeNum = RepeatCharges.Insert(repeatCharge);
            ProcedureCode procedureCodeOld = ProcedureCodes.GetProcCode("D0140");
            ProcedureCode procedureCode = ProcedureCodes.GetProcCode("D0140");
            procedureCode.ProcCat = defNum;//Hide D-code that is in repeatCharge.
            ProcedureCodes.Update(procedureCode);
            DateTime dateTimeNow = MiscData.GetNowDateTime();
            //Action
            Procedure proc = null;
            ...
            {//We should have thrown in tried method.
                ex.DoNothing();
            }
            ProcedureCodes.Update(procedureCodeOld);//Reset the proc code
                                                    //Assertion
