...
namespace UnitTests.Claims_Tests
{
    ...
    public class ClaimsTests : TestBase
    {
        ...
        [TestMethod]
        public void Claims_GetAllCreditsGreaterThanProcFees_ProcOverpaid()
        {
            //Build Account
            string suffix = MethodBase.GetCurrentMethod().Name;
            Patient pat = PatientT.CreatePatient(suffix);
            ...
            //Test
            List<string> listResults = Claims.GetAllCreditsGreaterThanProcFees(listClaimProcs);
            Assert.IsTrue(listResults.Count == 1);
        }

        [TestMethod]
        public void Claims_GetAllCreditsGreaterThanProcFees_NoneOverPaid()
        {
            //Build Account
            string suffix = MethodBase.GetCurrentMethod().Name;
            Patient pat = PatientT.CreatePatient(suffix);
        ...
        [TestMethod]
        public void Claims_GetWriteOffsGreaterThanProcFees_ProcOverpaid()
        {
            //Build Account
            string suffix = MethodBase.GetCurrentMethod().Name;
            Patient pat = PatientT.CreatePatient(suffix);
            ...
            listClaimProcs = ClaimProcs.Refresh(pat.PatNum);
            ClaimT.ReceiveClaim(claim, listClaimProcs, true);
            //Create writeoff that covers more than the procedure fee.
            ...
            //Test
            List<string> listResults = Claims.GetWriteOffsGreaterThanProcFees(listClaimProcs);
            Assert.IsTrue(listResults.Count == 1);
        }

        [TestMethod]
        public void Claims_GetWriteOffsGreaterThanProcFees_NoneOverPaid()
        {
            //Build Account
            string suffix = MethodBase.GetCurrentMethod().Name;
            Patient pat = PatientT.CreatePatient(suffix);
            ...
            listClaimProcs = ClaimProcs.Refresh(pat.PatNum);
            ClaimT.ReceiveClaim(claim, listClaimProcs, true);
            //Create writeoff that covers the procedure fee exactly.
            ClaimProc claimProc = listClaimProcs.FirstOrDefault(x => x.ProcNum == proc.ProcNum);
        ...
        [TestMethod]
        public void Claims_GetInitialPrimaryInsGreaterThanProcFees_ProcOverpaid()
        {
            //Build Account
            string suffix = MethodBase.GetCurrentMethod().Name;
            Patient pat = PatientT.CreatePatient(suffix);
            Carrier carrier = CarrierT.CreateCarrier(suffix);
            Carrier carrierSecondary = CarrierT.CreateCarrier(suffix + "_Secondary");
            InsPlan planPrimary = InsPlanT.CreateInsPlan(carrier.CarrierNum);
            InsPlan planSecondary = InsPlanT.CreateInsPlan(carrierSecondary.CarrierNum);
            InsSub subPrimary = InsSubT.CreateInsSub(pat.PatNum, planPrimary.PlanNum);
            InsSub subSecoondary = InsSubT.CreateInsSub(pat.PatNum, planSecondary.PlanNum);
            long subNumPrimary = subPrimary.InsSubNum;
            long subNumSecondary = subSecoondary.InsSubNum;
            PatPlanT.CreatePatPlan(1, pat.PatNum, subNumPrimary);
            PatPlanT.CreatePatPlan(2, pat.PatNum, subNumSecondary);
            ProcedureCode originalProcCode = ProcedureCodeT.CreateProcCode("D2391");
            //Chart procedure for $100.00, with both insurances covering 80%
            long ucrFeeSchedNum = FeeSchedT.CreateFeeSched(FeeScheduleType.Normal, "UCR Fees" + suffix);
            BenefitT.CreatePercentForProc(planPrimary.PlanNum, originalProcCode.CodeNum, 80);
            BenefitT.CreatePercentForProc(planSecondary.PlanNum, originalProcCode.CodeNum, 80);
            FeeT.CreateFee(ucrFeeSchedNum, originalProcCode.CodeNum, 100);
            Procedure proc = ProcedureT.CreateProcedure(pat, originalProcCode.ProcCode, ProcStat.C, "1", 100);//Tooth 1
                                                                                                              //Creates the claim in the same manner as the account module, including estimates.
            Family fam = Patients.GetFamily(pat.PatNum);
            List<InsSub> listInsSubs = InsSubs.RefreshForFam(fam);
            List<InsPlan> listInsPlans = InsPlans.RefreshForSubList(listInsSubs);
            ...
            List<Procedure> listProcs = Procedures.Refresh(pat.PatNum);
            List<ClaimProc> listClaimProcs = ClaimProcs.Refresh(pat.PatNum);
            Claim claimPrimary = ClaimT.CreateClaim("P", listPatPlans, listInsPlans, listClaimProcs, listProcs, pat, listProcs, listBenefits, listInsSubs);
            Claim claimSecondary = ClaimT.CreateClaim("S", listPatPlans, listInsPlans, listClaimProcs, listProcs, pat, listProcs, listBenefits, listInsSubs);
            listClaimProcs = ClaimProcs.Refresh(pat.PatNum);
            ClaimT.ReceiveClaim(claimPrimary, listClaimProcs, true);
            ClaimT.ReceiveClaim(claimSecondary, listClaimProcs, true);
            List<ClaimProc> listClaimProcsPrimary = listClaimProcs.Where(x => x.ClaimNum == claimPrimary.ClaimNum).ToList();
            List<ClaimProc> listClaimProcsSecondary = listClaimProcs.Where(x => x.ClaimNum == claimSecondary.ClaimNum).ToList();
            //Set writeoffs so that both claims slightly over-credit the procedure between payment and writeoff.
            listClaimProcsPrimary[0].WriteOff = 20.01;
            listClaimProcsSecondary[0].WriteOff = 20.01;
            ClaimProcs.Update(listClaimProcsPrimary[0]);
            ClaimProcs.Update(listClaimProcsSecondary[0]);
            //Test
            List<string> listResultsPrimary = Claims.GetInitialPrimaryInsGreaterThanProcFees(listClaimProcsPrimary);
            List<string> listResultsSecondary = Claims.GetInitialPrimaryInsGreaterThanProcFees(listClaimProcsSecondary);
            //The method only cares about write-offs and payments from initial primary insurance payments,
            //so a description for the procedure being over-credited will exist for primary claimprocs, but not for secondary claimprocs.
            Assert.IsTrue(listResultsPrimary.Count == 1);
            Assert.IsTrue(listResultsSecondary.Count == 0);
        }

        [TestMethod]
        public void Claims_GetInitialPrimaryInsGreaterThanProcFees_NoneOverpaid()
        {
            //Build Account
            string suffix = MethodBase.GetCurrentMethod().Name;
            Patient pat = PatientT.CreatePatient(suffix);
            ...
            List<ClaimProc> listClaimProcsPrimary = listClaimProcs.Where(x => x.ClaimNum == claimPrimary.ClaimNum).ToList();
            List<ClaimProc> listClaimProcsSecondary = listClaimProcs.Where(x => x.ClaimNum == claimSecondary.ClaimNum).ToList();
            //Set writeoffs so that only secondary over-credits the procedure between its payment and writeoff.
            listClaimProcsPrimary[0].WriteOff = 20.00;
            listClaimProcsSecondary[0].WriteOff = 20.01;
            ClaimProcs.Update(listClaimProcsPrimary[0]);
            ClaimProcs.Update(listClaimProcsSecondary[0]);
            //Test
            ...
            List<string> listResultsSecondary = Claims.GetInitialPrimaryInsGreaterThanProcFees(listClaimProcsSecondary);
            //The method only cares about write-offs and payments from initial primary insurance payments,
            //so no description will exist for primary or secondary claimprocs.
            Assert.IsTrue(listResultsPrimary.Count == 0);
            Assert.IsTrue(listResultsSecondary.Count == 0);
        }
    }
}
