    ...
    public class ParamCheckTests : TestBase
    {
        ...
        public void MiddleTier_ReturnAfterVoid()
        {
            ...
            List<MethodDefinition> listAllSClassMethods = asm.Modules
                .SelectMany(x => x.Types)
                .Where(x => listSClassNames.Contains(x.Name.ToLower()))
                .SelectMany(x => x.Methods)
        ...
        ///nature have no MiddleTierRole check. Then, it looks at all references to these methods. It will trace its way up each potential
        ///call stack until it reaches a public static method. If it has a MiddleTierRole check, on to the next one. Otherwise, there is a path the
        ///code could take to reach methods that are not behind a MiddleTierRole check. Other tests ensure public methods have MiddleTierRole checks
        ///in the case of database calls directly in that method. Also, other tests ensure that private static methods do not have an erroneous MiddleTierRole checks.</summary>
        [TestMethod]
