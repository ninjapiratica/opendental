    ...
    public class D3Helper
    {
        public static D3ObjectGroup ImportObjectsFromObjFile(string strObjData)
        {
            ...
            while (i < lines.Length)
            {
                ...
                {//vertex.  For example "v -0.34671119 0.22176000 0.83703486"
                    items = lines[i].Split(' ');
                    vertex = new Vertex3f();
                ...
                {//vertex.  For example "vt 0.72553915 0.63685900"
                    items = lines[i].Split(' ');
                ...
                {//normal.  For example "vn -0.32559605 -0.84121753 -0.43167149"
                    items = lines[i].Split(' ');
                    normal = new Vertex3f();
                ...
                {//group.  For example "g cylinder2_default"
                    if (d3Object != null)
                    {
                ...
                {//face.  For example "f 12//384 51//443 384//336 383//335".  Format is v//n
                    items = lines[i].Split(' ');//5 items in this example
                                                //create triangle fan, preserving counterclockwise order
                    for (int f = 2; f < items.Length - 1; f++)
                    {//one face/triangle per loop, 2 triangles in this case
                        face = new Face();
