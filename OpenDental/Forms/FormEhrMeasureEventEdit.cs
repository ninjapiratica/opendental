...
namespace OpenDental
{
    ...
    public partial class FormEhrMeasureEventEdit : FormODBase
    {
        ...
        public void CalcTobaccoDuration()
        {
            ...
            {//startdate anniversary was in a previous
                months = DateTime.Now.Month - startDate.Month;
                if (DateTime.Now.Day > startDate.Day)
                {//start month is before current month, and start day is before current day, don't count current month
                    months--;
                }
            }
            else if (startDate.Month == DateTime.Now.Month)
            {//startdate anniversary this month
                if (startDate.Day > DateTime.Now.Day)
                {//start date anniversary hasn't happened this month, subtract a year and months=11
                    years--;
                    months = 11;
                }
            }
            else
            {//startdate anniversary later in the year
                years--;
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            {//should never happen, only updates happen here
