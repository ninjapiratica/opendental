            ...
            string amountMin, string amountMax, string controlId, bool doShowAutomatableCarriersOnly)
        {
            ...
            progressOD.ActionMain = () =>
            {
                ...
                if (showStatusAndClinics)
                {
                    ...
                    {//Add the selected statuses to the list.
                        listX835Statuses.Add(_listX835Statuses[listStatus.SelectedIndices[i]]);
                    }
                    if (listX835Statuses.Contains(X835Status.Finalized))
                    {//Our list in the UI only allows the user to select "Finalized" thus we include all 3 finalized statuses.
