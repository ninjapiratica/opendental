...
namespace OpenDental
{
    ...
    public partial class FormPayPlanCredits : FormODBase
    {
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < _listPayPlanEntries.Count; i++)
        ...
        private List<PayPlanEdit.PayPlanEntry> CreatePayPlanEntries(bool showImplicitlyPaidOffProcs)
        {
            ...
            if (ListPayPlanChargesCredit.Exists(x => x.ProcNum == 0))
            {//only add "Unattached" if there is a credit without a procedure. 
        ...
        private void SetTextBoxes()
        {
            ...
            else if (listPayPlanEntriesSelected.Count == 1)
            { //if there is one entry selected
                ...
                { //if it's a PayPlanCharge
                  //button should say Update, text boxes should fill with info from that charge.
                    ...
                    if (payPlanEntrySelected.ProcStatOrd == ProcStat.TP && payPlanEntrySelected.ProcNumOrd != 0)
                    {//if tp, grey out the date textbox. it should always be maxvalue.
                    ...
                    { //selected charge could be unattached.
                ...
                {// selected line item is a procedure (or the "Unattached" entry)
                 //button should say "Add", text boxes should fill with info from that procedure (or "unattached").
                    ...
                    { //if it is a procedure (and not the "unattached" row)
                        ...
                        if (listPayPlanEntriesForProc.Count == 0)
                        { //if there are no other charges attached to the procedure
                            textAmt.Text = payPlanEntrySelected.RemBefStr; //set textAmt to the value in RemBefore
                        }
                        else
                        {//if there are other charges attached, fill the amount textbox with the minimum value in the RemAftr column.
            ...
            { //if they selected multiple line items
        ...
        {
            List<PayPlanEdit.PayPlanEntry> listPayPlanEntriesSelected = gridMain.SelectedTags<PayPlanEdit.PayPlanEntry>();
            if (listPayPlanEntriesSelected.Count <= 1)
            { //validation (doesn't matter if multiple are selected)
            ...
            { //if they have more than one entry selected
              //remove everythig that doesn't have a procnum from the list
                ...
                if (listPayPlanEntriesSelected.Count == 0)
                { //if the list is then empty, there's nothing to do.
