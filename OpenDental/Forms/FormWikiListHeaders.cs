...
namespace OpenDental
{
    public partial class FormWikiListHeaders : FormODBase
    {
        ...
        {
            if (_gridCurCell.Y > 0 && _gridCurCell.Y < _listTableHeaders.Count)
            {//no pick list option for the PK col
                _listTableHeaders[_gridCurCell.Y].PickList = string.Join("\r\n", _listStringPick);
            }
            #region Validation
            List<string> listColNames = new List<string>();
            for (int i = 1; i < gridMain.ListGridRows.Count; i++)
            {//start with index 1, first col is PK
                ...
                if (i == 0)
                {//don't check PK column name, since it can't be changed
                    continue;
                }
            ...
            {
                if (i > 0)
                {//don't allow renaming the first column, it's the PK
