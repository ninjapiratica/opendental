...
namespace OpenDental
{
    public partial class FormHelpBrowser : FormODBase
    {
        ...
        private FormFaqPicker _formFaqPicker;
        ...
        public static FormHelpBrowser GetFormHelpBrowser(bool enableUI)
        {
            ...
            if (!enableUI)
            {
                ...
                ((Control)_formHelpBrowser.webBrowserManual).Enabled = false;
                ((Control)_formHelpBrowser.webBrowserFAQ).Enabled = false;
                _formHelpBrowser.menuStrip1.Enabled = false;
            }
        ...
        public static void InitializeIfNull()
        {
            ...
            FormHelpBrowser formHelpBrowser = GetFormHelpBrowser(enableUI: true);
        ...
        private string ManualUrlToFaqUrl(string manualPageUrl)
        {
            ...
            if (urlParams[0] == "manual")
            {//Jordan uses "manual" to signify the stable version
                version = Faqs.GetStableManualVersion().ToString();
            }
            else
            {//Jordan appends the version to the manual page if it is not the stable version, for example '/manual183/'
        ...
        private bool IsManualPageUrl(string url)
        {
            ...
            if (url.EndsWith("searchmanual.html"))
            {//The user is most likely to hit 'search' from the help browser
                return false;
            }
            return true;
        }
        ...
        {
            _formFaqPicker.Show();
            if (_formFaqPicker.WindowState == FormWindowState.Minimized)
            {
                _formFaqPicker.WindowState = FormWindowState.Normal;
            }
            _formFaqPicker.BringToFront();
        }

        private void ManageToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (_formFaqEdit != null && !_formFaqEdit.IsDisposed)
            {//FormFaqEdit currently opened.
            ...
            if (_formFaqPicker == null || _formFaqPicker.IsDisposed)
            {
                _formFaqPicker = new FormFaqPicker(url, programVersion);
            }
            ShowFormFaqPicker();
        }
        ...
        private string FormatVersionNumber(string url)
        {
            ...
            {//We're on a manual page for stable so no version number will be present. In that case, grab the latest stable from the mp database.
        ...
        {
            if (!webBrowserManual.CanGoBack)
            {//If nothing to navigate back to in history, do nothing
                return;
            }
        ...
        {
            if (!webBrowserManual.CanGoForward)
            {//If nothing to navigate forward to in history, do nothing
                return;
            }
            Cursor = Cursors.WaitCursor;//Is set back to default cursor after the document loads inside the browser.
            Application.DoEvents();//To show cursor change.
        ...
        {
            using FormFaqVersionRelease formFaqVersionRelease = new FormFaqVersionRelease();
            formFaqVersionRelease.ShowDialog();
        }
        ...
        {
            if (_formFaqPicker != null && !_formFaqPicker.IsDisposed)
            {//FormFaqPicker currently opened.
            ...
            if (_formFaqEdit == null || _formFaqEdit.IsDisposed)
            {
                ...
                _formFaqEdit.FaqCur = new Faq();
                _formFaqEdit.FaqCur.IsNew = true;
                _formFaqEdit.IsQuickAdd = true;
                _formFaqEdit.ManualPage = GetPageTopicFromUrl(webBrowserManual.Url.ToString());
                _formFaqEdit.Version = FormatVersionNumber(webBrowserManual.Url.ToString());
            }
            else
            {
