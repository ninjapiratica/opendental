...
namespace OpenDental
{
    public partial class FormReplicationEdit : FormODBase
    {
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            else if (ReplicationServerCur.ReplicationServerNum == PrefC.GetLong(PrefName.ReplicationUserQueryServer))
            {//If this replication server was the original report server, set the current server to 0.
                if (Prefs.UpdateLong(PrefName.ReplicationUserQueryServer, 0))
                {
                    DataValid.SetInvalid(InvalidType.Prefs);
                }
