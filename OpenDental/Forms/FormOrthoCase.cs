    ...
    public partial class FormOrthoCase : FormODBase
    {
        ...
        private void FormOrthoCase_Load(object sender, EventArgs e)
        {
            ...
            if (!_orthoCase.IsTransfer)
            {
                ...
                if (_procedureBanding.ProcNum == 0)
                {//Set to null if GetOneProc returned a blank proc.
        ...
        private void SetExistingOrthoCaseControls()
        {
            ...
            if (_orthoSchedule.VisitAmount == 0)
            {//Set visits planned to zero if we divided by 0 above.
        ...
        private void CheckIsTransfer_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                labelBandingDate.Text = Lans.g(this, "Banding Date");
                if (_doClearAndDisableTextFields)
                {//Only want to clear text fields and disable them if we have unchecked this without adding a banding proc.
                    ClearTextFields();
                    DisableTextFields();
                }
            }
            ...
            , ValidDouble validDoubleSecondLinkedField = null)
        {
            ...
            if (CompareDecimal.IsGreaterThanOrEqualToZero(changedFieldNumber) && changedFieldNumber <= changedFieldLimit)
            {
                switch (fieldTypeLinked)
                {
                    case FieldType.Fee:
                        if (!_isShowingSecondary)
                        {//Can only auto-calculate a linked Fee field when we are working with one insurance plan.
        ...
        private void EnableTextFields()
        {
            ...
            if (!checkIsTransfer.Checked)
            {//If this isn't a transfer, enable banding fields.
        ...
        private void ClearSubordinateFields(object sender)
        {
            ...
            if (sender == textTotalFee)
            {
                textPrimaryInsuranceFee.Text = "";
                if (_isShowingSecondary)
                {//Keep the 0.00 for future calculations
        ...
        {
            if (gridOrthoSchedule.SelectedGridRows.Where(x => x.Tag != null).ToList().Count <= 0)
            {//Return if no selected rows have an attached procedure.
                return;
            }
        ...
        {
            if (_procedureDebond != null && _procedureDebond.ProcNum == procNum)
            {//detching debond
                _procedureDebond = null;
        ...
        private void ButPatPayPlan_Click(object sender, EventArgs e)
        {
            ...
            else if (_payPlanPatient == null && _orthoPlanLinkPatPayPlan != null)
            {//Get from link if we have not opened pay plan yet.
                _payPlanPatient = PayPlans.GetOne(_orthoPlanLinkPatPayPlan.FKey);
            }
            if (_payPlanPatient == null)
            {//If pay plan is null it was never created or it was deleted since the window opened.
            ...
            else
            {
                ...
                if (_payPlanPatient == null)
                {//In case pay plan was deleted from form.
                    _orthoPlanLinkPatPayPlan = null;
                }
            }
        }
        ...
        private void UpdateOrthoCase()
        {
            ...
            if (checkIsTransfer.Checked && _orthoProcLinkBanding != null)
            {//OrthoCase has been changed to a transfer. Delete banding proc link if it exists.
                OrthoProcLinks.Delete(_orthoProcLinkBanding.OrthoProcLinkNum);
            }
            else if (!checkIsTransfer.Checked)
            {
                if (_orthoProcLinkBanding == null)
                {//OrthoCase has been changed to a non-transfer. Link banding proc.
                ...
                {//OrthoCase is still a non-transfer, but banding proc linked may have changed so update.
                ...
                if (_procedureBanding != null && _procedureBanding.AptNum != 0)
                {//Banding proc may have changed, so check to see if it is scheduled and update bandingDate.
        ...
        private void ButOK_Click(object sender, EventArgs e)
        {
            ...
            if (_isNew)
            {
                ...
                if (_procedureBanding != null && _procedureBanding.AptNum != 0)
                {//If banding is scheduled save the appointment date instead.
