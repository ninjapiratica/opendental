...
namespace OpenDental
{
    public partial class FormEmailInbox : FormODBase
    {
        ...
        private void FormEmailInbox_Resize(object sender, EventArgs e)
        {
            ...
            if (!comboEmailAddress.Items.GetAll<EmailAddress>().IsNullOrEmpty())
            {//Since Resize() is called once before Load(), we must ensure FillComboEmail() has been called at least once.
                FillInboxOrSent();
            }
        }
        ...
        private void FillComboEmail()
        {
            ...
            {//Only providers have access to see Webmail messages.
        ...
        private void GetMessages()
        {
            ...
            if (GetSelectedAddress().EmailUsername == "" || GetSelectedAddress().Pop3ServerIncoming == "")
        ...
        private void FillGridInbox()
        {
            ...
            if (_isSearching)
            { //if searching, use the search list. Should already be filled.
            ...
            { //collapse the panel if they don't have exactly one email selected.
        ...
        private void FillGridSent()
        {
            ...
            if (_isSearching)
            { //if searching, use the search list. Should be prefilled.
            ...
            {
                if (!checkShowHiddenEmails.Checked && listEmailsFiltered[i].HideIn.HasFlag(HideInFlags.EmailInbox))
                {//We might need a separate HideInFlags.EmailSent option later.
                    continue;
                }
                if (IsWebMail())
                {
        ...
        private void RefreshShowIn()
        {
            ...
            if (GetActiveGrid().SelectedGridRows.Count != 0)
            { //at least one selected email
        ...
        {
            SplitContainerNoFlicker splitContainerNoFlickerActive = (GetActiveMailbox() == MailboxType.Inbox) ? splitContainerNoFlicker : splitContainerSent;
            if (splitContainerNoFlickerActive.Focused)
            {
                //We only want to resize when the user has moved the split container.
                Console.WriteLine($"{splitContainerNoFlickerActive.Name} {splitContainerNoFlickerActive.Focused}");
                LayoutManager.LayoutControlBoundsAndFonts(splitContainerNoFlickerActive);
            }
        }
        ...
        {
            SplitContainerNoFlicker splitContainerNoFlickerActive = (GetActiveMailbox() == MailboxType.Inbox) ? splitContainerNoFlicker : splitContainerSent;
            EmailPreviewControl emailPreviewControlActive = (GetActiveMailbox() == MailboxType.Inbox) ? emailPreview : emailPreviewControl1;
            SetButtonsEnabled();
            RefreshShowIn();
            if (GetActiveGrid().SelectedIndices.Length >= 2)
            {
                splitContainerNoFlickerActive.Panel2Collapsed = true;//Do not show preview if there are more than one emails selected.
                LayoutManager.LayoutControlBoundsAndFonts(splitContainerNoFlickerActive);
                return;
            }
            ...
            if (EmailMessages.IsSecureWebMail(emailMessage.SentOrReceived))
            {
                ...
                splitContainerNoFlickerActive.Panel2Collapsed = true;
                LayoutManager.LayoutControlBoundsAndFonts(splitContainerNoFlickerActive);
                return;
            }
            Cursor = Cursors.WaitCursor;
            ...
            splitContainerNoFlickerActive.Panel2Collapsed = false;
            LayoutManager.LayoutControlBoundsAndFonts(splitContainerNoFlickerActive);
        ...
        private void butRefresh_Click(object sender, EventArgs e)
        {
            ...
            if (GetActiveMailbox() == MailboxType.Inbox)
            {
                try
                {
                    ...
                    {//Insert a signal that will cause the Open Dental Service to retrieve emails when using the inbox feature.
        ...
        private void tabControl1_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            System.Windows.Forms.TabPage tabPage = tabControl1.TabPages[e.Index];
            tabPage.Focus();
            Rectangle rectangleBounds = tabControl1.GetTabRect(e.Index);
            using Image image = tabControl1.ImageList.Images[e.Index];
            Rectangle rectangleImage = new Rectangle(rectangleBounds.X + 6, rectangleBounds.Y + 10, image.Width, image.Height);
