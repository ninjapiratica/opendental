...
namespace OpenDental
{
    public partial class FormDefinitionPicker : FormODBase
    {
        ...
        private void FillListDefs(DefCat defCat, long defNumExclude)
        {
            ...
            if (defExclude == null)
            {//either defNum is 0 or it is an invalid DefNum, either way nothing to exclude
                return;
            }
            ...
            for (int i = 0; i < listInvalidNums.Count; i++)
            {
                ...
                List<long> listDefNums = _listDefs.FindAll(x => x.ItemValue == strDefNum).Select(x => x.DefNum).ToList();//children
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < _listDefs.Count; i++)
            {
                ...
                if (ListDefsSelected.Any(x => _listDefs[i].DefNum == x.DefNum))
                {//after adding row, see if it should be reselected after grid update
