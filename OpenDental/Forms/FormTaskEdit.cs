...
namespace OpenDental
{
    ...
    public partial class FormTaskEdit : FormODBase
    {
        ...
        private void LoadTask()
        {
            ...
            {
                if (!IsNew)
                {//to simplify the code, only allow jobs to be attached to existing tasks at HQ, not new tasks.
            ...
            else
            {
                ...
                Width = splitContainerDescriptNote.Right + 25;
            }
            FillTextAttachments();
            if (IsNew)
            {
            ...
            {//trying to edit an existing task, so need to block some things
                ...
                if (TaskCur.TaskListNum != Security.CurUser.TaskListInBox)
                ...
                {//need to block them if they don't have TaskDelete permission
            ...
            if (_defNumPrioritySelected == 0)
            {//The task does not yet have a priority assigned.  Find the default and assign it, if available.
                for (int i = 0; i < _listDefsTaskPriorities.Count; i++)
                {
            ...
            for (int i = 0; i < _listDefsTaskPriorities.Count; i++)
            {//Add non-hidden defs first
                comboTaskPriorities.Items.Add(_listDefsTaskPriorities[i].ItemName);
                if (_defNumPrioritySelected == _listDefsTaskPriorities[i].DefNum)
                {//Use priority listed within the database.
                    comboTaskPriorities.SelectedIndex = i;//Sets combo text too
                }
            }
            if (_defNumPrioritySelected == 0 && !hasDefault)
            {//If no default has been set in the definitions, select the last item in the list.
            ...
            if (_taskList != null && IsNew && _taskList.TaskListNum == 1697 && PrefC.GetBool(PrefName.DockPhonePanelShow))
            {//Set to triage blue if HQ, triage list, and is new.
                for (int i = 0; i < _listDefsTaskPriorities.Count; i++)
                {
                    if (_listDefsTaskPriorities[i].DefNum == _triageNumBlue)
                    {//Finding the option that is triageBlue to select it in the combobox (Combobox mirrors _listTaskPriorityDefs)
            ...
            if (comboTaskPriorities.SelectedIndex == -1)
            {//Priority for task wasn't found in the non-hidden priorities list (and isn't triageBlue), so it must be a hidden priority.
                ...
                {
                    if (listDefsCategories[i].DefNum == _defNumPrioritySelected)
                    {//We find the hidden priority and set the text of the combo box.
            ...
            else if (_listTaskNotes.Count == 0 && TaskCur.UserNum == Security.CurUser.UserNum)
            {//if this is my task
                labelReply.Visible = false;
                butReply.Visible = false;
            }
            ...
            {//Show red and blue buttons for HQ always
                butRed.Visible = true;
            ...
            _patNum = 0;
            if (_taskOld.ObjectType == TaskObjectType.Patient && _taskOld.KeyNum > 0)
            {//Patient is attached.
        ...
        private void SetTaskStartingLocation()
        {
            ...
            if (listFormTaskEdits.Count <= 1)
            {
                ...
                { //Should never be null.
                    ...
                    if (screenArray.Any(x => x.WorkingArea.Contains(pointStart)))
                    {//Make sure the task would actually be visible in the manual location.
        ...
        private void FillGrid()
        {
            ...
            {
                if (_numNotes == -1)
                {//Only fill _numNotes here the first time FillGrid is called.  This is used for coloring triage tasks.
                    _numNotes = _listTaskNotes.Count;
                }
                if (_defNumPrioritySelected == _triageNumBlue && _numNotes == 0 && _listTaskNotes.Count != 0)
                {//Blue triage task with an added note
                ...
                else if (_defNumPrioritySelected == _triageNumWhite && _numNotes != 0 && _listTaskNotes.Count == 0)
                {//White triage task with note that has been deleted, change it back to blue.
                    _defNumPrioritySelected = _triageNumBlue;
                    for (int i = 0; i < _listDefsTaskPriorities.Count; i++)
                    {
                        if (_listDefsTaskPriorities[i].DefNum == _triageNumBlue)
        ...
        {
            if (checkIsReadOnly.Checked)
            {   //If read only, dont open formTaskNoteEdit
                return;
            }
            if (_isTaskDeleted)
            {
        ...
        private void FormTaskNoteEdit_FormClosed_DoubleClick(object sender, EventArgs e)
        {
            ...
            {//If task was marked Done when opened, we uncheck the Done checkbox so people can see the changes.
                checkDone.Checked = false;
            }
            FillGrid();
        ...
        private void FormTaskNoteEdit_FormClosed_Edit(object sender, EventArgs e)
        {
            ...
            {//'new' box is checked
                checkNew.Checked = false;
                _hasStatusChanged = true;
                _isNotRead = false;//so that the automation won't happen again
            }
            Signalods.SetInvalid(InvalidType.TaskPopup, KeyType.Task, TaskCur.TaskNum);//popup
            TaskUnreads.AddUnreads(TaskCur, Security.CurUser.UserNum);//we also need to tell the database about all the users with unread tasks
            SendSignalsRefillLocal(TaskCur);
        ...
        private void checkNew_Click(object sender, EventArgs e)
        {
            ...
            _hasStatusChanged = true;
            _isNotRead = false;//don't override user's intent
        }
        ...
        {
            if (_defNumPrioritySelected == _triageNumBlue)
            {//Blue button is clicked while it's already blue
        ...
        {
            if (_defNumPrioritySelected == _triageNumRed)
            {//Red button is clicked while it's already red
                _defNumPrioritySelected = _triageNumWhite;//Change to white.
                for (int i = 0; i < _listDefsTaskPriorities.Count; i++)
                {
                    if (_listDefsTaskPriorities[i].DefNum == _triageNumWhite)
        ...
        private bool SaveCur()
        {
            ...
            TaskReminderType taskReminderType = _listTaskReminderTypes[comboReminderRepeat.SelectedIndex];
            if (textDateTimeEntry.Text != "" || taskReminderType != TaskReminderType.NoReminder)
            {//Reminders always require a DateTimeEntry
                try
                {
            ...
            if (taskReminderType != TaskReminderType.NoReminder && !PrefC.GetBool(PrefName.TasksUseRepeating))
            {//Is a reminder and not using legacy task system
                if (taskReminderType != TaskReminderType.Once &&
                ...
                {//Make a new ID if it's blank no matter what.  Could be an old task being changed.
            ...
            else
            {
                ...
                if (taskReminderType != TaskReminderType.NoReminder && IsNew && DateTime.Now > TaskCur.DateTimeEntry)
                { //New Reminder Task.
            ...
            {//because it can't be both new and done.
                if (PrefC.GetBool(PrefName.TasksNewTrackedByUser))
                {
                    ...
                    if (Tasks.IsReminderTask(TaskCur) && TaskCur.DateTimeEntry > DateTime.Now)
                    {//Future Reminder.
                        if (!TaskCur.IsUnread)
                        {
                    ...
                    {
                        if (_isNotRead && TaskCur.IsUnread)
                        {//This user opened the task, therefore, unless they manually changed checkNew, it's now read.
                            checkNew.Checked = false;
                            _hasStatusChanged = true;
                        }
                    ...
                    {//Just in case, checkbox should not be enabled or checked when _startedInOthersInbox is true.
        ...
        private void butReply_Click(object sender, EventArgs e)
        {
            ...
            if (!DidNotesChange && textDescript.Text == TaskCur.Descript)
            {//nothing changed
                _formTaskNoteEdit = new FormTaskNoteEdit();
                _formTaskNoteEdit.TaskNoteCur = new TaskNote();
                _formTaskNoteEdit.TaskNoteCur.TaskNum = TaskCur.TaskNum;
        ...
        private bool SaveCopy(List<long> listTaskListNums)
        {
            for (int i = 0; i < listTaskListNums.Count; i++)
            {
                ...
                if (taskCopy.ReminderType != TaskReminderType.NoReminder && !PrefC.GetBool(PrefName.TasksUseRepeating))
                {//Make a new ID if it's blank no matter what.  Could be an old task being changed.
                ...
                if (taskCopy.TaskListNum == Userods.GetInbox(Security.CurUser.UserNum))
                {//My inbox.
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (TaskCur.Equals(_taskOld) && !_hasStatusChanged)
        ...
        private void SendSignalsRefillLocal(Task task, long taskListNumOld = -1, bool canKeepTask = true)
        {
            ...
            if (_userNumFrom != task.UserNum)
            {//From User has changed.
                if (_userNumFrom != 0)
                {//Should never be 0, but might be 0 in much older databases?
        ...
        private void FormTaskEdit_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (DialogResult == DialogResult.None && (_formTaskNoteEdit != null && !_formTaskNoteEdit.IsDisposed))
            ...
            else if (DidNotesChange)
            {//Note changed and dialogue result was not OK
            ...
            if (checkDone.Checked)
            {//Will happen when the Done checkbox has been manually re-checked by the user or refreshing the task checked the box.
                return;
            }
            TaskCur.TaskStatus = TaskStatusEnum.Viewed;
            try
