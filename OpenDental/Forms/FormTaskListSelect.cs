    ...
    public partial class FormTaskListSelect : FormODBase
    {
        ...
        private List<TaskList> GetListUser()
        {
            ...
            if (_taskListUserInbox != null)
            {
                ...
                else if (listTaskListsRetVal.Any(x => x.TaskListNum == Security.CurUser.TaskListInBox))
                {//Show "My Inbox:" shortcut if current users inbox is in filtered list.
                    listTaskListsRetVal.Insert(0, _taskListUserInbox);
                }
            }
        ...
        {
            if (checkMulti.Checked && PrefC.IsODHQ)
            {//If user is in checkMulti state, deselect the list
