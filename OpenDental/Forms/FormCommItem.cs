    ...
    public partial class FormCommItem : FormODBase
    {
        ...
        private void FormCommItem_Load(object sender, EventArgs e)
        {
            ...
            if (PrefC.IsODHQ)
            {
                ...
                if (!_commlog.IsNew && !IsHistoric)
                {//Commlog must exist in the db in order to have history.
        ...
        private bool IsValid(bool showMsg)
        {
            ...
            if (DoOmitDefaults)
            { //Only validate if in enterprise mode
                if (listType.SelectedIndex == -1)
                {
        ...
        private bool SaveCommItem(bool showMsg)
        {
            ...
            if (IsPersistent && string.IsNullOrWhiteSpace(_commlog.Note))
        ...
        private void timerAutoSave_Tick(object sender, EventArgs e)
        {
            ...
            if (_commlogOld.Compare(_commlog))
            {//They're equal, don't bother saving
                return;
            }
