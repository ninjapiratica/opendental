...
namespace OpenDental
{
    ...
    public partial class FormScreenEdit : FormODBase
    {
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (!_isValid)
            {//If validation failed and they still want to continue, do Cancel instead.
                DialogResult = DialogResult.Cancel;
                return;
            }
            ...
            if (textBirthdate.Text != "" && textAge.Text == "")
            {//Birthdate is present but age isn't entered, calculate it.
                ScreenCur.Age = PIn.Byte(Patients.DateToAge(PIn.DateT(textBirthdate.Text)).ToString());
            }
            else if (textAge.Text != "")
            {//Age was manually entered, use it.
                ScreenCur.Age = PIn.Byte(textAge.Text);
            }
            else
            {//No age information was entered at all.
                ScreenCur.Age = 0;
            }
