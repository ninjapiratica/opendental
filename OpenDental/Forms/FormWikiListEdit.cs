...
namespace OpenDental
{
    public partial class FormWikiListEdit : FormODBase
    {
        ...
        private void FillGrid()
        {
            ...
            if (_table.Rows.Count > 0 && _listColumnHeaders.Count != _table.Columns.Count)
            {//if these do not match, something happened to be desynched at the right moment.
                WikiListHeaderWidths.RefreshCache();
                _table = WikiLists.GetByName(WikiListCurName);
                _listColumnHeaders = WikiListHeaderWidths.GetForList(WikiListCurName);
                if (_listColumnHeaders.Count != _table.Columns.Count)
                {//if they still do not match, one of them did not get synched correctly.
            ...
            {
                if (checkBoxIncludeHiddenColumns.Checked || !_listColumnHeaders[i].IsHidden)
                {//only add columns that should be visible right now
        ...
        private void butColumnLeft_Click(object sender, EventArgs e)
        {
            ...
            if (gridMain.SelectedCell.X < 2)
            {//can't shift first col nor the 2nd col left since, the first col is the PK and can't be moved right
                return;
            }
            SetIsEdited();
        ...
        private void butColumnRight_Click(object sender, EventArgs e)
        {
            ...
            if (gridMain.SelectedCell.X > gridMain.Columns.Count - 2)
            {//can't shift the last column right
                return;
            }
            SetIsEdited();
            Point pointNewSelectedCell = gridMain.SelectedCell;
