...
namespace OpenDental
{
    public partial class FormVitalsignEdit2014 : FormODBase
    {
        ...
        private List<Loinc> _listLoincHeightCodes;
        private List<Loinc> _listLoincWeightCodes;
        private List<Loinc> _listLoincBMICodes;
        ...
        private List<GenderAge_LMS> _listGenderAge_LMS;
        ...
        {
            _listGenderAge_LMS = Vitalsigns.GetListLMS();
            ...
            _listLoincHeightCodes = new List<Loinc>();
            _listLoincWeightCodes = new List<Loinc>();
            _listLoincBMICodes = new List<Loinc>();
            FillBMICodeLists();
            ...
            for (int i = 0; i < _listLoincHeightCodes.Count; i++)
            {
                if (_listLoincHeightCodes[i].NameShort == "" || _listLoincHeightCodes[i].NameLongCommon.Length < 30)
                {//30 is roughly the number of characters that will fit in the combo box
                    comboHeightExamCode.Items.Add(_listLoincHeightCodes[i].NameLongCommon, _listLoincHeightCodes[i]);
                }
                else
                {
                    comboHeightExamCode.Items.Add(_listLoincHeightCodes[i].NameShort, _listLoincHeightCodes[i]);
                }
                if (i == 0 || VitalsignCur.HeightExamCode == _listLoincHeightCodes[i].LoincCode)
                {
            ...
            for (int i = 0; i < _listLoincWeightCodes.Count; i++)
            {
                if (_listLoincWeightCodes[i].NameShort == "" || _listLoincWeightCodes[i].NameLongCommon.Length < 30)
                {//30 is roughly the number of characters that will fit in the combo box
                    comboWeightExamCode.Items.Add(_listLoincWeightCodes[i].NameLongCommon, _listLoincWeightCodes[i]);
                }
                else
                {
                    comboWeightExamCode.Items.Add(_listLoincWeightCodes[i].NameShort, _listLoincWeightCodes[i]);
                }
                if (i == 0 || VitalsignCur.WeightExamCode == _listLoincWeightCodes[i].LoincCode)
                {
        ...
        private void SetPregCodeAndDescript()
        {
            ...
            {//not currently attached to a disease
                ...
                {//only try to find existing problem if a valid exam date is entered before checking the box, otherwise we do not know what date to compare to the active dates of the pregnancy dx
                    ...
                    for (int i = 0; i < listDiseasesPatient.Count; i++)
                    {//loop through all diseases for this patient, shouldn't be very many
                        ...
                        for (int j = 0; j < listDiseaseDefsPreg.Count; j++)
                        {//loop through preg disease defs in the db, shouldn't be very many
                            if (listDiseasesPatient[i].DiseaseDefNum != listDiseaseDefsPreg[j].DiseaseDefNum)
                            {//see if this problem is a pregnancy problem
                                continue;
                            }
                            if (disease == null || listDiseasesPatient[i].DateStart > disease.DateStart)
        ...
        private void FillBMICodeLists()
        {
            ...
            _listLoincBMICodes.AddRange(listLoincs);
            listLoincs = Vitalsigns.GetLoincsHeight();
            if (listLoincs.Count < 6)
            {
                isInLoincTable = false;
            }
            _listLoincHeightCodes.AddRange(listLoincs);
            listLoincs = Vitalsigns.GetLoincsWeight();
            if (listLoincs.Count < 6)
            {
                isInLoincTable = false;
            }
            _listLoincWeightCodes.AddRange(listLoincs);
            if (!isInLoincTable)
            {
        ...
        private void CalcBMI()
        {
            ...
            {//calc and show BMI percentile if patient is >= 3 and < 17 on 01/01 of the year of the exam
                bmiPercentile = Vitalsigns.GetBMIPercentile(bmi, _patient, PIn.Date(textDateTaken.Text), _listGenderAge_LMS);
            }
            if (bmiPercentile > -1)
            {
            ...
            else if (AgeBeforeJanFirst >= 18 && labelWeightCode.Text != "")
        ...
        private void textDateTaken_Leave(object sender, EventArgs e)
        {
            ...
            {//the currently attached preg disease is valid, will be null if they checked the box on new exam but haven't hit ok to save
                if (dateExam.Date < disease.DateStart || (disease.DateStop.Year > 1880 && disease.DateStop < dateExam.Date))
...
Do you want to remove the pregnancy diagnosis?"))
        ...
        private void butChangeDefault_Click(object sender, EventArgs e)
        {
            ...
            {//text is "To Problem" only when vital sign has a valid PregDiseaseNum and the preg box is checked
                ...
                using FormDiseaseEdit formDiseaseEdit = new FormDiseaseEdit(disease);
                formDiseaseEdit.IsNew = false;
                formDiseaseEdit.ShowDialog();
                if (formDiseaseEdit.DialogResult == DialogResult.OK)
                {
        ...
        private void gridInterventions_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            {//grid can contain MedicationPat or Intervention objects, launch appropriate window
                using FormInterventionEdit formInterventionEdit = new FormInterventionEdit();
                formInterventionEdit.InterventionCur = (Intervention)objCur;
                formInterventionEdit.IsAllTypes = false;
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            Loinc selectedHeightCode = comboHeightExamCode.GetSelected<Loinc>();
            if (selectedHeightCode == null)
            {
                MsgBox.Show(this, "Please select Height code first.");
                return;
            }
            Loinc selectedWeightCode = comboWeightExamCode.GetSelected<Loinc>();
            if (selectedWeightCode == null)
            {
            ...
            #endregion
            #region Save
            VitalsignCur = Vitalsigns.SetFields(VitalsignCur, date, pulse, height, weight, BPdia, BPsys, PIn.Int(textBMIPercentile.Text), selectedHeightCode, selectedWeightCode);
