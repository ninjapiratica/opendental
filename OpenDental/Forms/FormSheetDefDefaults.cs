...
namespace OpenDental
{
    public partial class FormSheetDefDefaults : FormODBase
    {
        ...
        private void UpdateSheetDefDefault(UI.ComboBox comboBox, PrefName prefName, bool isIndependentOfClinic = false)
        {
            ...
            if (clinicPref == null)
            {//If the clinic pref hasn't been created yet.
                if (comboBox.GetSelected<SheetDef>().SheetDefNum != PrefC.GetLong(prefName))
                {//and the selected sheet def is the different from the default preference
            ...
            {//If the clinic pref has been created
                if (comboBox.GetSelected<SheetDef>().SheetDefNum == PrefC.GetLong(prefName))
                {//and the selected sheet def is the same as the default preferance
                    ClinicPrefs.Delete(clinicPref.ClinicPrefNum);
                }
                else
                {//and the selected sheet def is different from the default preferance
