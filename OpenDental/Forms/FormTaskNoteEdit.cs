...
namespace OpenDental
{
    public partial class FormTaskNoteEdit : FormODBase
    {
        ...
        private void FormTaskNoteEdit_Load(object sender, EventArgs e)
        {
            ...
            {//Tasknotes are not editable unless user has TaskNoteEdit permission.
                butOK.Enabled = false;
            }
            if (!Security.IsAuthorized(Permissions.TaskDelete, true))
            {//Tasknotes are not deletable unless user has TaskDelete  permission.
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            if (Tasks.IsTaskDeleted(TaskNoteCur.TaskNum))
            { //If this is for a new task we do have a valid TaskNum because of pre-insert
