...
namespace OpenDental
{
    ...
    public partial class FormEtrans835ClaimPay : FormODBase
    {
        ...
        private void gridPayments_CellClick(object sender, ODGridClickEventArgs e)
        {
            ...
            switch (e.Col)
            {
                case 0://Split claim column
                    ...
                    if (gridPayments.ListGridRows[e.Row].Cells[e.Col].Text == "")
                    {//Row is not selected to split currently, will select for split.
                        isSpliting = true;
                    }
                    gridPayments.BeginUpdate();
        ...
        private void SaveGridChanges()
        {
            ...
            for (int i = 0; i < gridPayments.ListGridRows.Count; i++)
            {
                if (gridPayments.ListGridRows[i].Cells[_idxDeduct].Text != "")
                {//deduct
                    try
                    {
                ...
                if (gridPayments.ListGridRows[i].Cells[_idxInsPayEst].Text != "")
                {//inspay
                    try
                    {
                ...
                if (gridPayments.ListGridRows[i].Cells[_idxWriteoff].Text != "")
                {//writeoff
                    try
                    {
                    ...
                    if (dbl < 0 && !_hx835_Claim.IsReversal)
                    {//Claim reversals have negative writeoffs.
            ...
            for (int i = 0; i < gridPayments.ListGridRows.Count; i++)
            {
                ...
                if (PrefC.GetBool(PrefName.ClaimEditShowPayTracking))
                {
                    ...
                    if (index != 0)
                    {//Otherwise set to chosen DefNum.
        ...
        private void butSplitProcs_Click(object sender, EventArgs e)
        {
            ...
            if (gridPayments.ListGridRows.Count == listClaimProcsSelected.Count)
