...
namespace OpenDental
{
    ...
    public partial class FormAutoCodeEdit : FormODBase
    {
        ...
        private void FormAutoCodeEdit_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            for (int i = 1; i < _listAutoCodeItems.Count; i++)
            ...
            if (_listAutoCodeItems[0].ListConditions.Count == 0)
            ...
            {//start at 1
                ...
                {//loop through the lower-indexed entries
                    int matches = 0;
                    for (int k = 0; k < _listAutoCodeItems[i].ListConditions.Count; k++)
                    {//For each condition in i, check for matches with conditions in j
                        if (_listAutoCodeItems[i].ListConditions[k].Cond == _listAutoCodeItems[j].ListConditions[k].Cond)
                        {//if the same condition is in both rows.
                            matches++;
                        }
                    }
                    if (matches == _listAutoCodeItems[i].ListConditions.Count)
            ...
            if (numCategories != _listAutoCodeItems[0].ListConditions.Count)
