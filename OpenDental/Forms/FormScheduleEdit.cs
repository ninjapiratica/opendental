    ...
    public partial class FormScheduleEdit : FormODBase
    {
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (!_isHolidayOrNote)
            {
                ...
                {//Go see if there is going to be overlapping issues with this new schedule.
            ...
            else if (ScheduleCur.Status != SchedStatus.Holiday && textNote.Text == "")
            {//don't allow blank schedule notes
                MsgBox.Show(this, "Please enter a note first.");
                return;
            }
            long clinicNum = 0;
            if (_isHolidayOrNote && ScheduleCur.SchedType == ScheduleType.Practice && PrefC.HasClinicsEnabled)
            {//prov notes do not have a clinic
                clinicNum = comboClinic.SelectedClinicNum;
                if (ScheduleCur.Status == SchedStatus.Holiday)
