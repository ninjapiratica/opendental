...
namespace OpenDental
{
    public partial class FormSheetPicker : FormODBase
    {
        ...
        private void FormSheetPicker_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPreFill && SheetType == SheetTypeEnum.PatientForm)
            {//we will also show medical history, and possibly consent forms
                ...
                if (!IsWebForm && PrefC.GetBool(PrefName.PatientFormsShowConsent))
                {//only if they want to see consent forms with patient forms.
                    if (listSheetDefsCon.Count == 0)
        ...
        {
            if (ListSheetDefsSelected == null || ListSheetDefsSelected.Count == 0 || AllowMultiSelect)
            {
