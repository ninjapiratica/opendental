...
namespace OpenDental
{
    public partial class FormOIDRegistryInternal : FormODBase
    {
        ...
        private void butRetrieveOIDs_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i < _listOIDInternals.Count; i++)
            {//start with 1, root is 0 and is updated above
                if (_listOIDInternals[i].IDRoot != "" && _listOIDInternals[i].IDRoot != _rootOIDString + "." + i.ToString())
                {//if any of the OIDs other than root are filled and not what we are going to fill it with using the root with CustomerID, we will be overwriting their manually entered data, ask first
