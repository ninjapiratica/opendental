    ...
    public partial class FormStatementOptions : FormODBase
    {
        ...
        private void FormStatementOptions_Load(object sender, System.EventArgs e)
        {
            if (ListStatements == null)
            {
                ...
                {//Must be called before SetEnabled below
                ...
                {//either a limited statement or super family show feature is disabled or statements are not using sheets
            ...
            else
            {
                ...
                {
                    if (ListStatements[0].DateSent != ListStatements[i].DateSent)
        ...
        {
            if (_wasInitiallySent && !checkIsSent.Checked)
        ...
        private bool SaveAsDocument(bool doPrintSheet = false, string pdfFileName = "", Sheet sheet = null, DataSet dataSet = null)
        {
            ...
            if (pdfFileName == "")
            {
                ...
                SheetPrinting.CreatePdf(sheet, tempPath, StatementCur, dataSet, null);
            }
            else
            {
            ...
            finally
            {
                ...
                {
                    if (pdfFileName == "")
                    {//If they're passing in a PDF file name, they probably have it open somewhere else.
            ...
            if (doPrintSheet)
            {
                ...
                if (StatementCur.IsInvoice && checkIsInvoiceCopy.Visible)
        ...
        private void listMode_Click(object sender, EventArgs e)
        {
            ...
            if (_isFromBilling)
            {//Disable single patient if we started in the billing window
                checkSinglePatient.Checked = false;
                checkSinglePatient.Enabled = false;
            }
        }
        ...
        private bool SaveToDb()
        {
            ...
            if (ListStatements == null)
            {
                ...
                if (checkBoxBillShowTransSinceZero.Checked)
                {
                    ...
                    if (tableBals.Rows.Count > 0)
                    {
                        ...
                        {//patient has a zero or credit balance.
            ...
            if (checkBoxBillShowTransSinceZero.Checked)
            {//populate lookup table of patients and dates
            ...
            for (int i = 0; i < ListStatements.Count; i++)
            {
                ...
                if (checkBoxBillShowTransSinceZero.Checked)
                {
                    ...
                    if (dateFrom != DateTime.MinValue)
                    {//patient does not have a zero or credit balance.
