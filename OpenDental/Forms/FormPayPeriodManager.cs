...
namespace OpenDental
{
    ...
    public partial class FormPayPeriodManager : FormODBase
    {
        ...
        private void FormPayPeriodManager_Load(object sender, System.EventArgs e)
        {
            ...
            if (dayOfWeek != 0)
            {//They have a day of the week selected
        ...
        private void GenerateSemiMonthlyPayPeriods()
        {
            ...
            while (numPeriodsGenerated < numPeriods)
            {//will add either one or zero items to the list on each pass. zero only on first pass
                if (checkLast.Checked)
                {
                    period2Day = System.DateTime.DaysInMonth(dateTimeStartPayPeriod.Year, dateTimeStartPayPeriod.Month);
                }
                ...
                if (dateTimeStartPayPeriod.Day > period1Day && dateTimeStartPayPeriod.Day > period2Day)
                {           //the current day is after both periods of the month 
                ...
                {
                    if (dateTimeStartPayPeriod.Month == 12)
                    {   //case where pay period is split between years
                        dateTimeStartPayPeriod = new DateTime(dateTimeStartPayPeriod.Year, dateTimeStartPayPeriod.Month, period2Day).AddDays(1);
                        if (checkLast.Checked)
                        {   //case where it spans years and period 2 day is the 31st. dateTimeStartPayPeriod is jan 1st
                ...
                if (_listPayPeriods.Count > 0)
                {
                    if (_listPayPeriods.Last().DatePaycheck.DayOfWeek == DayOfWeek.Saturday && doExcludeWeekends)
                    {
                        ...
                        {   //Paying before the weekend affects period end date. It is moved back until it is the specified num of days before payday.		
        ...
        private void butGenerate_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < numPeriods; i++)
            {
                ...
                {//Add time to "startDate" to get the new start date for the next iteration as well as figuring out the end date for current payperiod.
                ...
                if (payPeriod.DatePaycheck.DayOfWeek == DayOfWeek.Saturday && checkExcludeWeekends.Checked)
                {
                    ...
                    {
                        if (payPeriod.DatePaycheck.Subtract(TimeSpan.FromDays(1)) <= payPeriod.DateStop)
                        {//Can't move the paycheck date to the same day (or before) than the date end.
                ...
                else if (payPeriod.DatePaycheck.DayOfWeek == DayOfWeek.Sunday && checkExcludeWeekends.Checked)
                {
                    if (radioPayBefore.Checked)
                    {
                        if (payPeriod.DatePaycheck.Subtract(TimeSpan.FromDays(2)) <= payPeriod.DateStop)
                        {//Can't move the paycheck date to the same day (or before) than the date end.
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            //Save payperiods
            for (int i = 0; i < _listPayPeriods.Count; i++)
            {//PayPeriods are always new in this form.
