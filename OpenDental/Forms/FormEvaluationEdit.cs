    ...
    public partial class FormEvaluationEdit : FormODBase
    {
        ...
        private void FormEvaluationEdit_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _listEvaluationCriterions.Count; i++)
            {
                ...
                { //New grading scale isn't a duplicate, so add it to our list of grading scales.
        ...
        private void FillGridCriterion()
        {
            ...
            for (int i = 0; i < _listEvaluationCriterions.Count; i++)
            {
                ...
                if (_listEvaluationCriterions[i].IsCategoryName)
                {//Categories do not get filled
                    row.Bold = true;
                    row.Cells.Add("");
                    row.Cells.Add("");
                    row.Cells.Add("");
        ...
        private void FillGridGradingWithWeighted(long gradingScaleNum)
        {
            ...
            if (_listEvaluationCriterions[gridCriterion.SelectedCell.Y].IsCategoryName)
            {//Refill the grid with nothing if a category is selected.
        ...
        private void gridCriterion_CellLeave(object sender, ODGridClickEventArgs e)
        {
            ...
            if (_listEvaluationCriterions[gridCriterion.SelectedCell.Y].IsCategoryName)
            {//This must happen first so categories are always set to blank columns
                gridCriterion.ListGridRows[gridCriterion.SelectedCell.Y].Cells[gridCriterion.SelectedCell.X].Text = "";
                return;
            }
            if (gridCriterion.SelectedCell.X == 4)
            {//Cell 4 is the Notes Column
            ...
            float number = 0;
            if (!float.TryParse(gridCriterion.ListGridRows[gridCriterion.SelectedCell.Y].Cells[3].Text, out number))
            {// Cell 3 is the GradeNumber column
