...
namespace OpenDental
{
    public partial class FormEcwDiagAdv : FormODBase
    {
        ...
        private void FillTables()
        {
            try
            {
                _table = MySqlHelper.ExecuteDataset(_connectionString, "SHOW TABLES").Tables[0];
            }
            catch (Exception ex)
            {
        ...
        {
            if (textConnString.Text != _dummyConnString && textConnString.Text != "")
        ...
        private void gridTables_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            try
            {
                _tableQueryResults = MySqlHelper.ExecuteDataset(_connectionString, _command).Tables[0];
            }
            catch (Exception ex)
            {
                MsgBox.Show(this, ex.Message);
