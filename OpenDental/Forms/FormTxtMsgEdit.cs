...
namespace OpenDental
{
    public partial class FormTxtMsgEdit : FormODBase
    {
        ...
        public bool SendText(long patNum, string wirelessPhone, string message, YN txtMsgOk, long clinicNum, SmsMessageSource smsMessageSource, bool canIncreaseLimit = false)
        {
            ...
            {
                if (!PrefC.HasClinicsEnabled && PrefC.GetDateT(PrefName.SmsContractDate).Year < 1880)
                { //Checking for practice (clinics turned off).
                    MsgBox.Show(this, "Integrated Texting has not been enabled.");
                    return false;
                }
                else if (PrefC.HasClinicsEnabled && !Clinics.IsTextingEnabled(clinicNum))
                { //Checking for specific clinic.
            ...
            else
            {
                ...
                {//only a limitation for CallFire
        ...
        private bool SendCallFire(long patNum, string wirelessPhone, string message)
        {
            ...
            if (patNum == 0)
            {  //No patient selected, do not make commlog.
                return true;
            }
        ...
        private void butPatFind_Click(object sender, EventArgs e)
        {
            ...
            Patient patCur = Patients.GetPat(formP.PatNumSelected);
        ...
        private void radioOther_Click(object sender, EventArgs e)
        {
            ...
            {//The user clicked the "Another Person" radio button multiple times consecutively.
        ...
        {
            if (textWirelessPhone.ReadOnly)
            {//The user clicked the "Patient" radio button multiple times consecutively.
                return;//Leave so that the phone number is not wiped out unnecessarily.
            }
            butPatFind.Enabled = true;
            textPatient.Enabled = true;
        ...
        private void butSend_Click(object sender, EventArgs e)
        {
            ...
            {  //No patient selected
                ...
                {  //0 as PatNum to denote no pat specified
