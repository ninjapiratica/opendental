    ...
    public partial class FormSheetImport : FormODBase
    {
        ...
        private void FillRows()
        {
            ...
            if (SheetCur.SheetType == SheetTypeEnum.PatientForm)
            {
                ...
                if (fieldVal != null)
                {
                    ...
                    {//Patient entered blank date, consider this to be valid blank date.
                        row.NewValDisplay = "";
                        row.ImpValDisplay = "";
                    }
                    else if (((DateTime)row.NewValObj).Year < 1880)
                    {//Patient entered date in incorrect format, consider this invalid (imports as MinValue)
                        row.NewValDisplay = fieldVal;
            ...
            else if (SheetCur.SheetType == SheetTypeEnum.MedicalHistory)
            {
                ...
                for (int i = 0; i < listSheetFieldsAllergy.Count; i++)
                {
                    ...
                    if (listSheetFieldsAllergy[i].FieldValue == "")
                    {//Current box not checked.
                        if (sheetFieldOppositeBox == null || sheetFieldOppositeBox.FieldValue == "")
                ...
                for (int i = 0; i < listSheetFieldsCurrentMed.Count; i++)
                {
                    ...
                    for (int j = 0; j < listMedicationPatsFull.Count; j++)
                    {
                        ...
                        if (listMedicationPatsFull[j].MedicationNum != 0)
                    ...
                    for (int j = 0; j < listSheetFieldsRelatedChkBoxes.Count; j++)
                    {//Figure out which corresponding checkbox is checked.
                        if (listSheetFieldsRelatedChkBoxes[j].FieldValue != "")
                ...
                for (int i = 0; i < listSheetFieldsNewMed.Count; i++)
                {
                    ...
                    if (listSheetFieldsNewMed[i].FieldValue == "")
                ...
                for (int i = 0; i < listSheetFieldsProblems.Count; i++)
                {
                    ...
                    if (listSheetFieldsProblems[i].FieldValue == "")
                    {//Current box not checked.
                        if (sheetFieldOppositeBox == null || sheetFieldOppositeBox.FieldValue == "")
                        {//No opposite box or both boxes are not checked.
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            else if (_listSheetImportRows[e.Row].ObjType == typeof(string))
            {
                ...
                if (_listSheetImportRows[e.Row].OldValDisplay == inputBox.textResult.Text)
            ...
            else if (_listSheetImportRows[e.Row].ObjType.IsEnum)
            {
                ...
                if (formSheetImportEnumPicker.DialogResult == DialogResult.OK)
                {
                    ...
                    if (_listSheetImportRows[e.Row].ImpValObj == null)
                    {//was initially null
                        if (selectedI != -1)
                    ...
                    {//was not initially null
                        if ((int)_listSheetImportRows[e.Row].ImpValObj != selectedI)
                        {//value was changed.
                    ...
                    else if (_listSheetImportRows[e.Row].OldValObj != null && (int)_listSheetImportRows[e.Row].ImpValObj == (int)_listSheetImportRows[e.Row].OldValObj)
            ...
            #endregion
            #region DateTime
            else if (_listSheetImportRows[e.Row].ObjType == typeof(DateTime))
            {//this is only for one field so far: Birthdate
                ...
                if (_listSheetImportRows[e.Row].ImpValDisplay == _listSheetImportRows[e.Row].OldValDisplay)
                {//value is now same as original
                    _listSheetImportRows[e.Row].DoImport = false;
                }
                else
                ...
                || _listSheetImportRows[e.Row].ObjType == typeof(Disease))
            {
                ...
                else
                {
                    ...
                    if (_listSheetImportRows[e.Row].OldValDisplay == _listSheetImportRows[e.Row].ImpValDisplay)
                    {//value is now same as original
                        _listSheetImportRows[e.Row].DoImport = false;
                    }
                    else
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            else if (SheetCur.SheetType == SheetTypeEnum.MedicalHistory)
            {
                for (int i = 0; i < _listSheetImportRows.Count; i++)
                {
                    ...
                    if (_listSheetImportRows[i].ObjType == null)
                    {//Should never happen.
                        continue;
                    }
