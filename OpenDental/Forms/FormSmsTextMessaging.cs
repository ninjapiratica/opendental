    ...
    public partial class FormSmsTextMessaging : FormODBase
    {
        ...
        private void FormSmsTextMessaging_Load(object sender, EventArgs e)
        {
            ...
            if (PrefC.HasClinicsEnabled)
            {
                ...
                {//This form behaves differently when compared to the other 6 forms
        ...
        private void FillGridMessageThread()
        {
            ...
            if (GetSelectedPatNum() == 0 && GetSelectedSmsGroup() == null)
            { //A message with no patNum was selected and is not a group, or nothing is selected at all.
                if (GetSelectedSmsToMobile() != null)
                { //SmsToMobile is selected.
                ...
                { //Nothing selected at all, clear the message thread.
            ...
            if (radioGroupByPhone.Checked)
            {//Grouped by phone. Get all messages from the database for the given phone number. Don't specify patNum.
                patNumForDb = -1;
                strNumberForDb = GetSelectedMobileNumber();
            }
            else if (GetSelectedPatNum() == 0 && radioGroupByPatient.Checked)
            {//Grouped by Patient and the patient's patnum is 0. Find messages with the patnum/number combo.
                patNumForDb = GetSelectedPatNum();
                strNumberForDb = GetSelectedMobileNumber();
            }
            else
            {//Normal patnum. Find all messages for the patnum regardless of number.
                patNumForDb = GetSelectedPatNum();
                strNumberForDb = "";
            }
            ...
            string patNames = "";
            labelPatientsForPhone.Visible = false;
            if (listPatNumsInMessages.Count > 1)
            {//Only display the patient names if there are more than 1.
        ...
        private void menuItemSelectPatient_Click(object sender, EventArgs e)
        {
            ...
            if (GetSelectedPatNum() == 0 || GetPatientName(GetSelectedPatNum()) == Lan.g(this, "Not found"))
            {//If the patNum is 0 or patient could not be found.
        ...
        private void butSend_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < gridMessages.ListGridRows.Count; i++)
            {
                ...
                { //Verify that the row the user appears to have selected is actually the selected row.
                    continue;
                }
