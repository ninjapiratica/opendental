    ...
    public partial class FormClaimEdit : FormODBase
    {
        ...
        private void FormClaimEdit_Load(object sender, System.EventArgs e)
        {
            ...
            {//Canadian. en-CA or fr-CA
                ...
                if (_claim.DateSent.Date == MiscData.GetNowDateTime().Date)
            ...
            else if (_claim.ClaimStatus.In(ClaimStatus.Sent.GetDescription(true), ClaimStatus.Received.GetDescription(true)))
            {//sent or received
        ...
        private void SetFormReadOnly(System.Windows.Forms.Control controlsInput)
        {
            for (int i = 0; i < controlsInput.Controls.Count; i++)
            {
                ...
                {//Make sure all sub controls are read only.
        ...
        private void comboClaimStatus_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            if (claimStatus == ClaimStatus.HoldForInProcess && _listClaimStatuses[comboClaimStatus.SelectedIndex] != ClaimStatus.HoldForInProcess)
            { //Status is I and trying to change it to something else
            ...
            if (_claim.ClaimType == "PreAuth")
            {//we're in a PreAuth and trying to change the status from the dropdown
        ...
        public void FillForm()
        {
            ...
            comboClinic.SelectedClinicNum = _claim.ClinicNum;
            if (Defs.GetDefsForCategory(DefCat.ClaimCustomTracking, true).Count == 0)
            {  // Disable Add button if all defs are hidden (nothing to add)
        ...
        {
            if (PatPlans.GetCountForPatAndInsSub(_claim.InsSubNum, _claim.PatNum) == 0)
            {//If the plan has been dropped
            ...
            FillGrids();
            if (!_recalcErrorProvider.GetError(this.butRecalc).Equals(String.Empty))
            {//ClaimProcedures values have been modified.
        ...
        private void FillGrids(bool doRefreshData = true)
        {
            ...
            for (int i = 0; i < _listClaimProcsForClaim.Count; i++)
            {
                ...
                if (CultureInfo.CurrentCulture.Name.EndsWith("CA") && listClaimProcsForLab.Count != 0)
                {//This proc has one or more labs attached.
                    for (int j = 0; j < listClaimProcsForLab.Count; j++)
                    {
                        ...
                        if (_listClaimProcsForClaim[i].Status == ClaimProcStatus.Supplemental && _listClaimProcsForClaim[i].DateCP != listClaimProcsForLab[j].DateCP)
                        {//Needed when there are multiple supplemental payments.
        ...
        {
            if (_claim.ClaimStatus.In(ClaimStatus.Sent.GetDescription(true), ClaimStatus.Received.GetDescription(true)))
            {//sent or received
        ...
        private void SelectLabProcs()
        {
            ...
            for (int i = 0; i < gridProc.SelectedIndices.Count(); i++)
            {
                ...
                else if (procedure.ProcNumLab != 0)
                {//Otherwise, if it has a ProcNumLab set, this is a lab procedure.
                    gridProc.SetSelected(index - 1, true);//Guaranteed the row above is either a parent or another lab.  See FillGrids().
                    if (GetListLabsForProc(procedure.ProcNum, _listClaimProcsForClaim[index]).Count > 1)
                    {//User already clicked the 1 lab and it is already selected when 1.
                        if (_listClaimProcsForClaim[index - 1].ProcNum == procedure.ProcNumLab)
                        {//The row above the currently clicked row is the parent proc.
                        ...
                        {//The above row is the first lab and the current row is the second lab.
        ...
        private List<ClaimProc> GetListClaimProcsForProcNumLabsParent(long procNumParent)
        {
            ...
            {//For each lab proc associated to the passed in procNumParent, add it to the list
        ...
        private bool CheckRecalcEstimates(ClaimProc claimProc)
        {
            ...
            if (formClaimProc.DialogResult == DialogResult.Abort)
            {//Claim was deleted. Close claim edit window.
            ...
            if (isClaimProcRecalcNeeded)
            {//Claim Procedure values have changed.
        ...
        private void butPayTotal_Click(object sender, System.EventArgs e)
        {
            ...
            if (PrefC.GetBool(PrefName.ClaimPayByTotalSplitsAuto) && !hasCapClaimProcs)
            {//Determines if the user prefers line item accounting.
                if (_listClaimProcsForClaim.Any(x => x.Status == ClaimProcStatus.Received))
                {//This prevents the user from making multiple as total payments if there are recieved claims procs.
        ...
        private bool PayAsTotal()
        {
            ...
            {//Claim still exists, and user didn't click cancel
                for (int i = 0; i < _listClaimProcsForClaim.Count; i++)
                {
                    if (_listClaimProcsForClaim[i].Status != ClaimProcStatus.NotReceived)
                    {
        ...
        {
            if (_claim.ClaimType != "PreAuth" && !Security.IsAuthorized(Permissions.InsPayCreate))
            {//date not checked here, but it will be checked when actually creating the check
                return;
            }
            ...
            if (gridProc.SelectedIndices.Length == 0)
            {
                ...
                {
                    if (_listClaimProcsForClaim[i].Status == ClaimProcStatus.NotReceived
                        && _listClaimProcsForClaim[i].ProcNum > 0)
                    {//and is procedure
                        gridProc.SetSelected(i, true);
                    }
                }
            ...
            for (int i = 0; i < gridProc.SelectedIndices.Length; i++)
            {
                if (_listClaimProcsForClaim[gridProc.SelectedIndices[i]].ClaimPaymentNum != 0)
            ...
            if (DoConsolidateOrthoPayments())
            {
                ...
                if (listIntOrthoAutoGridRows.Count > 0)
                {
                    ...
                    {//Show the claim edit window and have the insurance payment window automatically show.
                        if (!Security.IsAuthorized(Permissions.ClaimView))
                        {//this should ever get hit if this permission is not granted, but just in case
                            return;
                        }
        ...
        private void butSplit_Click(object sender, System.EventArgs e)
        {
            ...
            if (!hasProcLeft)
            {//All procedures are selected for the split...
        ...
        private void butExport_Click(object sender, EventArgs e)
        {
            ...
            if (!Directory.Exists(claimAttachExportPath))
            {
                    ...
                    + Lan.g(this, "Would you like to create it?"), "", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    {   //May throw an exception for a variety of reasons.
        ...
        private void butPreview_Click(object sender, System.EventArgs e)
        {
            ...
            else if (formClaimPrint.DialogResult == DialogResult.Abort)
        ...
        private void SendClaim()
        {
            ...
            if (claimSendQueueItemsArrayCA.IsNullOrEmpty())
            {//Happens if the Claim has been deleted.
        ...
        {
            if (IsNew)
            {//Must have a claim object to do claim attachments
                MsgBox.Show(this, "The claim must be saved before you can start adding attachments.");
                return;
            }
            if (comboClaimStatus.SelectedIndex == _listClaimStatuses.IndexOf(ClaimStatus.WaitingToSend))
            {//Waiting to send claim
        ...
        {
            if (valCodeIdx < listClaimValCodeLogs.Count)
            {//update existing ClaimValCodeLog
            ...
            else if (PIn.Double(textBoxVCAmount.Text) > 0 || textBoxVCCode.Text != "")
            {//add a new ClaimValCodeLog
        ...
        private void butDelete_Click(object sender, System.EventArgs e)
        {
            ...
            if (_claim.ClaimStatus == ClaimStatus.Received.GetDescription(true))
            {//received
        ...
        private void DeleteClaimHelper(List<long> list835Attaches = null)
        {
            ...
            {//all other claim types use original estimate claimproc.
                ...
                for (int i = 0; i < _listClaimProcsForClaim.Count; i++)
                {
                    ...
                    if (listClaimProcsForLab.Count != 0)
                    {//And proc "i" has labs. 
                        for (int j = 0; j < listClaimProcsForLab.Count; j++)
                        {//Foreach lab
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (comboClaimStatus.SelectedIndex == _listClaimStatuses.IndexOf(ClaimStatus.HoldForInProcess)
                && _claimOld.ClaimStatus != ClaimStatus.HoldForInProcess.GetDescription(useShortVersionIfAvailable: true))
            { //If status changed to I from something else
            ...
            {//waiting to send
                ClaimSendQueueItem[] claimSendQueueItemsArray = updateData.ListSendQueueItems;
                if (claimSendQueueItemsArray.Length == 0 || !claimSendQueueItemsArray[0].CanSendElect)
                {//listQueue can be empty if another workstation deleted the claim
                    DialogResult = DialogResult.OK;
                    return;
                }
                //string warnings;
                ...
                {//No missing data, but can send attachments electronically
                 //Validating for attachments is currently only supported for ClaimConnect customers
                    ...
                    if (validateClaimResponse != null//catches do not return
                        && validateClaimResponse.IsAttachmentRequired)
                    {
                        if (_claim.AttachedFlags != "Misc")
                        {//No need to visit the database again if this is already the case
                    ...
                    else if (_claim.Attachments.Count == 0 && _claim.AttachedFlags != "Mail")
                    {//Don't set to 'Mail' on claims with attachments or make unneccessary DB trips.
