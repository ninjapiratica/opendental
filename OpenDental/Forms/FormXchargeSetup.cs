...
namespace OpenDental
{
    ...
    public partial class FormXchargeSetup : FormODBase
    {
        ...
        private void FormXchargeSetup_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _listProgProps.Count; i++)
            {
                if (_listProgProps[i].IsHighSecurity)
                {
                    _listProgProps[i].TagOD = _listProgProps[i].PropertyValue;
                }
            }
            if (!ProgramProperties.CanEditProperties(_listProgProps))
            {
        ...
        {
            if (comboClinic.SelectedClinicNum == _clinicNumRevert)
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            for (int i = 0; i < _listProgProps.Count; i++)
            {
                if (_listProgProps[i].IsHighSecurity && _listProgProps[i].PropertyValue != _listProgProps[i].TagOD.ToString())
                {
                    StringBuilder stringBuilderLogText = new StringBuilder();
                    stringBuilderLogText.Append(_progCur.ProgDesc + "'s " + _listProgProps[i].PropertyDesc);
                    if (PrefC.HasClinicsEnabled)
                    {
                        long clinicNumCur = _listProgProps[i].ClinicNum;
                        if (clinicNumCur == 0)
                        {
