...
namespace OpenDental
{
    ...
    public partial class FormBilling : FormODBase
    {
        ...
        {
            if (IsDisposed)
            {//Attempted bug fix for an exception which occurred in FillGrid() when the grid was already disposed.
                return;
            }
        ...
        private void butEdit_Click(object sender, EventArgs e)
        {
            ...
            FormStatementOptions.ListStatements = Statements.GetStatements(listStatementNums);
            FormStatementOptions.ShowDialog();
        ...
        ///<summary>Concat all the pdf's together to create one print job. Returns false if the printing was canceled. Changes to this method will also need to be made to OpenDentalService.ScheduledProcessThread.SendEmails().</summary>
            ...
            , ref Dictionary<long, StatementData> dictionaryStatementData)
        {
            ...
            if (PrefC.GetDate(PrefName.DateLastAging).Date != MiscData.GetNowDateTime().Date && !RunAgingEnterprise())
            {//run aging for all patients
            ...
            foreach (Statement statement in listStatements)
            {
                ...
                if (statement.Mode_ == StatementMode.Electronic)
                {
                    ...
                    {//EHG
                        listElectErrors = OpenDentBusiness.Bridges.EHG_statements.Validate(clinicNum);
                    }
        ...
        ///<summary>Attempt to send electronic bills if needed. Returns false if the sending was canceled. Changes to this method will also need to be made to OpenDentalService.ScheduledProcessThread.SendEBills().</summary>
            ...
            ref int curStatementsProcessed, ref int skippedDeleted)
        {
            ...
            if (listEbillStatements.Count == 0)
            {//All statements have been sent for the current clinic.  Nothing more to do.
                return true;
            }
            ...
            if (PrefC.GetString(PrefName.BillingUseElectronic) == "1")
            {
                OpenDentBusiness.Bridges.EHG_statements.GeneratePracticeInfo(xmlWriterElect, clinicNum);
            }
            else if (PrefC.GetString(PrefName.BillingUseElectronic) == "2")
            {
                OpenDentBusiness.Bridges.POS_statements.GeneratePracticeInfo(xmlWriterElect, clinicNum);
            }
            else if (PrefC.GetString(PrefName.BillingUseElectronic) == "3")
            {
                OpenDentBusiness.Bridges.ClaimX_Statements.GeneratePracticeInfo(xmlWriterElect, clinicNum);
            }
            else if (PrefC.GetString(PrefName.BillingUseElectronic) == "4")
            {
                OpenDentBusiness.Bridges.EDS_Statements.GeneratePracticeInfo(xmlWriterElect, clinicNum);
            }
            ...
            for (int j = listEbillStatements.Count - 1; j >= 0; j--)
            {//Construct the string for sending this clinic's ebills
                if (!BillingProgressPause())
                {
                    return false;
                }
                ...
                try
                {
                    ...
                    using (XmlWriter xmlWriterStatement = XmlWriter.Create(strBuildStatement, xmlWriterElect.Settings))
                    {
                        ...
                        {
                            OpenDentBusiness.Bridges.EHG_statements.GenerateOneStatement(xmlWriterStatement, statementCur, patient, family, dataSet);
                        }
                        else if (PrefC.GetString(PrefName.BillingUseElectronic) == "2")
                        {
                            OpenDentBusiness.Bridges.POS_statements.GenerateOneStatement(xmlWriterStatement, statementCur, patient, family, dataSet);
                        }
                        else if (PrefC.GetString(PrefName.BillingUseElectronic) == "3")
                        {
                            OpenDentBusiness.Bridges.ClaimX_Statements.GenerateOneStatement(xmlWriterStatement, statementCur, patient, family, dataSet);
                        }
                        else if (PrefC.GetString(PrefName.BillingUseElectronic) == "4")
                        {
                            OpenDentBusiness.Bridges.EDS_Statements.GenerateOneStatement(xmlWriterStatement, statementCur, patient, family, dataSet);
                        }
                    }
            ...
            if (PrefC.GetString(PrefName.BillingUseElectronic) == "1")
            {
                ...
                for (int attempts = 0; attempts < 3; attempts++)
                {
                    ...
                    try
                    {
                        OpenDentBusiness.Bridges.EHG_statements.Send(strBuildElect.ToString(), clinicNum, out alertMsg);
            ...
            if (PrefC.GetString(PrefName.BillingUseElectronic) == "2")
            {
                ...
                {//Default Output path not set, however, User already chose a filepath to output to on the previous pass.
                    filePath = selectedFile;
                }
                else if (selectedFile == "")
                {//User canceled when prompted for output path, since preference path is invalid.  Skip all batches.
                    sentElect -= listElectStmtNums.Count;
                    return true;//Go on to next batch
                }
                else
                ...
                filePath = Statements.GetEbillFilePathForClinic(filePath, clinicNum);
            ...
            if (PrefC.GetString(PrefName.BillingUseElectronic) == "3")
            {
                ...
                {//User already chose a filepath to output to on the previous pass.
                 //We will reuse the selectedPath below.
                }
                else if (selectedFile == "")
                {//User canceled when prompted for output path.  Skip all batches.
                    sentElect -= listElectStmtNums.Count;
                    return true;//Go on to next batch
                }
                else
                ...
                string filePath = Statements.GetEbillFilePathForClinic(selectedFile, clinicNum);
                File.WriteAllText(filePath, strBuildElect.ToString());
                for (int i = 0; i < listElectStmtNums.Count; i++)
                {
                    _listStatementNumsSent.Add(listElectStmtNums[i]);
        ...
        private void butClose_Click(object sender, EventArgs e)
        {
            ...
            if (dictionaryClinicStatmentsToDelete != null)
            {//No error. (jordan I don't understand this line)
