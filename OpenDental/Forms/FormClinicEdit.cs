...
namespace OpenDental
{
    ...
    public partial class FormClinicEdit : FormODBase
    {
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (comboDefaultBillingType.SelectedIndex > 0)
            {//If default billing type is not set to default, update/insert clinicpref row.
            ...
            {//"Use Global Preference" selected, delete the pref if it exists.
                listClinicPrefsToDelete.Add(PrefName.PracticeDefaultBillType);
            }
            if (checkAlwaysAssignBenToPatient.Checked)
            {//If checked, we will upsert into clinicpref table. False accurately represents the pref.
                ClinicPrefs.Upsert(PrefName.InsDefaultAssignBen, ClinicCur.ClinicNum, "0");
            }
            else
            {//Since we aren't overriding, we want to remove the clinicpref to not confuse as boolean state.
                listClinicPrefsToDelete.Add(PrefName.InsDefaultAssignBen);
            }
            if (comboInsAutoReceiveNoAssign.SelectedIndex == 2)
            {//2=Use Global Preference
        ...
        private void checkHidden_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (numPatients > 0)
            { //Patients attached to this clinic
