    ...
    public partial class FormRxEdit : FormODBase
    {
        ...
        private void FormRxEdit_Load(object sender, System.EventArgs e)
        {
            ...
            if (IsNew)
            {
                ...
                if (_userProvider != null && !_userProvider.IsSecondary)
        ...
        private void FillComboProvNum()
        {
            ...
            if (PrefC.GetBool(PrefName.RxHideProvsWithoutDEA))
            {
                ...
                if (_rxPat.ProvNum != 0 && listProviders.FindIndex(x => x.ProvNum == _rxPat.ProvNum) == -1)
                {//because List<T>.Contains can't manage to find a provider that is in the list
            ...
            {//If new Rx 
                ...
                else if (listProviders.Any(x => x.ProvNum == _patient.PriProv))
                {//Select the patient's primary provider, if possible.
        ...
        private bool SaveRx()
        {
            ...
            {//Happens if the patient's primary provider is not in the list of providers or the logged in user is not a provider. 
            ...
            if (PrefC.GetBool(PrefName.RxHasProc))
            {
                ...
                if (comboProcCode.SelectedIndex == 0)
                {//none
                    _rxPat.ProcNum = 0;
                }
                else
