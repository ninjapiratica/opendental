    ...
    public partial class FormQueryMonitor : FormODBase
    {
        ...
        private void TimerProcessQueue_Tick(object sender, EventArgs e)
        {
            ...
            while (_concurrentQueueDbQueryObjs.Count != 0)
            {
                ...
                if ((IsPayloadMonitor && query.IsPayload) || (!IsPayloadMonitor && !query.IsPayload))
                { //Payload monitor and query is payload, OR query monitor and query is query
        ...
        private void AddQueriesToGrid(params DbQueryObj[] dbQueryObjArray)
        {
            ...
            for (int i = 0; i < listDbQueryObjs.Count; i++)
            {
                ...
                {
                    row = new GridRow(command, listDbQueryObjs[i].DateTimeInit.ToString(), "");
                }
                else
                {
                    row = new GridRow(command, listDbQueryObjs[i].DateTimeInit.ToString(), listDbQueryObjs[i].Elapsed.ToString("G"));
                }
        ...
        {
            if (butStart.Text == "Stop")
            {//currently monitoring queries.
