...
namespace OpenDental
{
    ...
    public partial class FormProviderSetup : FormODBase
    {
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (!PrefC.GetBool(PrefName.EasyHideDentalSchools) && Providers.IsAttachedToUser(providerSelected.ProvNum))
            {//Dental schools is turned on and the provider selected is attached to a user.
             //provSelected could be a student, an instructor, or other provider at this point.
                if (!providerSelected.IsInstructor && providerSelected.SchoolClassNum != 0 && !Security.IsAuthorized(Permissions.AdminDentalStudents))
                {//student
                    return;
                }
                if (providerSelected.IsInstructor && !Security.IsAuthorized(Permissions.AdminDentalInstructors))
                {//instructor
                    return;
                }
                if (!providerSelected.IsInstructor && providerSelected.SchoolClassNum == 0 && !Security.IsAuthorized(Permissions.ProviderEdit))
                {//other provider
                    return;
                }
                if (!radioStudents.Checked)
                {
                    ...
                    if (Security.IsAuthorized(Permissions.ProviderEdit, suppressMessage: true))
                    {//Other provider. Dental schools specific permissions do not log.
            ...
            {//No Dental Schools or provider is not attached to a user
