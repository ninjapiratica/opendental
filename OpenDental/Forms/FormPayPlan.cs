    ...
    public partial class FormPayPlan : FormODBase
    {
        ...
        private void FillCharges()
        {
            ...
            for (int i = 0; i < listGridRowsPayPlan.Count; i++)
            {
                ...
                if (DateTime.Parse(listGridRowsPayPlan[i].Cells[0].Text) <= DateTime.Today)
                {//Totals row
            ...
            for (int i = 0; i < _listPayPlanCharges.Count; i++)
            {
                ...
                countDebits++;
                if (_listPayPlanCharges[i].ChargeType == PayPlanChargeType.Debit && _listPayPlanCharges[i].Principal >= 0)
                {//Not an adjustment
        ...
        private void butRecalculate_Click(object sender, EventArgs e)
        {
            ...
            if (_listPayPlanCharges.Count(x => x.ChargeType == PayPlanChargeType.Debit) == 0)
            {//This is only possible if they manually delete all of their rows and try to press recalculate.
        ...
        private void gridCharges_CellDoubleClick(object sender, OpenDental.UI.ODGridClickEventArgs e)
        {
            ...
            if (gridCharges.ListGridRows[e.Row].Tag.GetType() == typeof(PayPlanCharge))
            {
                ...
                {//The user deleted the payplancharge.
        ...
        private void butAdj_Click(object sender, EventArgs e)
        {
            ...
            {//Make adjustment visible in account module.
        ...
        private void UpdateTxPaymentPlanChargeSummary(List<PayPlanCharge> listPayPlanChargesCredit)
        {
            ...
            {
                if (listPayPlanChargesCredit[i].ChargeDate.Date != DateTime.MaxValue.Date)
                { //do not take into account maxvalue (tp'd) charges
        ...
        private void SaveData(bool isPrinting = false)
        {
            ...
            //PlanNum set already
            if (IsInsPayPlan)
            { //if insurance payment plan, remove all other credits and create one credit for the completed amt.
