...
namespace OpenDental
{
    public partial class FormBugSubmissions : FormODBase
    {
        ...
        private void gridSubs_RightClickHelper(object sender, EventArgs e)
        {
            ...
            switch (((MenuItem)sender).Index)
            {
                ...
                case 3://Link or Unlink bug
                    if (bugSubGridRow.BugSubmissionDisplay.BugId == 0)
                    {//Not linked to existing bug, so link
        ...
        private void FillSubGrid(bool isRefreshNeeded = false, string grouping95 = "")
        {
            ...
            if (comboGrouping.SelectedIndex == 0)
            {//Group by 'None'
        ...
        {
            if (_formBugSubmissionMode == FormBugSubmissionMode.ViewOnly)
            {//Not allowed to create a bug.
                return;
            }
        ...
        private void OpenBug(BugSubmission bugSubmission)
        {
            ...
            if (FormBugEdit.ShowDialog() == DialogResult.OK && FormBugEdit.BugCur == null)
            {//Bug was deleted.
                FillSubGrid(true);
            }
        }
        ...
        private void comboVersions_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            if (sender == comboGrouping && comboGrouping.SelectedIndex == 3)
            {//95%
        ...
        {
            if (listShowHideOptions.SelectedIndices.Contains(3) && _minGroupingCount >= 0)
            {//Already Set and still selected, another item was clicked.
                return;
            }
            ...
            else if (comboGrouping.SelectedIndex <= 0)
            {//Do not allow when 'None' selected.
        ...
        {
            if (_formBugSubmissionMode == FormBugSubmissionMode.SelectionMode)
            {//Text is set to "Ok" when SelectionMode
            ...
            if (butAddJob.Text == "View Job" && butAddJob.Tag is JobLink)
            {//Assocaited to job, see gridSubs_CellClick(...)	
