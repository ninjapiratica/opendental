...
namespace OpenDental
{
    ...
    public partial class FormAuditOneType : FormODBase
    {
        ...
        private List<Permissions> _listPermissions;
        private long _fKey;
        ...
        public FormAuditOneType(long patNum, List<Permissions> listPermissions, string title, long fKey)
        {
            //
            ...
            Text = title;
            _patNum = patNum;
            _listPermissions = new List<Permissions>(listPermissions);
            _fKey = fKey;
        }
        ...
        private void FormAuditOneType_Load(object sender, System.EventArgs e)
        {
            ...
            if (_listPermissions.Contains(Permissions.ProcFeeEdit))
            {
            ...
            else if (_listPermissions.Contains(Permissions.InsPlanChangeCarrierName))
            {
            ...
            else if (_listPermissions.Contains(Permissions.RxEdit))
            {
            ...
            else if (_listPermissions.Contains(Permissions.OrthoChartEditFull))
            {
            ...
            else if (_listPermissions.Contains(Permissions.ImageEdit)
                || _listPermissions.Contains(Permissions.ImageDelete)
                || _listPermissions.Contains(Permissions.ImageCreate)
                || _listPermissions.Contains(Permissions.ImageExport))
            {
            ...
            else if (_listPermissions.Contains(Permissions.EhrMeasureEventEdit))
            {
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < securityLogArray.Length; i++)
            {
                ...
                if (userod == null)
                {//Will be null for audit trails made by outside entities that do not require users to be logged in.  E.g. Web Sched.
