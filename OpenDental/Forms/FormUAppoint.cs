...
namespace OpenDental
{
    ...
    public partial class FormUAppoint : FormODBase
    {
        ...
        private List<ProgramProperty> _propertyList;
        ...
        private void FillForm()
        {
            ...
            _propertyList = ProgramProperties.GetForProgram(ProgramCur.ProgramNum);
            textUsername.Text = GetProp("Username");
            textPassword.Text = GetProp("Password");
            for (int i = 0; i < _propertyList.Count; i++)
            {
                if (_propertyList[i].IsHighSecurity)
                {
                    _propertyList[i].TagOD = _propertyList[i].PropertyValue;
                }
            }
            if (!ProgramProperties.CanEditProperties(_propertyList))
            {
        ...
        private string GetProp(string desc)
        {
            for (int i = 0; i < _propertyList.Count; i++)
            {
                if (_propertyList[i].PropertyDesc == desc)
                {
                    if (_propertyList[i].IsMasked)
                    {
                        return CDT.Class1.TryDecrypt((_propertyList[i].PropertyValue));
                    }
                    return _propertyList[i].PropertyValue;
                }
            }
        ...
        private bool SaveToDb()
        {
            ...
            for (int i = 0; i < _propertyList.Count; i++)
            {
                if (_propertyList[i].IsHighSecurity && ProgramProperties.GetPropVal(ProgramCur.ProgramNum, _propertyList[i].PropertyDesc) != _propertyList[i].TagOD.ToString())
                {
                    string logText = $"{ProgramCur.ProgDesc}+'s {_propertyList[i].PropertyDesc} for headquarters was altered.";
