...
namespace OpenDental
{
    ...
    public partial class FormInsPlan : FormODBase
    {
        ...
        public FormInsPlan(InsPlan insPlan, PatPlan patPlan, InsSub insSub)
        {
            ...
            if (CultureInfo.CurrentCulture.Name.Length >= 4 && CultureInfo.CurrentCulture.Name.Substring(3) == "GB")
            {//en-GB
        ...
        private void FormInsPlan_Load(object sender, System.EventArgs e)
        {
            ...
            if (insVerifyBenefits != null && insVerifyBenefits.DateLastVerified.Year > 1880)
            {//Only show a date if this insurance has ever been verified
        ...
        private void FillFormWithPlanCur(bool isPicked)
        {
            ...
            {//Select the default claim form if no selection (always for new plans)
        ...
        private void butPick_Click(object sender, EventArgs e)
        {
            ...
            if (formInsPlans.InsPlanSelected.PlanNum == 0)
            {//user clicked Blank
            ...
            FillBenefits();
            if (IsNewPlan || formInsPlans.InsPlanSelected.PlanNum == 0)
        ...
        private void textElectID_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (StringArrayElectIDs.Length == 0)
        ...
        private bool DropClickHelper()
        {
            ...
            {
                if (listClaims[j].PlanNum != _insPlan.PlanNum)
                {//different insplan
                    continue;
                }
                if (listClaims[j].DateService != DateTime.Today)
                {//not today
                    continue;
                }
        ...
        private void FillBenefits()
        {
            ...
            {
                row = new GridRow();
                if (_listBenefits[i].PatPlanNum == 0)
                {//attached to plan
                    row.Cells.Add("");
                }
                else
        ...
        {
            if (IsNewPlan && _insPlan.PlanNum != _insPlanOld.PlanNum)
            {  //If adding a new plan and picked existing plan from list
                ...
                if (etrans != null)
                {
                    ...
                    if (formEtrans270Edit.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        for (int i = 0; i < listDTP271Dates.Count; i++)
                        {
                            ...
                            if (listDTP271Dates[i].Segment.Get(1) == "347")
                            {//347 => Plan End
        ...
        private bool IsEmployerValid()
        {
            ...
            {//no employer was previously entered.
                ...
                {
                    if (!Security.IsAuthorized(Permissions.InsPlanEdit, true))
                    {//Employer was changed and they don't have perms to make new insplan (they picked plan from list).
                     //Validate plan's employer in DB
                        ...
                        if (hasExistingEmployerChanged)
                        {//not a new insplan, and the employer was changed compared to what's in the DB.
            ...
            {//an employer was previously entered
                ...
                {
                    if (!Security.IsAuthorized(Permissions.InsPlanEdit, true))
                    {//Employer is now empty.  Need to see if the insplan in DB also has empty employer, or if someone else put one on it.
                        if (hasExistingEmployerChanged)
                        {//Not a new insplan and employer was changed
                            MsgBox.Show(this, "The Employer for this insurance plan has been changed since the plan was loaded.  Please choose another insurance plan.");
                            return false;
                        }
                    }
                ...
                {//Employer text was changed since the window was loaded (picked from list or manually edited)
                    if (!Security.IsAuthorized(Permissions.InsPlanEdit, true))
                    {//Without permission, they must have picked from list.  Verify employer still exists.  If it does, verify the insplan still has same employer.
                        Employer employerDB = Employers.GetByName(textEmployer.Text);
                        if (employerDB == null)
                        {
                            MsgBox.Show(this, "The Employer for this insurance plan has been combined or deleted since the plan was loaded.  Please choose another insurance plan.");
                    ...
                    {//Are authorized
                        if (_employerNameCur == textEmployer.Text)
                        { //They picked from list and didn't change it manually.
                            Employer employerDB = Employers.GetByName(textEmployer.Text);
        ...
        private bool IsCarrierValid()
        {
            ...
            if (_carrier.CarrierNum != _carrierNumOrig && Carriers.Compare(carrierForm, _carrier))
            {//Carrier was changed via "Pick From List" and not edited manually
                if (Security.IsAuthorized(Permissions.InsPlanEdit, true))
                {
                    if (Carriers.GetCarrierDB(_carrier.CarrierNum) == null && !MsgBox.Show(this, MsgBoxButtons.YesNo, "The Carrier selected has been combined or deleted since it was last picked.  Would you like to override those changes?"))
                    {//Someone deleted/combined the carrier while the window was open.
                        return false;
                    }
                    if (hasExistingCarrierChanged && !MsgBox.Show(this, MsgBoxButtons.YesNo, "The selected insurance plan has had its Carrier changed since it was loaded.  Would you like to override those changes?"))
                    {//Someone changed this insplan's carrier while the window was open.
                        return false;
                    }
                }
                else
            ...
            else if (!Carriers.Compare(carrierForm, _carrier))
            {//Carrier edited manually (doesn't matter if it was picked from list or not, user without perms can't edit manually)
                if (hasExistingCarrierChanged && !MsgBox.Show(this, MsgBoxButtons.YesNo, "The selected insurance plan has had its Carrier changed since it was loaded.  Would you like to override those changes?"))
                {//Someone changed this insplan's carrier while the window was open.
                    return false;
                }
            ...
            else if (_insPlanOld.PlanNum != _insPlan.PlanNum)
            {//Plan was picked from list
                if (Security.IsAuthorized(Permissions.InsPlanEdit, true))
                {
            ...
            {//Carrier information is the same from when it was loaded into the form and "Pick From List" wasn't used to change information.
        ...
        private bool FillPlanCurFromForm()
        {
            ...
            _insPlan.DivisionNo = textDivisionNo.Text;//only visible in Canada
                                                      //carrier-----------------------------------------------------------------------------------------------------
            if (Security.IsAuthorized(Permissions.InsPlanEdit, true))
            {//User has the ability to edit carrier information.  Check for matches, create new Carrier if applicable.
                Carrier carrierForm = GetCarrierFromForm();
            ...
            {//User does not have permission to edit carrier information.  
             ...
             //If it's removed, have them choose another.  If it's simply changed, just use the same prikey.
                if (Carriers.GetCarrier(_carrier.CarrierNum).CarrierName == "" && _insPlan.PlanNum != _insPlanOld.PlanNum)
                {//Carrier not found, it must have been deleted or combined
                    MsgBox.Show(this, "Selected carrier has been combined or deleted.  Please choose another insurance plan.");
                    return false;
                }
                else if (_insPlan.PlanNum == _insPlanOld.PlanNum)
                {//Didn't switch insplan, they were only viewing.
            ...
            {//Hidden fee schedule selected in comboFeeSched
            ...
            else if (comboCopay.SelectedIndex == -1)
            {//CopayFeeSched is hidden or switching to/from PPO Fixed Benefits plan
            ...
            else if (comboOutOfNetwork.SelectedIndex == -1)
            {//Hidden fee schedule selected in comboAllowedFeeSched
        ...
        private bool CheckForReceivedClaims()
        {
            ...
            int claimCount = 0;
            if (patNum == 0)
            {//Editing insurance plans from Lists > Insurance Plans.
            ...
            {//Editing insurance plans from Family module.
                if (radioChangeAll.Checked == true)
                {//Check radio button
                    claimCount = Claims.GetCountReceived(_insPlanOriginal.PlanNum);
                    if (claimCount != 0)
                    {//Check all claims for plan
                ...
                {//Check claims for plan and patient only
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                if (_insPlan.CarrierNum != _insPlanOriginal.CarrierNum)
                {
                    ...
                    if (carrierNameOrig != carrierNameNew)
                    {//The CarrierNum could have changed but the CarrierName might not have changed.  Only warn the name changed.
            ...
            try
            {
                ...
                if (_insSubOld != null)
                {//Editing an old plan for a subscriber.
            ...
            try
            {
                ...
                if (_patPlan != null)
                {
                    if (PIn.Long(textOrdinal.Text) != _patPlan.Ordinal)
                    {//Ordinal changed by user
            ...
            if (Security.IsAuthorized(Permissions.InsPlanEdit, true))
            {
                ...
                {//editing from big list.  No subscriber.  'pick from list' button not visible, making logic easier.
                    ...
                    try
                    {
                        if (InsPlans.AreEqualValue(_insPlan, _insPlanOriginal))
                        {//If no changes
                ...
                {//(subCur!=null) editing from within patient
                 ...
                 //So we always compare with PlanNumOriginal.
                    if (IsNewPlan)
                    {
                        ...
                        {//New plan, no changes
                            ...
                            try
                            {
                                if (_insPlan.PlanNum != _insPlanOld.PlanNum)
                            ...
                            { //catch any other exceptions and display
            ...
            {//User does not have the InsPlanEdit permission.
                if (_insSub != null)
                {
                    if (IsNewPlan)
                    {
                        ...
                        {
                            if (_insPlan.PlanNum != _insPlanOld.PlanNum)
                            {//user clicked the "pick from list" button. 
            ...
            try
            {
                ...
                if (_listBenefitsOld.Count > 0 || _listBenefits.Count > 0)
                {//Synch benefits
                ...
                {//Update SubCur if needed
                    ...
                    if (_insSubOld.Subscriber != _insSub.Subscriber)
                    {//If the subscriber was changed, include an audit trail entry
        ...
        private bool DoShowBluebookDeletionMsgBox()
        {
            ...
            if (!isBluebookOn)
            {//Bluebook feature must be on to warn user.
                return false;
            }
            if (IsNewPlan && _insPlanOld.PlanNum == _insPlanOriginal.PlanNum)
            {//Never have to warn user because the plan is new and was not picked from the list.
                return false;
            }
            if (_insPlanOriginal.PlanType != "")
            {//Not Category Percentage plan so not bluebook eligible regardless if fee schedule changes.
                return false;
            }
