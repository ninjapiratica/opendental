...
namespace OpenDental
{
    public partial class FormEtrans270Edit : FormODBase
    {
        ...
        private List<EB271> RemoveDuplicateBenefitsFrom271(List<EB271> listEB271s)
        {
            ...
            {
                if (listEB271s[i].Benefitt == null)
                { //if Benefitt is null, we want to check the EB271's ToString method while ignoring free-form text.
                    if (!listEBStrings.Contains(listEB271s[i].ToString(false)))
                    { //If we've seen this string already, skip it. If we haven't, add it and keep note of it.
                        listEBStrings.Add(listEB271s[i].ToString(false));
                        listEB21sDistinct.Add(listEB271s[i]);
                    }
                }
                else
                { //Benefitt is not null, so we want to check Benefitt's ToString.
                    if (!listEBStrings.Contains(listEB271s[i].Benefitt.ToString()))
                    { //If we've seen this string already, skip it. If we haven't, add it and keep note of it.
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            {//generated benefit
                if (_listEB271s[e.Row].Benefitt == null)
                {//create new benefit
                    ...
                    if (formBenefitEdit.BenefitCur == null)
        ...
        private void butImport_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < gridMain.SelectedIndices.Length; i++)
            {
                ...
                if (_isDependent && benefit.CoverageLevel != BenefitCoverageLevel.Family)
                {//Dependent level benefit, set all benefits as patient overrides.
        ...
        private void butPrint_Click(object sender, EventArgs e)
        {
            ...
            if (isClaimConnect && string.IsNullOrEmpty(_htmlResponse))
            {
                ...
                string x12response = "";
                try
                {//Similar to x270Controller.RequestBenefits().
            ...
            if (radioModeElect.Checked)
            {//gridMain is wider for Electronic Import viewing mode, print/view in landscape to fit.
