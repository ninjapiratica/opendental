    ...
    public partial class FormPayPlanDynamic : FormODBase
    {
        ...
        private void FormPayPlanDynamic_Load(object sender, System.EventArgs e)
        {
            ...
            if (_dynamicPaymentPlanData.PayPlan.IsNew)
            {
                ...
                if (_sumAttachedProduction != 0)
                {//New plan may have attached production if created from ortho case.
            ...
            {//already saved payment plan, user needs to unlock before they can edit anything.
            ...
            if (!_hasGoToButtons)
            {//Can't go to pat because another window (ex: OrthoCase) will still be open.
        ...
        private void gridCharges_RightClickDelete(object sender, EventArgs e)
        {
            ...
            if (index == -1)
            {//Should not happen, menu item is only enabled when exactly 1 row is selected.
                return;
            }
        ...
        private void FillProduction()
        {
            ...
            for (int i = 0; i < _dynamicPaymentPlanData.ListPayPlanProductionEntries.Count; i++)
            {
                GridRow row = new GridRow();
                if (_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].CreditDate == DateTime.MinValue)
                {
                ...
                {
                    row.Cells.Add(_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].CreditDate.ToShortDateString());
                }
                row.Cells.Add(_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].ProductionDate.ToShortDateString());
                row.Cells.Add(Providers.GetAbbr(_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].ProvNum));
                if (PrefC.HasClinicsEnabled)
                {
                    row.Cells.Add(Clinics.GetAbbr(_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].ClinicNum));
                }
                row.Cells.Add(_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].Description);
                row.Cells.Add(_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].AmountOriginal.ToString("f"));
                if (_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].AmountOverride == 0)
                {
                    row.Cells.Add("");//if no override was entered cell should be blank.
                }
                else
                {
                    row.Cells.Add(_dynamicPaymentPlanData.ListPayPlanProductionEntries[i].AmountOverride.ToString("f"));
                }
                row.Tag = _dynamicPaymentPlanData.ListPayPlanProductionEntries[i];
        ...
        private bool TryGetTermsFromUI(out PayPlanTerms payPlanTerms, bool isSilent = false, bool doValidateTerms = true)
        {
            ...
            if (doValidateTerms && !ValidateTerms(isSilent))
            {//saveData relies on this, if removed from this method, needs to be addded to SaveData()
                return false;
            }
        ...
        private void CheckProductionLock_CheckedChanged(object sender, EventArgs e)
        {
            ...
            else if (checkProductionLock.Checked)
            {
                ...
                {
                    if (CreateSchedule())
                    {//create schedule button will get disabled once this box is checked so we need to do this for the user 
                        butAddProd.Enabled = false;
                        butDeleteProduction.Enabled = false;
                    }
                    else
        ...
        {
            if (ValidateTerms(doCheckAPR: false))
            {//Don't need to validate APR and full lock because we will auto-lock when APR is set (only for creating schedule).
        ...
        {
            if (gridCharges.ListGridRows[e.Row].Tag == null)
            {//Prevent double clicking on the "Current Totals" row
                return;
            }
            ...
            else if (rowData.IsPaymentRow())
            {
                ...
                {//Get changes to the payment from DB
            ...
            else if (gridCharges.ListGridRows[e.Row].Tag.GetType() == typeof(DataRow))
            {//Claim payment or bundle.
        ...
        private void butSignPrint_Click(object sender, EventArgs e)
        {
            ...
            //save signature
            if (_dynamicPaymentPlanData.PayPlan.Signature == "")
            {//clear signature and hide sigbox if blank sig was saved
        ...
        private bool SaveData(bool isPrinting = false, bool isUiValid = true)
        {
            ...
            if (!isUiValid || !TryGetTermsFromUI(out PayPlanTerms terms))
            {//also validates terms
                return false;
            }
