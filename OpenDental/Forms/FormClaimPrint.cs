    ...
    public partial class FormClaimPrint : FormODBase
    {
        ...
        public bool PrintImmediate(string auditDescription, PrintSituation printSituation, long auditPatNum, bool doUseLastPrinterSettingsIfAvailable = false)
        {
            if (DoPrintBlank)
            {
                ...
                {//Could happen if printing a blank form and no default claim form is set.
        ...
        {//raised for each page to be printed.
            if (!TryFillDisplayStrings(false))
            {//if failed
            ...
            {
                if (ClaimFormCur.Items[i].ImageFileName == "")
                {//field
        ...
        private bool TryFillDisplayStrings(bool isRenaissance)
        {
            ...
            if (family.GetIndex(_insSub.Subscriber) == -1)
            {//from another family
            ...
            {//Patient for this InsSub could not be found.  Likely db corruption.
            ...
            {//if secondary insurance exists
                if (family.GetIndex(insSub2.Subscriber) == -1)
                {//from another family
            ...
            for (int j = listStringsMissingTeeth.Count - 1; j >= 0; j--)
            ...
            for (int i = 0; i < ClaimFormCur.Items.Count; i++)
            {
                ...
                switch (ClaimFormCur.Items[i].FieldName)
                {
                    ...
                    case "SubscrPhone"://needs work.  Only used for 1500
                        if (isRenaissance)
                        {
                            ...
                            if (patientSubscriber.HmPhone.Length > 14)
                    ...
                    case "PatIDFromPatPlan": //Dependant Code for Canada
                        listPatPlans = PatPlans.Refresh(PatNum);
                        if (CultureInfo.CurrentCulture.Name.EndsWith("CA") && _carrier.ElectID == "000064")
                        {//Canadian. en-CA or fr-CA and Pacific Blue Cross
                    ...
                    case "BillingDentistNPI":
                        ...
                        if (CultureInfo.CurrentCulture.Name.EndsWith("CA") && //Canadian. en-CA or fr-CA
                            ...
                            _stringArrayDisplay[i].Length == 9)
                    ...
                    case "PayToDentistAddress": //Behaves just like the old BillingDentistAddress field, but is overridden by the Pay-To address if the Pay-To address has been specified.
                        ...
                        else if (PrefC.GetString(PrefName.PracticePayToAddress) != "")
                    ...
                    case "MedAccidentCode": //For UB04.
                        ...
                        else if (_claim.AccidentRelated == "E")
                ...
                if (CultureInfo.CurrentCulture.Name == "nl-BE" && _stringArrayDisplay[i] == "")
                {//Dutch Belgium
                    _stringArrayDisplay[i] = "*   *   *";
                }
        ...
        private void FillMedInsStrings()
        {
            ...
            for (int i = 0; i < _listClaimProcsForClaim.Count; i++)
            {
                for (int j = 0; j < _listClaimProcsForPat.Count; j++)
                {
                    ...
                    { //Medicaid
                        ...
                        if (strInsFilingCodeClaimProc == "16" || strInsFilingCodeClaimProc == "MB")
            ...
            for (int i = 0; i < ClaimFormCur.Items.Count; i++)
            {
                ...
                {
                    if (strInsFilingCode == "MB")
                    { //Medicare Part B
                        _stringArrayDisplay[i] = "XOVR";
                    }
                }
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "Name")
                { //MedInsAName, MedInsBName, MedInsCName
                    _stringArrayDisplay[i] = Carriers.GetName(_insPlan.CarrierNum);
                }
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "PlanID")
                { //MedInsAPlanID, MedInsBPlanID, MedInsCPlanID
                  //Not used. Leave blank.
                }
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "RelInfo")
                { //MedInsARelInfo, MedInsBRelInfo, MedInsCRelInfo
                    _stringArrayDisplay[i] = _insSub.ReleaseInfo ? "X" : "";
                }
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "AssignBen")
                ...
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "AmtDue")
                ...
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "OtherProvID")
                ...
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "InsuredName")
                ...
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "Relation")
                ...
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "InsuredID")
                { //MedInsAInsuredID, MedInsBInsuredID, MedInsCInsuredID
                    _stringArrayDisplay[i] = _insSub.SubscriberID;
                }
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "GroupName")
                { //MedInsAGroupName, MedInsBGroupName, MedInsCGroupName
                    _stringArrayDisplay[i] = _insPlan.GroupName;
                }
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "GroupNum")
                { //MedInsAGroupNum, MedInsBGroupNum, MedInsCGroupNum
                    _stringArrayDisplay[i] = _insPlan.GroupNum;
                }
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "AuthCode")
                { //MedInsAAuthCode, MedInsBAuthCode, MedInsCAuthCode
                    _stringArrayDisplay[i] = _claim.PreAuthString;
                }
                else if (ClaimFormCur.Items[i].FieldName == "MedIns" + insLine + "Employer")
        ...
        private int GetToothRangeCount(string toothNums)
        {
            ...
            if (listStringsToothNums.Count <= 1)
            {//Proc had 0 or 1 toothNum specified
                return -1;
            }
            else
            {//The procedure has a toothrange, return the number of teeth.
        ...
        {
            if (!butPrint.Enabled)
            {//if the claim has been deleted and the print button has been disabled.
