...
namespace OpenDental
{
    public partial class FormXchargeTokenTool : FormODBase
    {
        ...
        private void FilterCardList()
        {
            ...
            textVerified.Text = "0";
            textInvalid.Text = "0";
            for (int i = _listCreditCards.Count - 1; i >= 0; i--)
            {//looping backwards to remove cards that are valid
                Program prog = Programs.GetCur(ProgramName.Xcharge);
                string path = Programs.GetProgramPath(prog);
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            gridMain.BeginUpdate();
            for (int i = gridMain.ListGridRows.Count - 1; i > -1; i--)
            {//loop through backwards and remove any cards that are no longer in the list
                CreditCard ccGrid = (CreditCard)gridMain.ListGridRows[i].Tag;
                if (cc.PatNum != ccGrid.PatNum)
                {
                    continue;
                }
                if (!listCardsForPat.Any(x => x.CreditCardNum == ccGrid.CreditCardNum))
                {//this row is one of the cards for the patient
