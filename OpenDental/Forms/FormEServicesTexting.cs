...
namespace OpenDental
{
    ...
    public partial class FormEServicesTexting : FormODBase
    {
        ...
        private void FillGridSmsUsage()
        {
            ...
            { //No clinics so just get the practice as a clinic.
                listClinics.Clear();
            ...
            if (listClinics.Count > 1)
            {//Total row if there is more than one clinic (Will not display for practice because practice will have no clinics.
                GridRow row = new GridRow();
                row.Cells.Add("");
                row.Cells.Add("");
                row.Cells.Add("");
