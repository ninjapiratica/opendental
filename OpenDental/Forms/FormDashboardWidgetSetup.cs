...
namespace OpenDental
{
    public partial class FormDashboardWidgetSetup : FormODBase
    {
        ...
        {
            if (gridCustom.SelectedCell.X == -1 || gridCustom.SelectedCell.Y == -1)
            {//Invalid cell.
                return;
            }
            if (gridCustom.SelectedCell.X == _colAllowed)
            {//Do not open the edit window when double clicking the 'Allowed' column.
                return;
            }
        ...
        private void gridCustom_CellClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (gridCustom.SelectedCell.X != _colAllowed)
            {//Not setting/unsetting permission.
                return;
            }
