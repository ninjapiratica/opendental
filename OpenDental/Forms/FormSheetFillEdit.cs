...
namespace OpenDental
{
    ...
    public partial class FormSheetFillEdit : FormODBase
    {
        ...
        private void FormSheetFillEdit_Load(object sender, EventArgs e)
        {
            ...
            if (_isAutoSave && !IsInTerminal)
            {//only visible if the Autosave Form usage has been set and is not in kiosk mode
            ...
            catch (Exception ex)
            {
                ...
                {//The user does not have the permission. Show them the message and return.
            ...
            if (SheetCur.IsNew && SheetCur.SheetType != SheetTypeEnum.PaymentPlan)
            {//payplan does not get saved to db so sheet is always new
                butChangePat.Enabled = false;
                return;
            }
        ...
        private string LayoutFields()
        {
            ...
            for (int i = 0; i < SheetCur.SheetFields.Count; i++)
            {
                ...
                if (SheetCur.SheetFields[i].FontSize <= 0)
                {//Sheet did not save correctly.
            ...
            for (int i = 0; i < SheetCur.SheetFields.Count; i++)
            {
                ...
                if (SheetCur.SheetFields[i].FieldValue.Length > 0)
        ...
        private void FieldValueChanged(object sender)
        {
            ...
            if (sender.GetType() == typeof(SheetCheckBox))
            {
                ...
                { //set some other radiobuttons to be not checked
                    ...
                    if (sheetFieldThis.FieldName != sheetFieldOther.FieldName)
            ...
            if (sender.GetType() != typeof(RichTextBox) && sender.GetType() != typeof(System.Windows.Controls.RichTextBox))
            {//One for WinForms one for WPF
            ...
            if (calcH <= sheetField.Height)
            {//no growth needed.  If this is ever removed then SheetUtil.MoveAllUpBelowThis() needs to be considered below.
                return;
            }
            ...
            for (int i = 0; i < panelMain.Controls.Count; i++)
            {
                ...
                {//Pasting can throw off the WPF RTB and it just doesn't matter enough to crash the system for this.
                    ...
                    else if (panelMain.Controls[i].GetType() == typeof(System.Windows.Forms.Integration.ElementHost))
                    {//This will never be hit, since we are no longer using WPF RichTextBox. Leaving for documentation
        ...
        private void RefreshPanel()
        {
            ...
            for (int i = 0; i < listSheetFieldsOutput.Count; i++)
            { //rectangles around specific output fields
            ...
            for (int i = 0; i < listSheetFieldsSigBox.Count; i++)
            { //rectangles around specific output fields
        ...
        private void butPrint_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetOutputFormat.DialogResult != DialogResult.OK)
            {
                ...
                if (hasSheetFromDb)
                {//Only re-layout the fields if we actually changed SheetCur to a new object from the db.
                    LayoutFields();
                }
                return;
            }
        ...
        private string EmailSheet(string addr_to, string subject)
        {
            ...
            if (!Security.IsAuthorized(Permissions.EmailSend, false))
            {//Still need to return after printing, but not send emails.
                DialogResult = DialogResult.OK;
                Close();
                return "";
            }
        ...
        private bool VerifyRequiredFields()
        {
            ...
            for (int i = 0; i < panelMain.Controls.Count; i++)
            {
                ...
                else if (panelMain.Controls[i].GetType() == typeof(SheetCheckBox))
                {//Radio button groups or misc checkboxes
                    SheetField sheetField = (SheetField)panelMain.Controls[i].Tag;
                    if (sheetField.IsRequired && sheetField.FieldValue != "X")
        ...
        private void SaveSignaturePayPlan()
        {
            ...
            for (int i = 0; i < panelMain.Controls.Count; i++)
            {
                ...
                {//invalid sig. Do not save signature.
            ...
            if ((payPlan.IsNew || hasSigChanged) && isValidSig)
            {
                ...
                if (category == 0)
                {
                    ...
                    else if (listImageCatDefsLong.Count != 0)
        ...
        private void butSave_Click(object sender, EventArgs e)
        {
            ...
            if (SheetCur.SheetType == SheetTypeEnum.PaymentPlan)
            {//Payment plan saves and closes the window.
            ...
            else if (SheetCur.SheetType == SheetTypeEnum.ExamSheet)
            {//Exam sheet saves and keeps the window open
                ...
                if (TryToSaveData())
                {//If saved successful, show saved on the button.
                    ShowSaved();
                }
            }
        }
