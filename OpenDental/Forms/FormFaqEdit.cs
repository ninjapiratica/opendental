...
namespace OpenDental
{
    public partial class FormFaqEdit : FormODBase
    {
        ...
        {
            if (FaqCur.IsNew && !IsQuickAdd)
            {//Creating a new faq, no data to load
                return;
            }
            if (IsQuickAdd)
            {//We're going to have a new FAQ we're creating but we do have a new FAQ object that won't have a bunch of data set yet.
        ...
        private bool IsValid()
        {
            ...
            if (!String.IsNullOrEmpty(stringBuilderErrMsg.ToString()))
            {//Errors found, inform user
        ...
        {
            if (textAnswer.SelectedText.Length > 0)
            ...
            else if (doInsertWhenNoSelection)
            {//No text selected, insert tags at cursor position
        ...
        private void ButOK_Click(object sender, EventArgs e)
        {
            ...
            if (_listVersions == null || _listVersions.Count == 0)
            {//This could be the case if we are in a quick add. We know the version text isn't empty because IsValid didn't return false.
            ...
            {//Users can only edit 1 Faq for a specific version at a time.
