    ...
    public partial class FormCCDPrint : FormODBase
    {
        ...
        private void Init()
        {
            ...
            { //Some transactions are not patient specific.
                ...
                if (_claim == null)
                {//for eligibility or when the claim was deleted by the customer before receiving the response from the mailbox.
                 //Get primary info
            ...
            {//Version 02
             //Since there is no FormID field in version 02, we figure out what the formId should be based on the transaction type.
                if (_transactionCode == "10")
                {//Eligibility Response.
                    _formId = "08";//Eligibility Form
                }
                else if (_transactionCode == "11")
                {//Claim Response.
                    _formId = "03";//Claim Acknowledgement Form
                }
                else if (_transactionCode == "21")
                {//EOB
                    _formId = "01";//EOB Form
            ...
            { //Show the form on screen if there are no copies to print.
                ...
                if (_responseStatus != "R" && _formId == "05")
                { //Manual claim form
            ...
            {
                if (_responseStatus != "R" && _formId == "05")
                { //Manual claim form (CDA claim form)
                    if (ODBuild.IsDebug())
                    {
        ...
        private void butOverride_Click(object sender, EventArgs e)
        {
            ...
            if (_claim == null)
            {//When claim is null then claimprocs is also null.  See Init() for when claim can be null.
        ...
        private void PrintDentaide(Graphics g)
        {
            ...
            { //Claim ACK or EOB
              //Retreive the predetermination number for this claim from the transaction history.
                ...
                {//Etrans entries are chronological, so we find the most recent one starting at the end of the list.
                    if (listEtrans[i].Etype == EtransType.PredetermEOB_CA)
            ...
            switch (Canadian.GetRelationshipCode(relatPri))
            {//Field C03
                case "1":
            ...
            {
                _text = "";
                switch (Canadian.GetRelationshipCode(_patPlanSec.Relationship))
                {//Field E06
                    case "1":
                        _text = _isFrench ? "Époux(se)" : "Spouse";
                        break;
            ...
            {
                _text = "";
                switch (_patient.CanadianEligibilityCode)
                {//Field C09
        ...
        private void PrintProcedureListEOB(Graphics g)
        {
            ...
            for (int p = 0; p < ccdFieldsArrayProcedureLineNumbers.Length; p++)
            {
                ...
                List<Procedure> listProceduresLab = Procedures.GetCanadianLabFees(procedure.ProcNum, listProcedures);
                if (listProceduresLab.Count > 0 && ccdFieldsArrayEligibleLabAmounts1.Length > 0)
                ...
                if (listProceduresLab.Count > 1 && ccdFieldsArrayEligibleLabAmounts2.Length > 0)
                {//In version 2 the lab fee is rolled into the procedure amount and so there is no lab section.
        ...
        private SizeF PrintDentistPhone(Graphics g, float X, float Y)
        {
            ...
            if (_text.Length == 10 && TelephoneNumbers.IsFormattingAllowed)
        ...
        private void PrintPaymentSummary(Graphics g)
        {
            ...
            else if (payableTo == "4" || payableTo == "0")
            {//Dentist
        ...
        private void PrintMissingToothList(Graphics g)
        {
            ...
            if (ccdFieldInitialPlacementUpper != null && ccdFieldInitialPlacementLower != null &&
                ...
                (ccdFieldInitialPlacementLower.valuestr == "Y" || ccdFieldInitialPlacementLower.valuestr == "O"))
            {//Only print extractions when F15 or F18 are "Yes"
                ...
                int j = 0;
                for (int i = 0; i < _listProcedureExtracted.Count; i++)
                {//Count specified by field F22
                    if (j % cycleOrthoDateCount == 0)
                    {
                        _x = _documentGenerator.StartElement();
                    }
                    if (IsValidDate(_listProcedureExtracted[i].ProcDate))
        ...
        private void PrintNoteList(Graphics g)
        {
            ...
            _documentGenerator.StartElement(_verticalLine);
            for (int i = 0; i < ccdFieldsArrayNoteTexts.Length; i++)
            {//noteTexts.Length<=32
                if (i < ccdFieldsArrayNoteOutputFlags.Length)
