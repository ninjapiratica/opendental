...
namespace OpenDental
{
    ...
    public partial class FormApptEdit : FormODBase
    {
        ...
        private void FormApptEdit_Load(object sender, System.EventArgs e)
        {
            ...
            {//Can happen if appointment was deleted by another WS.
            ...
            else
            {
                ...
                if (_appointment.AptStatus != ApptStatus.Complete && !Security.IsAuthorized(Permissions.AppointmentEdit)
                    || (_appointment.AptStatus == ApptStatus.Complete && !Security.IsAuthorized(Permissions.AppointmentCompleteEdit)))
                ...
                if (_appointment.AptStatus != ApptStatus.Complete && !Security.IsAuthorized(Permissions.AppointmentDelete, suppressMessage: true))
                {//Suppress message because it would be very annoying to users.
                    butDelete.Enabled = false;
                }
                if (_appointment.AptStatus == ApptStatus.Complete && !Security.IsAuthorized(Permissions.AppointmentCompleteDelete, suppressMessage: true))
                {//Suppress message because it would be very annoying to users.
                    butDelete.Enabled = false;
                }
            }
            if (!Security.IsAuthorized(Permissions.ApptConfirmStatusEdit, suppressMessage: true))
            {//Suppress message because it would be very annoying to users.
                comboConfirmed.Enabled = false;
            }
            else if (_isSchedulingUnscheduledAppt)
            ...
            int selectedIndex = -1;
            if (IsNew && _appointmentTypeSelected != null)
            { //_appointmentTypeSelected will be null if they didn't select anything.
        ...
        private void butPickHyg_Click(object sender, EventArgs e)
        {
            ...
            formProviderPick.ProvNumSelected = comboProvHyg.GetSelectedProvNum();
            formProviderPick.ShowDialog();
            if (formProviderPick.DialogResult != DialogResult.OK)
            {
                return;
            }
            comboProvHyg.SetSelectedProvNum(formProviderPick.ProvNumSelected);
            SetTimeSliderColors();
        }
        ...
        private void butLab_Click(object sender, EventArgs e)
        {
            ...
            {//no labcase
             //so let user pick one to add
                ...
                if (_isPlanned)
                {
                    LabCases.AttachToPlannedAppt(formLabCaseSelect.LabCaseNumSelected, _appointment.AptNum);
                }
                else
                {
                    LabCases.AttachToAppt(formLabCaseSelect.LabCaseNumSelected, _appointment.AptNum);
                }
            }
            else
        ...
        private void butDeleteProc_Click(object sender, EventArgs e)
        {
            ...
            if (PrefC.GetBool(PrefName.ApptsRequireProc))
            {
                ...
                {//Will be empty if there is only the current appointment in the grid.
            ...
            if (!MsgBox.Show(this, MsgBoxButtons.OKCancel, deleteMsg))
            {//deleteMsg can only be of two strings, okay to use in MsgBox.Show()
                return;
            }
            int skipped = 0;
        ...
        private bool AptTypeHelper()
        {
            ...
            if (listProcedureCodesApptType.Count > 0)
            {//AppointmentType is associated to procs.
                ...
                for (int i = 0; i < listProcedureCodesApptType.Count; i++)
                {
                    ...
                    if (listProceduresSelectedApptType.Count > 0)
                    { // If procedures with this code were already selected, preserve those selections
        ...
        private bool DoPreventCompletedApptProcChange(Procedure proc, out string msg)
        {
            msg = "";
            if (proc == null)
            {//Explicitly checking proc that was passed in because it is required.
        ...
        private void FillComm()
        {
            ...
            for (int i = 0; i < _tableComms.Rows.Count; i++)
            {
                ...
                if (!checkShowCommAuto.Checked && isCommlogAutomated)
                { //Skip automated commlogs if not checked.
                    continue;
                }
                row = new GridRow();
        ...
        private void FillProcedures()
        {
            ...
            if (_isOnLoad && !_isInsertRequired)
            {//First time filling the grid and not a new appointment.
                ...
                {//for eCW only and only if not in 'Revise' mode, select completed procs from _listProcedureForAppointments with ProcDate==AptDateTime
            ...
            for (int i = 0; i < listProcedures.Count; i++)
            {
                ...
                for (int j = 0; j < listDisplayFieldsAppts.Count; j++)
                {
                    switch (listDisplayFieldsAppts[j].InternalName)
                    {
                        ...
                        case "Description":
                            string descript = "";
                            if (listProcedures[i].ProcNumLab != 0)
                            {//Proc is a Canadian Lab.
        ...
        {
            if (_appointment.AptStatus == ApptStatus.Planned)
            {//Planned is not a selectable choice in the comboStatus box.
            ...
            else if (_appointment.AptStatus == ApptStatus.PtNote || _appointment.AptStatus == ApptStatus.PtNoteCompleted)
            {
                ...
                {//Ptnote selected from comboStatus.
                    return ApptStatus.PtNote;
                }
                return ApptStatus.PtNoteCompleted;
            }
            else if (comboStatus.SelectedIndex == 3)
            {//Broken is SelectedIndex 3 but enum value 5, so when selected, we return ApptStatus.Broken.
                return ApptStatus.Broken;
            }
            else
            {//Scheduled, Complete, and Unscheduled are SelectedIndex 0,1,2 but enum values 1,2,3, so we index by 1 and return the appropriate ApptStatus.
        ...
        private string GetLabCaseDescript()
        {
            ...
            if (laboratory != null)
            {  //Laboratory won't be set if the program closed in the middle of creating a new lab entry.
            ...
            if (_labCase.DateTimeChecked.Year > 1880)
            {//Logic from Appointments.cs lines 1098 to 1117
        ...
        private void InvertCurProcSelected(int index)
        {
            ...
            {//Canadian. en-CA or fr-CA
                ...
                if (procedureSelected.ProcNumLab == 0)
                {//Not a lab, but could be a parent to a lab.
                    for (int i = 0; i < gridProc.ListGridRows.Count; i++)
                    {
                        Procedure proc = (Procedure)gridProc.ListGridRows[i].Tag;
                        if (proc.ProcNumLab == procedureSelected.ProcNum)
                        {//Is lab of selected procedure.
                            listIndicies.Add(i);
                        }
                    }
                }
                ...
                {//Is a lab.
                    for (int i = 0; i < gridProc.ListGridRows.Count; i++)
                    {
                        ...
                        if (procedure.ProcNum == procedureSelected.ProcNumLab)
                        {//Parent of selected lab.
                            listIndicies.Add(i);
                        }
        ...
        private void OnDelete_Click(bool isSkipDeletePrompt = false)
        {
            ...
            {//ordinary appointment
                ...
                if (_appointment.AptNum > 0 && HieClinics.IsEnabled())
                {//Ignore new appointment delete
        ...
        private bool UpdateListAndDB(bool isClosing = true, bool doCreateSecLog = false, bool doInsertHL7 = false)
        {
            ...
            if (_appointmentOld.AptStatus != ApptStatus.UnschedList && comboStatus.SelectedIndex == 2)
            ...
            if (_appointment.AptStatus != ApptStatus.Complete && (comboProv.GetSelectedProvNum() != _appointmentOld.ProvNum || comboProvHyg.GetSelectedProvNum() != _appointmentOld.ProvHyg))
        ...
        {
            if (_appointment == null)
            {//Could not find _appointment in the Db on load.
                return;
            }
