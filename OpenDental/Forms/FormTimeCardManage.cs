...
namespace OpenDental
{
    public partial class FormTimeCardManage : FormODBase
    {
        ...
        private GridOD FillMain(bool isForGridPrint = false)
        {
            ...
            for (int i = 0; i < MainTable.Rows.Count; i++)
            {
                ...
                if (PrefC.GetBool(PrefName.TimeCardsUseDecimalInsteadOfColon))
                {
                    ...
                    row.Cells.Add(PIn.Time(MainTable.Rows[i]["rate3OTHours"].ToString()).TotalHours.ToString("n"));
                    row.Cells.Add(PIn.Time(MainTable.Rows[i]["PTOHours"].ToString()).TotalHours.ToString("n"));
                ...
                {//Colon format with seconds
                    ...
                    row.Cells.Add(PIn.Time(MainTable.Rows[i]["rate3OTHours"].ToString()).ToStringHmmss());
                    row.Cells.Add(PIn.Time(MainTable.Rows[i]["PTOHours"].ToString()).ToStringHmmss());
                ...
                {//Colon format without seconds
                    ...
                    row.Cells.Add(PIn.Time(MainTable.Rows[i]["rate3OTHours"].ToString()).ToStringHmm());
                    row.Cells.Add(PIn.Time(MainTable.Rows[i]["PTOHours"].ToString()).ToStringHmm());
        ...
        private GridOD GetGridForPrinting(Employee emp)
        {
            ...
            if (_timeAdjustNote != null)
            { //a note row exists for this pay period.
            ...
            TimeSpan rate3span = new TimeSpan(0);//rate3 hours total
            ...
            rate3Time = rate3span.ToStringHmm();
            ...
            rate3Time2 = rate3span.TotalHours.ToString("n");
        ...
        private void PrintTimeCard(object sender, System.Drawing.Printing.PrintPageEventArgs e, Employee employeeCur, int pagesToPrint)
        {
            ...
            yPos += 16;
            g.DrawString(Lan.g(this, "Rate 3 Time") + ": " + rate3Time + " (" + rate3Time2 + ")", fontHeader, brush, xPos, yPos);
            yPos += 16;
        ...
        private void butExportADP_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < MainTable.Rows.Count; i++)
            {
                ...
                string r3OThours = (PIn.TSpan(MainTable.Rows[i]["rate3OTHours"].ToString())).TotalHours.ToString("F2");//adp allows 2 digit precision
                if (r3OThours == "0.00")
                {
                    r3OThours = "";
                }
                string ptoHours = (PIn.TSpan(MainTable.Rows[i]["PTOHours"].ToString())).TotalHours.ToString("F2");//adp allows 2 digit precision
                if (ptoHours == "0.00")
                {
                    ptoHours = "";
                }
                string textToAdd = "";
                if (r1hours != "" || r1OThours != "" || ptoHours != "")
        ...
        private void butExportADPRun_Click(object sender, EventArgs e)
        {
            ...
            strb.AppendLine("IID,Pay Frequency,Pay Period Start Date,Pay Period End Date,Employee ID,Earnings Code,Pay Hours,Dollars,Separate Check,Worked In Dept,Rate Code");
            ...
            for (int i = 0; i < MainTable.Rows.Count; i++)
            {
                ...
                if (r3OThours != "0.00")
                {
                    strb.AppendLine(textToAdd + "OVT," + r3OThours + "," + dollars + "," + separateCheck + "," + workedDepartment + ",RATE_3");
                }
                string ptoHours = (PIn.TSpan(row["PTOHours"].ToString())).TotalHours.ToString("F2");
                if (ptoHours != "0.00")
                {
                    strb.AppendLine(textToAdd + "VAC," + ptoHours + "," + dollars + "," + separateCheck + "," + workedDepartment + ",BASE");
                }
                #endregion
            }//end file contents
            using FolderBrowserDialog fbd = new FolderBrowserDialog();
