...
namespace OpenDental
{
    ...
    public partial class FormConfirmList : FormODBase
    {
        ...
        private void FillMain()
        {
            ...
            if (comboShowRecall.SelectedIndex == 0 || comboShowRecall.SelectedIndex == 3)
            {//All or Hygiene Prescheduled
        ...
        private void butPostcards_Click(object sender, System.EventArgs e)
        {
            ...
            if (!isDialogOk)
            { //dialog result not OK, postcards weren't sent to the printer.
                return;
            }
            for (int i = 0; i < _tableAddresses.Rows.Count; i++)
            { //loop through the address table and create commlog entries for all selected.
        ...
        private void pdCards_PrintPage(object sender, PrintPageEventArgs ev)
        {
            ...
            while (yPos < ev.PageBounds.Height - bottomPageMargin && _patientsPrinted < _tableAddresses.Rows.Count)
            {
                ...
                {
                    if (!PrefC.HasClinicsEnabled || PIn.Long(_tableAddresses.Rows[_patientsPrinted]["ClinicNum"].ToString()) == 0)
        ...
        private void butEmail_Click(object sender, EventArgs e)
        {
            ...
            if (gridMain.SelectedIndices.Length == 0)
            {
                ...
                for (int i = 0; i < _tableAppointments.Rows.Count; i++)
                {
                    ...
                    if (_tableAppointments.Rows[i]["confirmed"].ToString() == Defs.GetName(DefCat.ApptConfirmed, PrefC.GetLong(PrefName.ConfirmStatusEmailed)))
            ...
            for (int i = 0; i < gridMain.SelectedIndices.Length; i++)
            {
                ...
                {//build the list of appointments a family has scheduled
                ...
                catch (Exception ex)
                {
                    ...
                    if (!errors.Contains("Message send fail for Patnum:" + emailMessage.PatNum + ":  " + ex.Message))
            ...
            Cursor = Cursors.Default;
            if (listPatNumsFailed.Count == gridMain.SelectedIndices.Length)
            { //all failed
              //no need to refresh
            ...
            else if (listPatNumsFailed.Count > 0)
            {//if some failed
                FillMain();
        ...
        private void butText_Click(object sender, EventArgs e)
        {
            ...
            {//None selected. Select all of type text that are not yet confirmed by text message.
                ...
                for (int i = 0; i < _tableAppointments.Rows.Count; i++)
                {
                    ...
                    if (_tableAppointments.Rows[i]["confirmed"].ToString() == Defs.GetName(DefCat.ApptConfirmed, PrefC.GetLong(PrefName.ConfirmStatusTextMessaged)))
                    {//Already confirmed by text
                        continue;
                    }
                    if (!_tableAppointments.Rows[i]["contactMethod"].ToString().StartsWith("Text:"))
            ...
            for (int i = gridMain.SelectedIndices.Length - 1; i >= 0; i--)
            {
                ...
                if (YNtxtMsgOk == YN.Unknown && PrefC.GetBool(PrefName.TextMsgOkStatusTreatAsNo))
                {//Check if OK to text
                    skipped++;
                    gridMain.SetSelected(gridMain.SelectedIndices[i], false);
                    continue;
                ...
                if (PrefC.HasClinicsEnabled && SmsPhones.IsIntegratedTextingEnabled())
                {//using clinics with Integrated texting must have a non-zero clinic num.
                    ...
                    if (clinicNum == 0 || Clinics.GetClinic(clinicNum).SmsContractDate.Year < 1880)
                    {//no clinic or assigned clinic is not enabled.
                        skipped++;
                        gridMain.SetSelected(gridMain.SelectedIndices[i], false);
                        continue;
                    }
            ...
            for (int i = 0; i < gridMain.SelectedIndices.Length; i++)
            {
                ...
                {//There was an exception thrown in FormTME.SendText() meaning something went wrong.  Give the user an option to stop sending messages.
