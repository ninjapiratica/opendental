...
namespace OpenDental
{
    public partial class FormOrthoChart : FormODBase
    {
        ...
        private void FormOrthoChart_Load(object sender, EventArgs e)
        {
            ...
            if (_userNumLocked == _userodCurUser.UserNum)
            {//we will usually have control
            ...
            {//Tab index hasn't changed, fill the grid.
        ...
        {
            if (_isReadOnly)
            {//Don't bother using locking logic, locking ability has been disabled.
                return;
            }
        ...
        private void timerLocked_Tick(object sender, EventArgs e)
        {
            //Every 4 seconds.
            if (_isReadOnly)
            {//Don't bother using timer logic, locking ability has been disabled.
                return;
            }
            long userNum = PatientNotes.GetUserNumOrthoLocked(_patient.PatNum);
            if (userNum == -5)
            {//-5 indicates that an instance of OD changed users locally to that form.
                return;
            }
            ...
            if (userNum == 0)
            {//Someone else just closed their ortho chart for this patient.
        ...
        private void buttonTakeControl_Click(object sender, EventArgs e)
        {
            ...
            long userNum = PatientNotes.GetUserNumOrthoLocked(_patient.PatNum);
            if (userNum == -5)
            {//We are taking control from a workstation that changed users to sign an orthochartrow.
            ...
            if (userNum == _userodCurUser.UserNum)
            {//Current user already has lock on different workstation.
        ...
        private void gridMain_CellClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (textUser.Text != _userodCurUser.UserName || !_isLockedByMe)
            {//Don't allow signature to be overwritten if someone else previously signed without first changing users.
        ...
        private void SaveSignatureToDict(int idxRow)
        {
            ...
            DateTime dateTime = orthoChartRow.DateTimeService;
            if (_changedUser)
            {//Changed user to sign signature box, so update the selected orthochart row to the changed user's UserNum.
        ...
        {
            if (gridPat.ListGridRows[e.Row].Tag is PatFieldDef)
            ...
            else if (gridPat.ListGridRows[e.Row].Tag is PatField)
        ...
        {
            using FormOrthoChartAddDate formOrthoChartAddDate = new FormOrthoChartAddDate();
            formOrthoChartAddDate.ShowDialog();
            if (formOrthoChartAddDate.DialogResult != DialogResult.OK)
            {
                return;
            }
            ...
            OrthoChartRow orthoChartRow = new OrthoChartRow();
            orthoChartRow.PatNum = _patient.PatNum;
            orthoChartRow.DateTimeService = formOrthoChartAddDate.DateSelected;
            orthoChartRow.ProvNum = formOrthoChartAddDate.ProvNumSelected;
            orthoChartRow.UserNum = _userodCurUser.UserNum;
            orthoChartRow.Signature = "";
            orthoChartRow.IsNew = true;
            OrthoChartRows.Insert(orthoChartRow);
        ...
        private void butChangeUser_Click(object sender, EventArgs e)
        {
            ...
            if (userNumNew == userNumOld)
            {//Current user is the same as the changed user. Don't update DB.
        ...
        {
            if (_isReadOnly)
            {//Form was opened in read only mode so don't warn about unsaved changes.
                DialogResult = DialogResult.Cancel;
                return;
            }
            if (_hasChanged || signatureBoxWrapper.SigChanged)
        ...
        private void FormOrthoChart_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (_isReadOnly)
            {//Clicking cancel or [X] sets DialogResult.Cancel so we shouldn't get here, but just in case.
                return;
            }
