...
namespace OpenDental
{
    public partial class FormXWeb : FormODBase
    {
        ...
        private double _originalPayAmt;
        ...
        public FormXWeb(long patNum, CreditCard creditCard, XWebTransactionType tranType, bool createPayment, double originalPayAmt = 0)
        {
            InitializeComponent();
            ...
            _originalPayAmt = originalPayAmt;
        }
        ...
        private void FormXWeb_Load(object sender, EventArgs e)
        {
            ...
            if (!CompareDouble.IsZero(_originalPayAmt))
            {
                textAmount.Text = _originalPayAmt.ToString();
            }
            if (LockCardInfo)
            {
        ...
        private bool VerifyData()
        {
            ...
            if (_originalPayAmt != 0 && CompareDouble.IsGreaterThan(Math.Abs(PIn.Double(textAmount.Text)), Math.Abs(_originalPayAmt)))
            {
