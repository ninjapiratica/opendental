...
namespace OpenDental
{
    public partial class FormBackup : FormODBase
    {
        ...
        private void FormBackup_Load(object sender, System.EventArgs e)
        {
            ...
            {//Archive not supported over MT connection. Show warning if Middle Tier is in use.
        ...
        private void butBackup_Click(object sender, System.EventArgs e)
        {
            ...
            if (!Directory.Exists(ODFileUtils.CombinePaths(textBackupFromPath.Text, dbName)))
            ...
            if (!hasDriveSpace(textBackupToPath.Text, dbSize))
            { // ensure drive space before attempting backup
        ...
        private bool hasDriveSpace(string backupToPath, double fileSize)
        {
            ...
            {
                if ((ulong)fileSize * 1024 * 1024 >= driveFreeSpace)
                {//dbSize is in megabytes, cast to ulong to compare. It will never be negative so this is safe.
                    return false;
                }
            }
            return true;
        ...
        private void InstanceMethodDatabaseBackup(string databaseName, string backupFromPath, string backupToPath, double databaseSize, bool isInnoDb)
        {
            ...
            if (Directory.Exists(dbBackupToPath))
            {// D:\opendental
                int loopCount = 1;
            ...
            for (int i = 0; i < fileInfoArray.Length; i++)
            {
                ...
                {
                    if (fileInfoArray[i].LastWriteTime != File.GetLastWriteTime(toFile))
        ...
        {
            if (!Directory.Exists(ODFileUtils.CombinePaths(backupToPath, aToZDirectory)))
            {// ex. D:\OpenDentalData
                try
                {
        ...
        private void InstanceMethodRestore(string aToZFullPath, string restoreFromPath)
        {
            ...
            if (!Directory.Exists(aToZFullPath))
            {// C:\OpenDentalData\
        ...
        private void butRestore_Click(object sender, System.EventArgs e)
        {
            ...
            if (!Directory.Exists(ODFileUtils.CombinePaths(textBackupRestoreFromPath.Text, dbName)))
            {// D:\opendental
                MessageBox.Show(Lan.g(this, "Restore FROM path is invalid.  Unable to find folder named ") + dbName);
                return;
            }
            if (!Directory.Exists(ODFileUtils.CombinePaths(textBackupRestoreToPath.Text, dbName)))
            {// C:\mysql\data\opendental
            ...
            if (ShouldUseAtoZFolder())
            {
                ...
                string aToZDirectory = aToZFullPath.Substring(aToZFullPath.LastIndexOf(Path.DirectorySeparatorChar) + 1);// OpenDentalData
                if (!Directory.Exists(ODFileUtils.CombinePaths(textBackupRestoreFromPath.Text, aToZDirectory)))
                {// D:\OpenDentalData
            ...
            if (Directory.Exists(ODFileUtils.CombinePaths(textBackupRestoreToPath.Text, newDb)))
        ...
        private void butArchive_Click(object sender, EventArgs e)
        {
            ...
            { //We only need to validate the backup settings if the user wants to make a backup first
                ...
                if (string.IsNullOrWhiteSpace(PrefC.GetString(PrefName.ArchiveKey)))
                {//If archive key isn't set, generate a new one.
