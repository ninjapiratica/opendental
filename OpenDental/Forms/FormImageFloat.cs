    ...
    public partial class FormImageFloat : FormODBase
    {
		...
		*/
        ...
        public FormImageFloat()
        {
            ...
            if (!ODBuild.IsWeb())
            {//WebView2 does not work on Cloud
            ...
            {//For cloud, instead use CloudIframe.
        ...
        public List<MountItem> GetAvailSlots(int countNeed = -1)
        {
            ...
            {//Second loop to catch items lower than the first loop
                idxItem = 0;
        ...
        public void SelectTreeNode(NodeTypeAndKey nodeTypeAndKey, string localPathImportedCloud = "")
        {
            ...
            if (nodeTypeAndKey.NodeType == EnumImageNodeType.Mount)
            {
                ...
                if (listMissingMountNums.Count > 0)
                {//Notify user of any files that were unable to load
        ...
        public void SetWindowingSlider()
        {
            ...
            if (IsDocumentShowing())
            {
                if (GetDocumentShowing(0).WindowingMax == 0)
                {//default
            ...
            {
                if (_documentArrayShowing[_idxSelectedInMount].WindowingMax == 0)
                {//default
                    windowingEventArgs.MinVal = 0;
                    windowingEventArgs.MaxVal = 255;
                }
        ...
        public void ToolBarCopy_Click()
        {
            ...
            else if (IsDocumentShowing())
            {
                ...
                if (ImageHelper.HasImageExtension(GetDocumentShowing(0).FileName) || GetDocumentShowing(0).FileName.EndsWith(".dcm"))
                {
                    ...
                    if (!ImageHelper.HasImageBeenEdited(GetDocumentShowing(0)))
                    {//Do a file copy since no changes were made
        ...
        public void ToolBarDelete_Click()
        {
            ...
            for (int i = 0; i < documentArrayInMountDb.Length; i++)
            {
                ...
                if (document != null)
                {//this document from db is showing to user, so no concurrency issue.
                    continue;
                }
        ...
        public void ToolBarPaste_Click()
        {
            ...
            if (bitmapPaste == null)
            {//if no bitmap on clipboard, try to get file names
                try
                {
        ...
        private void panelMain_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (_drawMode == EnumDrawMode.Line)
            {
                PointF pointFPenInBitmap = ControlPointToBitmapPointF(e.Location);
                if (_isLineStarted)
                {//if this is the second click to finish a line that was already started.
                    if (_idxImageDraw == -1)
                    {
                        return;
                    }
        ...
        private void panelMain_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (_drawMode == EnumDrawMode.Text)
            {
                ...
                {//not dragging. Just change cursor.
                    panelMain.Cursor = Cursors.IBeam;
                    int idxText = HitTestText(e.Location);
                    if (idxText != -1)
                    {
            ...
            if (_drawMode == EnumDrawMode.Line)
            {
                ...
                if (_listImageDraws != null && _idxImageDraw != -1 && _listImageDraws.Count > _idxImageDraw)
                {//ensure big enough list for index
        ...
        private void panelMain_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (_drawMode == EnumDrawMode.Text)
            {
                ...
                if (_idxTextMouseDown != -1)
                {//releasing from a drag, click, or double click
                    if (_isDraggingText)
                    {//excludes the click and double click
            ...
            if (_drawMode == EnumDrawMode.Line)
            {
                ...
                {//treat this as the mouse up of a drag
                    if (_idxImageDraw == -1)
                    {
            ...
            _isMouseDownPanel = false;
            if (GetSelectedType() == EnumImageNodeType.Mount && isClick && _cropPanAdj == EnumCropPanAdj.Pan)
            {//Adj is handled on mouse down 
            ...
            if (GetSelectedType() == EnumImageNodeType.Mount && !isClick && _cropPanAdj == EnumCropPanAdj.Adj && _isDraggingMount)
            {
                ...
                if (idxNewPos != _idxSelectedInMount && _documentArrayShowing[idxNewPos] != null)
                {
                    ...
                    if (_documentArrayShowing[_idxSelectedInMount].ToothNumbers == _listMountItems[_idxSelectedInMount].ToothNumbers)
                    {//only if unchanged by user
                    ...
                    if (_documentArrayShowing[idxNewPos].ToothNumbers == _listMountItems[idxNewPos].ToothNumbers)
                    {//only if unchanged by user
                ...
                if (idxNewPos != _idxSelectedInMount)
                {
                    ...
                    if (movedPosNoCrop)
                    {//not adding crop, so nothing else to do
                        _isDraggingMount = false;
                        ThumbnailRefresh();
                        panelMain.Invalidate();
                        return;
        ...
        private void DownloadDocumentNoteFile(NodeTypeAndKey nodeTypeAndKey)
        {
            ...
            if (!document.Note.StartsWith(downloadPrefix) && !document.Note.StartsWith(base64Prefix))
            { //only process notes that contain a url or base64
                return;
            }
            ProgressOD progressOD = new ProgressOD();
        ...
        private void DrawMount(Graphics g)
        {
            ...
            for (int i = 0; i < _listMountItems.Count; i++)
            {
                if (_isDraggingMount && i == _idxSelectedInMount)
                {// && _dateTimeMouseDown.AddMilliseconds(600) < DateTime.Now){//_pointDragStart!=_pointDragNow){
            ...
            if (_isDraggingMount && _idxSelectedInMount > -1)
            {// && _dateTimeMouseDown.AddMilliseconds(600) < DateTime.Now){//_pointDragStart!=_pointDragNow){
        ...
        private void DrawMountOne(Graphics g, int i, bool translateToItemPos = true)
        {
            ...
            if (_listMountItems[i].TextShowing != "")
            {
                ...
                using Font font = new Font(FontFamily.GenericSansSerif, _listMountItems[i].FontSize);
            ...
            for (int i = 0; i < _listImageDraws.Count; i++)
            {
                ...
                if (_listImageDraws[i].DrawType == ImageDrawType.Text)
                {
                    ...
                    Rectangle rectangleText = new Rectangle(pointText.X - circleDiameter, pointText.Y - circleDiameter,
                        widthText + circleDiameter, font.Height + circleDiameter);
                    if (!rectangleText.Contains(Point.Round(pointF)))
                    {
                        continue;
                    }
        ...
        {
            if (_nodeTypeKeyCatSelected == null || _nodeTypeKeyCatSelected.ImageNodeType == EnumImageNodeType.None || _nodeTypeKeyCatSelected.DefNumCategory == 0)
            {//No Image Category selected.
        ...
        private int HitTestText(Point pointPanel)
        {
            for (int i = 0; i < _listImageDraws.Count; i++)
            {
                ...
                float sizeFont = _listImageDraws[i].FontSize * ZoomSliderValue / 100f;
                ...
                Rectangle rectangleText = new Rectangle(pointTextInPanel.X, pointTextInPanel.Y, widthText, font.Height);
        ...
        private async void LoadPdf(string atoZFolder, string atoZFileName, string localPath, string downloadMessage)
        {
            try
            {
                ...
                else
                {
                    ...
                    _odWebView2.CoreWebView2.Navigate(_odWebView2FilePath);
                    _isExportable = true;
                    IsImageFloatLocked = true;
                }
            }
        ...
        private void ToolBarImportMount()
        {
            ...
            if (Prefs.GetContainsKey(nameof(PrefName.UseAlternateOpenFileDialogWindow)) && PrefC.GetBool(PrefName.UseAlternateOpenFileDialogWindow))
            {//Hidden pref, almost always false.
        ...
        {
            if (Plugins.HookMethod(this, "ContrImages.ToolBarImport_Click_Start", PatientCur))
            {//Named differently for backwards compatibility
                EventFillTree?.Invoke(this, true);
                return;
            }
            OpenFileDialog openFileDialog = new OpenFileDialog();
