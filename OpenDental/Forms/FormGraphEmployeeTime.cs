    ...
    public partial class FormGraphEmployeeTime : FormODBase
    {
        ...
        private void RefreshDataForDay()
        {
            ...
            for (int i = 0; i < _listSchedules.Count; i++)
            {
                ...
                {//no phone graph entry found (likely for a future date which does not have entries created yet OR past date where current employee didn't work here yet)
                    if (_dateShowing <= DateTime.Today)
                    ...
                    {//we will default to PhoneEmpDefault.IsGraphed so make sure the default exists
                        continue;
                    }
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < _listPhoneEmpDefaultsRaw.Count; i++)
            {
                ...
                {
                    if (!_listSchedules.Exists(x => x.EmployeeNum == _listPhoneEmpDefaultsRaw[i].EmployeeNum))
                    {//no normal sched
                        if (phoneGraph == null || phoneGraph.DateTimeStart1.Year < 1880)
                        {//and no override added
                            continue;
                        }
                    }
                }
                ...
                {
                    if (!_listPhoneEmpDefaultsRaw[i].IsGraphed)
                    {//maybe include a check for override?
                        continue;
                    }
                }
                if (textTime.Text != "")
            ...
            for (int i = 0; i < _listPhoneEmpDefaultsShowing.Count; i++)
            {
                ...
                else
                {
                    ...
                    {//override different from default
