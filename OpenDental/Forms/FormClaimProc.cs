    ...
    public partial class FormClaimProc : FormODBase
    {
        ...
        public void Initialize()
        {
            ...
            if (_claimProc.ClaimNum > 0)
            {
                ...
                else if ((claim.ClaimStatus == "S" || claim.ClaimStatus == "R"))
                {//sent or received 
            ...
            if ((butOK.Enabled || butDelete.Enabled) && NoPermissionProc)
            ...
            if (_claimProc.ClaimPaymentNum > 0)
            ...
            {
                IsProc = true;
                _listBenefits = Benefits.RefreshForPlan(_claimProc.PlanNum, _patPlanNum);
                if (_procedure == null)
                {
            ...
            {
                if (!Security.IsAuthorized(Permissions.InsPayCreate, true))
                { //user not allowed to create an insurance payment
                    textInsPayAmt.ReadOnly = true;
                }
            }
            else
            {
                if (!Security.IsAuthorized(Permissions.InsPayEdit, _claimProc.DateCP, true))
                { //user not allowed to edit an insurance payment
                    textInsPayAmt.ReadOnly = true;
                }
            }
            if (!Security.IsAuthorized(Permissions.InsWriteOffEdit, dateWriteoffSec, true))
            {//user not allowed to edit/create a writeoff
            ...
            else if (_claimProc.PlanNum > 0 && _claimProc.Status.In(ClaimProcStatus.CapEstimate, ClaimProcStatus.CapComplete))
            ...
            if (CultureInfo.CurrentCulture.Name.EndsWith("CA"))
            {
                ...
                {//We're loading this form with a lab procedure selected.
        ...
        private void FillInitialAmounts()
        {
            if (IsProc)
            {
                ...
                {//if ppo
                    ...
                    {//if standard fee is greater than ins fee for a PPO plan, show standard fee sched
        ...
        private void textCopayOverride_KeyUp(object sender, System.Windows.Forms.KeyEventArgs e)
        {
            ...
            if (textCopayOverride.Text != "")
            {//If override is specified, use that amount instead
        ...
        {
            if (!textInsPayAmt.ReadOnly)
            {//If this box is readonly, show them the security warning that disabled it when the user clicks into it.
                return;
            }
            if (CompareDouble.IsZero(_claimProc.InsPayAmt))
            {
        ...
        {
            if (!textWriteOff.ReadOnly)
            {//In this window if the box is readonly when the user clicks into it show them the security warning that disabled it.
                return;
            }
        ...
        {
            if (!textWriteOffEstOverride.ReadOnly)
            {//In this window if the box is readonly when the user clicks into it show them the security warning that disabled it.
                return;
            }
            DateTime dateWriteoffSec = _claimProc.SecDateEntry;
        ...
        private void MakeAuditTrailEntries()
        {
            ...
            {//Could happen with pay "As Total". Still want to log, just without procedure.
