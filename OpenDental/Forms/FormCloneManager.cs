...
namespace OpenDental
{
    ...
    public partial class FormCloneManager : FormODBase
    {
        ...
        private List<Patient> _listPatientsClones;
        ...
        {
            _listPatientsClones = Patients.GetAllPatients(); //change this to get a list of all patients WITH possible clones
            ...
            GridRow row;
            for (int i = 0; i < _listPatientsClones.Count; i++)
            {
                row = new GridRow();
                row.Cells.Add(_listPatientsClones[i].FName.ToString());
                row.Cells.Add(_listPatientsClones[i].LName.ToString());
                row.Cells.Add(_listPatientsClones[i].MiddleI.ToString());
                row.Cells.Add(_listPatientsClones[i].Gender.ToString());
                row.Cells.Add(_listPatientsClones[i].Birthdate.ToShortDateString());
                row.Cells.Add(Providers.GetLongDesc(Patients.GetProvNum(_listPatientsClones[i])));
                row.Cells.Add(Providers.GetLongDesc(_listPatientsClones[i].SecProv));
                row.Tag = _listPatientsClones[i];
