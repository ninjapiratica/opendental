...
namespace OpenDental
{
    ...
    public partial class FormClinics : FormODBase
    {
        ...
        private void butDown_Click(object sender, EventArgs e)
        {
            ...
            Clinic clinicDest = ((Clinic)gridMain.ListGridRows[selectedIdx + 1].Tag);
            if (clinicSource.ItemOrder == clinicDest.ItemOrder)
            {//just in case, an issue in the past would cause ItemOrder inconsitencies, shouldn't happen
                clinicSource.ItemOrder++;
            }
            else
            {
        ...
        {
            if (DoIncludeHQInList)
            {//always keep the HQ clinic at the top if it's in the list
                if (x.ClinicNum == 0)
                {
                    return -1;
                }
            ...
            if (retval == 0)
            {//if Abbr's are alphabetically the same or ItemOrder's are the same, order alphabetical by Description
                retval = x.Description.CompareTo(y.Description);
            }
            if (retval == 0)
            {//if Abbrs/ItemOrders are the same and Descriptions are alphabetically the same, order by ClinicNum (guaranteed deterministic)
