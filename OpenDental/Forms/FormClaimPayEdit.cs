...
namespace OpenDental
{
    ...
    public partial class FormClaimPayEdit : FormODBase
    {
        ...
        {
            if (IsNew)
            {//security for new and finalized claim payments already checked before this form opens
                textAmount.Select();
            }
            else
            {
        ...
        private Deposit GetDepositCur()
        {
            ...
            if (_depositOld != null)
            {//Maintain the values of the existing deposit if it exists
        ...
        private void CheckUIState()
        {
            ...
            if (progXcharge == null || progPayConnect == null || progPaySimple == null || progEdgeExpress == null)
            {//Should not happen.
            ...
            if (!progPayConnect.Enabled && !progXcharge.Enabled && !progPaySimple.Enabled && !progEdgeExpress.Enabled)
            {//if none enabled
                ...
                && !PIn.Bool(ProgramProperties.GetPropVal(progPayConnect.ProgramNum, PayConnect.ProgramProperties.PayConnectPreventSavingNewCC, clinicNum)))
            {
                ...
                {//if clinics are disabled, PayConnect button will be visible if PayConnect has been enabled in program links
                    butPayConnect.Visible = true;
                }
            }
                ...
                && !PIn.Bool(ProgramProperties.GetPropVal(progXcharge.ProgramNum, ProgramProperties.PropertyDescs.XCharge.XChargePreventSavingNewCC, clinicNum)))
            {
                ...
                {//if clinics are disabled, X-Charge button will be visible if X-Charge has been enabled in program links
                    panelXcharge.Visible = true;
                }
            }
                ...
                && !PIn.Bool(ProgramProperties.GetPropVal(progPaySimple.ProgramNum, PaySimple.PropertyDescs.PaySimplePreventSavingNewCC, clinicNum)))
            {
                ...
                {//if clinics are disabled, PaySimple button will be visible if PaySimple has been enabled in program links
                    butPaySimple.Visible = true;
                }
            }
                ...
                && !PIn.Bool(ProgramProperties.GetPropVal(progEdgeExpress.ProgramNum, EdgeExpressProps.PreventSavingNewCC, clinicNum)))
            {
                ...
                {//if clinics are disabled, EdgeExpress button will be visible if EdgeExpress has been enabled in program links
                    panelEdgeExpress.Visible = true;
                }
            }
        ...
        private void butDepositEdit_Click(object sender, EventArgs e)
        {
            ...
            if (formDepositEdit.DialogResult == DialogResult.OK)
            {//Made changes, update deposit values
            ...
            for (int i = 0; i < _listRequiredFields.Count; i++)
            {
                ...
                switch (_listRequiredFields[i].FieldName)
                {
                    ...
                    case RequiredFieldName.DepositDate:
                        if (!groupBoxDeposit.Visible)
                        {//don't set required if you can't see it
                            break;
                        }
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            else
            {
                ...
                if (IsFinalizePayment)
                {//finalizing a claim payment should use the InsPayCreate permission, not InsPayEdit
                    if (!Security.IsAuthorized(Permissions.InsPayCreate, PIn.Date(textDate.Text)))
                    {
                        return;
                    }
            ...
            if (comboPayGroup.SelectedIndex != -1)
            {//If they didn't select anything, leave what was originally there
            ...
            else
            {
                ...
                if (IsCreateLogEntry)
                { //need a InsPayCreate Log entry because it just was pre-inserted.
                    SecurityLogs.MakeLogEntry(Permissions.InsPayCreate, 0,
                        Lan.g(this, "Carrier Name: ") + ClaimPaymentCur.CarrierName + ", "
                        + Lan.g(this, "Total Amount: ") + ClaimPaymentCur.CheckAmt.ToString("c") + ", "
                        + Lan.g(this, "Check Date: ") + ClaimPaymentCur.CheckDate.ToShortDateString() + ", "//Date the check is entered in the system (i.e. today)
