...
namespace OpenDental
{
    public partial class FormPatientRaceEthnicity : FormODBase
    {
        ...
        private void FillRaceData()
        {
            ...
            if (_listCdcrecsRaceAll.Count > 1 && _listCdcrecsEthnicityAll.Count > 1)
            {//Greater than 1 because we always add 'Declined to Specify'
        ...
        private void FillTree(TreeView treeView, List<Cdcrec> _listCdcrecs)
        {
            ...
            for (int i = 0; i < _listCdcrecs.Count; i++)
            {
                ...
                if (!_listCdcrecs[i].CdcrecCode.StartsWith("ASKU"))
                {//If not Declined to Specify, include the CdcrecCode.
