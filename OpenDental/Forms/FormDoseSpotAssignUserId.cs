...
namespace OpenDental
{
    public partial class FormDoseSpotAssignUserId : FormODBase
    {
        ...
        private void FormDoseSpotAssignUserId_Load(object sender, EventArgs e)
        {
            ...
            if (_listUserods.Count == 0)
            {//empty list, populate with all users
        ...
        private void butUserPick_Click(object sender, EventArgs e)
        {
            ...
            formUserPick.ListUserodsFiltered = _listUserods;
            formUserPick.IsPickAllAllowed = false;
            formUserPick.ShowDialog();
            if (formUserPick.DialogResult != DialogResult.OK)
            {
                return;
            }
            _userNumSelected = formUserPick.SelectedUserNum;
            FillComboBox();
        }
