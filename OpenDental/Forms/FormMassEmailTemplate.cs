...
namespace OpenDental
{
    public partial class FormMassEmailTemplate : FormODBase
    {
        ...
        {
            if (_emailHostingTemplate.IsNew)
            {//Wont get hit as the button is set to not be visible if tempCur.IsNew=true;
        ...
        private bool Save()
        {
            ...
            //Create an API instance with the clinic num for this template.
            IAccountApi iAccountApi = EmailHostingTemplates.GetAccountApi(_emailHostingTemplate.ClinicNum);
            if (_emailHostingTemplate.IsNew || _emailHostingTemplate.TemplateId == 0)
            {//templates inserted during convert didn't have their Id's set, we need to create them.
