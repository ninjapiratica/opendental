...
namespace OpenDental
{
    public partial class FormClaimCustomTrackingUpdate : FormODBase
    {
        ...
        private void butUpdate_Click(object sender, EventArgs e)
        {
            ...
            { //when not called from FormRpOutstandingIns, this should only have one claim.
                ...
                if (claimTrackingCur.ClaimTrackingNum == 0)
                { //new claim tracking status.
                    ClaimTrackings.Insert(claimTrackingCur);
                }
                else
                { //existing claim tracking status
