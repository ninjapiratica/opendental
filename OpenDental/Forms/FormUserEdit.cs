...
namespace OpenDental
{
    ...
    public partial class FormUserEdit : FormODBase
    {
        ...
        private void FormUserEdit_Load(object sender, System.EventArgs e)
        {
            ...
            if (_listUserAlertTypesOld.Select(x => x.ClinicNum).Contains(-1))
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (PrefC.HasClinicsEnabled && checkClinicIsRestricted.Checked)
            ...
            try
            {
                ...
                if (UserClinics.Sync(listUserClinics, UserCur.UserNum))
            ...
            foreach (long alertCatNum in listUserAlertCats)
            {
                if (!PrefC.HasClinicsEnabled)
                {
                    if (!_listUserAlertTypesOld.Exists(x => x.AlertCategoryNum == alertCatNum))
                ...
                {//Clinics enabled.
                    ...
                    {
                        if (!_listUserAlertTypesOld.Exists(x => x.ClinicNum == clinicNumCur && x.AlertCategoryNum == alertCatNum))
                        {//Was not subscribed to type.
            ...
            isUserOdPrefCacheInvalid |= UserOdPrefs.Sync(_listDoseSpotUserPrefNew, _listDoseSpotUserPrefOld);
