...
namespace OpenDental
{
    ...
    public partial class FormLateCharges : FormODBase
    {
        ...
        private void FormLateCharges_Load(object sender, EventArgs e)
        {
            ...
            if (Defs.GetDefsForCategory(DefCat.BillingTypes, true).Count == 0)
        ...
        private void butSaveDefaults_Click(object sender, EventArgs e)
        {
            ...
            RoundDoubleFields();
            if (!AreLateChargeSettingsValid())
            {//Check again because a blank cell will not be considered invalid, even when the minimum value is not zero.
                MsgBox.Show(this, "Please fix data entry errors first.");
                return;
            }
        ...
        private void butRun_Click(object sender, EventArgs e)
        {
            ...
            if (listLateChargeAdjNums.IsNullOrEmpty())
            {//No adjustments were made so we don't want to run aging.
                DialogResult = DialogResult.OK;
                return;
            }
            if (RunAgingEnterprise(false))
