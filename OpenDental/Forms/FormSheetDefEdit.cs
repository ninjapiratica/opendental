...
namespace OpenDental
{
    ...
    public partial class FormSheetDefEdit : FormODBase
    {
        ...
        private string GetSelectedLanguageThreeLetters()
        {
            ...
            if (comboLanguages.SelectedIndex == 1)
            {//'Default' or no seleciton
                return "";
            }
        ...
        private string GetSelectedLanguageDisplay()
        {
            ...
            if (comboLanguages.SelectedIndex - 1 == 0)
            {//'Add New'
                return null;
            }
            if (comboLanguages.SelectedIndex == 1)
        ...
        private void butDelete_Click(object sender, EventArgs e)
        {
            ...
            if (!GetSelectedLanguageThreeLetters().IsNullOrEmpty())
            {//User clicked 'Delete' while viewing a translated view.
        ...
        private void butEdit_Click(object sender, EventArgs e)
        {
            ...
            FillFieldList();
            panelMain.Invalidate();
            if (_sheetDef.HasMobileLayout)
            { //Always open the mobile editor since they want to use the mobile layout.
                ShowMobile();
            }
        ...
        private void butPageRemove_Click(object sender, EventArgs e)
        {
            ...
            if (minimumPageCount > _sheetDef.PageCount)
            { //There are fields that have a YPos and heights that push them past the bottom of the page.
        ...
        private void comboLanguages_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            {//Null when 'Add New' clicked.
                if (!TryAddTranslation(out selectedThreeLetterLanguage))
                {//Open prompt to select a new language selection, returns false if user cancels.
            ...
            if (listSheetFieldDefsTranslatedFields.IsNullOrEmpty())
            {//New translation
        ...
        private void FormSheetDefEdit_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (_isCtrlDown && e.KeyCode == Keys.C)
            { //CTRL-C
                CopyControlsToMemory();
                return;
            }
            if (_isCtrlDown && e.KeyCode == Keys.V)
            { //CTRL-V
                PasteControlsFromMemory(new Point(0, 0));//this does panel.Invalidate, etc.
                return;
            }
            if (_isCtrlDown && e.KeyCode == Keys.Z)
            { //CTRL-Z
                butUndo_Click(this, new EventArgs());
                return;
            }
            if (_isCtrlDown && e.KeyCode == Keys.Y)
            { //CTRL-Y
        ...
        private void FormSheetDefEdit_Load(object sender, EventArgs e)
        {
            ...
            textDescription.Text = _sheetDef.Description;
            if (!TryInitLayoutModes())
            {//TryInitLayoutModes() must be called before initial FillFieldList().
        ...
        private void listFields_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (sheetFieldDef.TabOrder != SheetFieldDefOld.TabOrder)
        ...
        private void panelMain_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            else if (_gridSnapDistance > 0)
            {//Attempt to autosnap field to nearest cell top left corner.
                for (int i = 0; i < listBoxFields.SelectedIndices.Count; i++)
                {
                    SheetFieldDef sheetFieldDef = listSheetFieldDefsShowing[listBoxFields.SelectedIndices[i]];
        ...
        private void butOutputText_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldOutput.DialogResult != DialogResult.OK || formSheetFieldOutput.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butInputField_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldInput.DialogResult != DialogResult.OK || formSheetFieldInput.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butStaticText_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldStatic.DialogResult != DialogResult.OK || formSheetFieldStatic.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butCheckBox_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldCheckBox.DialogResult != DialogResult.OK || formSheetFieldCheckBox.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butComboBox_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldComboBox.DialogResult != DialogResult.OK || formSheetFieldComboBox.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butImage_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldImage.DialogResult != DialogResult.OK || formSheetFieldImage.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butPatImage_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldPatImage.DialogResult != DialogResult.OK || formSheetFieldPatImage.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butLine_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldLine.DialogResult != DialogResult.OK || formSheetFieldLine.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butRectangle_Click(object sender, EventArgs e)
        {
            ...
            if (formSheetFieldRect.DialogResult != DialogResult.OK || formSheetFieldRect.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void AddSignatureBox(SheetFieldType sheetFieldType)
        {
            ...
            if (formSheetFieldSigBox.DialogResult != DialogResult.OK || formSheetFieldSigBox.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
        ...
        private void butGrid_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                formSheetFieldGrid.SheetFieldDefCur = SheetFieldDef.NewGrid(formSheetFieldGridType.SheetGridTypeSelected, 0, 0, 100, 100); //is resized from dialog window.
            ...
            if (formSheetFieldGrid.DialogResult != DialogResult.OK || formSheetFieldGrid.SheetFieldDefCur == null)
            {//SheetFieldDefCur==null if it was Deleted
                return;
            }
            List<SheetFieldDef> listSheetFieldDefsPertinent = _sheetDef.SheetFieldDefs.FindAll(x => x.LayoutMode == _sheetFieldLayoutMode && x.Language == GetSelectedLanguageThreeLetters());
        ...
        private void AddNewSheetFieldDefsForTranslations(SheetFieldDef sheetFieldDefDefault, params string[] arrThreeLetterLanguages)
        {
            ...
            {
                if (arrThreeLetterLanguages[i].IsNullOrEmpty())
                {//Ignore 'Default' 
                    continue;
                }
            ...
            for (int i = 0; i < listBoxFields.SelectedIndices.Count; i++)
            {
                ...
                switch (sheetFieldDef.FieldType)
                {
                    ...
                    case SheetFieldType.CheckBox:
                        if (sheetFieldDef.FieldName != "misc")
        ...
        private bool DoRefreshImages()
        {
            ...
            for (int i = 0; i < listSheetFieldDefs.Count(); i++)
            {//any images altered
                SheetFieldDef sheetFieldDefImgOld = _listSheetFieldDefsDisplayedImages.FirstOrDefault(x => x.SheetFieldDefNum == listSheetFieldDefs[i].SheetFieldDefNum);
                if (sheetFieldDefImgOld is null)
                {
                    return true;
                }
                if (!IsEquivalentSheetDef(listSheetFieldDefs[i], sheetFieldDefImgOld))
                {
                    return true;
                }
            }
        ...
        private void FillFieldList()
        {
            ...
            for (int i = 0; i < _sheetDef.SheetFieldDefs.Count(); i++)
            {
                if (_sheetDef.SheetFieldDefs[i].LayoutMode != _sheetFieldLayoutMode || _sheetDef.SheetFieldDefs[i].Language != selectedLanguage)
                {//Currently both of these can not be set at the same time.
                    continue;
                }
        ...
        private List<SheetFieldDef> GetPertinentSheetFieldDefs(SheetDef sheetDef = null)
        {
            ...
            for (int i = 0; i < sheetDef.SheetFieldDefs.Count; i++)
            {
                ...
                if (sheetDef.SheetFieldDefs[i].Language != selectedLanguage)
                {//Currently both of these can not be set at the same time.
                    continue;
                }
        ...
        private bool HasSelectedDiagonalLine(SheetFieldDef sheetFieldDef, Rectangle rectangleCheck)
        {
            ...
            {//Step pixel by pixel
                ...
                {//Move along the y axis
                    ...
                    if (sheetFieldDef.Bounds.Height < 0)
                    {//Move backwards along the y axis instead
                        pointOnLine = new Point((int)Math.Round((-i / slope) + lineXStart, MidpointRounding.AwayFromZero), (int)lineYStart - i);
                    }
                }
                else
                {//Move along the x axis
                    ...
                    if (sheetFieldDef.Bounds.Width < 0)
                    {//Move backwards along the x axis instead
                        pointOnLine = new Point((int)lineXStart - i, (int)Math.Round((-i * slope) + lineYStart, MidpointRounding.AwayFromZero));
                    }
                }
        ...
        private bool HasTranslatedSheetDefS(SheetFieldDef sheetFieldDef, out List<SheetFieldDef> listSheetFieldDefsMatched)
        {
            ...
            if (sheetFieldDef.Language.IsNullOrEmpty())
            {//Method should only be called using 'Default' SheetFieldDefs
        ...
        private void LaunchEditWindow(SheetFieldDef sheetFieldDef, bool isEditMobile)
        {
            ...
            if (sheetFieldDefCopy.FieldType != SheetFieldType.MobileHeader)
            {
                ...
                else if (HasTranslatedSheetDefS(sheetFieldDefCopy, out List<SheetFieldDef> listSheetFieldDefsMatched))
                {//def was not deleted.
        ...
        private void PasteControlsFromMemory(Point pointOrigin)
        {
            ...
            if (pointOrigin.X == 0 && pointOrigin.Y == 0)
            { //allows for cascading pastes in the upper right hand corner.
                ...
                int maxH = 0;
                int maxW = 0;
                for (int i = 0; i < _listSheetFieldDefsCopyPaste.Count; i++)
        ...
        private bool PromptUpdateEClipboardSheetDefs()
        {
            ...
            inputBoxParam.ParamType = InputBoxType.CheckBox;
            inputBoxParam.LabelText = message;
            inputBoxParam.Text = "Force patients to fill out again";
            inputBoxParam.ParamSize = new Size(200, 80);
            inputBoxParam.Position = new Point(15, 30);
        ...
        private bool UpdateWebSheetDef()
        {
            ...
            while (!_hasSheetsDownloaded)
            {//The thread has not finished getting the list. 
                Application.DoEvents();
                Thread.Sleep(100);
            }
            Cursor = Cursors.Default;
            if (_listWebForms_SheetDefs == null || _listWebForms_SheetDefs.Count == 0)
            {//No web forms use this sheet def.
                return true;
            }
        ...
        private bool VerifyDesign()
        {
            ...
            for (int i = 0; i < _sheetDef.SheetFieldDefs.Count; i++)
            {
                ...
                {
                    for (int j = 0; j < listSheetFieldDefMedCheckBox.Count(); j++)
                ...
                {
                    for (int j = 0; j < listSheetFieldDefInputMedList.Count(); j++)
                    { //check for duplicates
                ...
                {
                    for (int j = 0; j < listSheetFieldDefCheckMedList.Count(); j++)
                    { //check for duplicates
        ...
        private void DrawGrid(SheetFieldDef sheetFieldDef, Graphics g, bool isSelected)
        {
            ...
            else if (GetIsDynamicSheetType())
            {//Always highlight dynamic module grid defs so user can see min height and width.
                Color color = _colorGray;
                if (checkBlue.Checked)
                {
                    color = _colorBlue;
        ...
        private void DrawSpecial(SheetFieldDef sheetFieldDef, Graphics g, int sheetDefWidth, bool isSelected)
        {
            ...
            switch (sheetFieldDef.FieldName)
            {
                ...
                case "ChartModuleTabs":
                    System.Windows.Forms.TabControl tabControlPrimary = new System.Windows.Forms.TabControl();
        ...
        private void DrawStringWordByWord(string str, Font font, Brush brush, SheetFieldDef sheetFieldDef, bool isRightToLeft, Point[] pointArrayCharPositions, Graphics g)
        {
            ...
            for (int i = 0; i < listWords.Count(); i++)
            {
                ...
                g.DrawString(listWords[i], font, brush, xPos, yPos, stringFormat);
                posIndex += listWords[i].Length;
            }
        }
        ...
        private void DrawTabMode(SheetFieldDef sheetFieldDef, Graphics g, bool isHighlightEligible)
        {
            ...
            Rectangle rectangleTab = new Rectangle(sheetFieldDef.XPos - 1, //X
                ...
                (int)g.MeasureString(sheetFieldDef.TabOrder.ToString(), _fontTabOrder).Width + 1, //Width
                12); //height
            if (isHighlightEligible && _listSheetFieldDefsTabOrder.Contains(sheetFieldDef))
            { //blue border, white box, blue letters
        ...
        private void DrawToothChart(SheetFieldDef sheetFieldDef, Graphics g, bool isSelected)
        {
            ...
            if (sheetFieldDef.FieldValue[0] == '1')
            {//Primary teeth chart
        ...
        {
            if (sheetFieldDef.Language.IsNullOrEmpty())
            {//Always use defaultPen when working with a non-translated def.
                return colorDefault;
            }
        ...
        private List<string> SplitStringByWhitespace(string str, bool isWhitespaceIncluded = true)
        {
            ...
            for (int i = 0; i < charArray.Length; i++)
            {
                ...
                else
                {
                    ...
                    {//If we encounter consecutive white space characters in a row.
