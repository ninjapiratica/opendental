    ...
    public partial class FormProgramLinkHideClinics : FormODBase
    {
        ...
        public FormProgramLinkHideClinics(Program program, List<ProgramProperty> listProgramProperties, List<Clinic> listClinicsUser)
        {
            ...
            if (!listProgramProperties.IsNullOrEmpty())
            {
                ...
                for (int i = listClinicsVisible.Count - 1; i >= 0; i--)
                {//Count backwards to allow for clean removal from lists.
                    if (listProgramProperties.Any(x => x.ClinicNum == listClinicsVisible[i].ClinicNum))
        ...
        private void SyncHiddenProgramProperties()
        {
            ...
            {
                if (!listProgramPropertiesHiddenClinics.Select(x => x.ProgramPropertyNum).Contains(listProgramPropertiesHidden[i].ProgramPropertyNum))
            ...
            {
                if (!listProgramPropertiesHidden.Select(x => x.ProgramPropertyNum).Contains(listProgramPropertiesHiddenClinics[i].ProgramPropertyNum))
        ...
        private List<long> MoveClinics(ListBoxOD listBoxFrom, ListBoxOD listBoxTo)
        {
        ...
        private void ReselectClinics(ListBoxOD listBox, List<long> listClinicNumsSelected)
        {
