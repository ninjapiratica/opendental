    ...
    public partial class FormPayment : FormODBase
    {
        ...
        private void FormPayment_Load(object sender, System.EventArgs e)
        {
            ...
            if (IsNew)
            {
                ...
                if (!Security.IsAuthorized(Permissions.PaymentCreate, _payment.PayDate))
                {//to prevent backdating of payments, check for date when this form is loaded
            ...
            for (int i = 0; i < _listDefsPaymentType.Count; i++)
            {
                ...
                if (IsNew && PrefC.GetBool(PrefName.PaymentsPromptForPayType))
        ...
        private void gridSplits_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (formPaySplitEdit.PaySplitCur == null)
        ...
        private void comboGroupBy_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            if (comboGroupBy.SelectedIndex == 2)
        ...
        private void AddCreditCardsToCombo(List<CreditCard> listCreditCards, Func<CreditCard, bool> funcSelectCard = null)
        {
            ...
            for (int i = 0; i < listCreditCards.Count; i++)
            {
                ...
                if (PrefC.HasClinicsEnabled)
                {
                    ...
                    if (clinic != null)
                    {  //adds abbr if cc has clinic attached
        ...
        private void CheckUIState()
        {
            ...
            if (_programX == null || programPayConnect == null || programPaySimple == null || programCareCredit == null || programEdgeExpress == null)
            {//Should not happen.
            ...
            if (!programPayConnect.Enabled && !_programX.Enabled && !programEdgeExpress.Enabled && !programPaySimple.Enabled)
            {//if none enabled
        ...
        private void FillGridCharges()
        {
            ...
            for (int i = 0; i < listAccountEntriesOutstandingCharges.Count; i++)
            {
                ...
                {//Just add the index for the splits associated to the current entryCharge.
        ...
        private void FillGridSplits()
        {
            ...
            for (int i = 0; i < _listPaySplits.Count; i++)
            {
                ...
                else if (_listPaySplits[i].PayPlanChargeNum > 0)
                {//Might be payment plan interest when not a procedure.
        ...
        private bool HasPaySimple()
        {
            ...
            {//Program link not enabled.  Launch a promo website.
                ODException.SwallowAnyException(() =>
            ...
            string aliasToken, string transactionId, double prepaidAmount = 0, long clinicNum = -1)
        {
            ...
            switch (edgeExpressTransType)
            {
                case EdgeExpressTransType.CreditSale:
                    ...
                    using (FormWebBrowser formWebBrowser = new FormWebBrowser(xWebResponse.HpfUrl))
                    {//Braces required within switch statements.
                ...
                case EdgeExpressTransType.CreditAuth:
                    ...
                    _payment.Receipt = EdgeExpress.CNP.BuildReceiptString(xWebResponseProcessed, false);
                    if (xWebResponseProcessed.XWebResponseCode == XWebResponseCodes.Approval)
                    {// only print receipt if its an approved transaction
                        _payment.IsCcCompleted = true;
                        textNote.Text += payNote;
                        if (_doPrintReceipt)
                        {
                ...
                case EdgeExpressTransType.CreditVoid:
                    ...
                    if (xWebResponse.XWebResponseCode == XWebResponseCodes.Approval)
                    {// only continue if we got a approval code back from Edge Express
        ...
        private bool SavePaymentToDb()
        {
            ...
            {
                if (_listPaySplits.Any(x => x.ProcNum == 0 && x.UnearnedType == 0 && x.AdjNum == 0 && x.PayPlanChargeNum == 0))
                {//if no procs, no adjust, not an unearned type, and not a payment plan.
            ...
            {//Not visable when IsNew or _loadData.Transaction is null or if listPayType is clicked.
        ...
        private bool ValidateForCreditCardPayment(bool isPrepaidCard, out CreditCard creditCardSelected)
        {
            ...
            {//Validation for regular credit cards (not prepaid cards).
                ...
                { // make sure there is an entry here, 0 is valid entry.
        ...
        private void VoidPayConnectTransaction(string refNum, string amount)
        {
            ...
            string receiptStr = "";
            if (_creditCardRequestPayConnect == null)
            {//The payment was made through the terminal.
            ...
            {//The payment was made through the web service.
                Cursor = Cursors.WaitCursor;
        ...
        public string MakePayConnectTransaction(double prepaidAmt = 0)
        {
            ...
            if (prepaidAmt == 0 && formPayConnect.GetResponse() != null)
            {//Regular credit cards (not prepaid cards).
            ...
            if (prepaidAmt != 0)
            {
                if (formPayConnect.GetResponse() != null && formPayConnect.GetResponse().StatusCode == "0")
            ...
            if (formPayConnect.GetResponse() != null)
            {
                if (formPayConnect.GetResponse().StatusCode == "0")
                { //The transaction succeeded.
                    _payment.IsCcCompleted = true;
                    _isCCDeclined = false;
                    ...
                    if (formPayConnect.TransType == PayConnectService.transType.VOID)
                    {//Close FormPayment window now so the user will not have the option to hit Cancel
                        if (IsNew)
                        {
                            if (!_wasCreditCardSuccessful)
                            {
            ...
            if (formPayConnect.WasPaymentAttempted && !_payment.IsCcCompleted && (formPayConnect.GetResponse() == null || formPayConnect.GetResponse().StatusCode != "0"))
        ...
        public string MakePaySimpleTransaction(double prepaidAmt = 0, string carrierName = "")
        {
            ...
            { //The transaction succeeded.
                ...
                if (formPaySimple.ApiResponseOut.TransType == PaySimple.TransType.VOID)
                {//Close FormPayment window now so the user will not have the option to hit Cancel
                    if (IsNew)
                    {
                        if (!_wasCreditCardSuccessful)
            ...
            if (formPaySimple.WasPaymentAttempted && !_payment.IsCcCompleted && (formPaySimple.ApiResponseOut == null || formPaySimple.ApiResponseOut.Status.ToLower() == "failed"))
            { //The transaction failed.
        ...
        public string MakeXChargeTransaction(double prepaidAmt = 0)
        {
            ...
            if (creditCard != null && !string.IsNullOrEmpty(creditCard.XChargeToken))
			...
			*/
            ...
            if (doNeedToken && !string.IsNullOrEmpty(xChargeToken) && prepaidAmt == 0)
            ...
            if (doUpdateCard && newAccount != "" && dateNewExpiration.Year > 1880 && prepaidAmt == 0)
        ...
        public string MakeEdgeExpressTransaction(double prepaidAmt = 0, long clinicNum = -1)
        {
            ...
            if (amt < 0)
            {//EdgeExpress always wants a positive number, even for returns.
                amt *= -1;
            }
            string note = null;
            //Web entry - CNP
        ...
        private void FormPayment_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            for (int i = 0; i < listNoteFields.Count; i++)
            {
                ...
                if (listNoteFields[i].StartsWith("TRANSACTIONID: "))
                {//EdgeExpress
                    transactionID = StringTools.SubstringAfter(listNoteFields[i], "TRANSACTIONID: ");
                    isEdgeExpress = true;
                }
                if (listNoteFields[i].StartsWith("Transaction ID: "))
                {//EdgeExpress
