...
namespace OpenDental
{
    ...
    public partial class FormClaimPayTotal : FormODBase
    {
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < ClaimProcArray.Length; i++)
            {
                ...
                if (PrefC.GetBool(PrefName.ClaimEditShowPayTracking))
                {
                    ...
                    if (!isDefPresent)
                    { //The ClaimPaymentTracking definition has been hidden or ClaimPaymentTracking==0 (not possible to hide last pay tracking def)
                        row.Cells.Add("");
        ...
        public void butWriteOff_Click(object sender, EventArgs e)
        {
            ...
            if (isCanadian && dialogResult == DialogResult.Yes)
            {//Canadian lab fees should be written off.
                ...
                for (int i = 0; i < ClaimProcArray.Length; i++)
                {
                    ...
                    if (claimProcOverpayment > procLabTotal)
                ...
                if (claimProcTotalLabs.InsPayAmt > 0 || claimProcTotalLabs.WriteOff > 0)
            ...
            for (int i = 0; i < ClaimProcArray.Length; i++)
            {
                ...
                if (unpaidAmt <= 0)
                { // clear out the writeoff if determined to be negative or 0
