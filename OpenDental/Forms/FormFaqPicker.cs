    ...
    public partial class FormFaqPicker : FormODBase
    {
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < listFaqsForPageName.Count(); i++)
            {
                ...
                if (manualPageUnlinkedItem is null)
                {//Null because the FaqNum is linked to a valid manual page
                    row.Cells["LinkedStatus"].Value = "Linked";
                }
                else if (manualPageUnlinkedItem.ManualPageNum == 0)
                {//FaqNum not linked to manual page 
                    row.Cells["LinkedStatus"].Value = "Unlinked";
                }
                else
                {//FaqNum is linked to a manual page that no longer exists in jordans_mp database
