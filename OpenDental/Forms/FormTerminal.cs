...
namespace OpenDental
{
    ...
    public partial class FormTerminal : FormODBase
    {
        ...
        {
            if (IsSimpleMode)
            { //Process signals ONLY if not simple mode.
                return;
            }
        ...
        private void LoadPatient(bool isRefreshOnly)
        {
            ...
            {//NOT IsSimpleMode
                ...
                {//either terminal.PatNum is 0 or no sheets for pat
                    labelWelcome.Visible = true;
                    labelForms.Visible = false;
                    listForms.Visible = false;
                    butDone.Visible = false;
                    if (terminalActive.PatNum > 0)
                    {//pat loaded but no sheets to show, unload pat, update db, send signal
        ...
        private void AutoShowSheets()
        {
            ...
            while (patNum == PatNum && sheetNum > 0)
            {//if patient is changed or no more sheets to display, break out of loop
        ...
        private void butDone_Click(object sender, EventArgs e)
        {
            ...
            butDone.Visible = false;
            if (IsSimpleMode)
            {//not subscribed to signals if IsSimpleMode
                PatNum = 0;
                _listSheets = new List<Sheet>();
        ...
        private void panelClose_Click(object sender, EventArgs e)
        {
            ...
            {
                if (count > 0)
                {//we already passed through once, so the entered password must have been invalid. 
