...
namespace OpenDental
{
    ...
    public partial class FormAdjMulti : FormODBase
    {
        ...
        private void FillGrid()
        {
            ...
            {
                if (i == 0)
                {//Always add the 'Unassigned' header row first. This is because procedures typically act as the 'header' but these adjustments don't have procedures.
        ...
        private bool AddAdjustments()
        {
            ...
            {//Add a singular adjustment if no procedures are selected.
                _listAdjustments.Add(GetAdjFromUI());
            }
            else
            {//Add an adjustment for every procedure selected.
        ...
        private void butUpdate_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listAdjustments.Count; i++)
            {
                ...
                {//There is a procedure associated to the selected adjustment.
        ...
        {
            if (_listAdjustments.IsNullOrEmpty())
            ...
            if (enumAdjustmentBlockOrWarn == EnumAdjustmentBlockOrWarn.Block && hasNegAmt)
            {//block preference set
            ...
            if (enumAdjustmentBlockOrWarn == EnumAdjustmentBlockOrWarn.Warn && hasNegAmt)
            {//warning preference set
                if (!MsgBox.Show(this, MsgBoxButtons.YesNo, "Remaining amount on a procedure is negative. Continue?", "Overpaid Procedure Warning"))
                {
                    return;
                }
            }
            if (!Security.IsAuthorized(Permissions.AdjustmentCreate, PIn.Date(dateAdjustment.Text), true))
            {//User does not have full edit permission.
