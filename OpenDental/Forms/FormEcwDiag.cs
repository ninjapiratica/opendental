...
namespace OpenDental
{
    public partial class FormEcwDiag : FormODBase
    {
        ...
        private void HL7Verification(bool verbose)
        {
            ...
            for (int ifaceIndex = 0; ifaceIndex < listHL7InterfaceIDs.Count; ifaceIndex++)
            {//validate one interface at a time.
                ...
                for (int i = 0; i < listHL7MessageIDs.Count; i++)
                {  //2, in our sample
            ...
            for (int r = 0; r < tableHL7Segments.Rows.Count; r++)
            {
                ...
                switch (stringArraySegmentFields[0])
                {
                    case "MSH":
                        ...
                        continue;
                    case "SCH":
                        if (stringArraySegmentFields[2] != "{ENCID}")
        ...
        private void checkShow_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (_stringBuilder.ToString().EndsWith("X"))
