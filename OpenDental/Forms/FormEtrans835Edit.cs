...
namespace OpenDental
{
    public partial class FormEtrans835Edit : FormODBase
    {
        ...
        private void gridClaimDetails_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            else if (Security.IsAuthorized(Permissions.InsPayCreate))
            {//Claim found and is not received.  Date not checked here, but it will be checked when actually creating the check.
            ...
            if (!gridClaimDetails.IsDisposed)
            {//Not sure how the grid is sometimes disposed, perhaps because of it being non-modal.
        ...
        private Claim ClaimSelectionHelper(Hx835_Claim hx835_ClaimPaid)
        {
            ...
            if (!String.IsNullOrEmpty(hx835_ClaimPaid.ClaimTrackingNumber) && hx835_ClaimPaid.ClaimTrackingNumber != "0")
            {//Claim was not printed, it is an eclaim.
        ...
        private void gridClaimDetails_RightClickHelper(object sender, EventArgs e)
        {
            ...
            else if (idxMenuItem == 1)
            {
                ...
                Etrans835Attaches.CreateForClaim(_x835, hx835_Claim, claim.ClaimNum, isAttachNeeded, _listEtrans835Attaches);
                if (FormClaimEdit.AddClaimCustomTracking(claim, hx835_Claim.GetRemarks()))
                {//A tracking status was chosen and the tracking status contains an error code.
        ...
        private void FormEtrans835Edit_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            for (int i = 0; i < gridClaimDetails.ListGridRows.Count; i++)
            {
                ...
                {//Will count as Recieved even if some claims are manually detached from the ERA ("Recd" column shows "N/A")
                    isReceived = false;
                    break;
                }
            }
