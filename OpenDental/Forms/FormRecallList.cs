...
namespace OpenDental
{
    ...
    public partial class FormRecallList : FormODBase
    {
        ...
        private DataTable GetRecallTable()
        {//long clinicNum, long siteNum, long maxReminders){
            long siteNum = 0;
        ...
        private void SetFamilyColors()
        {
            ...
            for (int i = 0; i < _grid.ListGridRows.Count; i++)
            {
                if (((PatRowTag)_grid.ListGridRows[i].Tag).GuarantorNum == guar)
                { //family member
                    famCount++;
        ...
        {
            if (gridRecalls.Columns[e.Col].Tag.ToString() == "WebSched")
        ...
        private void pdCards_PrintPage(object sender, PrintPageEventArgs ev)
        {
            ...
            while (yPos < ev.PageBounds.Height - bottomPageMargin && patientsPrinted < _tableAddress.Rows.Count)
            {
                ...
                if (PrefC.HasClinicsEnabled && Clinics.GetClinic(clinicNum) != null)
        ...
        private void butSetStatusRecalls_Click(object sender, System.EventArgs e)
        {
            ...
            long newStatus = 0;
            if (comboSetStatusRecalls.SelectedIndex > 0)
            {//not None or no selection
        ...
        private void CommCreate(CommItemTypeAuto commItemTypeAuto, bool doIncludeNote, long defNumStatus = 0)
        {
            ...
            if (doIncludeNote)
            {
                ...
                else if (commItemTypeAuto == CommItemTypeAuto.RECALL && comboSetStatusRecalls.SelectedIndex > 0)
                {//comboStatus not None
                    status = comboSetStatusRecalls.GetSelected<Def>().ItemName;
                }
                else if (commItemTypeAuto == CommItemTypeAuto.REACT && comboSetStatusReact.SelectedIndex > 0)
                {//comboReactStatus not None
        ...
        private void gridReactivations_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (formReactivationEdit.DialogResult == DialogResult.Yes)
        ...
        {
            long status = 0;
            if (comboSetStatusReact.SelectedIndex > 0)
            {//not None or no selection
            ...
            {
                if (listPatRowTags[i].PriKeyNum == 0)
        ...
        private void butSched_Click(object sender, EventArgs e)
        {
            ...
            ShrinkWindowBeforeMinMax();
            WindowState = FormWindowState.Minimized;
            if (isRecall && listAppointmentsPinned[listAppointmentsPinned.Count - 1].AptDateTime >= DateTime.Today)
            { //we're dealing with a future recall(s), so pin it and jump to it's date
            ...
            { //it's a reactivation / it's a recall that is overdue or unsafe to jump to, so just pin it and jump to today's date
