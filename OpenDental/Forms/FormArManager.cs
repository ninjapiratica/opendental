    ...
    public partial class FormArManager : FormODBase
    {
        ...
        private void SetTotalsLocAndVisible(DisplayFieldCategory displayCat, int hScrollValue)
        {
            ...
            {//if any of the display fields that have a textbox are selected
                ...
                foreach (string colName in dictColPosAndWidth.Keys.ToList())
        ...
        private void butSend_Click(object sender, EventArgs e)
        {
            ...
            foreach (long guarCur in listPatAging.Select(x => x.PatNum))
            {//for each guarantor selected, get the family
                if (!dictFams.TryGetValue(guarCur, out Family famCur))
                {
                    continue;
                }
                foreach (long patCur in famCur.ListPats.Select(x => x.PatNum))
            ...
            if (outstandingFound)
            {
                ...
                if (MsgBox.Show(MsgBoxButtons.YesNo, message))
                {
                    ...
                    for (int i = 0; i < listFamiliesOutstanding.Count; i++)
                    ...
                    for (int i = 0; i < listPatNumsRestricted.Count; i++)
