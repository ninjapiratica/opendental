...
namespace OpenDental
{
    public partial class FormInfobutton : FormODBase
    {
        ...
        public string AgeGroupCodeHelper(DateTime dateTime)
        {
            ...
            //UI = D000369
            #endregion
            if (PatientCur.Birthdate.AddMonths(1) > DateTime.Now)
            {//less than 1mo old, newborn
                return "D007231";
            }
            else if (PatientCur.Birthdate.AddYears(2) > DateTime.Now)
            {//less than 2 yrs old, Infant
                return "D007223";
            }
            else if (PatientCur.Birthdate.AddYears(5) > DateTime.Now)
            {//2 to 5 yrs old, Preschool
                return "D007675";
            }
            else if (PatientCur.Birthdate.AddYears(12) > DateTime.Now)
            {//6 to 12 yrs old, Child
                return "D002648";
            }
            else if (PatientCur.Birthdate.AddYears(18) > DateTime.Now)
            {//13 to 18 yrs old, Adolescent
                return "D000293";
            }
            else if (PatientCur.Birthdate.AddYears(44) > DateTime.Now)
            {//19 to 44 yrs old, Adult
                return "D000328";
            }
            else if (PatientCur.Birthdate.AddYears(64) > DateTime.Now)
            {//45 to 64 yrs old, Middle Aged
                return "D008875";
            }
            else if (PatientCur.Birthdate.AddYears(79) > DateTime.Now)
    ...
    public class KnowledgeRequestNotification
    {
        ...
        public string ToUrl()
        {
            ...
            for (int i = 0; i < ListSubject3s.Count; i++)
            {
                ...
                if (ListSubject3s[i].MainSearchCriteria_.OriginalText != ListSubject3s[i].MainSearchCriteria_.Value_.DisplayName)
