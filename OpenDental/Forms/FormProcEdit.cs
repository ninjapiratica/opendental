...
namespace OpenDental
{
    ...
    public partial class FormProcEdit : FormODBase
    {
        ...
        private void FormProcInfo_Load(object sender, System.EventArgs e)
        {
            ...
            {//Canadian. en-CA or fr-CA
             //groupCanadianProcType.Location=new Point(106,301);
                ...
                if (_procedureCode.IsCanadianLab)
        ...
        {
            if (_procedure.ProcStatus == ProcStat.D && _procedure.IsLocked)
            ...
            else
            {
                ...
                if (_procedure.ProcStatus == ProcStat.TPi)
            ...
            if (comboProcStatus.GetSelected<ProcStat>() == ProcStat.D)
            {//an invalidated proc
                comboProcStatus.Enabled = false;
                butInvalidate.Visible = false;
                butOK.Enabled = false;
        ...
        private void FormProcEdit_Shown(object sender, EventArgs e)
        {
            ...
            {//Replace [[text]] sections within the note with AutoNotes.
                PromptForAutoNotes();
            }
        ...
        private void SetControlsUpperLeft()
        {
            ...
            if (_isStartingUp)
            {
                ...
                {//Hide Orig Date Comp if same as current procedure date.
                ...
                || _procedureCode.AreaAlsoToothRange)
            {
                ...
                for (int toothNum = 1; toothNum <= 16; toothNum++)
                {
                    ...
                    if (_listPriTeeths.Contains(toothNum.ToString()))
                    {//Is Primary
                ...
                for (int i = 0; i < stringArrayToothNums.Length; i++)
                {
                    if (Tooth.IsMaxillary(stringArrayToothNums[i]))
                    {//Works for primary or permanent tooth numbers.
        ...
        private void comboProcStatus_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            if (comboProcStatus.GetSelected<ProcStat>() == ProcStat.TP)
            ...
            if (comboProcStatus.GetSelected<ProcStat>() == ProcStat.C)
            {
                ...
                {//if attached to an appointment
                    ...
                    if (appointment.AptDateTime.Date > MiscData.GetNowDateTime().Date)
        ...
        private void butSetComplete_Click(object sender, System.EventArgs e)
        {
            ...
            {//if attached to an appointment
                Appointment appointment = Appointments.GetOneApt(_procedure.AptNum);
                if (appointment == null)
                {//Appointment was deleted before proc could be set complete.
            ...
            if (_procedureCode.PaintType == ToothPaintingType.Extraction)
        ...
        private void ProcNoteUiHelper()
        {
            ...
            if (textNotes.Text != "" && procNoteDefault != "")
            { //check to see if a default note is defined.
        ...
        private bool EntriesAreValid()
        {
            ...
            if (_procedure.ProcStatus != ProcStat.EO && textDate.Text == "")
            {//Only ProcStat.EO can be empty.
        ...
        private void SaveAndClose()
        {
            ...
            {
                for (int i = 0; i < _listAdjustments.Count; i++)
                {//update the attached adjustments
        ...
        private void ClearAdultToothSelections()
        {
            ...
            if (_procedureCode.TreatArea == TreatmentArea.ToothRange || _procedureCode.AreaAlsoToothRange)
            {
                ...
                {
                    if (listBoxTeeth.Items[j].ToString() == "")
                    {//Can be blank when the tooth is flagged as primary when it is an adult tooth.
                ...
                {
                    if (listBoxTeeth2.Items[j].ToString() == "")
                    {//Can be blank when the tooth is flagged as primary when it is an adult tooth.
