...
namespace OpenDental
{
    ...
    public partial class FormEServicesMassEmail : FormODBase
    {
        ...
        private void FormEServicesMassEmail_Load(object sender, EventArgs e)
        {
            ...
            bool canEdit = Security.IsAuthorized(Permissions.EServicesSetup, suppressMessage: true);
            ...
            checkIsMassEmailEnabled.Enabled = canEdit;
            checkIsSecureEnabled.Enabled = canEdit;
            butActivate.Enabled = canEdit;
            WebServiceMainHQProxy.EServiceSetup.SignupOut.SignupOutEService signupOutEService = _signupOut.EServices
                .FirstOrDefault(x => x.EService == eServiceCode.EmailMassUsage);
            if (signupOutEService != null)
            {
                webBrowser1.Navigate(signupOutEService.HostedUrl);
            }
            else
            {
        ...
        private void butActivate_Click(object sender, EventArgs e)
        {
            ...
            {
                PayloadItem payloadItem = new PayloadItem(clinicNum, "ClinicNum");
        ...
        {
            if (prefName == PrefName.MassEmailStatus && isEnabling && !MsgBox.Show(this, MsgBoxButtons.YesNo, _activateMessage))
            {//box is being checked
                checkIsMassEmailEnabled.Checked = false;
                return;
            }
            long clinicNum = GetClinicPromoCur().ClinicNum;
            HostedEmailStatus hostedEmailStatus = HostedEmailStatus.NotActivated;
            if (clinicNum == 0)
            {
                hostedEmailStatus = PrefC.GetEnum<HostedEmailStatus>(prefName);
                if (isEnabling)
                {
                    hostedEmailStatus = EnumTools.AddFlag(hostedEmailStatus, HostedEmailStatus.Enabled);
                }
                else
                {
                    hostedEmailStatus = EnumTools.RemoveFlag(hostedEmailStatus, HostedEmailStatus.Enabled);
                }
            ...
            else
            {
                ...
                ClinicPrefs.Upsert(prefName, clinicNum, ((int)hostedEmailStatus).ToString());
                ClinicPrefs.RefreshCache();
                _doSetInvalidClinicPrefs = true;
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (clinicNum == 0)
            {
                HostedEmailStatus hostedEmailStatus = PrefC.GetEnum<HostedEmailStatus>(PrefName.EmailSecureStatus);
                hostedEmailStatus = hostedEmailStatus ^ HostedEmailStatus.SignedUp;
                Prefs.UpdateInt(PrefName.EmailSecureStatus, (int)hostedEmailStatus);
                Prefs.RefreshCache();
            }
            else
            {
                HostedEmailStatus hostedEmailStatus = (HostedEmailStatus)ClinicPrefs.GetInt(PrefName.EmailSecureStatus, clinicNum);
                hostedEmailStatus = hostedEmailStatus ^ HostedEmailStatus.SignedUp;
                ClinicPrefs.Upsert(PrefName.EmailSecureStatus, clinicNum, $"{(long)hostedEmailStatus}");
                ClinicPrefs.RefreshCache();
            }
            FillPromotionForClinic();
        }
