...
namespace OpenDental
{
    ...
    public partial class FormProcCodes : FormODBase
    {
        ...
        private void FormProcCodes_Load(object sender, System.EventArgs e)
        {
            ...
            {//checkShowHidden will always be unchecked.
        ...
        private void FillComboBoxes()
        {
            ...
            {
                if (comboFeeSched1.GetSelectedKey<FeeSched>(x => x.FeeSchedNum) > 0)
                {//comboFeeSched1 doesn't usually contain a 'None' option, but can if there are no FeeScheds in DB
            ...
            if (PrefC.HasClinicsEnabled)
            {
                if (comboClinic3.Enabled == false)
                {//Previously selected FeeSched WAS global
        ...
        {
            if (feeSchedGroup == null || feeSchedGroup.ListClinicNumsAll.Count == 0)
            { //skip if empty group
                return;
            }
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < listProcedureCodes.Count; i++)
            {
                ...
                {//This proc code is in the same category or we are not sorting by category.
                    row.Cells.Add("");
                }
        ...
        private void gridMain_CellLeave(object sender, ODGridClickEventArgs e)
        {
            ...
            if (!Security.IsAuthorized(Permissions.FeeSchedEdit, true))
            { //Don't do anything if they don't have permission.
                return;
            }
            ...
            if (PrefC.GetBool(PrefName.ShowFeeSchedGroups) && provNum == 0 && !isEditingGroup)
            {//Ignore provider fees and don't block from editing a group.
            ...
            { //Global fee schedules have only one fee so blindly insert/update the fee. There will be no more localized copy.
                if (fee == null)
                { //Fee doesn't exist, insert
                    fee = new Fee();
                ...
                { //Fee does exist, update or delete.
                    if (feeAmtNewStr == "")
                    { //They want to delete the fee
                        _listFees.Remove(fee);
                    }
                    else
                    { //They want to update the fee
                        fee.Amount = feeAmtNew;
                    }
                }
            }
            else
            { //FeeSched isn't global.
                if (feeAmtNewStr == "")
                { //They want to delete the fee
                  //NOTE: If they are deleting the HQ fee we insert a blank (-1) for the current settings.
                    ...
                    {//They want to delete a fee for their current settings.
                        _listFees.Remove(fee);
                        if (isEditingGroup
                            && feeSchedGroupSelected != null
                            && !feeSchedGroupSelected.ListClinicNumsAll.IsNullOrEmpty()
                ...
                { //Fee isn't null, is for our current clinic, is for the selected provider, and they don't want to delete it.  Just update.
                    fee.Amount = feeAmtNew;
                    if (isEditingGroup
                        && feeSchedGroupSelected != null
                        && !feeSchedGroupSelected.ListClinicNumsAll.IsNullOrEmpty()
        ...
        public static int ImportProcCodes(string path, List<ProcedureCode> listProcedureCodes, string xmlData)
        {
            ...
            for (int i = 0; i < listProcedureCodes.Count; i++)
            {
                ...
                if (listProcedureCodes[i].ProcCat == 0)
        ...
        private void butPickFeeSched_Click(object sender, EventArgs e)
        {
            ...
            if (buttonPicker == butPickSched1)
            { //First FeeSched combobox doesn't have "None" option.
