...
namespace OpenDental
{
    ...
    public partial class FormInsPlans : FormODBase
    {
        ...
        private void butMerge_Click(object sender, EventArgs e)
        {
            ...
            if (isBluebookOn && planCount > 0)
            {//Make sure user wants to merge selected plans that are Category Percentage with no fee schedule attached, if applicable.
        ...
        {
            int planCount = 0;
            if (insPlanToMergeTo.IsBlueBookEnabled && insPlanToMergeTo.PlanType == "")
            {//Plan to merge to is blue book eligible, so no insbluebooks will be deleted.
                return planCount;
            }
            for (int i = 0; i < insPlanSeletedArray.Length; i++)
            {
                if (insPlanSeletedArray[i].PlanType == "" && insPlanSeletedArray[i].IsBlueBookEnabled)
                {//This will exclude the planToMergeTo which is in the array.
                    planCount += 1;
                }
            }
            return planCount;
