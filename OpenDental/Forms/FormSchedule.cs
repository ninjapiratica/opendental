...
namespace OpenDental
{
    ...
    public partial class FormSchedule : FormODBase
    {
        ...
        private bool ValidateInputs(bool isQuiet = false)
        {
            ...
            if (listBoxProvs.SelectedIndices.Count > 1 && listBoxProvs.SelectedIndices.Contains(0))
            {//'none' selected with additional provs
                listErrorMsgs.Add(Lan.g(this, "Invalid selection of providers."));
            }
            if (listBoxEmps.SelectedIndices.Count > 1 && listBoxEmps.SelectedIndices.Contains(0))
            {//'none' selected with additional emps
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (listProvidersSelected.Count == 1 && listEmployeesSelected.Count == 0)
            {//only 1 provider selected, pass into schedule day filter
                provAbbr = listProvidersSelected[0].Abbr;
            }
            else if (listEmployeesSelected.Count == 1 && listProvidersSelected.Count == 0)
            {//only 1 employee selected, pass into schedule day filter
                empFName = listEmployeesSelected[0].FName;
            }
            else if (listProvidersSelected.Count == 1 && listEmployeesSelected.Count == 1)
            {//1 provider and 1 employee selected
        ...
        {
            if (!checkPracticeNotes.Checked && !checkClinicNotes.Checked)
            {//No holidays should be showing
            ...
            else if (comboClinic.SelectedClinicNum != 0)
            {//A clinic is selected
