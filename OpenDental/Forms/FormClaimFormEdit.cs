...
namespace OpenDental
{
    ...
    public partial class FormClaimFormEdit : FormODBase
    {
        ...
        private void FormClaimFormEdit_Load(object sender, System.EventArgs e)
        {
            ...
            panelClaimForm.Width = _claimForm.Width;
            panelClaimForm.Height = _claimForm.Height;
            FillItems();
        ...
        private void FormClaimFormEdit_Layout(object sender, System.Windows.Forms.LayoutEventArgs e)
        {
            ...
            vScrollBar1.Maximum = _claimForm.Width > _claimFormVisibleWidth ? panelClaimForm.Height + hScrollBar.Height : panelClaimForm.Height;//1100;
        ...
        {
            if (panelClaimForm.Width > _claimFormVisibleWidth)
            {//If they set the width to greater than the default, show the scroll bar
                ...
                bool wasHScrollBarVisible = hScrollBar.Visible;
        ...
        private void FillItems()
        {
            ...
            if (_claimForm.Items.Count > 0)
            {
                listItems.ColumnWidth = _claimForm.Items.Max(x => TextRenderer.MeasureText(x.FieldName, listItems.Font).Width);
            }
        }
        ...
        private void panelClaimForm_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
        {
            ...
            for (int i = 0; i < _claimForm.Items.Count; i++)
            {
                ...
                {//field
                    ...
                    else colorPanel = Color.Blue;
                    xPosRect = _claimForm.Items[i].XPos;
                    xPosText = xPosRect;
                    if (_stringArrayDisplays[i] == "1234.00" || _stringArrayDisplays[i] == "AB")
                    {//right aligned fields: any amount field, or ICDindicatorAB
                    ...
                    grfx.DrawRectangle(new Pen(colorPanel)
                        , xPosRect, _claimForm.Items[i].YPos
        ...
        private void butAdd_Click(object sender, System.EventArgs e)
        {
            ...
            panelClaimForm.Invalidate();//also Fills _stringArrayDisplays
            FillForm();
        }
        ...
        private void panelClaimForm_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            ...
            for (int i = _claimForm.Items.Count - 1; i >= 0; i--)
            {
                ...
                {
                    width = grfx.MeasureString(_stringArrayDisplays[i]
                        , new Font(_claimForm.FontName, _claimForm.FontSize)).Width;
                    height = grfx.MeasureString(_stringArrayDisplays[i]
                        , new Font(_claimForm.FontName, _claimForm.FontSize)).Height;
                }
                else
                {//a width and height are available, so use them
                    width = _claimForm.Items[i].Width;
                    height = _claimForm.Items[i].Height;
                }
                bool isRightAligned = _stringArrayDisplays[i] == "1234.00" || _stringArrayDisplays[i] == "AB";
                if ((isRightAligned ? e.X < _claimForm.Items[i].XPos : e.X > _claimForm.Items[i].XPos)
                    && (isRightAligned ? e.X > _claimForm.Items[i].XPos - width : e.X < _claimForm.Items[i].XPos + width)
                    && e.Y > _claimForm.Items[i].YPos
                    && e.Y < _claimForm.Items[i].YPos + height)
                {
                    if (_isControlDown)
                    {
                        if (listItems.SelectedIndices.Contains(i))
                    ...
                    {//control not down
                        if (listItems.SelectedIndices.Count > 1//if multiple items already selected
                            && listItems.SelectedIndices.Contains(i))
            ...
            for (int i = 0; i < listItems.SelectedIndices.Count; i++)
            {//then a normal loop to set _pointFArrayOldItemLocs for dragging
                _pointFArrayOldItemLocs[i] = new PointF((float)_claimForm.Items[listItems.SelectedIndices[i]].XPos
                    , (float)_claimForm.Items[listItems.SelectedIndices[i]].YPos);
            }
            panelClaimForm.Invalidate();
        }
