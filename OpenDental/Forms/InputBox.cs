...
namespace OpenDental
{
    ...
    public partial class InputBox : FormODBase
    {
        ...
        public System.Windows.Forms.CheckBox checkBoxResult
        {
            get
            {
                return (System.Windows.Forms.CheckBox)_listInputControls.FirstOrDefault(x => x is System.Windows.Forms.CheckBox);
            }
        }
        ...
        private void AddInputControls()
        {
            ...
            foreach (InputBoxParam inputParam in _listInputParams)
            {
                ...
                if (!string.IsNullOrEmpty(inputParam.LabelText))
                {
                    ...
                    label.Size = inputParam.ParamSize == Size.Empty ? new Size(controlWidth, controlHeight) : new Size(inputParam.ParamSize.Width, inputParam.ParamSize.Height);
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (_listInputControls.OfType<UI.ComboBox>().Any(x => !x.SelectionModeMulti && x.SelectedIndex == -1))
            {//single selection
                MsgBox.Show(this, "Please make a selection.");
                return;
            }
            if (_listInputControls.OfType<UI.ComboBox>().Any(x => x.SelectionModeMulti && x.SelectedIndices.Count == 0))
            {//multi selection
                MsgBox.Show(this, "Please make at least one selection.");
                return;
            }
            if (_listInputControls.OfType<System.Windows.Forms.CheckBox>().ToList().Count > 1 && _listInputControls.OfType<System.Windows.Forms.CheckBox>().Where(x => x.Checked).Count() == 0)
            {
                MsgBox.Show(this, "Please make a selection.");
                return;
            }
            if (_listInputControls.OfType<System.Windows.Forms.CheckBox>().ToList().Count > 1 && _listInputControls.OfType<System.Windows.Forms.CheckBox>().Where(x => x.Checked).Count() > 1)
            {
