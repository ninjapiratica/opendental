    ...
    public partial class FormProvEdit : FormODBase
    {
        ...
        private void FormProvEdit_Load(object sender, System.EventArgs e)
        {
            ...
            for (int i = 0; i < defArray.Length; i++)
            {
                ...
                if (i == 0 || ProviderCur.Specialty == defArray[i].DefNum)
        ...
        {
            using FormProviderIdentEdit FormProviderIdentEdit = new FormProviderIdentEdit();
            FormProviderIdentEdit.ProviderIdentCur = ProviderIdentArray[e.Row];
            FormProviderIdentEdit.ShowDialog();
            FillGridProvIdent();
        }
        ...
        {
            using FormProviderIdentEdit FormProviderIdentEdit = new FormProviderIdentEdit();
            FormProviderIdentEdit.ProviderIdentCur = new ProviderIdent();
            FormProviderIdentEdit.ProviderIdentCur.ProvNum = ProviderCur.ProvNum;
            FormProviderIdentEdit.IsNew = true;
            FormProviderIdentEdit.ShowDialog();
            FillGridProvIdent();
        }
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (!PrefC.GetBool(PrefName.EasyHideDentalSchools) && (ProviderCur.IsInstructor || ProviderCur.SchoolClassNum != 0))
            ...
            if (provNumClaimBillingOverride != 0)
            {
                ...
                if (providerClaimBillingOverride == null)
                {//Hidden provider, need to get them the normal way
                    providerClaimBillingOverride = Providers.GetProv(provNumClaimBillingOverride);//Can return null if the provider doesn't exist
                }
                if (providerClaimBillingOverride != null && !providerClaimBillingOverride.IsNotPerson)
                {//Override is a person.
                    MsgBox.Show(this, "E-claim Billing Prov Override cannot be a person.");
                    return;
                }
            }
            if (ProviderCur.IsNew == false && comboProv.GetSelectedProvNum() == ProviderCur.ProvNum)
            {//Override is the same provider.
                MsgBox.Show(this, "E-claim Billing Prov Override cannot be the same provider.");
                return;
            }
            if (ProviderCur.IsErxEnabled == ErxEnabledStatus.Disabled && checkUseErx.Checked)
