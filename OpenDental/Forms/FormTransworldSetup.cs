...
namespace OpenDental
{
    public partial class FormTransworldSetup : FormODBase
    {
        ...
        private void FormTransworldSetup_Load(object sender, EventArgs e)
        {
            ...
            {//Using clinics
                ...
                foreach (Clinic clinicCur in listClinics)
                {
                    ...
                    if (Clinics.ClinicNum == clinicCur.ClinicNum)
                    {//set selected index to the currently selected clinic in FormOpenDental
                        _selectedClinicNum = clinicCur.ClinicNum;
                        comboClinic.SelectedIndex = comboClinic.Items.Count - 1;
                    }
                }
            }
            _listProgramProperties = ProgramProperties.GetForProgram(_progCur.ProgramNum);//get list of all props for the program
            _dictClinicListProgProps = _listProgramProperties
            ...
            {//if not set, default to repeat once a day
        ...
        private void SyncWithHQ()
        {
            ...
            foreach (ProgramProperty propCur in listPropsCur)
            {//update the currently selected props with the current form values
                switch (propCur.PropertyDesc)
                {
                    case "SftpServerAddress":
            ...
            else
            {
                ...
                else if (!_dictClinicListProgProps.ContainsKey(_selectedClinicNum))
                {//if the clinic is not in the dict (otherwise values are already updated)
        ...
        private void MakeLogEntry(ProgramProperty programProperty)
        {
            ...
            stringBuilderLogText.Append(_progCur.ProgDesc + "'s " + programProperty.PropertyDesc);
            if (PrefC.HasClinicsEnabled)
            {
            ...
            SecurityLogs.MakeLogEntry(Permissions.ManageHighSecurityProgProperties, 0, stringBuilderLogText.ToString(), _progCur.ProgramNum, DateTime.Now);
        }
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _listProgramProperties.Count; i++)
            {//Iterates through the original properties for each clinic as found when we load this form
                if (_listProgramProperties[i].IsHighSecurity)
                {
                    ...
                    if (!_dictClinicListProgProps.TryGetValue(_listProgramProperties[i].ClinicNum, out listProgramPropertiesClinic))
                    {
                        ...
                        _dictClinicListProgProps.TryGetValue(0, out listProgramPropertiesClinic);
                    }
            ...
            {
                _dictClinicListProgProps.Remove(listClinicNums[i]);
            }
            //Flatten the dictionary and get the properties for the clinics that didn't have their own set of properties before. 
            List<ProgramProperty> listProgramPropertiesNew = _dictClinicListProgProps.Where(x => x.Key != 0).SelectMany(x => x.Value).ToList();
            _dictClinicListProgProps.TryGetValue(0, out List<ProgramProperty> listProgramPropertiesHQ); //Get the HQ clinic set of properties.
