...
namespace OpenDental
{
    ...
    public partial class FormBillingOptions : FormODBase
    {
        ...
        private void SetFiltersForClinicNums(List<long> listClinicNums, bool isTextNoteExcluded = false)
        {
                ...
                || !_dictionaryClinicPrefsNew.ContainsKey(listClinicNums[0]))//They have not saved their default filter options for the selected clinic. Use default prefs.
            {
                ...
                for (int i = 0; i < stringArrayselectedBillTypes.Length; ++i)
                {
                    ...
                    {//cannot convert string to int, just continue
                        continue;
                    }
                }
                ...
                if (stringArraySelectedInsFilingCodes.Length == 0)
                {//Set to None.
                    comboInsFilingCodes.SelectedIndex = 0;
                }
                else if (stringArraySelectedInsFilingCodes.Contains("-1"))
                {//(all)
            ...
            {//Update filter UI to reflect ClinicPrefs. //there has to be ONE item in ClinicNums. It MUST be in the dictionary and it MUST NOT be -1 or 0.
        ...
        private void butSaveDefault_Click(object sender, System.EventArgs e)
        {
            ...
            else if (_dictionaryClinicPrefsNew.Keys.Contains(listClinicNums[0]))
            {//ClincPrefs exist, update them.
            ...
            {//No existing ClinicPrefs for the currently selected clinic in comboClinics.
        ...
        ///<summary>Sets UI controls to the practice defaults preference values. The OpenDentalService.ScheduledProcesses.CreateStatementHelper() uses this logic to set default practice preference values. Changes to this method will also need to be made there, if they apply.</summary>
        ...
        ///<summary>Runs enterprise aging. Returns false if there are any errors while running aging. This logic is used in OpenDentalService.ProcessStatements.RunAgingEnterprise(). Any changes to this method will also need to be made there, if they apply.</summary>
        ...
        ///<summary>Creates statements. This method was used to create the OpenDentalService.ScheduledProcessThread.TryGenerateList() method. Changes to this method will also need to be made there if they apply.</summary>
        ...
        {
            if (comboInsFilingCodes.SelectedIndices.Count < 1)
            {//No selections made, default to None.
        ...
        ///<summary>Creates statements for a single clinic.  Returns number of statements created, or -1 if there was an error or user cancelled. The logic in this method is used in OpenDentalService.ScheduledProcessThread.CreateStatementHelper(). Changes to this method will also need to be considered there.</summary>
        ...
        private int CreateHelper(long clinicNum, bool useClinicPrefs = false, bool suppressPopup = false, Dictionary<long, PatAgingData> dictionaryPatAgingData = null)
        {
            ...
            if (comboInsFilingCodes.SelectedIndices[0] == 0)
            {//None
                listInsFilingCodeNums.Clear();
                filterByInsFilingCodes = false;
            }
            else if (comboInsFilingCodes.SelectedIndices[0] == 1)
            {//(all)
            ...
            if (dictionaryPatAgingData == null)
            {//If not passed in, we must generate the data here.
            ...
            if (checkExcludeInsPending.Checked || checkExcludeIfProcs.Checked || checkIncludeChanged.Checked)
            {
                foreach (KeyValuePair<long, PatAgingData> kvp in dictionaryPatAgingData)
                {
                    ...
                    if (checkExcludeInsPending.Checked && kvp.Value.HasPendingIns)
                    {//don't fill list if not excluding if pending ins
                        listPendingInsPatNums.Add(kvp.Key);
                    }
                    if (checkExcludeIfProcs.Checked && kvp.Value.HasUnsentProcs)
                    {//don't fill list if not excluding if unsent procs
                        listUnsentPatNums.Add(kvp.Key);
                    }
                    if (checkIncludeChanged.Checked)
                    {//don't fill dictionary if not including if changed
            ...
            if (checkSuperFam.Checked)
            {
                ...
                for (int i = listPatAgings.Count - 1; i >= 0; i--)
                {//Go through each PatAging in the retrieved list
