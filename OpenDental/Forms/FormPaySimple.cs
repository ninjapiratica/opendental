...
namespace OpenDental
{
    public partial class FormPaySimple : FormODBase
    {
        ...
        private PaySimple.ApiResponse ProcessPayment(int expYear, int expMonth)
        {
            ...
            try
            {
                switch (_transType)
                {
                    ...
                    case PaySimple.TransType.AUTH:
                        ...
                        if (paySimpleCustomerId == 0)
                        {//Insurance payment, make a new customer id every time per Nathan on 04/26/2018
                            if ((_patient == null || _patient.PatNum == 0))
                            {
            ...
            if (_transType.In(PaySimple.TransType.SALE, PaySimple.TransType.RETURN, PaySimple.TransType.VOID))
        ...
        private bool VerifyData(out int expYear, out int expMonth)
        {
            ...
            if (_creditCard == null)
            {//if the user selected a new CC, verify through PaySimple
            ...
            else if (_creditCard.PaySimpleToken == "" && Regex.IsMatch(textCardNumber.Text, @"X+[0-9]{4}"))
            {//using a stored CC
            ...
            if (textNameOnCard.Text.Trim() == "" && _patient != null && _patient.PatNum > 0)
