    ...
    public partial class FormInsEditLogs : FormODBase
    {
        ...
        private List<GridRow> ConstructGridRows()
        {
            ...
            for (int i = 0; i < listGridRows.Count; i++)
            {
                ...
                if (insEditLog.FieldName == "BenefitNum")
                {//child cannot be BenefitNum
                    continue;
                }
                ...
                for (int j = 0; j < listGridRows.Count; j++)
                {
                    ...
                    if (insEditLogParent.FieldName != "BenefitNum")
                    {//parent must be BenefitNum
                        continue;
                    }
            ...
            for (int i = 0; i < _listInsEditLogs.Count; i++)
            {
                ...
                switch (_listInsEditLogs[i].FieldName)
                {
                    case "CarrierNum":
                        ...
                        if (_listInsEditLogs[i].LogType == InsEditLogType.InsPlan && carrierNameBefore == carrierNameAfter)
                        {//Edits to carrier.
