...
namespace OpenDental
{
    public partial class FormConnectionLost : FormODBase
    {
        ...
        public FormConnectionLost(Func<bool> funcShouldWindowClose, ODEventType odEventType = ODEventType.Undefined, string errorMessage = "", Type type = null)
        {
            InitializeComponent();
            ...
            if (type == null)
            {
                type = typeof(DataConnectionEvent);
            }
            //Make sure that the event type passed in has a "Fired" event to listen to.
            ODException.SwallowAnyException(() => _eventInfoFired = type.GetEvent("Fired"));
            if (_eventInfoFired == null)
            {
                throw new ApplicationException("The 'eventType' passed into FormConnectionLost does not have a 'Fired' event.\r\n"
                    + "Type passed in: " + type.GetType());
            }
            Delegate delegateFired = GetDelegateFired(_eventInfoFired.EventHandlerType);
            MethodInfo methodInfo = _eventInfoFired.GetAddMethod();
            methodInfo.Invoke(this, new object[] { delegateFired });
        }
        ...
        private Delegate GetDelegateFired(Type typeEventHandler)
        {
        ...
        private void FormConnectionLost_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            Delegate delegateFired = GetDelegateFired(_eventInfoFired.EventHandlerType);
            MethodInfo methodInfo = _eventInfoFired.GetRemoveMethod();
            methodInfo.Invoke(this, new object[] { delegateFired });
        }
    }
}