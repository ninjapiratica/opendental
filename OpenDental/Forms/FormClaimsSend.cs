...
namespace OpenDental
{
    ...
    public partial class FormClaimsSend : FormODBase
    {
        ...
        private void FormClaimsSend_Load(object sender, System.EventArgs e)
        {
            ...
            if (PrefC.RandomKeys && PrefC.HasClinicsEnabled)
        ...
        public void FillClinicsList(long claimCustomTrackingNum)
        {
            int previousSelection = -1;
            if (comboClinic.SelectedIndex != -1)
            {//Only -1 the first time this method is run.
        ...
        private void FillGrid(bool rememberSelection = false, bool isRefreshRequired = true)
        {
            ...
            if (isRefreshRequired)
            {
                ...
                for (int i = 0; i < claimSendQueueItemArray.Length; i++)
                {
                    ...
                    for (int j = 0; j < _claimSendQueueItemArrayQueueAll.Length; j++)
                    {//Go through the old list of all claims.
                        if (claimSendQueueItemArray[i].ClaimNum == _claimSendQueueItemArrayQueueAll[j].ClaimNum)
                        {//The same claim is in both the old and new "all" lists.
                    ...
                    if (claimSendQueueItemArray[i].MissingData == null)
                    {//Can only be null if the claim was not in the old "all" list.  For example when undo.
        ...
        private ClaimSendQueueItem[] GetListQueueFiltered()
        {
            ...
            {
                if (Security.CurUser.ClinicIsRestricted)
                {//If the user is restricted to specific clinics (has no Unassigned/Default option)
                    clinicNum = _listClinics[comboClinic.SelectedIndex].ClinicNum;
                }
                else if (comboClinic.SelectedIndex != 0)
                {//If not restricted to specific clinics and not selecting Unassigned/Default
            ...
            if (PrefC.HasClinicsEnabled)
            {//Filter by clinic only when clinics are enabled.
                listClaimSendQueueItems = listClaimSendQueueItems.FindAll(c => c.ClinicNum == clinicNum);
            }
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            {
                ClaimL.PromptForSecondaryClaim(ClaimProcs.RefreshForClaim(formClaimPrint.ClaimNum));
            }
            FillGrid();
        ...
        private void RefreshAndValidateSelections()
        {
            ...
            for (int j = 0; j < claimSendQueueItemArrayRefreshItems.Length; j++)
            {//Loop through all the refreshed ClaimSendQueueItems
                for (int k = 0; k < _claimSendQueueItemArrayQueueAll.Length; k++)
                {//Loop through all the ClaimSendQueueItems in the grid's main list
                    if (claimSendQueueItemArrayRefreshItems[j].ClaimNum == _claimSendQueueItemArrayQueueAll[k].ClaimNum)
                    {//If you found the matching ClaimSendQueueItem
            ...
            {//At least one claim still exists
        ...
        private void SendEclaimsToClearinghouse(long hqClearinghouseNum)
        {
            ...
            RefreshAndValidateSelections();
            if (gridMain.SelectedTags<ClaimSendQueueItem>().Count == 0)
            {//No claims selected after Validation
                MsgBox.Show(this, "No claims to send.");
                return;
            }
        ...
        private void FillHistory()
        {
            ...
            for (int i = 0; i < comboHistoryType.SelectedIndices.Count; i++)
            {//Some selected, add only those selected
                int selectedIdx = (int)comboHistoryType.SelectedIndices[i];
                listSelectedEtransTypes.Add(_listEtransTypes[selectedIdx]);
            }
            if (comboHistoryType.SelectedIndices.Count == 0)
            {//None selected.  The user can unselect each option manually.
