    ...
    public partial class FormToothChartingBig : FormODBase
    {
        ...
        private void DrawProcGraphics()
        {
            ...
            for (int i = 0; i < ListDataRowsProcs.Count; i++)
            {
                ...
                ProcedureCode procCode = ProcedureCodes.GetProcCode(ListDataRowsProcs[i]["ProcCode"].ToString());
                ProcStat procStatus = (ProcStat)PIn.Long(ListDataRowsProcs[i]["ProcStatus"].ToString());
                long procProvNum = PIn.Long(ListDataRowsProcs[i]["ProvNum"].ToString());
                ...
                switch (ProcedureCodes.GetProcCode(ListDataRowsProcs[i]["ProcCode"].ToString()).PaintType)
                {
                    ...
                    case ToothPaintingType.DentureDark:
                        ...
                        {
                            teeth = ListDataRowsProcs[i]["ToothRange"].ToString().Split(new char[] { ',' });
                        }
                        for (int t = 0; t < teeth.Length; t++)
                        {
                            if (ToothInitials.ToothIsMissingOrHidden(ListToothInitials, teeth[t]))
                            {
                    ...
                    case ToothPaintingType.SpaceMaintainer:
                        if (procCode.TreatArea == TreatmentArea.Tooth && ListDataRowsProcs[i]["ToothNum"].ToString() != "")
                        {
                            _toothChartRelay.SetSpaceMaintainer(ListDataRowsProcs[i]["ToothNum"].ToString(), cDark);
                        }
                        else if ((procCode.TreatArea == TreatmentArea.ToothRange && ListDataRowsProcs[i]["ToothRange"].ToString() != "")
                            || (procCode.AreaAlsoToothRange == true && ListDataRowsProcs[i]["ToothRange"].ToString() != ""))
                        {
                            teeth = ListDataRowsProcs[i]["ToothRange"].ToString().Split(',');
                            for (int t = 0; t < teeth.Length; t++)
                            {
