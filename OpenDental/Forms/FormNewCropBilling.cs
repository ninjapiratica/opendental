...
namespace OpenDental
{
    public partial class FormNewCropBilling : FormODBase
    {
        ...
        private void CreateChargeList(bool isLoading)
        {
            ...
            for (int i = 2; i < stringArrayCsvLines.Length; i++)
            {//Skip the first row (the column names), skip the second row (the totals).
        ...
        private int GetChargeDayOfMonth(long patNum)
        {
            ...
            {
                if (repeatChargeArrayForPats[j].ProcCode == "001")
            ...
            for (int i = 0; i < _listNewCropCharges.Count; i++)
            {
                ...
                {//No such repeating charge exists yet for the given npi.
                 //We consider the provider a new provider and create a new repeating charge.
                    ...
                    if (dateErxCharge < DateTime.Today.AddMonths(-3))
                    ...
                    try
                    {
                        ...
                        if (isFutureDated)
                        {//Since we are future dating the repeating charge we need a second proc for the current month
                ...
                { //The repeating charge for eRx billing already exists for the given npi.
                    ...
                    if (_listNewCropCharges[i].repeatCharge.DateStop.Year > 2010)
                    {//eRx support for this provider was disabled at one point, but has been used since.
                        if (_listNewCropCharges[i].repeatCharge.DateStop < dateEndLastMonth)
