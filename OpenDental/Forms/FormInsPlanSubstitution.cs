...
namespace OpenDental
{
    public partial class FormInsPlanSubstitution : FormODBase
    {
        ...
        {
            if (e.Col != 2 || e.Row < 0 || e.Row >= gridMain.ListGridRows.Count)
            {//Return if not SubstOnlyIf column or invalid row
                return;
            }
            //Get the grid tag
        ...
        {
            if (e.Col != 3 || e.Row < 0)
            {//Return if not substitution code column or invalid row
                return;
            }
            InsPlanSubstitution insPlanSubstitutionOld = gridMain.SelectedTag<InsPlanSubstitution>();
            if (insPlanSubstitutionOld == null)
            {//This could happen if a substonlyif change occurred from the drop down
                return;
            }
        ...
        private void butDelete_Click(object sender, EventArgs e)
        {
            ...
            if (insPlanSubstitution.SubLink == null)
            {//User selected a procedure code level substitution code. Cannot delete
