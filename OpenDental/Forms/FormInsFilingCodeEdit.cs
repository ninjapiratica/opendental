...
namespace OpenDental
{
    ...
    public partial class FormInsFilingCodeEdit : FormODBase
    {
        ...
        {
            using FormInsFilingCodeSubtypeEdit formInsFilingCodeSubtypeEdit = new FormInsFilingCodeSubtypeEdit();
            formInsFilingCodeSubtypeEdit.InsFilingCodeSubtypeCur = _listInsFilingCodeSubtypes[e.Row].Clone();
            formInsFilingCodeSubtypeEdit.ShowDialog();
            if (formInsFilingCodeSubtypeEdit.DialogResult != DialogResult.OK)
            {
                FillGrid();
                return;
            }
            try
            {
                InsFilingCodeSubtypes.Update(formInsFilingCodeSubtypeEdit.InsFilingCodeSubtypeCur);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        {
            using FormInsFilingCodeSubtypeEdit formInsFilingCodeSubtypeEdit = new FormInsFilingCodeSubtypeEdit();
            formInsFilingCodeSubtypeEdit.InsFilingCodeSubtypeCur = new InsFilingCodeSubtype();
            formInsFilingCodeSubtypeEdit.InsFilingCodeSubtypeCur.IsNew = true;
            formInsFilingCodeSubtypeEdit.ShowDialog();
            if (formInsFilingCodeSubtypeEdit.DialogResult != DialogResult.OK)
            {
                return;
            ...
            formInsFilingCodeSubtypeEdit.InsFilingCodeSubtypeCur.InsFilingCodeNum = InsFilingCodeCur.InsFilingCodeNum;
            try
            {
                InsFilingCodeSubtypes.Insert(formInsFilingCodeSubtypeEdit.InsFilingCodeSubtypeCur);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
