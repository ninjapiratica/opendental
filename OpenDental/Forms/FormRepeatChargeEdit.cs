...
namespace OpenDental
{
    ...
    public partial class FormRepeatChargeEdit : FormODBase
    {
        ...
        private void FormRepeatChargeEdit_Load(object sender, EventArgs e)
        {
            ...
            _isErx = false;
            if (PrefC.GetBool(PrefName.DistributorKey) && Regex.IsMatch(_repeatCharge.ProcCode, "^Z[0-9]{3,}$"))
            {//Is eRx if HQ and a using an eRx Z code.
                _isErx = true;
                ...
                if (IsNew && _repeatCharge.ProcCode == "Z100")
                {//DoseSpot Procedure Code
            ...
            for (int i = 0; i < listDefNumsUnearnedTypeCur.Count; i++)
            {//There isn't an easy way to set selected multiple defs, so loop through each one that was selected on the RepeatCharge and set it selected if it matches an item from the list of all defs(listUnread)
        ...
        private void AddProcedureToCC()
        {
            ...
            { //Only one active card so ask the user to add the procedure to that one
            ...
            else if (listCreditCards.FindAll(x => x.Procedures != "").Count == 1)
            { //Only one card has procedures attached so ask the user to add to that card
            ...
            { //At least two cards have procedures attached to them or there are multiple active cards and none have procedures attached
        ...
        private void butManual_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                procedure = RepeatCharges.AddProcForRepeatCharge(chargeManual, DateTime.Today, DateTime.Today, orthoCaseProcLinkingData: new OrthoCaseProcLinkingData(_repeatCharge.PatNum));
            }
        ...
        private bool UpdateRepeatCharge(RepeatCharge repeatCharge)
        {
            ...
            if (PIn.Double(textChargeAmt.Text) < 0 && checkCreatesClaim.Checked)
            {//user entered a value less than zero while checkCreatesClaim is checked
            ...
            if (PIn.Date(textDateStart.Text) != _repeatCharge.DateStart)
            ...
            if (textErxAccountId.Text.Length > 2 && textErxAccountId.Text.Substring(0, 3).ToLower() == "ds;")
            {//support for DoseSpot account Ids
