...
namespace OpenDental
{
    ...
    public partial class FormPaySplitEdit : FormODBase
    {
        ...
        private void comboUnearnedTypes_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            if (PaySplitCur.UnearnedType > 0)
            {//If they use an unearned type the provnum must be zero if Edit Anyway isn't pressed
                PaySplitCur.ProvNum = 0;
        ...
        private void FillPatient()
        {
            ...
            if (PaySplitCur.PatNum == 0)
            {
                ...
                for (int i = 0; i < _family.ListPats.Length; i++)
                {//This is the same order added to listPatient.
        ...
        private void checkPatOtherFam_Click(object sender, System.EventArgs e)
        {
            ...
            if (checkPatOtherFam.Checked)
            {
                ...
                PaySplitCur.PatNum = formPatientSelect.PatNumSelected;
            }
            else
            ...
            if (!_isEditAnyway)
            {//When user clicks Edit Anyway they are specifically trying to correct a bad split, so don't clear it out.
                _procedure = null;
                DetachPayPlan();
                FillProcedure();
            }
        ...
        private void FillProcedure()
        {
            ...
            {//There is no good way to determine if a proc previously had TP unearned so we will just keep whatever loaded in and disable the box. 
        ...
        private bool IsValid()
        {
            ...
            if (_procedure != null && _adjustment != null)
            {//should not be possible, but as an extra precaution because this is not allowed. 
