...
namespace OpenDental
{
    ...
    public partial class FormScheduleDayEdit : FormODBase
    {
        ...
        private void FormScheduleDay_Load(object sender, System.EventArgs e)
        {
            ...
            if (_isFromSchedule && (_provAbbrFilter != "" || _employeeNameFilter != ""))
            {//single person was passed in from the schedule window.
                if (_provAbbrFilter != "")
                {//it was a provider schedule
                    textSearch.Text = _provAbbrFilter;
                }
                else
                {//it was an employee schedule
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < _listSchedules.Count; i++)
            {
                ...
                for (int j = 0; j < _listSchedules[i].Ops.Count; j++)
                {
                    ...
                    if (operatory == null || operatory.IsHidden)
                ...
                //note
                note = "";
                if (_listSchedules[i].SchedType == ScheduleType.Practice)
                {//note or holiday
                    string clinicAbbr = "";
                    if (PrefC.HasClinicsEnabled)
                    {
                        ...
                        {//must be a Note, only add 'Note' if clinics are enabled
        ...
        {
            if (scheduleX == scheduleY)
            {//also handles null==null
                return 0;
            }
            if (scheduleY == null)
            {
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (selectedSchedule.ProvNum > 0)
            {//Don't look for conflicts against a schedule with no provider.
        ...
        private bool ValidateLists()
        {
            ...
            if (listProv.SelectedIndices.Count > 1 && listProv.SelectedIndices.Contains(0))
        ...
        private void butAddTime_Click(object sender, System.EventArgs e)
        {
            ...
            if (!listProv.SelectedIndices.Contains(0))
            {// Add all selected providers to the provider number list
