...
namespace OpenDental
{
    public partial class FormProcEditAll : FormODBase
    {
        ...
        private void FillComboProv(bool tryMaintainOldSelection = false)
        {
            ...
            if (comboClinic.GetSelected<Clinic>() == null)
            {//Dummy clinic is selected either directly or on load due to procs having multiple clinics.
            ...
            if (tryMaintainOldSelection && comboProv.GetSelected<Provider>() != null)
            {//Only true on manual selection, not on load.
        ...
        private bool EntriesAreValid()
        {
            ...
            {
                if (!IsUserAuthorizedForProcDate(dateProc))
                {//Do not allow new ProcDate outside of date limitations.  Mimics behavior in FormProcEdit.
                    return false;
                }
                Appointment appointment;
                for (int i = 0; i < ListProcedures.Count; i++)
                {//first validate for all dates.
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ListProcedures.Count; i++)
            {
                ...
                if (dateProc != DateTime.MinValue && ListProcedures[i].ProcDate != dateProc)
                {//Using value entered in the textbox.
                ...
                if (comboProv.GetSelected<Provider>() != null && comboProv.GetSelected<Provider>().ProvNum != ListProcedures[i].ProvNum)
                {//Using selection
                ...
                if (comboClinic.GetSelected<Clinic>() != null && comboClinic.GetSelected<Clinic>().ClinicNum != ListProcedures[i].ClinicNum)
                {//Using selection
