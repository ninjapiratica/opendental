...
namespace OpenDental
{
    ...
    public partial class FormASAP : FormODBase
    {
        ...
        private void FormASAP_Load(object sender, System.EventArgs e)
        {
            ...
            {//Not signed up for Web Sched ASAP
        ...
        private void LayoutControls()
        {
            ...
            int splitContainerHeight = splitContainer.Height;
            int splitPanelHeight = splitContainer.Panel1.Height;
            int tabControlHeight = tabControl.Height;
            int clientSizeHeight = tabControl.ClientSize.Height;
            int tabPageHeight = tabPageRecalls.Height;
            LayoutManager.Move(gridAppts,
                new Rectangle(1, LayoutManager.Scale(33), tabPageRecalls.Width - 2, tabPageRecalls.Height - LayoutManager.Scale(33) - 2));
        ...
        private void FillGridAppts()
        {
            ...
            for (int i = 0; i < _listAppointmentsASAP.Count; i++)
            {
                ...
                if (_listAppointmentsASAP[i].AptDateTime.Year < 1880)
        ...
        private List<PatComm> GetPatCommsForTexting(Clinic clinic, GridOD grid, List<long> listPatNumsInGrid, Func<int, long> getPatNumFromGridRow)
        {
            ...
            {//None selected. Select all that can be texted.
            ...
            {//Backwards since we are deselecting rows.
        ...
        private string GetTextMessageText(Clinic curClinic)
        {
            ...
            if (DateTimeChosen.Year > 1880)
            {//The user clicked on the appt schedule to get here.
            ...
            if (listInputBoxParams.Count > 0 && inputBox.ShowDialog() == DialogResult.OK)
            {
                ...
                if (textTemplate.Contains("[Time]") && inputBox.ListTextEntered[listInputBoxParams.Count - 1] != "")
                {//A time was entered.
        ...
        {
            if (!IsSendingWebSched() || tabControl.SelectedIndex != 0)
            {//Recall tab is selected.
                return;
            }
        ...
        {
            if (!IsSendingWebSched() || tabControl.SelectedIndex != 1)
            {//Appt tab is selected.
                return;
            }
        ...
        {
            if (IsDisposed)
            {//The user closed the form while the thread checking Web Sched signups was still running.
                return;
            }
            if (_threadWebSchedSignups != null)
            {//The thread checking clinics that are signed up for Web Sched has not finished getting the list.
                _hasClickedWebSched = true;//The thread checking Web Sched signups will call this method on exit.
                Cursor = Cursors.AppStarting;
                return;
            }
            if (_listClinicNumsWebSched.Count == 0)
            ...
            if (!IsSendingWebSched())
            {//Did not get to this window by right-clicking in the Appointment module.
                MsgBox.Show(this,
