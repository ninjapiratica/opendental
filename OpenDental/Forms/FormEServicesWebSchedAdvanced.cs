...
namespace OpenDental
{
    public partial class FormEServicesWebSchedAdvanced : FormODBase
    {
        ...
        private void LoadAndDisplayClinic(EServiceClinicName eserviceClinicName)
        {
            ...
            if (userControlHostedURL == null)
            {//No controls displaying, so just display the current one.
                LayoutUserControlHostedURL(eserviceClinicName);
            }
            else
            {//Save the control if we haven't already.
                ...
                if (userControlHostedURLSaved == null)
                {//We haven't saved this control before.
                    _listUserControlHostedURLs.Add(userControlHostedURL);
                    LayoutUserControlHostedURL(eserviceClinicName);
                }
                else
                {//Load the saved control
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            if (userControlHostedURL != null && !_listUserControlHostedURLs.Any(x => x.GetClinicNum() == userControlHostedURL.GetClinicNum()))
            {//No duplicates.
                _listUserControlHostedURLs.Add(userControlHostedURL);
            }
