    ...
    public partial class FormPatientEdit : FormODBase
    {
        ...
        private void FormPatientEdit_Load(object sender, System.EventArgs e)
        {
            ...
            if (programErx.Enabled && (erxOption == ErxOption.DoseSpot || erxOption == ErxOption.DoseSpotWithNewCrop))
            {
                ...
                if (checkDoseSpotConsent.Checked)
                {//once checked, should never be able to be unchecked.
            ...
            listTextOk.SelectedIndex = (int)_patient.TxtMsgOk;
            ...
            string errorMsg)
        {
            ...
            while (parent != null)
            {
                ...
                {
                    ((System.Windows.Forms.TabPage)parent).Text = ((System.Windows.Forms.TabPage)parent).Text.Replace("*", "") + "*";
                }
        ...
        private void odDatePickerBirthDate_Validated(object sender, System.EventArgs e)
        {//Does fire even though textBox is read only.
            CalcAge();
        }
        ...
        {
            if (odDatePickerBirthDate.ReadOnly)
            {//showing xx/xx/xxxx
                return;
            }
            textAge.Text = "";
        ...
        private void comboZip_SelectionChangeCommitted(object sender, System.EventArgs e)
        {
            ...
            {//if somehow nothing is selected. Can happen if selecting something and draging up outside of the combobox.
                return;
            }
        ...
        private void UpdateLocalNameHelper()
        {
            ...
            {//update the Family object as well
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if ((bool)objectArrayParameters[0] == false)
            ...
            if (comboPriProv.GetSelectedProvNum() == 0)
            {//selected index could be -1 if the provider was selected and then hidden
            ...
            if (PrefC.GetBool(PrefName.ShowPreferredPronounsForPats))
            {//Only set when preference is enabled.
            ...
            if (!textSSN.ReadOnly && Patients.SSNRemoveDashes(textSSN.Text) != _patientOld.SSN)
            { //Only update SSN if it was changed, readonly must be false to edit, meaning it is also unmasked.
                if (CultureInfo.CurrentCulture.Name == "en-US")
                {
                    if (Regex.IsMatch(textSSN.Text, @"^\d\d\d-\d\d-\d\d\d\d$"))
                    {
            ...
            _patient.TxtMsgOk = (YN)listTextOk.SelectedIndex;
            ...
            if (!_isUsingNewRaceFeature)
            {
                ...
                else if (_listPatientRaces.Any(x => x.CdcrecCode == "2131-1"))
            ...
            if (comboExcludeECR.SelectedIndices.Contains(0))
            { //If "None" is selected
            ...
            if (!PrefC.GetBool(PrefName.EasyHideHospitals))
            { //Only update if Hospital tab is showing
            ...
            if (checkBillProvSame.Checked)
            {
                ...
                if (!isAuthArchivedEdit)
                {//Remove Archived patients if not allowed to edit so we don't create a log for them.
        ...
        private void FormPatientEdit_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (_hasGuardiansChanged)
            {  //If guardian information was changed, and user canceled.
