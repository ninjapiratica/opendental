...
namespace OpenDental
{
    ...
    public partial class FormFeeSchedTools : FormODBase
    {
        ...
        private void FillComboBoxes()
        {
            ...
            //Fee Sched Groups
            if (checkShowGroups.Visible)
            {//Always run the fill logic if they are using the groups feaure, not just if the combobox is showing.
        ...
        private void FillFeeSchedGroupComboBox(UI.ComboBox comboFeeSchedGroup, long feeSchedNumSelected, long feeSchedGroupNum)
        {
            ...
            {
                if (!listComboGroupNums.Contains(listFeeSchedGroups[i].FeeSchedGroupNum) && listFeeSchedGroups[i].ListClinicNumsAll.Count > 0)
                {//Skip duplicate/empty groups.
        ...
        private void butCopy_Click(object sender, System.EventArgs e)
        {
            ...
            {
                if (!PrefC.HasClinicsEnabled || listClinicNumsTo.Contains(fromClinicNum))
                {//If clinics disabled, can cause false negative so shortcircuit
        ...
        private bool EvaluateOverrides(long clinicNum, long provNum, long feeSchedNum, List<Fee> listFees)
        {
            ...
            if (clinicNum != 0 && provNum != 0)
            {//user seems to be trying to increase fee overrides for clinic and prov
                if (listFees.Count == 0)
        ...
        private void butUpdateFees_Click(object sender, EventArgs e)
        {
            ...
            {//no clinic - "Clinic" here is just a reference to the progress bar that updates Clinic progress instead of overall progress
        ...
        private void butUpdateWriteoffs_Click(object sender, EventArgs e)
        {
            ...
            {//This is assuming clinics are turned off
