    ...
    public partial class FormHL7DefSegmentEdit : FormODBase
    {
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            if (HL7DefSegmentCur.ItemOrder == 0 && HL7DefSegmentCur.SegmentName == SegmentNameHL7.MSH)
            {
                for (int i = 0; i < HL7DefSegmentCur.hl7DefFields.Count; i++)
                {
                    if (HL7DefSegmentCur.hl7DefFields[i].FieldName == "separators^~\\&" && HL7DefSegmentCur.hl7DefFields[i].OrdinalPos != 1)
                    {//we force the separators^~\\& to be in field 1 of the message header segment or we will not know how to split the messageType field (usually messageType^eventType, but the '^' is defined in the separator segment) to get the message type
                        MsgBox.Show(this, "The separators^~\\& field must be in position 1 of the message header segment.");
                        return;
                    }
                    if (HL7DefSegmentCur.hl7DefFields[i].FieldName == "messageType" && HL7DefSegmentCur.hl7DefFields[i].OrdinalPos != 8)
