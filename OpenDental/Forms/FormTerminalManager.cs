        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < listSheetDevices.Count(); i++)
            {
                ...
                void CellClick(object sender, EventArgs e)
                {
                    ...
                    if (sheetDeviceCurrent.GetPatNum() == 0)
                    { //we are trying to load the patient
                        ...
                        if (sheetDeviceCurrent.IsKiosk())
                        { //kiosk only
                            if (listSheets.Items.Count == 0)
                            { //eClipboard will allow to continue to load here in case we just want to take a photo
                    ...
                    { //we are trying to clear the patient
                        ...
                        if (sheetDeviceCurrent.MobileAppDevice != null)
                        { // device might have disconnected without the grid being updated yet
