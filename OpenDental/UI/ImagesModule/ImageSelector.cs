    ...
    public partial class ImageSelector : Control
    {
        ...
        public void LoadExpandedPrefs()
        {
            ...
            for (int i = 0; i < _listDefsImageCats.Count; i++)
            {
                ...
                {//User has a expanded preference for this image category.
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            for (int i = 0; i < _listNodeObjTags.Count; i++)
            {
                ...
                {
                    if (_listNodeObjTags[i].Thumbnail is null)
                    {//lazy load, so this doesn't actually happen on each paint.
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            {//already dragging before this click
            ...
            if ((e.Button & MouseButtons.Right) == MouseButtons.Right)
            {
                ...
                if (_listNodeObjTags[idx].IsMatching(_nodeObjTagSelected))
                {//On node already selected
                ...
                if (ContextMenu != null)
                {//When using FormImagePickerPatient, the ContextMenu control is not initialized so we need to do a null check to prevent errors.
        ...
        private void ComputeRows()
        {
            ...
            int hRowThumb = LayoutManager.Scale(100) + Font.Height * 2 + LayoutManager.Scale(10);
            for (int i = 0; i < _listNodeObjTags.Count; i++)
            {
                if (_listNodeObjTags[i].NodeType == EnumImageNodeType.Category)
                ...
                else if (_listNodeObjTags[i].IsThumbnail)
                {
                    ...
                    if (_listNodeObjTags[i].DocCategory != defNumCat)
                    ...
                    {
                        if (xPos + _listNodeObjTags[i].Width > Width - vScroll.Width + LayoutManager.ScaleF(3))
                        {//too wide to fit
        ...
        private void SaveExpandedPref(long defNum, bool isExpanding)
        {
            ...
            if (isExpanding)
            {//Insert a row to save an expanded category.
        ...
        private void ToolTipSetString(Point point)
        {
            ...
            if (_listNodeObjTags[idx].IsThumbnail)
            {
                ...
                int widthTxt = (int)g.MeasureString(_listNodeObjTags[idx].Description, Font).Width;
            ...
            int widthTxt2 = (int)g.MeasureString(str, Font).Width;
