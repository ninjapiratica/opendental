...
namespace OpenDental.UI
{
    ...
    {
        private System.Windows.Forms.TabControl tabHqBugControl;
        private System.Windows.Forms.TabPage tabBugsFixed;
        private GridOD gridBugsFixed;
        private System.Windows.Forms.TabPage tabBugSubs;
        ...
        private System.Windows.Forms.TabPage tabBugsInProgress;
        ...
        {
            if (_patCur == null || gridBugsFixed == null)
            {//Just in case.
                return;
            }
            ...
            if (isRefreshNeeded)
            {
                ...
                {//Should only occur if running in debug at HQ and no bugs database on local machine.
                    ex.DoNothing();
            ...
            foreach (BugSubmission sub in _listBugSubs)
            {
                ...
                if (sub.BugObj != null && !listBugNums.Contains(sub.BugObj.BugId))
                {//BugSubmission has an associated bug and it is not a duplicate.
                    if (string.IsNullOrEmpty(sub.BugObj.VersionsFixed))
                    {//Bug is not fixed
