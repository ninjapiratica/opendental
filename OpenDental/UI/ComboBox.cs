...
namespace OpenDental.UI
{
    ...
    //To replace an existing ComboBox with a UI.ComboBox or a ComboBoxClinicPicker:
    ...
    public partial class ComboBox : Control
    {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            base.OnMouseDown(e);
            if (Items.Count == 0)
            {//even if IncludeAll, we still don't want it to show.
                return;
            }
            ...
            if (!IsAllSelected)
            {//can't include other indices if "all" selected
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (char.IsLetterOrDigit(charKey) && !SelectionModeMulti)
            {//alpha character down
        ...
        private void _formComboPicker_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                if (!IsAllSelected)
                {//if all is selected, we ignore other selections
        ...
        {
            if (defNum == 0)
            {//special handling for 0. We don't want to go looking in the cache.
                _listSelectedIndices.Clear();
                _overrideText = "";
                _selectedKey = 0;
