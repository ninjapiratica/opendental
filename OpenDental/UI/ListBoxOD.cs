...
namespace OpenDental.UI
{
    ...
    public partial class ListBoxOD : Control
    {
        ...
        private static SolidBrush _brushSelectedBack = new SolidBrush(Color.FromArgb(186, 199, 219));//grid default was Silver: C0C0C0 (192 gray)
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (char.IsLetterOrDigit(charKey))
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (Items.Count > maxItems)
            {
                ...
                if (vScroll.Maximum != oldScrollMaxValue)
                {//set the scrollbar value on update/load of the ListBox items
                    SetVScrollValue();
                }
            }
            else
            ...
            for (int i = _indexTopShowing; i < totalToDraw; i++)
            {
                ...
                if (isSelected)
                { //Draw the selected index with gray background
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (_mouseDownIndex > Items.Count - 1)
            {//clicked below items
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            {
                if (e.Y < 0)
                {//if mouse is above the top of the ListBox while mouse down
                    VScrollMoveUp();
                }
                else if (e.Y > Height)
                {//if mouse is below the bottom of the ListBox while mouse down
                    VScrollMoveDown();
                }
            }
            ...
            if (!listSelectedOrig.SequenceEqual(_listSelectedIndices))
            {//just comparing to the local list for this move
                SelectionChangeCommitted?.Invoke(this, new EventArgs());
                SelectedIndexChanged?.Invoke(this, new EventArgs());
            }
            Invalidate();
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            {
                if (vScroll.Value > vScroll.Maximum - this.Height && vScroll.Maximum - this.Height >= 0)
                {//checks when resizing if the items need to move down
        ...
        public long GetSelectedKey<T>(Func<T, long> funcSelectKey)
        {
            ...
            if (Items.GetObjectAt(_listSelectedIndices[0]) == null)
            {//just in case
                return 0;
            }
        ...
        private bool SetSearchedIndex(int startIdx, char charKey)
        {
            ...
            {//loop through all of the items and only add the item if it is found
                if (Items.GetTextShowingAt(i).ToUpper().StartsWith(charKey.ToString()))
        ...
        private void SetVScrollValue()
        {
            ...
            if (vScroll.Value > vScroll.Maximum - this.Height)
            {//check if the scrollbar is below the last item
            ...
            if (_listSelectedIndices[_listSelectedIndices.Count - 1] < _indexTopShowing)
            {//the latest selected index is above the bounds
            ...
            else if ((_listSelectedIndices[_listSelectedIndices.Count - 1] - _indexTopShowing + 1) * _heightLine > this.Height)
            {//the latest selected index is hidden below bounds
        ...
        public class ListBoxItemCollection
        {
            ...
            public bool Contains<T>(T item)
            {
                for (int i = 0; i < _listListBoxItems.Count; i++)
                {
                    ...
                    {
                        if (_listListBoxItems[i].Item.Equals(item))
                        {//this would fail if this method was not generic
                            return true;
                        }
                    }
                }
