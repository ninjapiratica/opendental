...
namespace OpenDental.UI
{
    ...
    public partial class ComboBoxClinicPicker : UserControl
    {
        ...
        private void _formComboPicker_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {//this needs to be set to null if a hidden clinic is no longer selected.  See OnMouseDown.
        ...
        private void FillClinics()
        {
            ...
            try
            {
                ...
                {
                    if (!PrefC.HasClinicsEnabled)
                    {//Clinics are turned off, but
                        if (Visible && !Enabled)
                        {//this combobox was set visible after the Load (FormProcCodes)
