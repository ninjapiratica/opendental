...
namespace OpenDental.UI
{
    ...
    public partial class ControlApptProvSlider : Control
    {
        ...
        public string Pattern
        {
            ...
            {
                if (value == null)
                {//Appointments.CalculatePattern can pass in null to indicate no change
                    return;
                }
                if (_pattern == value)
                {
                ...
                { //The slider already handles changing the length of the patterns
        ...
        public string ProvBarText
        {
            get
            {
                if (_provBarText.Length > SelectedLength)
                {//if longer than what's showing
        ...
        private int SelectedLength
        {
            ...
            set
            {
                ...
                if (MinPerIncr == 10 && SelectedIndex > lengthNew / 2 - 1)
                {//If we are hiding the cursor, then officially set it to -1
                    SelectedIndex = -1;
                }
        ...
        protected override void OnKeyPress(KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == '\r')
            {//Enter implies user wants to force text down
                if (SelectedIndex < _provBarText.Length)
                {//if this is within existing text, insert a space 
            ...
            {//backspace
                ...
                if (SelectedIndex < _provBarText.Length + 1)
                {//if this is within or immediately after existing text, delete
            ...
            //}
            if (SelectedIndex < _provBarText.Length - 1 && _provBarText[SelectedIndex] == ' ')
            {//if the current pos is an intermediate space, replace it
                ProvBarText = _provBarText.Substring(0, SelectedIndex) + e.KeyChar + _provBarText.Substring(SelectedIndex + 1);
                SelectedIndex++;
                return;
            }
            if (SelectedIndex > _provBarText.Length)
            {//if there is any gap after the end, fill it. Example 3>2
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            Focus();
            if (e.X < _marginLR + _widthLeft)
            {//in the prov area at the left
                SelectedIndex = -1;
                _indexMouseDownProv = (int)(e.Y / _heightRow);
                _isProv2 = false;
                ToggleProv(_indexMouseDownProv);
            }
            else if (e.X < _marginLR + _widthLeft * 2)
            {//in the prov2 area
                SelectedIndex = -1;
                _indexMouseDownProv = (int)(e.Y / _heightRow);
                _isProv2 = true;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (_isMouseDown)
            {
                ...
                else if (_indexMouseDownProv != -1)
                {//dragging in the prov area
                    if (e.Y < 0)
                    {
                        return;
                    }
                    ...
                    if (idxNew != _indexMouseDownProv)
                    {//in a new cell
                        if (_isProv2)
                        {//in the second prov area
                            ToggleProv2(idxNew);
                        }
                        else
                        {//in the prov area at the left
        ...
        protected override bool ProcessDialogChar(char charCode)
        {
            ...
            {//But if Alt-char, then it will bubble up
