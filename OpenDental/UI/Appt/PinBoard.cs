...
namespace OpenDental.UI
{
    public partial class PinBoard : Control
    {
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            {//managed resources
                ...
                for (int i = 0; i < ListPinBoardItems.Count; i++)
                {
                    ...
                    {//date was updated since put on the pinboard
                        continue;
                    }
                    Appointments.Delete(appt.AptNum, true);
                    if (Security.CurUser == null)
                    {// E.g. clicking Log Off invalidates the user.
                        continue;
                    }
        ...
        {
            if (_isMouseDown)
            {//User right clicked while dragging appt around.
                return;
            }
            ...
            if (contrTempPinAppt.Right < Left + Parent.Left + Parent.Parent.Left + _pointMouseOrigin.X)
            {//appointment is very narrow
        ...
        public void SetBitmapTempPinAppt(Bitmap bitmap)
        {
            ...
            if (bitmap == null)
            {//ContrAppt is telling pinboard that we can't drag appt off.
