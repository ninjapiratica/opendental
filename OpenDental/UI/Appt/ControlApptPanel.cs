    ...
    public partial class ControlApptPanel : UserControl
    {
        ...
        public List<Provider> ListProvsVisible
        {
            ...
            set
            {
                ...
                if (_showProvBars)
                {//this is also present in IsWeeklyView.set and LayoutRecalcAfterResize()
        ...
        public bool IsWeeklyView
        {
            ...
            set
            {
                ...
                if (_showProvBars)
                {//this is also present in ListProvsVisible.set and LayoutRecalcAfterResize()
                    _widthMainVisible = this.Width - (int)_widthTime * 2 - (int)(_widthProv * _listProvsVisible.Count) - vScrollBar1.Width - 1;
                    hScrollBar1.Left = (int)(_widthTime + _widthProv * _listProvsVisible.Count) + 1;
                }
                else
        ...
        private void LayoutRecalcAfterResize()
        {
            ...
            if (_showProvBars)
            {//this is also present in ListProvsVisible.set and IsWeeklyView.set
                _widthMainVisible = this.Width - (int)_widthTime * 2 - (int)(_widthProv * _listProvsVisible.Count) - vScrollBar1.Width - 1;
            }
            else
            {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (timeClicked == null)
            {//clicked outside the main bitmap
                return;//?
            }
            ...
            {//clicked on empty space, not appt
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            {
                if (contrTempAppt.Visible)
                {//mouse was down when some other program stole focus, leaving orphaned appt.
                    contrTempAppt.Visible = false;
                    Invalidate();
                }
                int idxAppt = HitTestAppt(e.Location);
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            try
            {
                ...
                if (_isResizingAppt)
                {
                    ...
                    if (dateTimeTempAppt.Day != (dateTimeTempAppt + newspan).Day)
                    {//I don't think this can get hit
                    ...
                    {//max length of 390 minutes. Not sure why.  I want to increase this to 9 hours.
                        newpatternL = 78;
                    }
                ...
                if ((Math.Abs(e.X - _pointMouseOrigin.X) < 7) && (Math.Abs(e.Y - _pointMouseOrigin.Y) < 7))
                ...
                if (contrTempAppt.Location.X > this.Right)
                { //Dragging to pinboard, so place a copy there.
                    ...
                    if ((ApptStatus)PIn.Int(_dataRowTempAppt["AptStatus"].ToString()) == ApptStatus.Complete)
                    {//could cause completed procs to change date
                ...
                if (appt.AptStatus == ApptStatus.Complete && isMovingApptToDifferentDay)
                {//Completed appointment moving to different day.
        ...
        public void PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            if (dHoursPerPage < 1 && dHoursPerPage >= .5)
            {//Defaults to 30 min per page.
                dHoursPerPage = .5;
            }
            else if (dHoursPerPage < .5 && dHoursPerPage >= .25)
            {//Defaults to 15 min per page.
                dHoursPerPage = .25;
            }
            else if (dHoursPerPage < .25 && dHoursPerPage >= 0.2)
            {//Defaults to 12 min per page.
                dHoursPerPage = .2;
            }
            else if (dHoursPerPage < .2)
            {//Defaults to 6 min per page.
                dHoursPerPage = .10;
            }
            if (IsWeeklyView)
            {
            ...
            if (dHoursPerPage < 1)
            {//Only for extreme edge cases where one hour is taller than a page.
            ...
            if (hourEnd > stopHour)
            {//Don't show too many hours on the last page
                hourEnd = stopHour;
            }
        ...
        public string GetPatternShowing(string dbPattern)
        {
            ...
            for (int i = 0; i < dbPattern.Length; i++)
            {
                ...
                {//Example: 10 MinPerIncr, 2 RowsPerIncr, so 12 per hour
        ...
        public void RedrawAsNeeded()
        {
            ...
            if (!_isValidHeaders)
            {//need widths above in order to calc whether text will wrap
        ...
        {
            if (AppointmentRules.GetCount() == 0)
            { //If no rules exist then don't bother checking.
                return false;
            }
        ...
        private void SetBitmapTempAppt()
        {
            ...
            if (patternShowing.Length < 96)
            {//8hrs x 12 increments
            ...
            {//only one day showing
             ...
             //SchedListPeriod contains scheds for only one day, not for a week
                ...
                {//ops per page in day view
                    if (i == ListOpsVisible.Count)
                    {//for printing, when _countCol is a smaller per-page loop
                        break;
                    }
        ...
        private void DrawStringWithSpacing(Graphics g, string str, Font font, Brush brush, RectangleF rectangleLayout, float heightLine)
        {
            ...
            while (true)
            {
                ...
                if (linesFilled == 0 || charactersFitted == 0)
                {//can happen when half height rows can't draw			
        ...
        private void DrawProvBarsAppts(Graphics g)
        {
            ...
            {//_provBar.Length;j++) {
             //loop through increments.  If two rowsPerIncr, then the loop will handle both at the same time
                for (int i = 0; i < 24 * (int)(60 / _minPerIncr); i++)
                {//_rowsPerHr
        ...
        private void DrawOneAppt(Graphics g, DataRow dataRoww, string patternShowing, float widthAppt, float heightAppt, bool showBrokenX = true)
        {
            ...
            if (widthAppt < 14)
            {//Narrow appointments will look very simple, with no rounded corners or text
                if (_isPrinting && PrintColorBehavior == ApptPrintColorBehavior.LessColor)
                {
                    if (PIn.Long(dataRoww["AptStatus"].ToString()) == (int)ApptStatus.Complete)
                    {
                ...
                if (widthAppt < 6)
                {//below 6, the black side lines are distracting
            ...
            while (pointDraw.Y < heightAppt && elementI < ListApptViewItemRowElements.Count)
            {
                ...
                {
                    if (ListApptViewItemRowElements[elementI].ElementDesc != "ProcsColored")
                    {//ProcsColored will behave differently than other patfields and will only be drawn in Main, even if in UR or LR
                        elementI++;
                        continue;
                    }
                }
            ...
            //broken X
            if (showBrokenX)
            {//this gets skipped when dragging appt size
        ...
        {
            if (widthAppt < WidthProvOnAppt + 5)
            {//Appointment is too narrow to draw anything
            ...
            {//always stacks vertical
                ...
                {
                    if (text == "")
                    {//the text should have the status and if it doesn't, return the point we're currently at becuase there is nothing to draw
                        return pointDraw;
                    }
                    Size sizeDesired = new Size((int)_heightLine, (int)_heightLine);
        ...
        private void DrawOutlineSelected(Graphics g)
        {
            ...
            for (int i = 0; i < TableAppointments.Rows.Count; i++)
            {
                ...
                Pen penProvOutline;
                if (dataRow["ProvNum"].ToString() != "0" && dataRow["IsHygiene"].ToString() == "0")
                {//dentist
                    penProvOutline = new Pen(Providers.GetOutlineColor(PIn.Long(dataRow["ProvNum"].ToString())), 3f);
                }
                else if (dataRow["ProvHyg"].ToString() != "0" && dataRow["IsHygiene"].ToString() == "1")
                {//hygienist
                ...
                if (rectangleF.Bottom < 0 || rectangleF.Top > _heightMainVisible + _heightProvOpHeaders)
                {//if outside visible area
                    return;
                }
        ...
        private void DrawRedTimeLine(Graphics g)
        {
            ...
            if (yVal < _heightProvOpHeaders || yVal > _heightMainVisible + _heightProvOpHeaders)
            {//if outside visible area
                return;
            }
        ...
        private void BubbleDraw(Point point, int idxAppt)
        {
            ...
            int yval = point.Y + 10;
            int xval = point.X + 10;
            if (idxAppt == _bubbleApptIdx)
            {//if the pointer is still on the same appt as last time
            ...
            for (int i = 0; i < listDisplayFieldsBubble.Count; i++)
            {
                ...
                if (CompareFloat.IsEqual(x, LayoutManager.ScaleF(8)))
                ...
                switch (listDisplayFieldsBubble[i].InternalName)
                {
                    case "Patient Name":
                        ...
                        if (isNameWrapping)
                        {//Patient name is first and wraps.
                    ...
                    case "Note":
                        ...
                        if (s.Trim() != "" && maxNoteLength > 0 && s.Length > maxNoteLength)
                        {//Trim text
                    ...
                    case "Horizontal Line":
                        if (showingPatientPicture && y < LayoutManager.Scale(100) + imageOffset)
                        { //height of pat picture is 100 plus the image offset
                    ...
                    case "Fam Note":
                        ...
                        {
                            if (s.Length > 32000)
                            {//GDI+ only allows 32,000 characters at a time
            ...
            if (showingPatientPicture)
            { //only draw the pat picture if the display field is present
        ...
        private void ComputeHeight()
        {
            ...
            if (_heightMain > _heightMainVisible)
            {//the normal situation, where part of the main area is hidden by scrolling
        ...
        private void HeaderTipDraw(Point point)
        {
            ...
            else if (point.X > _widthTime + _widthProv * _listProvsVisible.Count)
            {//to the right of prov header, so in ops area
                if (ListOpsVisible.Count == 0)
                {
                    panelHeaderTip.Visible = false;
                    return;
