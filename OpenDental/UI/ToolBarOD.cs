...
namespace OpenDental.UI
{
    ...
    public class ToolBarOD : System.Windows.Forms.UserControl
    {
        ...
        protected override void OnMouseMove(System.Windows.Forms.MouseEventArgs e)
        {
            ...
            if (_toolBarButtonHot != null)
            {//first handle any old hot button
                if (_toolBarButtonHot != toolBarButtonOver)
                {//if we have moved to hover over a new button, or to hover over nothing
                    _toolBarButtonHot.State = ToolBarButtonState.Normal;
                    Invalidate();//_toolBarButtonHot.Bounds);
                }
            }
            if (toolBarButtonOver != null)
            {//then, the new button
                if (_toolBarButtonHot != toolBarButtonOver)
        ...
        protected override void OnMouseUp(System.Windows.Forms.MouseEventArgs e)
        {
            ...
            {//there was a hotButton
                ...
                Invalidate();//_toolBarButtonHot.Bounds);//invalidate the old button
                             //CLICK: 
                if (_toolBarButtonHot == toolBarButton)
                {//if mouse was released over the same button as it was depressed
                        ...
                        && HitTestDrop(toolBarButton, e.X, e.Y))//and we are in the dropdown area on the right
                    {
                        ...
                        {//there is a dropdown menu to display
                            _toolBarButtonHot = null;
                    ...
                    else if (toolBarButton.Style == ODToolBarButtonStyle.ToggleButton)
        ...
        private void ComputeButtonsizes(Graphics g)
        {
            ...
            for (int i = 0; i < buttons.Count; i++)
            {
                ...
                else if (buttons[i].Icon != EnumIcons.None)
                {//button with Icon
                ...
                else if (imageList == null || buttons[i].ImageIndex == -1)
                {//Normal button
                    width = (int)g.MeasureString(buttons[i].Text, Font).Width + LayoutManager.Scale(6);
                }
                else
                {//Image List button (being deprecated)
        ...
        private void DrawButton(Graphics g, ODToolBarButton button)
        {
            ...
            if (isNotify)
            {//Override dropdown background to show notification color.
            ...
            if (button.Icon != EnumIcons.None || bitmap != null)
            {//draw image and text
                if (!button.Enabled)
                {
                ...
                else if (button.State == ToolBarButtonState.Pressed)
