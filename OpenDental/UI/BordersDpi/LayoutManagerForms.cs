		...
		*/
        ...
        public LayoutManagerForms(FormODBase formODBase, bool isLayoutMS, bool is96dpi)
        {
            ...
            for (int i = 0; i < listControls.Count; i++)
            {
                ...
                {
                    if (listControls[i].Parent is DomainUpDown)
        ...
        ///<summary>Deprecated. Only for Windows.Forms.TabPage. For all other Remove situations, don't run them through LayoutManager.  Just use native Remove, Clear, etc.  The only gotcha is that Form.Controls.Clear() needs to change to Form.PanelClient.Controls.Clear()</summary>
        public void Remove(Control control)
        {
            //Removes the tabPage from its parent. This does not do control.Dispose, nor does it remove the control from our tracked list.  This way, if it (and its children) are later added back, they are still available.  But, if a control is disposed in form code, we should add another method here to remove it from our list.
            if (control is System.Windows.Forms.TabPage forms_TabPage)
            {
                if (forms_TabPage.Parent != null)
                {
                    forms_TabPage.Parent.Controls.Remove(forms_TabPage);
                }
                return;
            }
            //This didn't work. Better to just track down all incoming references and fix them.
            //if(control is UI.TabPage ui_TabPage){
            //	throw new Exception("Instead, use TabControl.TabPageRemove.");
            //}
        ...
        public void Move(Control control, Rectangle boundsScaled)
        {
            ...
            {//this is here to keep it from crashing further down.
        ...
        public void SetPanelClient(FormODBase formODBase)
        {
            ...
            for (int i = 0; i < listControls.Count; i++)
            {//this list is just the direct children
        ...
        private void LayoutChildren(Control control)
        {
            ...
            for (int i = 0; i < control.Controls.Count; i++)
            {
                ...
                if ((control96Info.Anchor & (AnchorStyles.Left | AnchorStyles.Right)) == (AnchorStyles.Left | AnchorStyles.Right))
                {//left and right
                    width += sizeParentClientNow96.Width - control96InfoParent.ClientSize96.Width;
                }
                else if ((control96Info.Anchor & (AnchorStyles.Left | AnchorStyles.Right)) == AnchorStyles.Right)
                {//only right
                    x += sizeParentClientNow96.Width - control96InfoParent.ClientSize96.Width;
                }
                if ((control96Info.Anchor & (AnchorStyles.Top | AnchorStyles.Bottom)) == (AnchorStyles.Top | AnchorStyles.Bottom))
                {//top and bottom
        ...
        private bool LayoutSkipChildren(Control96Info control96Info, Control control)
        {
            ...
            {
                gridOD.SetScaleAndFont(ScaleMy());//grids are over in OpenDentBusiness, so this is instead of a ref to LayoutManager.
                return true;//grids handle their own layout of scrollbars, etc.
            }
            if (control is UI.GridOld gridOld)
            {
                gridOld.ScaleMy = ScaleMy();
                return true;
            }
        ...
        private bool LayoutSkipSelf(Control96Info control96Info, Control control)
        {
            if (control96Info == null)
            {
                ...
                if (control is SparksToothChart.ToothChartDirectX)
                {//this one can't be added manually, so we'll add it.
                    AddToList96(control, control.Parent);
            ...
            if (control is UI.MenuStripOD)
            {//This is not MenuOD, but is the MenuStrip inside of it.
        ...
        private void PassLayoutManager(Control control)
        {
            ...
            if (control is DashFamilyInsurance dashFamilyInsurance)
            {//only gets hit during non-dashboard, in controlTreat
                dashFamilyInsurance.LayoutManager = this;
                return;
            }
            if (control is DashIndividualDiscount dashIndividualDiscount)
            {//only gets hit during non-dashboard, in controlTreat
                dashIndividualDiscount.LayoutManager = this;
                return;
            }
            if (control is DashIndividualInsurance dashIndividualInsurance)
            {//only gets hit during non-dashboard, in controlTreat
