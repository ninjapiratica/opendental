...
namespace OpenDental
{
    ...
    public class ODtextBox : RichTextBox
    {//System.ComponentModel.Component
        ...
        protected override CreateParams CreateParams
        {
            get
            {
                ...
                try
                {
                    ...
                    {//only try to load the library if not loaded already.
                        _libPtr = LoadLibrary("msftedit.dll");
                    }
                    if (_libPtr == IntPtr.Zero)
                    {//still zero, library was not loaded successfully
                        return cParams;
                    }
        ...
        private void contextMenu_Popup(object sender, System.EventArgs e)
        {
            ...
            if (!this._spellCheckIsEnabled
              ...
              || !IsOnMisspelled(PositionOfClick))
        ...
        {
            if (ReadOnly && contextMenu.MenuItems.IndexOf((MenuItem)sender) != 14)
            {//14 = copy.
            ...
            switch (contextMenu.MenuItems.IndexOf((MenuItem)sender))
            {
                ...
                case 3:
                case 4:
                    if (!this._spellCheckIsEnabled || !PrefC.GetBool(PrefName.SpellCheckIsEnabled))
        ...
        private void ODtextBox_ContentsResized(object sender, ContentsResizedEventArgs e)
        {
            try
            {
                if (DesignMode || !this._spellCheckIsEnabled || !PrefC.GetBool(PrefName.SpellCheckIsEnabled))
                {//if spell check disabled, return
                    return;
                }
            }
        ...
        public void ClearWavyLines(int widthOverride = -1, SpellCheckResult spellCheckResult = null)
        {
            ...
            {//Width or Height can be 0 if the window or textbox is resized.  Causes a UE when creating a Bitmap. 
                return;//No lines.
            }
            ...
            foreach (int lineHeight in listVisibleLineHeights)
            {
                ...
                {//y pos of the bottom of the line is below the visible area, with scroll bar active.
                    break;
                }
        ...
        public SpellCheckResult SpellCheck()
        {
            ...
            foreach (MatchOD wordCur in listVisibleWords)
            {
                ...
                {//A word we have not checked yet.
        ...
        private void DrawWave(MatchOD match, List<int> listVisibleLineHeights, List<WavyLine> listWavyLines = null)
        {
            ...
            if (pointStart.Y <= 4 || pointStart.Y >= this.Height)
            ...
            if (pointEnd.Y > pointStart.Y)
            {
                ...
                while (tempEnd.Y <= pointEnd.Y && lineIndex < listVisibleLineHeights.Count)
                {//One line at a time.
                    if (ODBuild.IsDebug())
                    {
                        if (wavyLine == null)
