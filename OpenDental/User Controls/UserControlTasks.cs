...
namespace OpenDental
{
    public partial class UserControlTasks : UserControl
    {
        ...
        public UserControlTasksTab TaskTab
        {
            ...
            set
            {
                System.Windows.Forms.TabPage tabOld = tabContr.SelectedTab;
                ...
                if (tabContr.SelectedTab != tabOld)
        ...
        private void SetFiltersToDefault(TaskList taskListSelected = null)
        {
            ...
            if (taskListSelected == null && tree.SelectedNode != null)
            ...
            if (globalFilterType == GlobalTaskFilterType.Default)
            {//Get the default filter setting.
        ...
        private GlobalTaskFilterType DowngradeFilterTypeIfNeeded(GlobalTaskFilterType globalFilterType)
        {
            ...
            {//Downgrade to None if Clinics are disabled.
                globalFilterType = GlobalTaskFilterType.None;
            }
        ...
        private void menuItemFilterClinic_Click(object sender, EventArgs e)
        {
            ...
            Clinics.GetAllForUserod(Security.CurUser).ForEach(x =>
            {//Only Clinics the user has access to.
        ...
        public static void RefillLocalTaskGrids(Task task, List<TaskNote> listTaskNotes, List<long> listSentSignalNums, bool canKeepTask = true)
        {
            ...
            foreach (UserControlTasks control in _listInstances)
            {
                    ...
                    && (!control._isShowFinishedTasks || (control._isShowFinishedTasks && control.tabContr.SelectedTab == control.tabNew)))
                {
                    ...
                    || (control.tabContr.SelectedTab == control.tabPatientTickets && task.KeyNum == FormOpenDental.PatNumCur)))//Patient tab
                {
        ...
        {
            if (parent != 0 || tabContr.SelectedTab == tabNew)
        ...
        private bool IsInNewTab(Task task)
        {
            ...
            else if (!PrefC.GetBool(PrefName.TasksNewTrackedByUser) && task.TaskStatus == TaskStatusEnum.New)
        ...
        private void FillGrid(List<Signalod> listSignals = null, bool isManualRefresh = false)
        {
            ...
            else
            {
                ...
                {//Individual Task signals. Only refreshes if the task is in the currently displayed list of Tasks. Add/Remove is addressed with TaskList signals.
                    foreach (Signalod signal in listSignals)
                    {
                        ...
                        {
                            if (_listTasks.Exists(x => x.TaskNum == signal.FKey))
                ...
                && (tabContr.SelectedTab == tabDate || tabContr.SelectedTab == tabWeek || tabContr.SelectedTab == tabMonth))
            {
                ...
                for (int i = 0; i < repeatingLists.Count; i++)
                {
                    ...
                    alreadyExists = false;
                    for (int j = 0; j < _listTaskLists.Count; j++)
                ...
                for (int i = 0; i < repeatingTasks.Count; i++)
                {
                    //if already exists, skip
                    alreadyExists = false;
                    for (int j = 0; j < _listTasks.Count; j++)
                    {//loop through Main list
            ...
            {//HQ
                ...
                if (parent != _TriageListNum)
                {//Everything that's not triage
            ...
            for (int i = 0; i < _listTaskLists.Count; i++)
            {
                ...
                row.Cells.Add("");
                if (PrefC.GetBool(PrefName.DockPhonePanelShow))
                {//HQ.  Add if job manager is available
                    row.Cells.Add("");//ST
                    if (parent != _TriageListNum)
                    {//Everything that's not triage
                        row.Cells.Add("");//Job
            ...
            for (int i = 0; i < _listTasks.Count; i++)
            {
                ...
                else if (_listExpandedTaskNums.Contains(_listTasks[i].TaskNum))
                {//Expanded
                ...
                gridMain.ListGridRows.Add(row);
                if (_clickedTask is Task && _listTasks[i].TaskNum == _clickedTask.TaskNum)
        ...
        public static List<TaskList> GetSubscribedTaskLists(long userNum)
        {
            ...
            while (listQueueTaskLists.Count > 0)
            {
                ...
                if (!listSubscribedTaskLists.Contains(taskList))
                ...
                {
                    if (!listSubscribedTaskLists.Contains(child) && !listQueueTaskLists.Contains(child))
                    {//Avoid duplicate return values.
        ...
        private void AddList_Clicked()
        {
            ...
            if (tabContr.SelectedTab == tabUser && _listTaskListTreeHistory.Count == 0)
            {//trunk of user tab
        ...
        private void AddTask(bool isReminder)
        {
            ...
            else if (tabContr.SelectedTab == tabUser && _listTaskListTreeHistory.Count == 0)
            {//trunk of user tab
                task.TaskListNum = -1;//Force FormTaskEdit to ask user to pick a task list.
            }
            else
        ...
        public void Search_Clicked()
        {
            ...
            try
            {
                ...
                {//System.Windows.Forms.Clipboard fails for Thinfinity
                    string txtClip = System.Windows.Clipboard.GetText().Trim().ToLower();
                    if (Regex.IsMatch(txtClip, @"^tasknum:\d+$"))
        ...
        {
            if (_clickedI < _listTaskLists.Count)
            {//is list
        ...
        private void Paste_Clicked()
        {
            ...
            {//a task is on the clipboard
                ...
                if (_wasCut)
                { //cut
                    if (clipTaskTaskListNum == newT.TaskListNum)
        ...
        private void Delete_Clicked()
        {
            ...
            {//Is task
             //This security logic should match FormTaskEdit for when we enable the delete button.
                ...
                if (_clickedTask.UserNum != Security.CurUser.UserNum)
                ...
                if (_clickedTask.TaskListNum != Security.CurUser.TaskListInBox)
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            else if (tabContr.SelectedTab == tabNew && e.Col == 3 && !PrefC.GetBool(PrefName.TasksNewTrackedByUser))
            {//ST column (an index varaible would help)
                return;//Don't double click on ST column.
            }
            else if (tabContr.SelectedTab == tabNew && e.Col == 4 && !PrefC.GetBool(PrefName.TasksNewTrackedByUser))
        ...
        private void SetClickedIAndTask(object e)
        {
            ...
            if (_clickedI >= gridMain.ListGridRows.Count)
        ...
        {
            //Done----------------------------------
            if (gridMain.SelectedIndices.Length == 0 || _clickedI < _listTaskLists.Count)
            ...
            else if (tabContr.SelectedTab == tabUser && _clickedI < _listTaskLists.Count)
            {//user tab and is a list
                menuItemSubscribe.Enabled = false;
                menuItemUnsubscribe.Enabled = true;
            }
            else if (tabContr.SelectedTab == tabMain && _clickedI < _listTaskLists.Count)
            {//main and tasklist
                menuItemSubscribe.Enabled = true;
                menuItemUnsubscribe.Enabled = false;
            }
            else if (tabContr.SelectedTab == tabReminders && _clickedI < _listTaskLists.Count)
            ...
            if (_clickedI > -1 && tabContr.SelectedTab.In(tabUser, tabMain, tabReminders) && _clickedI < _listTaskLists.Count
        ...
        private void menuTaskPriority_Click(Task task, Def priorityDef)
        {
            ...
            try
            {
                ...
                if (PrefC.IsODHQ && priorityDef.DefNum == 502)
                {//They chose Blue as their priority
