    ...
    public partial class UserControlSecurityTree : UserControl
    {
        ...
        private void treePermissions_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            ...
            {//level 1
             //if(e.X<5 || e.X>17) {
                if (e.X < 24 || e.X > 35)
                {//These new ranges are needed because of the +/- boxes.  Tested at different zoom levels.
                    return;
                }
            }
            ...
            if (perm.PermType == Permissions.Reports)
            {//Reports permission is being checked.
            ...
            else if (perm.PermType == Permissions.AdjustmentTypeDeny)
            { //AdjustmentTypeDeny permission is being checked
            ...
            {//Partially checked (currently only applies to Reports permission)
                try
                {
                    GroupPermissions.RemovePermission(_listUserGroupNums.First(), (Permissions)_clickedPermNode.Tag);
                    SecurityLogs.MakeLogEntry(Permissions.SecurityAdmin, 0, "Permission '" + _clickedPermNode.Tag + "' revoked from '"
