		...
		*/
        ...
        private void DrawRow(int row, PaintEventArgs e)
        {
            ...
            {//loop through all columns in the row
                ...
                //test for clip later
                if (i == 0)
                {//first column, dates and row labels
                    float x = rect.Right - g.MeasureString(DataArray[i, row].Text, font).Width - 1;
                    float y = rect.Top - 1;
                    if (GetSection(row) != -1 && RowTypes[GetSection(row)][GetSectionRow(row)] == PerioSequenceType.Probing)
                    {//date
            ...
            {//exams, earliest through current
                ...
                {//sequences
                    ...
                    {//measurements
                        ...
                        for (int surfI = 0; surfI < Enum.GetNames(typeof(PerioSurf)).Length; surfI++)
                        {//surfaces(6or7)
        ...
        public void SaveCurExam(PerioExam PerioExamCur)
        {
            ...
            foreach (PerioMeasureItem measureItem in listChangedMeasurements)
            {
                ...
                //new measurement
                if (PerioMeasures.List[selectedExam, (int)seqI, toothI] == null || PerioExamCur.IsNew)
                {//.PerioMeasureNum==0){
        ...
        private void ClearDataArray()
        {
            ...
            try
            {
                for (int i = 1; i <= 16; i++)
                {
                    DataArray[3 * i - 1, curY].Text = Tooth.Display(i.ToString());
                    if (IsImplant(i))
                    {
                ...
                for (int i = 1; i <= 16; i++)
                {
                    DataArray[3 * i - 1, curY].Text = Tooth.Display((33 - i).ToString());
                    if (IsImplant(33 - i))
                    {
                        DataArray[3 * i - 1, curY].Text += "i";
                    }
        ...
        public static bool IsImplant(int toothNum)
        {
            ...
            {//Not initialized yet.
                _listPatProcs = Procedures.Refresh(FormOpenDental.PatNumCur);
            }
            List<Procedure> listProcsForTooth = _listPatProcs.FindAll(x => x.ToothNum == toothNum.ToString() && x.ProcStatus.In(
            ...
            else if (e.KeyCode == Keys.Left)
            {
                if (ThreeAtATime)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        {
                            AdvanceCell(GetSection(CurCell.Y).In(0, 2));
                        }
                        else
                        {
                            AdvanceCell(true);
            ...
            else if (e.KeyCode == Keys.Right)
            {
                if (ThreeAtATime)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        {
                            AdvanceCell(GetSection(CurCell.Y).In(1, 3));
                        }
                        else
                        {
                            AdvanceCell();
        ...
        private void EnterValue(string keyValue)
        {
            ...
            else
            {
                ...
                if (perioCell.Text == null || perioCell.Text == "")
        ...
        private void EnterValue(int keyValue)
        {
            ...
            if ((keyValue < 0 || keyValue > 19)
                && RowTypes[GetSection(CurCell.Y)][GetSectionRow(CurCell.Y)] != PerioSequenceType.GingMargin)
        ...
        public bool TryFindNextCell(Point curCell, bool isReverse, out Point nextCell, bool doSetDirection = true)
        {
            ...
            {//the 400 limit is just a last resort. Should always break manually.
                ...
                if (Direction == AutoAdvanceDirection.Custom)
                {//custom path
                    return TryAutoAdvanceCustom(curCell, startedOnSkipped, isReverse, out nextCell);
                }
                if ((Direction == AutoAdvanceDirection.Left && isReverse) || (Direction == AutoAdvanceDirection.Right && !isReverse))
                {//right
                ...
                {
                    if (Math.IEEERemainder(((double)nextCell.X + 1), 3) != 0)
                    {//{2,5,8,11};examples of acceptable cols
            ...
            out Point nextCell)
        {
            ...
            if (nextCell.X == 1 && !startedOnSkipped)
            {//if first column
        ...
        private bool TryAutoAdvanceLeft(Point pointCur, int section, bool doSetDirection, PerioSequenceType seqType, out Point nextCell)
        {
            ...
            if (nextCell.X == DataArray.GetLength(0) - 1)
            {//if last column
                if (section == 0)
                {
                    newSection = 1;
            ...
            List<AutoAdvanceCustom> listPaths)
        {
            ...
            if (pathPerioCustom != null && skippedTeeth.Contains(pathPerioCustom.ToothNum))
            {
