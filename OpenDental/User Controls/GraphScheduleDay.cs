...
namespace OpenDental
{
        ...
        private float GetFontHeight()
        {
            ...
            while (true)
            {
                using (Font newFont = new Font(Font.FontFamily, emSize, Font.Style))
                {
                    ...
                    if ((size.Height + .2f) < BarHeightPixels)
                    { //still too small so increment the font size and try again
                        emSize += .1F;
                        continue;
                    }
                    return emSize;
    ...
    public class ScheduleListComparer : IComparer<List<Schedule>>
    {
        ...
        public int Compare(List<Schedule> collectionX, List<Schedule> collectionY)
        {
            ...
            { //send this orders by 1) practice notes, 2) providers 3) employees
                return x.SchedType.CompareTo(y.SchedType);
            }
            else if ((x.SchedType == ScheduleType.Practice && y.SchedType == ScheduleType.Practice) ||
                (x.SchedType == ScheduleType.Blockout && y.SchedType == ScheduleType.Blockout))
            { //this is a note only so order alapha
            ...
            else if (Sort == SortBy.StartTime)
            {
                ...
                if (ret == 0)
                { //if start times are same then default to alpha
