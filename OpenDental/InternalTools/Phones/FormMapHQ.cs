...
namespace OpenDental
{
    public partial class FormMapHQ : FormODBase
    {
            ...
            List<WebChatSession> listWebChatSessions, List<PeerInfo> listPeerInfosRemoteSupportSessions)
        {
            ...
            for (int i = 0; i < listControlsAreaPanel.Count; i++)
            { //loop through all of our cubicles and labels and find the matches
                ...
                if (mapCubicle.MapAreaCur.Extension == 0)
                ...
                if (phone.DateTimeNeedsHelpStart.Date == DateTime.Today)
                { //if they need help, use that time.
                    TimeSpan timeSpan = DateTime.Now - phone.DateTimeNeedsHelpStart + _timeSpanDelta;
                    mapCubicle.TimeSpanElapsed = timeSpan;
                }
                else if (phone.DateTimeStart.Date == DateTime.Today && phone.Description != "")
                { //else if in a call, use call time.
                    TimeSpan timeSpan = DateTime.Now - phone.DateTimeStart + _timeSpanDelta;
                    mapCubicle.TimeSpanElapsed = timeSpan;
                }
                else if (phone.Description == "" && webChatSession != null)
                {//else if in a web chat session, use web chat session time
                    TimeSpan timeSpan = DateTime.Now - webChatSession.DateTcreated + _timeSpanDelta;
                    mapCubicle.TimeSpanElapsed = timeSpan;
                }
                else if (phone.Description == "" && chatuser != null && chatuser.CurrentSessions > 0)
                { //else if in a chat, use chat time.
                ...
                else if (phone.DateTimeStart.Date == DateTime.Today)
                { //else available, use that time.
                    TimeSpan timeSpan = DateTime.Now - phone.DateTimeStart + _timeSpanDelta;
                    mapCubicle.TimeSpanElapsed = timeSpan;
                }
                else
        ...
        {
            if (phoneEmpDefault.EscalationOrder <= 0)
            { //Filter out employees that do not have an escalation order set.
                return false;
            }
            ...
            if (phone == null || phone.Description != "")
            { //Filter out invalid employees or employees that are already on the phone.
                return false;
            }
            PhoneEmpSubGroupType phoneEmpSubGroupTypeSelected = (PhoneEmpSubGroupType)(tabMain.SelectedTab?.Tag ?? PhoneEmpSubGroupType.Escal);
            if (phoneEmpSubGroupTypeSelected == PhoneEmpSubGroupType.Avail)
            {//Special rules for the Avail escalation view
        ...
        public void SetOfficesDownList(List<Task> listTasksOfficeDown)
        {
            ...
            for (int i = 0; i < listTasksOfficeDown.Count; i++)
            {
                ...
                { //Filter out old tasks. Should not be any but just in case.
                    continue;
                }
        ...
        public void SetTriageUrgent(int calls, TimeSpan timeSpanBehind)
        {
            ...
            if (timeSpanBehind == TimeSpan.Zero)
            ...
            if (timeSpanBehind >= TimeSpan.FromMinutes(_triageRedTime))
            { //we are behind
        ...
        public void SetVoicemailRed(int calls, TimeSpan timeSpanBehind)
        {
            ...
            if (timeSpanBehind >= TimeSpan.FromMinutes(_voicemailTime))
            { //we are behind
        ...
        public void SetTriageNormal(int callsWithNotes, int callsWithNoNotes, TimeSpan timeSpanBehind, int triageRed)
        {
            ...
            if (callsWithNoNotes > 0 || triageRed > 0)
            { //we have calls which don't have notes or a red triage task so display that number
                labelTriageCalls.Text = (callsWithNoNotes + triageRed).ToString();
            }
            else
            { //we don't have any calls with no notes nor any red triage tasks so display count of total tasks
                labelTriageCalls.Text = "(" + callsWithNotes.ToString() + ")";
            }
            if (callsWithNoNotes + triageRed >= _triageCalls)
            { //we are behind
                labelTriageCalls.SetAlertColors();
            }
            else if (callsWithNoNotes + triageRed >= _triageCallsWarning)
            { //we are approaching being behind
            ...
            if (timeSpanBehind >= TimeSpan.FromMinutes(_triageTime))
            { //we are behind
                labelTriageTimeSpan.SetAlertColors();
            }
            else if (timeSpanBehind >= TimeSpan.FromMinutes(_triageTimeWarning))
            { //we are approaching being behind
        ...
        private void menuItemToggleTriage_Click(object sender, EventArgs e)
        {
            ...
            { // Don't expand mapAreaPanel to the left; don't cover up the other controls.
