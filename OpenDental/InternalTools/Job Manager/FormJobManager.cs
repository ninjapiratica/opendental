...
namespace OpenDental
{
    ...
    public partial class FormJobManager : FormODBase
    {
        ...
        private List<System.Windows.Forms.TabPage> _listHiddenTabs;
        ...
        private void FillGridActions()
        {
            ...
            foreach (Job job in JobManagerCore.ListJobsAll)
            {
                ...
                {//All is not selected, only select the jobs with the specified proposedversion
                    continue;
                }
            ...
            foreach (KeyValuePair<JobAction, List<Job>> kvp in dictActions)
            {
                ...
                foreach (Job job in listJobsSorted)
                {
                    ...
                    new GridRow(
                        ...
                        {
                            ColorBackG = (job.ToString().ToLower().Contains(textSearch.Text.ToLower()) && !string.IsNullOrWhiteSpace(textSearch.Text) ? Color.LightYellow :
        ...
        private void FillGridNeedsEngineer()
        {
            ...
            foreach (KeyValuePair<JobCategory, List<Job>> kvp in dictCategories)
            {
                ...
                if (comboProposedVersionNeedsEngineer.SelectedIndex != 0)
                {//All is not selected, only select the jobs with the specified proposedversion
        ...
        private void FillGridNeedsExpert()
        {
            ...
            foreach (KeyValuePair<JobCategory, List<Job>> kvp in dictCategories)
            {
                ...
                if (comboProposedVersionNeedsExpert.SelectedIndex != 0)
                {//All is not selected, only select the jobs with the specified proposedversion
        ...
        private void FillGridQueries()
        {
            ...
            gridQueries.Columns.Add(new GridColumn("Priority", 50, HorizontalAlignment.Center));
            gridQueries.Columns.Add(new GridColumn("Owner", 55, HorizontalAlignment.Center));// X for yes, - for unassigned
            gridQueries.Columns.Add(new GridColumn("", 245));
        ...
        private void FillGridUnresolvedIssues()
        {
            ...
            //Get a list of all jobs that should be tested.
            List<Job> listUnresolvedIssues = JobManagerCore.ListJobsAll.FindAll(x => x.Category.In(JobCategory.UnresolvedIssue)
                    && x.PhaseCur.In(JobPhase.Concept, JobPhase.Cancelled))
                //This is the reverse order of the actual priority of different categories of jobs
                //Purposefully put in this order so they appear correctly in the list for testing.
        ...
        private void FillGridSubmittedJobs()
        {
            ...
            if (selectedJobNum > 0 && selectedJobNum == userControlJobManagerEditor.JobNumCur)
            {
                for (int i = 0; i < gridSubmittedJobs.ListGridRows.Count; i++)
                {
                    if ((gridSubmittedJobs.ListGridRows[i].Tag is Job) && ((Job)gridSubmittedJobs.ListGridRows[i].Tag).JobNum == selectedJobNum)
                    {
                        gridSubmittedJobs.SetSelected(i, true);
                        break;
                    }
                }
            }
        ...
        {
            if (gridSubmittedJobs.ListGridRows[e.Row].Tag != null && gridSubmittedJobs.ListGridRows[e.Row].Tag is Job)
            {
                LoadJob((Job)gridSubmittedJobs.ListGridRows[e.Row].Tag, false);
            }
        }
        ...
        private void AddNewJob(long parentNum = 0)
        {
            ...
            {//Should only happen if we forget to stop them from being able to click the button
            ...
            {//Shouldn't ever happen, but I am leaving this here
            ...
            else if (jobNew.Category == JobCategory.Query || jobNew.Category == JobCategory.MarketingDesign)
            {
                ...
                {
                    FormPS.ListPatNumsExplicit = new List<long> { jobQuoteNew.PatNum };
                }
                FormPS.ShowDialog();
                ...
                Patient pat = Patients.GetPat(FormPS.PatNumSelected);
        ...
        {
            if (job == null)
            {//Double clicking on a title row, or something went wrong.
                return;
            }
