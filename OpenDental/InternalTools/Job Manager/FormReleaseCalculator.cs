...
namespace OpenDental
{
    public partial class FormReleaseCalculator : FormODBase
    {
        ...
        private void butCalculate_Click(object sender, EventArgs e)
        {
            ...
            List<Job> listJobs = _listJobsAll.Where(x => listPriorities.GetListSelected<Def>().Select(y => y.DefNum).Contains(x.Priority)
                && listPhases.GetListSelected<JobPhase>().Contains(x.PhaseCur)
                && listCategories.GetListSelected<JobCategory>().Contains(x.Category)).ToList();
            ...
            gridCalculatedJobs.Columns.Add(new GridColumn("EstHrs", 60) { TextAlign = HorizontalAlignment.Center, SortingStrategy = GridSortingStrategy.AmountParse });
            gridCalculatedJobs.Columns.Add(new GridColumn("ActHrs", 60) { TextAlign = HorizontalAlignment.Center, SortingStrategy = GridSortingStrategy.AmountParse });
            gridCalculatedJobs.Columns.Add(new GridColumn("", 300));
