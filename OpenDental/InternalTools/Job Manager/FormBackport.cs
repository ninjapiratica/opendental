...
namespace OpenDental
{
    public partial class FormBackport : FormODBase
    {
        ...
        private void RefreshData()
        {
            ...
            {
                if (_pathOnRefresh.Contains("OPEN DENTAL SUBVERSION"))
                {//look for open dental first as its naming scheme does not match the rest.
            ...
            ODThread odThread = new ODThread((o) =>
            {
                ...
                {//If window quit, this action will not run and the thread will die.
                    if (o.HasQuit)
                    {//If the user refreshed the path and this was marked to quit.
                        return;
                    }
            ...
            odThread.AddExceptionHandler(ex =>
            {
                ...
                _progressBarAction = null;
                this.InvokeIfNotDisposed(() =>
                {//If there's an exception after the form is closed, swallow and do not do anything.
        ...
        private void UpdateListVersions()
        {
            ...
            listVersions = listVersions.Where(x => Version.TryParse(x, out Version _)).OrderByDescending(x => x).Take(3).ToList();
        ...
        private bool DoBackport()
        {
            ...
            if (listFileChanges.Any(x => !x.ModificationType.In(FileModificationType.Modified, FileModificationType.Added, FileModificationType.Deleted)))
            {
