...
namespace OpenDental
{
    public partial class FormJobManagerDashboard : FormODBase
    {
        ...
        private void RefreshAll()
        {
            ...
            List<Job> listJobsNonBugEnhancement = _listJobsAll.FindAll(x => x.Category.In(
            ...
            List<Job> listRemainingNonBugJobs = listJobsNonBugEnhancement.FindAll(x => x.PhaseCur.In(
            ...
            List<Job> listCompletedBackportJobs = listJobsBugEnhancement.FindAll(x => x.PhaseCur.In(JobPhase.Documentation, JobPhase.Complete)
                ...
                && DateTimeOD.Between(_listJobLogs.FirstOrDefault(y => y.JobNum == x.JobNum && y.Description.Contains("Job implemented"))?.DateTimeEntry ?? DateTime.MinValue, dateStart, dateEnd));
            List<Job> listRemainingBugJobs = listJobsBugEnhancement.FindAll(x => x.Category == JobCategory.Bug && x.PhaseCur.In(
        ...
        private void FillGridOldBugs(List<Job> listStaleBugJobs)
        {
            ...
            gridBugJobs.Columns.Add(new GridColumn("Last Updated", 180, HorizontalAlignment.Center, GridSortingStrategy.DateParse));
        ...
        private void FillGridOldEnhancements(List<Job> listStaleEnhancementJobs)
        {
            ...
            gridEnhancementJobs.Columns.Add(new GridColumn("Last Updated", 180, HorizontalAlignment.Center, GridSortingStrategy.DateParse));
            gridEnhancementJobs.Columns.Add(new GridColumn("", 100) { IsWidthDynamic = true });
