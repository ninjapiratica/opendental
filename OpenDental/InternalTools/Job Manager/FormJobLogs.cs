...
namespace OpenDental
{
    public partial class FormJobLogs : FormODBase
    {
        ...
        private void FillGridLog()
        {
            ...
            gridLog.Columns.Add(new GridColumn("Date", 80));
            gridLog.Columns.Add(new GridColumn("User", 60) { TextAlign = HorizontalAlignment.Center });
            gridLog.Columns.Add(new GridColumn("Expert", 60) { TextAlign = HorizontalAlignment.Center });
            gridLog.Columns.Add(new GridColumn("Engineer", 60) { TextAlign = HorizontalAlignment.Center });
            gridLog.Columns.Add(new GridColumn("Conc", 35) { TextAlign = HorizontalAlignment.Center });//shows X if this row has a copy of job description text.
            gridLog.Columns.Add(new GridColumn("W/U", 35) { TextAlign = HorizontalAlignment.Center });//shows X if this row has a copy of job description text.
            gridLog.Columns.Add(new GridColumn("HrEst", 35) { TextAlign = HorizontalAlignment.Center });
            gridLog.Columns.Add(new GridColumn("Title", 60) { IsWidthDynamic = true });
            gridLog.Columns.Add(new GridColumn("Description", 60) { IsWidthDynamic = true });
