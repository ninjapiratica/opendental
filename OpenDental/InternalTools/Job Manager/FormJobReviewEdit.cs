...
namespace OpenDental
{
    public partial class FormJobReviewEdit : FormODBase
    {
        ...
        private void CheckPermissions()
        {
            ...
            if (_jobReviewCur.ReviewerNum == 0 && JobPermissions.IsAuthorized(JobPerm.Writeup, true, secUser.UserNum))
            {//allow any expert to change the expert if there is no expert.
                comboReviewer.Enabled = true;
            }
            if (_jobReviewCur.ReviewerNum == secUser.UserNum || (_jobReviewCur.ReviewerNum == 0 && JobPermissions.IsAuthorized(JobPerm.Writeup, true, secUser.UserNum)))
            { //allow current expert to edit things.
