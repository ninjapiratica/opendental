...
namespace OpenDental.InternalTools.Job_Manager
{
    ...
    public partial class UserControlJobEdit : UserControl
    {
        ...
        public void LoadJob(Job job, TreeNode treeNode)
        {
            ...
            if (job != null)
            {//re-enable control after we have loaded the job.
        ...
        private void treeRelatedJobs_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (!(sourceNode.Tag is Job))
            {//only allow move is source node was a job.
        ...
        private void CheckPermissions()
        {
            ...
            switch (_jobCur.PhaseCur)
            {
                ...
                case JobPhase.Definition:
                    ...
                    if (_jobCur.IsApprovalNeeded && !JobPermissions.IsAuthorized(JobPerm.Approval, true))
                    {//job needs approval and you are not authorized.
                        break;
                    }
            ...
            if (_isOverride)
            {//Enable everything and make everything visible
                textTitle.ReadOnly = false;
                comboPriority.Enabled = true;
        ...
        private void actionMenu_SaveCommitClick(object sender, EventArgs e)
        {
            ...
            if (FormCP.DialogResult == DialogResult.OK && (commitVal == 1 || commitVal == 3))
            {//Public Repo or Both
                process.StartInfo = startInfo;
                process.Start();
                process.WaitForExit();
            }
            if (FormCP.DialogResult == DialogResult.OK && (commitVal == 2 || commitVal == 3))
            {//Internal Repo or Both
                process = new Process();
        ...
        public void LoadMergeJob(Job newJob)
        {
            ...
            {
                if (_jobCur.Title == _jobOld.Title)
                {//Was edited, AND user has not already edited it themselves.
            ...
            {
                if (textJobEditor.ConceptRtf == _jobOld.Requirements)
                {//Was edited, AND user has not already edited it themselves.
            ...
            {
                if (JsonConvert.SerializeObject(textJobEditor.GetListJobRequirements()) == _jobOld.RequirementsJSON)
                {//Was edited, AND user has not already edited it themselves.
            ...
            {
                if (textJobEditor.WriteupRtf == _jobOld.Implementation)
                {//Was edited, AND user has not already edited it themselves.
            ...
            {
                if (textEditorDocumentation.MainRtf == _jobOld.Documentation || string.IsNullOrWhiteSpace(_jobOld.Documentation))
                {//Was edited, AND user has not already edited it themselves.
            ...
            //CATEGORY
            if (_jobCur.Category != jobMerge.Category)
            {//Was edited, AND user has not already edited it themselves.
            ...
            //VERSION
            if (_jobCur.JobVersion != jobMerge.JobVersion && _jobCur.JobVersion == _jobOld.JobVersion)
            {//Was edited, AND user has not already edited it themselves.
            ...
            //CONCEPT EST
            if (_jobCur.HoursEstimateConcept != jobMerge.HoursEstimateConcept && _jobCur.HoursEstimateConcept == _jobOld.HoursEstimateConcept)
            {//Was edited, AND user has not already edited it themselves.
            ...
            //WRITEUP EST
            if (_jobCur.HoursEstimateWriteup != jobMerge.HoursEstimateWriteup && _jobCur.HoursEstimateWriteup == _jobOld.HoursEstimateWriteup)
            {//Was edited, AND user has not already edited it themselves.
            ...
            //DEVELOPMENT EST
            if (_jobCur.HoursEstimateDevelopment != jobMerge.HoursEstimateDevelopment && _jobCur.HoursEstimateDevelopment == _jobOld.HoursEstimateDevelopment)
            {//Was edited, AND user has not already edited it themselves.
            ...
            //REVIEW EST
            if (_jobCur.HoursEstimateReview != jobMerge.HoursEstimateReview && _jobCur.HoursEstimateReview == _jobOld.HoursEstimateReview)
            {//Was edited, AND user has not already edited it themselves.
            ...
            //HOURS ACT
            if (_jobCur.HoursActual != jobMerge.HoursActual && _jobCur.HoursActual == _jobOld.HoursActual)
            {//Was edited, AND user has not already edited it themselves.
            ...
            //PARENT
            if (_jobCur.ParentNum != jobMerge.ParentNum && _jobCur.JobNum == _jobOld.JobNum)
            {//Parent was edited, AND user has not already edited the parent themselves.
        ...
        private void gridAppointments_TitleAddClick(object sender, EventArgs e)
        {
            ...
            AddAppointment(FormPS.PatNumSelected);
        }
        ...
        private void gridCustomerQuotes_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (FormJQE.JobQuoteCur != null)
            {//re-add altered version, iff the jobquote was modified.
                _jobCur.ListJobQuotes.Add(FormJQE.JobQuoteCur);
            }
            FillGridQuote();
        }
        ...
        {
            if (jobPrevious != null && jobPrevious.JobNum == _jobCur.JobNum)
            {//Skip out if you click on the same job twice
                return;
            }
