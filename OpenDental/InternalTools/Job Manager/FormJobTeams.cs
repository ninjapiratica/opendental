    ...
    public partial class FormJobTeams : FormODBase
    {
        ...
        private void FillGridTeams(int selectedRow = 0)
        {
            ...
            for (int i = 0; i < _listJobTeams.Count; i++)
            {
                ...
                if (teamLead == null)
                {//No team lead assigned to team yet.
                    row.Cells.Add("");
                }
                else
                {
                    ...
                    if (user == null)
                    {//Somehow, the user does not exist. Maybe the userod got deleted/hidden, but we didn't delete/hide the JobTeamUser.
                        row.Cells.Add("");
                    }
                    else
                    {
            ...
            {
                if (selectedRow < 0)
                {//Negative because we deleted the very first row.
                    selectedRow = 0;
                }
        ...
        private void FillListTeamUsers()
        {
            ...
            if (gridTeams.SelectedTag<JobTeam>() == null)
            {//Only happens if there are no teams. No need to try and fill team members then.
                return;
            }
            _listJobTeamUsersCur = _listJobTeamUsers.FindAll(x => x.JobTeamNum == gridTeams.SelectedTag<JobTeam>().JobTeamNum);
            if (_listJobTeamUsersCur == null)
            {//No team members assigned to team yet.
                return;
            }
