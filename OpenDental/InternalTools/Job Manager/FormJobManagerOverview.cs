...
namespace OpenDental
{
    public partial class FormJobManagerOverview : FormODBase
    {
        ...
        private void FillGridEngineerHours(List<Job> listJobs)
        {
            ...
            List<JobReview> listJobReviews = listJobs.SelectMany(x => x.ListJobTimeLogs).ToList(); //All time log entries for all passed in jobs
            ...
            foreach (long userNum in listUserNums)
            {
                ...
                row.Cells.Add(Userods.GetName(userNum));    //Engineer
                double hours = listJobReviews.Where(x => x.ReviewerNum == userNum).Sum(x => x.TimeReview.TotalHours);
                row.Cells.Add(hours.ToString("F2"));            //Hours
        ...
        private List<Job> ApplyFilters(List<Job> listJobs)
        {
            ...
            if (comboTeams.SelectedIndex > 0 && comboEngineers.SelectedIndex == 0)
            {//All engineers on one team selected.
            ...
            if (!string.IsNullOrWhiteSpace(textWithinLastXDays.Text))
            {
                ...
                //Jobs where the engineer or expert has logged time within the last X days.
                listJobs = listJobs
                    .Where(x => x.ListJobTimeLogs.Any(y => y.DateTStamp >= DateTime.Now.AddDays(-withinLastXDays) && y.ReviewerNum.In(x.UserNumExpert, x.UserNumEngineer)))
                    .ToList();
            }
