    ...
    public class AppointmentL
    {
        ...
        public static void BreakApptHelper(Appointment appt, Patient pat, ProcedureCode procCode)
        {
            ...
            if (procCode != null)
            {
                ...
                brokenProcedure.PlaceService = Clinics.GetPlaceService(brokenProcedure.ClinicNum);
                ...
                double procFee;
                long feeSch;
                bool noBillIns = InsPlanPreferences.NoBillIns(procCode, insPlanPrimary);
                ...
                FormPB.IsNew = true;
                FormPB.ShowDialog();
                brokenProcAmount = FormPB.GetAmountTotal();
                wasBrokenProcDeleted = FormPB.IsProcDeleted;
                Procedures.AfterProcsSetComplete(ListTools.FromSingle(brokenProcedure));
            }
            #endregion
        ...
        {
            List<PatientLink> listPatientLinksMerged = PatientLinks.GetLinks(patNum, PatientLinkType.Merge);
            if (!PatientLinks.WasPatientMerged(patNum, listPatientLinksMerged))
            {
                return null;
            }
            //This patient has been merged before.  Get a list of all patients that this patient has been merged into.
            List<Patient> listPatients = Patients.GetMultPats(listPatientLinksMerged.Where(x => x.PatNumTo != patNum).Select(x => x.PatNumTo).ToList()).ToList();
        ...
        public static PlannedApptStatus CreatePlannedAppt(Patient pat, int itemOrder, List<long> listPreSelectedProcNums = null)
        {
            ...
            Patient patientMerged = AppointmentL.GetPatientMergePrompt(pat.PatNum);
            if (patientMerged != null)
            {
                pat = patientMerged;
                FormOpenDental.S_Contr_PatientSelected(pat, isRefreshCurModule: true, isApptRefreshDataPat: false);
            }
            ...
            using FormApptEdit FormApptEdit = new FormApptEdit(AptCur.AptNum, listProcNumsPreSelected: listPreSelectedProcNums);
        ...
        public static void ShowKioskManagerIfNeeded(Appointment oldAppt, long newConfirmed)
        {
            ...
            if (MobileAppDevices.IsClinicSignedUpForEClipboard(clinicNum) && ClinicPrefs.GetBool(PrefName.EClipboardPopupKioskOnCheckIn, clinicNum))
            {
                ...
                using FormTerminalManager formTM = new FormTerminalManager(appointment: newAppt);
                formTM.ShowDialog();
            }
            else
            {
