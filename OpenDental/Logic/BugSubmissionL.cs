...
namespace OpenDental
{
    class BugSubmissionL
    {
        ...
        public static bool TryAssociateSimilarBugSubmissions(Point? pointFormLocaiton = null, bool isVerbose = true)
        {
            ...
            foreach (long key in listOrderedKeys)
            {//Loop through submissions that are already attached to bugs
                dictSimilarBugSubs[key] = new List<BugSubmission>();
                foreach (BugSubmission sub in dictAttachedSubs[key])
                {//Loop through the unique exception text from the submission with thie highest reported version.
