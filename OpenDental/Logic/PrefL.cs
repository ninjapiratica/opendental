...
namespace OpenDental
{
    public class PrefL
    {
        ...
        public static bool CheckProgramVersion(Form currentForm, bool isSilent, ChooseDatabaseInfo chooseDatabaseInfo, CommandLineArgs commandLineArgs)
        {
            if (ODBuild.IsDebug())
            {
            ...
            {
                if (isSilent)
                {//This should never happen after a silent update.
                ...
                {
                    LaunchCorrespondingVersion(storedVersion, currentVersion, isInDynamicModeFolder, chooseDatabaseInfo, commandLineArgs);
                    return false;
                }
            ...
            {
                if (isSilent)
                {//This should have already been thrown but we do it here anyway.
            ...
            //At this point we know 100% it's going to be an upgrade
                ...
                && (updateServerName == "" || ODEnvironment.IdIsThisComputer(updateServerName.ToLower())))
            {
                if (ODBuild.IsTrial() && PrefC.GetString(PrefName.RegistrationKey) != "")
                {//Allow databases with no reg key to continue.  Needed by our conversion department.
            ...
            bool isInvalidUpdateServerNameAllowed = false)
        {
            ...
            {//We do not want to install in case this is a pre-test cloud database.
                return funcDisplayError(Lans.g("ServicesHelper", "Not allowed to install the OpenDentalEConnector service in cloud mode."));
            }
            if (string.IsNullOrWhiteSpace(updateServerName))
            {
                ...
                if (doOverrideBlankUpdateServerName)
                {
                    ...
                    {
                        return funcDisplayError(Lans.g("ServicesHelper", "Failed to get host name:") + " " + ex.Message);
                    }
                }
            }
        ...
        private static bool TryInstallEConnector(out string error)
        {
            ...
            try
            {
                ...
                List<ServiceController> listServiceControllersEConnector = ServicesHelper.GetServicesByExe("OpenDentalEConnector.exe");
                if (listServiceControllersEConnector.Count > 0)
                {
                    return true;//An eConnector service is already installed.
                }
                string pathEConnectorExe = ODFileUtils.CombinePaths(Directory.GetCurrentDirectory(), "OpenDentalEConnector", "OpenDentalEConnector.exe");
                FileInfo fileInfoEConnectorExe = new FileInfo(pathEConnectorExe);
