...
namespace OpenDental
{
    ...
    public class FeeL
    {
        ...
        public static void SaveAllowedFeesFromClaimPayment(List<ClaimProc> listClaimProcs, List<InsPlan> listInsPlans)
        {
            ...
            List<Procedure> listProcs = Procedures.GetProcsFromClaimProcs(listClaimProcs);
            for (int i = 0; i < listClaimProcs.Count; i++)
            {
                ...
                Procedure proc = Procedures.GetProcFromList(listProcs, listClaimProcs[i].ProcNum);
                ...
                Fee fee = Fees.GetFee(codeNum, allowedFeeSchedNum, proc.ClinicNum, proc.ProvNum);
                DateTime datePrevious = DateTime.MinValue;
                if (fee == null)
                {
                    fee = new Fee();
                    ...
                    fee.CodeNum = codeNum;
                    fee.ClinicNum = (FeeScheds.GetFirst(x => x.FeeSchedNum == allowedFeeSchedNum).IsGlobal) ? 0 : proc.ClinicNum;
                    fee.ProvNum = (FeeScheds.GetFirst(x => x.FeeSchedNum == allowedFeeSchedNum).IsGlobal) ? 0 : proc.ProvNum;
