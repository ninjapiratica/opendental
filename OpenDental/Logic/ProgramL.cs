...
namespace OpenDental
{
    ...
    public class ProgramL
    {
        ...
        public static void Execute(long programNum, Patient pat)
        {
            ...
            if (ODBuild.IsWeb() && Programs.GetListDisabledForWeb().Select(x => x.ToString()).Contains(prog.ProgName))
            {
            ...
            else if (prog.ProgName == ProgramName.AiDental.ToString())
            {
                AiDental.SendData(prog, pat);
                return;
            }
            ...
            else if (prog.ProgName == ProgramName.MeditLink.ToString())
            {
                MeditLink.SendData(prog, pat);
                return;
            }
            ...
            else if (prog.ProgName == ProgramName.PDMP.ToString() || prog.ProgName == ProgramName.Appriss.ToString())
            {
                ...
                try
                {
                    ...
                    {
                        if (hasUrl)
                        {//Logicoy errors throw exceptions.
            ...
            else if (prog.ProgName == ProgramName.SteriSimple.ToString())
            {
                SteriSimple.SendData(prog, pat);
                return;
            }
