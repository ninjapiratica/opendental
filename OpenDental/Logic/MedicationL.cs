...
namespace OpenDental
{
    ...
    public class MedicationL
    {
        ...
        public static int ImportMedications(List<ODTuple<Medication, string>> listImportMeds, List<Medication> listMedsExisting)
        {
            ...
            foreach (ODTuple<Medication, string> medGenPair in listImportMeds)
        ...
        public static int ExportMedications(string filename, List<Medication> listMedications)
        {
            ...
            {
                listMedicationsExport.Add(new MedicationExport(listMedications[i]));
            }
            string json = JsonConvert.SerializeObject(listMedicationsExport, Formatting.Indented);
            if (ODBuild.IsWeb())
            {
        ...
        public static List<ODTuple<Medication, string>> GetMedicationsFromFile(string filename, bool isTempFile = false)
        {
            ...
            {//New method: deserialize a JSON file.
                List<MedicationExport> listMedicationsExport = JsonConvert.DeserializeObject<List<MedicationExport>>(medicationData);
                for (int i = 0; i < listMedicationsExport.Count; i++)
                {
                    Medication medication = new Medication();
                    medication.MedName = listMedicationsExport[i].MedName;
                    string genericName = listMedicationsExport[i].GenericName;
        ...
        private static List<ODTuple<Medication, string>> SortMedGenericsFirst(List<ODTuple<Medication, string>> listMedLines)
        {
            ...
            foreach (ODTuple<Medication, string> pair in listMedLines)
            {
                ...
                if (med.MedName.ToLower().In(genericName.ToLower(), ""))
