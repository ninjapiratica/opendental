    ...
    public class ClaimL
    {
            ...
            , bool hasPrimaryClaim = false, bool hasSecondaryClaim = false)
        {
            ...
            if (!hasPrimaryClaim)
            {
                ...
                if (isVerbose && claimPrimary.ClaimNum != 0)
                ...
                else if (claimPrimary.ClaimNum != 0)
        ...
        public static Claim CreateClaim(Claim claimCur, string claimType, bool isVerbose, CreateClaimDataWrapper createClaimDataWrapper, string msgBoxHeader = "")
        {
            ...
            switch (claimType)
            {
                ...
                case "Other":
                    using (FormClaimCreate FormCC = new FormClaimCreate(pat.PatNum))
                    {
                        ...
                        PlanCur = FormCC.InsPlanSelected;
                        SubCur = FormCC.InsSubSelected;
                        relatOther = FormCC.RelatPat;
                    }
                    break;
            }
            ...
            List<Procedure> listSelectedProcs = createClaimDataWrapper.ClaimData.ListProcs.FindAll(x => listSelectedProcNums.Contains(x.ProcNum));
            ...
            List<Procedure> listNoBillInsProcs = listSelectedProcs.FindAll(x => !listBillInsProcs.Select(y => y.ProcNum).Contains(x.ProcNum));
        ...
        public static void PromptForSecondaryClaim(List<ClaimProc> listClaimProcsForClaim)
        {
            ...
            listInputBoxParam.Add(new InputBoxParam(InputBoxType.RadioButton, msg, strChangeStatus, Size.Empty));
            listInputBoxParam.Add(new InputBoxParam(InputBoxType.RadioButton, "", strSendClaims, Size.Empty));
            listInputBoxParam.Add(new InputBoxParam(InputBoxType.RadioButton, "", strPrintClaims, Size.Empty));
            if (!PrefC.GetBool(PrefName.ClaimPrimaryRecievedForceSecondaryStatus))
            {
                listInputBoxParam.Add(new InputBoxParam(InputBoxType.RadioButton, "", strDoNothing, Size.Empty));
            }
            using InputBox inputBox = new InputBox(listInputBoxParam);
            ...
            if (selectedRadioButton.Text == strDoNothing)
            {
                return;
            }
            ...
            if (selectedRadioButton.Text == strSendClaims)
            {
                ...
                List<ClaimSendQueueItem> listQueueItemsSent = SendClaimSendQueueItems(listQueue.ToList(), 0);//Use clearinghouseNum of 0 to indicate automatic calculation of clearinghouse
                for (int i = 0; i < listQueueItemsSent.Count; i++)
                {
                    if (listQueueItemsSent[i].ClaimStatus != "S")
                    {
                        continue;
                    }
                    Claims.ReceiveAsNoPaymentIfNeeded(listQueueItemsSent[i].ClaimNum);
                }
            }
            else if (selectedRadioButton.Text == strPrintClaims)
            {
                ...
                if (listSecondaryClaims.Count > 1)
                {
                    ...
                    string stringMessage = stringSameSettings + listSecondaryClaims.Count.ToString() + stringClaims;
                    doUsePrinterSettingsForAll = MsgBox.Show(MsgBoxButtons.YesNo, stringMessage);
                }
                for (int i = 0; i < listSecondaryClaims.Count; i++)
                {
                    Claim claim = listSecondaryClaims[i];
        ...
        public static bool AreCreditsGreaterThanProcFee(List<ClaimProc> listClaimProcsHypothetical)
        {
            return IsInitialPrimaryInsGreaterThanProcFees(listClaimProcsHypothetical)
                || AreWriteOffsGreaterThanProcFees(listClaimProcsHypothetical)
                || AreAllCreditsGreaterThanProcFees(listClaimProcsHypothetical);
        }
