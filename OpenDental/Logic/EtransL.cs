    ...
    public class EtransL
    {
        ...
        public static void ViewFormForEra(Etrans etrans, Form formParent = null, long specificClaimNum = 0)
        {
            ...
            {//More than one EOB in the 835 and we do not know which one to pick.
                using FormEtrans835PickEob formPickEob = new FormEtrans835PickEob(listTranSetIds, messageText835, etrans, doOpenEtrans835: true);
        ...
        public static EraData GetEraDataFiltered(bool showStatusAndClinics, List<X835Status> listX835Statuses, List<long> listSelectedClinicNums,
            string amountMin, string amountMax, DateTime dateFrom, DateTime dateTo, string carrierName, string checkTraceNum, string controlId, bool doIncludeAutomatableCarriersOnly,
            bool areAllClinicsSelected, List<X835AutoProcessed> listX835AutoProcessedStatuses = null, bool doIncludeAcknowledged = true)
        {
            ...
            eraData.ListEtrans835s = Etrans835s.GetFiltered(dateFrom, dateTo, carrierName, checkTraceNum, insPaidMin, insPaidMax, controlId,
                listX835AutoProcessedStatuses, doIncludeAcknowledged, listX835Statuses.ToArray());
            ...
            if (showStatusAndClinics && PrefC.HasClinicsEnabled && !areAllClinicsSelected)
            {//Filter by selected clinics if applicable.
                ...
                eraData.ListAttached = eraData.ListAttached.FindAll(x => listSelectedClinicNums.Contains(x.ClinicNum));
            ...
            if (doIncludeAutomatableCarriersOnly)
            {
        ...
        private static void FilterAutomatableERAs(List<Etrans> listEtrans, List<Etrans835> listFilteredEtrans835s, List<Etrans835Attach> listAttached)
        {
        ...
        public static void GoToAccountForHx835_Claim(Hx835_Claim claimPaid)
        {
            ...
            {//By definition, capitation insurance pays in one lump-sum, not over an extended period of time.
             //By sending in ClaimNum, we ensure that we only get the payplan a claimproc from this claim was already attached to or payplans with no claimprocs attached.
                ...
                else if (listPayPlans.Count > 1)
                {
                    ...
                    {
                        insPayPlanNum = FormPPS.PayPlanNumSelected;
                    }
                }
            }
        ...
        ///<summary>For the given x834, imports all of the information about insurance plans.</summary>
        ///<param name="x834">The x834 object. This was generated by taking the .834 and parsing the text.</param>
        ///<param name="listPatientLimiteds">A list of all patients from the database.</param>
        ///<param name="doCreatePatient">Indicates if a new patient should be made automatically when no patients are found. Correlates to 
        ///PrefName.Ins834IsPatientCreate.</param>
        ///<param name="doDropExistingInsurance">Indicates that all patient plans that are not in the 834 will be dropped. Correlated to 
        ///PrefName.Ins834DropExistingPatPlans</param>
        ...
        ///<param name="showStatus">An optional action to take that shows the status of the process to the user via the UI. The current row/index of
        ///the patient is the first parameter to the action. The second is the current patient.</param>
        public static void ImportInsurancePlansUsingPatientLimited(X834 x834, List<PatientFor834Import> listPatientLimiteds, bool doCreatePatient, bool doCreateEmployer, bool doDropExistingInsurance,
            out int createdPatsCount, out int updatedPatsCount, out int skippedPatsCount, out int createdCarrierCount, out int createdInsPlanCount,
            out int updatedInsPlanCount, out int createdInsSubCount, out int updatedInsSubCount, out int createdPatPlanCount, out int droppedPatPlanCount,
            out int updatedPatPlanCount, out int createdEmployerCount, out StringBuilder sbErrorMessages, Action<int, Patient> showStatus = null)
        {
            ...
            for (int i = 0; i < x834.ListTransactions.Count; i++)
            {
                ...
                for (int j = 0; j < tran.ListMembers.Count; j++)
                {
                    ...
                    if (member.Pat.PatNum == 0)
                    {
                        ...
                        //If the patient was already inserted, then they would show up in _listPatients and also in the database.  Attempt to locate the patient
                        //in _listPatients again before inserting.
                        List<PatientFor834Import> listPatientMatches = PatientFor834Import.GetPatientLimitedsByNameAndBirthday(member.Pat, listPatientLimiteds);
                    ...
                    else if (member.Pat.PatNum == 0 && doCreatePatient)
                    {
                        ...
                        member.Pat.Guarantor = member.Pat.PatNum;
                        Patients.Update(member.Pat, memberPatOld);
                        PatientFor834Import patLimited = (PatientFor834Import)member.Pat;
                        int patIdx = listPatientLimiteds.BinarySearch(patLimited);//Preserve sort order by locating the index in which to insert the newly added patient.
                        int insertIdx = ~patIdx;//According to MSDN, the index returned by BinarySearch() is a "bitwise compliment", since not currently in list.
                        listPatientLimiteds.Insert(insertIdx, patLimited);
                        SecurityLogs.MakeLogEntry(Permissions.PatientCreate, member.Pat.PatNum, "Created from Import Ins Plans 834.", LogSources.InsPlanImport834);
                        isMemberImported = true;
                        createdPatsCount++;
                    }
                    ...
                    {//member.Pat.PatNum!=0
                        PatientFor834Import patDb = listPatientLimiteds.FirstOrDefault(x => x.PatNum == member.Pat.PatNum);//Locate by PatNum, in case user selected manually.
                        if (patDb == null)
                        {
                            listPatientLimiteds = Patients.GetAllPatsFor834Imports();
                        }
                        listPatientLimiteds.FirstOrDefault(x => x.PatNum == member.Pat.PatNum);//If its still null, the patient really doesn't exist in the DB.
                        if (patDb != null)
                        {
                            Patient patientDB = Patients.GetPat(patDb.PatNum);
                            member.MergePatientIntoDbPatient(patientDB);//Also updates the patient to the database and makes log entry.
                            listPatientLimiteds.Remove(patDb);//Remove patient from list so we can add it back in the correct location (in case name or bday changed).
                            int patIdx = listPatientLimiteds.BinarySearch(patDb);//Preserve sort order by locating the index in which to insert the newly added patient.
                                                                                 //patIdx could be positive if the user manually selected a patient when there were multiple matches found.
                                                                                 //If patIdx is negative, then according to MSDN, the index returned by BinarySearch() is a "bitwise compliment".
                                                                                 //If there were mult instances of patDb BinarySearch() would return 0, which should not be complimented (OutOfRangeException)
                            int insertIdx = (patIdx >= 0) ? patIdx : ~patIdx;
                            listPatientLimiteds.Insert(insertIdx, patDb);
                            isMemberImported = true;
                            updatedPatsCount++;
                        }
                    }
