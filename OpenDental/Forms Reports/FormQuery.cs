...
namespace OpenDental
{
	...
	//this test case worked, so now it is just a matter of finishing this off, and Excel export will be done.
        ...
        public void SubmitQueryThreaded(bool isSqlValidated = false)
        {
            ...
            { //Can't cancel User queries for Oracle. this is still from the main thread so we should be ok.
                try
                {
                    if (isSqlValidated || Db.IsSqlAllowed(_reportSimpleGrid.Query))
                    { //Throws Exception
                        SubmitQuery();
                    }
                }
        ...
        private void pd2_PrintPage(object sender, PrintPageEventArgs ev)
        {
            ...
            {//If we reach the end of the document, OR the end of the specificed range, stop printed. Otherwise, continue.
