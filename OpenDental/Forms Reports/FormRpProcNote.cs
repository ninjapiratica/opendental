...
namespace OpenDental
{
    public partial class FormRpProcNote : FormODBase
    {
        ...
        private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (screenCurrent.DeviceName == _screenParent.DeviceName)
            {//If on same screen as main OD
        ...
        private void pd_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            if (!_headingPrinted)
            {
                ...
                g.DrawString(text, subHeadingFont, Brushes.Black, center - g.MeasureString(text, subHeadingFont).Width / 2, yPos);
                yPos += (int)g.MeasureString(text, headingFont).Height;
                if (comboClinics.ListSelectedClinicNums.Count == 0 || comboClinics.IsAllSelected)
                {//No clinics selected or 'All' selected
        ...
        private void comboClinics_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            {//return all providers if "All" is selected.
                FillProvs(Providers.GetListReports());
            }
            else
            { //return providers associated with the selected clinics.
