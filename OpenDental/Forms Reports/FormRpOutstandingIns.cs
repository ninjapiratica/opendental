    ...
    public partial class FormRpOutstandingIns : FormODBase
    {
        ...
        private void FormRpOutIns_Load(object sender, EventArgs e)
        {
            ...
            rightClickMenu.Popup += new EventHandler((o, ea) =>
            {
                ...
                {//Only show 'Go to Account' when there is exactly 1 row selected.
                    return;
                }
        ...
        {
            if (!_hasFormLoaded)
            {//Prevents multiple fill grid calls when loading the form.
                return;
            }
            ...
            if (!comboUserAssigned.IsAllSelected)
            { //If we have all selected, we don't want to filter by UserNum at all.
            ...
            if (listRows == null)
            {//There was an error running the query and it did not finish.
                gridMain.EndUpdate();
                return;
            }
            ...
            , bool checkIgnoreCustomChecked)
        {
            ...
            foreach (RpOutstandingIns.OutstandingInsClaim claimCur in listOustandingInsClaims)
            {
                if (!checkIgnoreCustomChecked)
                {
                    ...
                    {//Custom Claim Tracking def.ValueString is way too big (observed bug) or way too small.
                        ex.DoNothing();
        ...
        {
            if (filterUserNum == -1)
            {//Defaults to 'All', filterUserNum will be -1 in this case
                comboUserAssigned.IsAllSelected = true;
            }
            else
            {
        ...
        private DateTime GetDateTo(System.Windows.Forms.TabPage tabPageCur)
        {
        ...
        ///<summary>Gets date control for report from a specific tab in tabControlDate</summary>
        private DateTime GetDateFrom(System.Windows.Forms.TabPage tabPageCur)
        {
        ...
        {
            System.Windows.Forms.TabPage tabPagePrevious = (tabControlDate.SelectedTab == tabDaysOld ? tabDateRange : tabDaysOld);//Get dates from tab we are leaving.
