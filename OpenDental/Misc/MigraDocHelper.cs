...
namespace OpenDental
{
    ...
    public class MigraDocHelper
    {
        ...
        public static void DrawGrid(Section section, GridOD grid)
        {
            ...
            //in 100ths/inch
            double widthAllColumns = (double)grid.Columns.WidthAll() / .96;
            ...
            for (int i = 0; i < grid.ListGridRows.Count; i++, edgeRows++)
            {
                ...
                if (grid.ListGridRows[i].Note != null && grid.ListGridRows[i].Note != "" && grid.NoteSpanStop > 0 && grid.NoteSpanStart < grid.Columns.Count)
                {
                    row = table.AddRow();
                    row.TopPadding = Unit.FromInch(.01);
                    row.BottomPadding = Unit.FromInch(0);
