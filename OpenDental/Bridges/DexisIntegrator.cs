    ...
    public static class DexisIntegrator
    {
        ...
        public static void SendDataCommFile(Program progCur, Patient pat)
        {
            ...
            try
            {
                ...
                using (StreamWriter streamWriter = new StreamWriter(memoryStream, encoding))
                {
                    ...
                    {//add optional bday only if valid
        ...
        public static void SendDataDDE(Program progCur, Patient pat)
        {
            try
            {
                ...
                if (pat.Birthdate.Year > 1880 && ddeCommand.Length < 244)
                {//add optional bday only if valid and it won't increase the length to >255 characters
        ...
        private static void PatientChangedEvent_Fired(ODEventArgs e)
        {
            ...
            Patient pat = null;
            if (prog.Enabled && _client != null && _client.IsConnected)
            {//prog enabled and connection made (i.e. user previously pressed the button)
                pat = Patients.GetPat((long)e.Tag);//the tag for this event is the newly selected PatNum
            }
            if (pat != null)
            {//pat is not null, so prog is enabled and connection has been previously made (i.e. user has pressed the button)
            ...
            if (!prog.Enabled)
            {//prog was enabled (otherwise this event wouldn't be triggered) but now is not enabled
