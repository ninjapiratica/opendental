...
namespace OpenDental.Bridges
{
    ...
    public class Ewoo
    {
        ...
        public static void SendData(Program ProgramCur, Patient pat)
        {
            ...
            if (!ODBuild.IsWeb() && !File.Exists(path))
            {//If ODCloud, this check is performed by the Cloud Client
                MessageBox.Show(path + " could not be found.");
                return;
            }
            string dir = Path.GetDirectoryName(path);
            string linkage = CodeBase.ODFileUtils.CombinePaths(dir, "linkage.xml");
            if (!ODBuild.IsWeb() && File.Exists(linkage))
            {//Will never exist for WEB version.
                File.Delete(linkage);
            }
            if (pat != null)
            {
