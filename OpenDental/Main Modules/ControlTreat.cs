...
namespace OpenDental
{
    ...
    public partial class ControlTreat : System.Windows.Forms.UserControl
    {
        ...
        public void InitializeOnStartup()
        {
            ...
            if (PrefC.RandomKeys)
            {//random PKs don't get the option or sorting by order entered
        ...
        private void RefreshModuleScreen(bool doRefreshData = true)
        {
            ...
            {//_listTreatPlans will be null when no patient is selected.
                textNote.Text = "";
                HasNoteChanged = false;
            }
        }
        ...
        private void menuConsent_Click(object sender, EventArgs e)
        {
            ...
            {//loop through selected procedures
        ...
        private void FillMainData()
        {
            ...
            if (treatPlanTemp.TPStatus == TreatPlanStatus.Active || treatPlanTemp.TPStatus == TreatPlanStatus.Inactive)
        ...
        private void FillMainDisplay(bool hasSelectedTpChanged = false)
        {
            ...
            if (hasSelectedTpChanged && !_listTreatPlans[gridPlans.SelectedIndices[0]].TPStatus.In(TreatPlanStatus.Active, TreatPlanStatus.Inactive))
            {
                ...
                if ((!wasShowInsChecked && checkShowIns.Checked) || (!wasShowDiscountChecked && checkShowDiscount.Checked))
                {//checkShowIns was changed to checked, which affects how FillMainData() sets each row.Fee.
                ...
                for (int j = 0; j < listDisplayFields.Count; j++)
                {
                    switch (listDisplayFields[j].InternalName)
                    {
                        ...
                        case "Allowed":
                            if (checkShowIns.Checked)
                            {
                                if (CompareDecimal.IsGreaterThan(_listTpRowsMain[i].FeeAllowed, -1))
        ...
        private void TrySendTreatPlan()
        {
            ...
            if (treatPlan == null)
            {//Shouldn't happen, control auto selects when loading.
        ...
        private long SendTreatPlanParam(TreatPlan treatPlan)
        {
            ...
            {//no need to create a treatPlanParam if not saving PDF
                return 0;
            }
        ...
        private PdfDocument GetTreatPlanPDF(out TreatPlan treatPlan, out bool hasPracticeSig)
        {
            ...
            if (gridPlans.SelectedIndices.Count() == 1)
            {
                ...
                {//generate and save a new document from scratch
        ...
        private List<ProcTP> LoadTP(TreatPlan treatPlan)
        {
            ...
            if (checkShowDiscount.Checked && CompareDecimal.IsZero(_listTpRowsMain.FirstOrDefault(x => x.RowType == TpRowType.Total)?.Discount ?? 0))
            { //mimics Saved TP logic
                checkShowDiscount.Checked = false;
            }
            //Change the note
        ...
        private void LoadArchivedTP(TreatPlan treatPlan)
        {
            ...
            { //mimics Active/Inactive TP logic
                checkShowDiscount.Checked = false;
            }
            textNote.Text = _listTreatPlans[gridPlans.SelectedIndices[0]].Note;
            HasNoteChanged = false;
        ...
        private void ComputeProcListFiltered()
        {
            ...
            for (int i = 0; i < _listProcedures.Count; i++)
            {
                ...
                if (_listProcedures[i].ProcStatus == ProcStat.R)
                { //always show all referred
                    _listProceduresFiltered.Add(_listProcedures[i]);
                }
                if (_listProcedures[i].ProcStatus == ProcStat.Cn)
        ...
        private void ToolBarMainUpdate_Click()
        {
            ...
            for (int i = 0; i < _listProceduresTP.Count; i++)
            {
                ...
                { //If needed, update the sales tax amount as well (checks HQ)
        ...
        private List<Document> SaveTPAsDocument(bool isSigSave, Sheet sheet = null)
        {
            ...
            for (int i = 0; i < listNumsCategories.Count; i++)
        ...
        private void ToolBarMainPreAuth_Click()
        {
            ...
            {//Canada
                ...
                if (gridMain.SelectedIndices.Length > 7)
                {
                    ...
                    if (listIndicesSelected.FindAll(x => gridMain.ListGridRows[x].Tag != null).Count - selectedLabCount > 7)
                    {//only if they selected more than 7 procedures, not 7 rows.
            ...
            {//Won't be null if there is only one PatPlan
            ...
            for (int i = 0; i < gridMain.SelectedIndices.Length; i++)
            {
                ...
                {//If it's already marked as Ortho (from a previous procedure), skip this
