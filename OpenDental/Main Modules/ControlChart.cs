...
namespace OpenDental
{
    ...
    public partial class ControlChart : UserControl
    {
        ...
        private void butChartViewAdd_Click(object sender, EventArgs e)
        {
            ...
            if (index < 0)
            { //If there are no chart views, then we do not set a selected chart view, simply return.
                return;
            }
            if (formChartView.DialogResult == DialogResult.Cancel)
            {
                if (selectedIndex.Between(0, index))
                { //if the user cancelled, i.e. did not add a chart, set to the originally selected value
                    index = selectedIndex;
                }
                else
                {
        ...
        private void checkTPChart_Click(object sender, EventArgs e)
        {
            ...
            {
                _chartViewCurDisplay.IsTpCharting = ((System.Windows.Forms.CheckBox)sender).Checked;
            }
            chartCustViewChanged = true;
            FillProgNotes();
        }
        ...
        private void gridPtInfo_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (gridPtInfo.ListGridRows[e.Row].Tag != null && gridPtInfo.ListGridRows[e.Row].Tag.ToString() != "DOB")
            {
                ...
                if (gridPtInfo.ListGridRows[e.Row].Tag.GetType() == typeof(RegistrationKey))
                {
                    ...
                    formRKE.RegistrationKeyCur = (RegistrationKey)gridPtInfo.ListGridRows[e.Row].Tag;
                    formRKE.ShowDialog();
                    FillPtInfo();
                    return;
                }
                ...
                if (gridPtInfo.ListGridRows[e.Row].Tag.ToString() == "References")
                {
                    ...
                    {
                        Patient pat = Patients.GetPat(formReference.PatNumGoto);
                ...
                else if (gridPtInfo.ListGridRows[e.Row].Tag is PatFieldDef patFieldDef)
        ...
        private void gridTpProcs_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            using FormProcEdit formProcEdit = new FormProcEdit(procCur, Pd.Patient, Pd.Family, listToothInitials: Pd.ListToothInitials);
        ...
        private void menuItemEditSelected_Click(object sender, EventArgs e)
        {
            ...
            formProcEditAll.ListProcedures = listProcs;
        ...
        private void menuItemGroupSelected_Click(object sender, EventArgs e)
        {
            ...
            formProcGroup.ProcedureGroup = procGroup;
            formProcGroup.ListProcGroupItems = listProcGroupItems;
            formProcGroup.ListProcedures = listProcs;
        ...
        private void butAddProc_Click(object sender, EventArgs e)
        {
            ...
            for (int n = 0; n == 0 || n < _toothChartRelay.SelectedTeeth.Count; n++)
            {
                ...
                procCur = new Procedure();//going to be an insert, so no need to set Procedures.CurOld
                                          //Procedure
                procCur.CodeNum = formProcCodes.CodeNumSelected;
        ...
        private void gridProg_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            switch (ChartModules.GetRowType(row, out long rowPk))
            {
                case ProgNotesRowType.Proc:
                    ...
                    if (ProcedureCodes.GetStringProcCode(proc.CodeNum, doThrowIfMissing: false) == ProcedureCodes.GroupProcCode)
                    {
                        ...
                        formProcGroup.ProcedureGroup = proc;
                        formProcGroup.ListProcGroupItems = listProcGroupItems;
                        formProcGroup.ListProcedures = listProcs;
                        formProcGroup.ShowDialog();
                        if (formProcGroup.DialogResult == DialogResult.OK)
                    ...
                    else
                    {
                        ...
                        using FormProcEdit formProcEdit = new FormProcEdit(proc, Pd.Patient, Pd.Family, listToothInitials: Pd.ListToothInitials);
                        formProcEdit.ListClaimProcHistsLoop = listClaimProcHistsLoop;
                        formProcEdit.ListClaimProcHists = Pd.ListClaimProcHists;
                ...
                case ProgNotesRowType.WebChatSession:
                    ...
                    {//We don't usually refresh prognotes after a double click
            ...
            ModuleSelected(Pd.PatNum);
            if (Pd.Patient == null)
            {//Don't worry about running the Allocator if the patient is already deleted.
                return;
            }
        ...
        public void TaskGoToEvent(object sender, CancelEventArgs e)
        {
            ...
            TaskObjectType gotoType = formTaskEdit.TaskObjectTypeGoTo;
        ...
        private void gridPlanned_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (Programs.UsingOrion)
            {
                ...
                {
                    if (_isModuleSelected)
                    {//module might have changed, such as when clicking GoTo on a task launched from the appointment edit window.
        ...
        private void toothChart_SegmentDrawn(object sender, ToothChartDrawEventArgs e)
        {
            ...
            else if (radioEraser.Checked)
            {
                ...
                for (int i = Pd.ListToothInitials.Count - 1; i >= 0; i--)
                {//go backwards
        ...
        private void CanadianLabFeeHelper(long procNumParent)
        {
            ...
            if (procParent.ProcNum == 0)
            {//Should never happen.
                return;
            }
            if (Procedures.IsAttachedToClaim(procParent.ProcNum))
            ...
            if (procNum != 0)
            {
                ...
                {//If a group note
                 ...
                 //procedures in this loop.
                    if (doCheckDb)
                    {
                        if (ProcGroupItems.GetCountCompletedProcsForGroup(procNum) == 0)
        ...
        private bool CanUngroupMultiVisit(DataRow row, bool doCheckDb, bool isSilent)
        {
            ...
            if (!doCheckDb)
            {//We'll check the db later before ungrouping.
        ...
        private bool DoesGridProgRowPassFilter(DataRow rowCur, List<ProcGroupItem> procGroupItems, List<long> procNumList)
        {
            ...
            {//if this is a commlog
                ...
                if (!IsPatientNull() && patNumCur != Pd.PatNum)
                {//if this is a different family member
                    ...
                    {//Don't show super family members
                        if (!Pd.ListPatients.Any(x => x.PatNum == patNumCur))
        ...
        private void DrawOrthoHardware()
        {
            ...
            List<OrthoHardware> listOrthoHardwares = Pd.ListOrthoHardwares.FindAll(x => x.DateExam == comboOrthoDate.GetSelected<DateTime>() && !x.IsHidden);
        ...
        private void FillDxProcImage(bool doRefreshData = true)
        {
            ...
            if (selectedImageTab < tabControlImages.TabPages.Count)
            {
        ...
        private void FillTpProcData()
        {
            ...
            for (int i = 0; i < gridTreatPlans.SelectedIndices.Length; i++)
            {
                ...
                for (int j = 0; j < listProcsForTP.Count; j++)
                {
                    ...
                    if (CultureInfo.CurrentCulture.Name.EndsWith("CA") && listProcsForTP[j].ProcNumLab != 0)
                    {//for canada lab fees
                        row.Description = $"^ ^ {descript}";
                    }
        ...
        private void ModuleSelectedDoseSpot()
        {
            ...
            if (_isModuleSelected)
            {//User might have switched to another module.
        ...
        private bool NewCropRefreshPrescriptions()
        {
            ...
            bool retVal = false;
            List<RxPat> listNewRxPats = new List<RxPat>();
            if (IsPatientNull())
            {
                return false;
            }
            try
            {
                retVal = NewCrop.RefreshPrescriptions(Pd.PatNum, ref listNewRxPats);
            }
            catch
            ...
            if (listNewRxPats.Count > 0)
            {
                AutomationL.Trigger(AutomationTrigger.RxCreate, new List<string>(), Pd.PatNum, 0, listNewRxPats);
            }
            return true;
        }
        ...
        private void RefreshModuleData(long patNum, bool isFullRefresh)
        {
            ...
            Pd.ClearAll();
            Pd.PatNum = patNum;
            List<EnumPdTable> listPatDataTypes = ChartModules.FillPatientDataList(panelImages.Visible, checkShowOrtho.Checked);
        ...
        private void Tool_eRx_Click(bool isShowRefillsAndErrors = false)
        {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (isEmp)
                        {
                            ...
                            formProviderPick.ProvNumSelected = prov.ProvNum;
                            ...
                            List<Userod> listDoseUsers = Userods.GetWhere(x => x.ProvNum == formProviderPick.ProvNumSelected, true);//Only consider non-hidden users.
                            ...
                            prov = Providers.GetProv(formProviderPick.ProvNumSelected);
                            #region Provider Term Date Check
                            //Prevents prescriptions from being added that have a provider selected that is past their term date
            ...
            try
            {
                ...
                if (PrefC.HasClinicsEnabled)
                {
                    ...
                    else if (Pd.Patient.ClinicNum != 0)
            ...
            byte[] arrayPostDataBytes = ErxXml.BuildNewCropPostDataBytes(prov, emp, Pd.Patient, clinicNumToSend, out clickThroughXml);
        ...
        private Dictionary<string, Control> LayoutSheet_GetControlDict()
        {
            ...
            for (int i = 0; i < listSheetFieldDefsAll.Count; i++)
            {
                ...
                switch (listSheetFieldDefsAll[i].FieldName)
                {
                    ...
                    case "toothChart":
                        if (odInternalCustomerGrids.Visible)
                        {//Replace toothchart with HQ bug submission control
        ...
        private bool AddProcHelper(Procedure procedure, List<Fee> listFees)
        {
            ...
            List<ClaimProcHist> listClaimProcHistsLoop = new List<ClaimProcHist>();
            if (!ChartModules.AddProcHelper(procedure, listFees, Pd, _procStatusNew, _listChartedProcs, _listTreatPlans, _listSubstitutionLinks, priorityNum, listTpNums,
            ...
            if (AutoCodeItems.ShouldPromptForCodeChange(procedure, procedureCode, Pd.Patient, isMandibular, listClaimProcs, out codeNumVerify))
            {
                using FormAutoCodeLessIntrusive formAutoCodeLessIntrusive = new FormAutoCodeLessIntrusive(Pd.Patient, procedure, procedureCode, codeNumVerify, Pd.ListPatPlans, Pd.ListInsSubs, Pd.ListInsPlans,
        ...
        private void EnterTypedCode()
        {
            ...
            {//always loops at least once.
                ...
                procCodes.Add(ProcedureCodes.GetProcCode(procedure.CodeNum).ProcCode);
            }//n selected teeth
             //this was requiring too many irrelevant queries and going too slowly   //ModuleSelected(PatCur.PatNum);
            Pd.ClearAndFill(EnumPdTable.ToothInitial);
            ClearButtons();
        ...
        private void ProcButtonClicked(ProcButton procButton, ProcButtonQuick procButtonQuick = null)
        {
            ...
            if (procButton != null && procButton.IsMultiVisit)
            {
                ...
                {
                    ProcMultiVisits.CreateGroup(_listChartedProcs);//Add all procedures within the procedure button to one group since there is an Auto code (ex Bridge).
                }
                else
                {
                    ...
                    List<Procedure> listProceduresRanged = _listChartedProcs.FindAll(x => x.ToothNum == "");
        ...
        private void HideOrUnhideOrthoHardware(bool doHide)
        {
            if (tabControlOrthoCategories.SelectedTab.Tag is OrthoChartTab)
            {
