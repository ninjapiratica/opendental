...
namespace OpenDental
{
    ...
    public partial class ControlAccount : UserControl
    {
        ...
        private void ButMakeOrthoCaseActive_Click(object sender, EventArgs e)
        {
            ...
            {//If link exists debond proc must be complete
        ...
        {
            if (e.Row == gridAcctPat.ListGridRows.Count - 1)
            {//last row
        ...
        private void gridPatInfo_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (gridPatInfo.ListGridRows[e.Row].Tag is PatFieldDef)
            ...
            else if (gridPatInfo.ListGridRows[e.Row].Tag is PatField)
        ...
        private void menuItemAddRefund_Click(object sender, EventArgs e)
        {
            ...
            {   //if the user clicks cancel or x, undo any changes to the database.
        ...
        private void toolBarButStatement_Click()
        {
            ...
            if (textDateStart.IsValid() && textDateStart.Text != "")
            {//textDateStart has ultimate precedence. User may have intentionally set the date range for statement.
            ...
            {//Use preferences to determine the "from" date.
                ...
                if (billingDefaultsLastDaysPref > 0)
                {//0 days means ignore preference and show everything.
                ...
                if (PrefC.GetBool(PrefName.BillingShowTransSinceBalZero))
                {
                    ...
                    if (tableBals.Rows.Count > 0)
                    {
                        ...
                        if (dateTimeFrom > statement.DateRangeFrom)
                        {//Zero balance date range has precedence if it's more recent than billing default date range.
        ...
        private void FillComm()
        {
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                {//one patient
                    if (table.Rows[i]["PatNum"].ToString() == _patCur.PatNum.ToString())
                    {//if this patient
                        row.Cells.Add("");
                    }
                    else
        ...
        private void FillPaymentPlans()
        {
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                row = new GridRow();
                row.Cells.Add(table.Rows[i]["date"].ToString());
                if (table.Rows[i]["InstallmentPlanNum"].ToString() != "0" && table.Rows[i]["PatNum"].ToString() != _patCur.Guarantor.ToString())
        ...
        public void LayoutPanelsAndRefreshMainGrids(bool doLogFillMain = false)
        {
            ...
            LayoutPanels();
            if (doLogFillMain)
            {//Only log this fill on the refresh method call
        ...
        public void LayoutPanels()
        {
            ...
            if (_listSplitsHidden.Count == 0)
            {//might need to get updated more often than from loadData. Not sure how much we care. 
