...
namespace OpenDental
{
    ...
    public partial class ControlAppt : UserControl
    {
        ...
        private void contrApptPanel_ApptMainAreaDoubleClicked(object sender, UI.ApptMainClickEventArgs e)
        {
            ...
            if (_patCur == null || formPatientSelect.PatNumSelected != _patCur.PatNum)
        ...
        private void contrApptPanel_ApptMoved(object sender, UI.ApptMovedEventArgs e)
        {
            ...
            if (listProcsOld.Exists(x => x.ProvNum != listProcsNew.FirstOrDefault(y => y.ProcNum == x.ProcNum).ProvNum))
        ...
        private void ContrApptPanel_ApptRightClicked(object sender, UI.ApptRightClickEventArgs e)
        {
            ...
            if (OpenDentBusiness.AutoComm.Byod.IsSetup(apptClinicNum, out string err))
            {//Check-In Links feature is enabled.  (handles PrefC.HasClinicsEnabled)
        ...
        private void contrApptPanel_SelectedApptChanged(object sender, UI.ApptSelectedChangedEventArgs e)
        {
            ...
            if (_patCur == null || _patCur.PatNum != e.PatNumNew)
            {//patient changed
        ...
        private void butComplete_Click()
        {
            ...
            Procedures.AfterProcsSetComplete(listProcs);
        ...
        private void butDelete_Click(bool showPrompt)
        {
            ...
            if ((appt.AptStatus != ApptStatus.Complete && !Security.IsAuthorized(Permissions.AppointmentDelete))
                || (appt.AptStatus == ApptStatus.Complete && !Security.IsAuthorized(Permissions.AppointmentCompleteDelete)))
            {
                return;
            }
            DataRow dataRow = contrApptPanel.GetDataRowForSelected();
            ...
            else
            {
                ...
                {// seperate log entry for editing completed appointments.
                    SecurityLogs.MakeLogEntry(Permissions.AppointmentCompleteDelete, _patCur.PatNum,
                        dataRow["procs"].ToString() + ", " + dataRow["AptDateTime"].ToString() + ", " + Lan.g(this, "Deleted"),
                        appt.AptNum, appt.DateTStamp);
                }
                else
                {
                    SecurityLogs.MakeLogEntry(Permissions.AppointmentDelete, _patCur.PatNum,
                        dataRow["procs"].ToString() + ", " + dataRow["AptDateTime"].ToString() + ", " + Lan.g(this, "Deleted"),
                        appt.AptNum, appt.DateTStamp);
                }
        ...
        private void pinBoard_ApptMovedFromPinboard(object sender, UI.ApptFromPinboardEventArgs e)
        {
            ...
            try
            {
                    ...
                    if ((assignedDent != 0 && assignedDent != apptCur.ProvNum) || (assignedHyg != 0 && assignedHyg != apptCur.ProvHyg))
                    {
                        if (MsgBox.Show(this, MsgBoxButtons.YesNo, "Change provider?"))
                        {
                            ...
                            if (apptCur.Pattern != calcPattern)
                            {
                                ...
                                {//appt time not locked. Do not give popup for Timelocked appointments.
                ...
                {
                    if (opCur.SetProspective && _patCur.PatStatus != PatientStatus.Prospective)
                ...
                if (!AppointmentL.IsSpecialtyMismatchAllowed(_patCur.PatNum, apptCur.ClinicNum))
                {//ClinicNum just set using either opCur or PatCur.
                    return;
                }
                if (apptCur.AptStatus == ApptStatus.Planned)
                ...
                {//simple drag off pinboard to a new date/time
                    ...
                    try
                    {
                        ...
                        if (apptOld.AptStatus == ApptStatus.UnschedList && apptOld.AptDateTime == DateTime.MinValue)
        ...
        private void menuBlockAdd_Click(object sender, EventArgs e)
        {
            ...
            if (!Security.IsAuthorized(Permissions.Blockouts, true))
            {
                ...
                if (listDefsBlockoutTypes.Count == 0 && !Security.IsAuthorized(Permissions.Blockouts))
        ...
        private void menuBlockCutCopyPaste_Click(object sender, EventArgs e)
        {
            ...
            if (ApptViews.IsNoneView(viewCur))
            {//None view.
        ...
        private void DoSearch(bool isForMakeRecall = false)
        {
            ...
            List<long> listBlockoutTypes = null;
            ...
            {
                List<string> listBlockoutTypesForAppointmentType = appointmentType.BlockoutTypes.Split(",", StringSplitOptions.RemoveEmptyEntries).ToList();
                //Convert into a list of longs
                List<long> listBlockoutTypeDefNumsForApptType = listBlockoutTypesForAppointmentType.Select(x => PIn.Long(x, hasExceptions: false)).ToList();
                listBlockoutTypes = listBlockoutTypeDefNumsForApptType;
            }
            _listScheduleOpenings = ApptSearch.GetSearchResults(aptNum, afterDate, afterDate.AddDays(731)
                , providerNums, listOpNums, listClinicNums, beforeTime, afterTime, listBlockoutTypes: listBlockoutTypes, isForMakeRecall: isForMakeRecall);
        ...
        public void SetInitialStartTime()
        {
            ...
            { //None view NOT selected.
                ...
                if (viewCur.IsScrollStartDynamic)
