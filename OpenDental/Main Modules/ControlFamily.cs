    ...
    public class ControlFamily : UserControl
    {
        ...
        private void gridPat_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            else if (_gridPat.ListGridRows[e.Row].Tag.ToString().Equals("Payor Types"))
            {
                ...
                formPayorTypes.PatientCur = _patCur;
                formPayorTypes.ShowDialog();
            }
            else if (_gridPat.ListGridRows[e.Row].Tag is PatFieldDef)
            ...
            else if (_gridPat.ListGridRows[e.Row].Tag is PatField)
            ...
            else if (_gridPat.ListGridRows[e.Row].Tag is Address)
            {
                ...
                if (address.IsNew)
                { //add the patCur's patNum is new
        ...
        private void menuPlansForFam_Click(object sender, EventArgs e)
        {
            ...
            formPlansForFamily.FamilyCur = _famCur;
        ...
        private void FillPatientData()
        {
            ...
            {
                ehrPatient = EhrPatients.Refresh(_patCur.PatNum);
            }
        ...
        private void ToolButAddSuper_Click()
        {
            ...
            Patient patSelected = Patients.GetPat(formPatSelect.PatNumSelected);
            ...
            if (patSelected.SuperFamily == patSelected.Guarantor)
            {//selected patient's guarantor is the super family head of another super family
                listSuperFamPats = Patients.GetBySuperFamily(patSelected.SuperFamily);
            }
            DialogResult diagResult = DialogResult.None;
            if (listSuperFamPats.Any(x => x.Guarantor != x.SuperFamily))
            {//super family consists of more than one family
            ...
            else if (diagResult.In(DialogResult.None, DialogResult.No))
            {//None = the fam doesn't belong to another super fam, just move into this super fam
                if (diagResult == DialogResult.No)
                {
                    ...
                    { //Create SecurityLog for each member of the superfamily being disbanded.
                        if (listPatientNumsInSuperFam[i] == patSelected.PatNum)
                        { //SecurityLog for the SuperHead moving to another SuperFamily is already handled.
                            continue;
                        }
                        SecurityLogs.MakeLogEntry(
                            Permissions.PatientEdit,
        ...
        private void FillInsData()
        {
            ...
            for (int y = 0; y < benefitMatrix.GetLength(1); y++)
            {//rows
                ...
                for (int x = 0; x < benefitMatrix.GetLength(0); x++)
                {//columns
                    ...
                    else if (benefitMatrix[x, y].CodeNum == 0 && procCode.AbbrDesc != null)
                    {//e.g. flo
                ...
                {//columns
                    ...
                    if (benefitMatrix[x, y].QuantityQualifier == BenefitQuantity.NumberOfServices)
                    ...
                    else if (benefitMatrix[x, y].QuantityQualifier == BenefitQuantity.Months)
                    {//eg Every 2 months
                    ...
                    else if (benefitMatrix[x, y].QuantityQualifier == BenefitQuantity.Years)
                    {//eg Every 2 years
                    ...
                    {
                        if (benefitMatrix[x, y].QuantityQualifier != BenefitQuantity.None && !hasSpecialAgeLimitAdded)
                        {//e.g. flo
                        ...
                        && benefitMatrix[x, y].QuantityQualifier.In(BenefitQuantity.Months, BenefitQuantity.Years))
                    {
                        ...
                        if (benefitMatrix[x, y].QuantityQualifier == BenefitQuantity.Months)
                        {//eg Every 2 months
        ...
        private void ToolButDiscount_Click()
        {
            ...
            if (_discountPlanSubCur == null)
            {//Patient does not have a discount plan.
        ...
        private void ToolButIns_Click()
        {
            ...
            {//patient is not subscriber
             //show list of patients in this family
                ...
                patSubscriber = Patients.GetPat(formSubsciberSelect.PatNumSelected);
            }
            ...
            else
            {
                ...
                if (formInsSelectSubscr.SelectedInsSubNum == 0)
                {//'New' option selected.
                    isNewPlan = true;
                }
                else
        ...
        private void RefreshModuleScreen()
        {
            ...
            {//no patient selected
             //Hide super family and patient clone grids, safe to run even if panel is already hidden.
                ...
                if (_toolBarMain.Buttons["AddSuper"] != null)
