    ...
    public partial class FormOpenDental : FormODBase
    {
        ...
        public void FormOpenDentalShown()
        {
            ...
            {//Most users will loop through once.  If user tries to connect to a db with replication failure, they will loop through again.
                ...
                if (!PrefsStartup(isSilentUpdate, chooseDatabaseInfo2))
        ...
        private bool PrefsStartup(bool isSilentUpdate, ChooseDatabaseInfo chooseDatabaseInfo)
        {
            ...
            {
                if (!PrefL.ConvertDB(this, chooseDatabaseInfo?.UseDynamicMode ?? false))
                {//refreshes Prefs if converted successfully.
                    return false;
                }
            }
        ...
        private void FillPatientButton(Patient patient)
        {
            ...
            if (patChanged)
            {
                ...
                if (AutomationL.Trigger(AutomationTrigger.OpenPatient, null, patient.PatNum))
        ...
        private void menuClick_OpenPickList(object sender, EventArgs e)
        {
            ...
            if (formClinics.ClinicNumSelected == 0)
        ...
        private void SignalsTickWhileInactive()
        {
            ...
            if (PrefC.IsODHQ)
            {//HQ stores important information within the sitelink table and should always process signals for the 'Sites' invalid type.
        ...
        private void SignalsTick(bool isAllInvalidTypes = true)
        {
            try
            {
                ...
                {
                    string errorMsg;
                    if (!ODBuild.IsDebug() && !IsDbConnectionSafe(out errorMsg))
        ...
        private void TaskPopupHelper(Task taskPopup, List<UserOdPref> listUserOdPrefsBlockedTasks, List<TaskNote> listTaskNotes = null)
        {
            try
            {
                ...
                if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT && !Security.IsUserLoggedIn)
                ...
                {//User is subscribed to this TaskList, or one of its ancestors.
                    ...
                    if (!listUserOdPrefsBlockedTasks.Any(x => x.Fkey == taskPopup.TaskListNum && PIn.Bool(x.ValueString)))
        ...
        private void FormMapHQ_RoomControlClicked(object sender, EventArgs e)
        {
            ...
            if (!mapCubicle.IsFlashing() && mapCubicle.Status != "OnWay")
            { //If OnWay we're setting them to available, don't return.
                return;
            }
