...
namespace OpenDentBusiness
{
    public class ErxXml
    {
        ...
        public static string BuildNewCropClickThroughXml(Provider prov, Employee emp, Patient pat, long clinicNum, out NCScript ncScript)
        {
            ...
            {
                if (vitalsign.Height != 0 && ncScript.Patient.PatientCharacteristics.height.IsNullOrEmpty())
                {//Only set once
                ...
                if (vitalsign.Weight != 0 && ncScript.Patient.PatientCharacteristics.weight.IsNullOrEmpty())
                {//Only set once
