...
namespace OpenDentBusiness
{
    public class PayConnect
    {
        ...
        public static void MakePaymentWithAlias(Patient pat, string email, string payNote, double amount, CreditCard cc, string logGuid = "",
            bool doForceDuplicate = false)
        {
            if (pat == null)
            {
                throw new ODException("No Patient Found", ODException.ErrorCodes.NoPatientFound);
            ...
            if (payConnectRequest != null && payConnectResponse.Status.code == 0)
            {//Success
                ...
                long payNum = Payments.InsertFromPayConnect(pat.PatNum, pat.PriProv, pat.ClinicNum, amount, formattedNote, receipt, CreditCardSource.PayConnectPortal, logGuid);
                PayConnectResponseWeb responseWeb = new PayConnectResponseWeb()
                {
                    Amount = amount,
                    PatNum = pat.PatNum,
                ...
                PayConnectResponseWebs.Insert(responseWeb);
                if (EmailAddresses.IsValidEmail(responseWeb.EmailResponse))
                {
        ...
        public class Monitor
        {
            ...
            public static void OnThreadRun(ODThread odThread)
            {
                ...
                try
                {
                    ...
                    if (ProcessOutstandingTransactions() > 0)
                    { //We still have pending OTKs, set next run interval to short.
            ...
            private static int ProcessOutstandingTransactions()
            {
                ...
                if (listPendingDue.Count <= 0)
                { //None are due this time around but we may have some still pending, return count of those.
                    ...
                    try
                    {
                        ...
                        switch (responseWebCur.ProcessingStatus)
                        {
                            ...
                            case PayConnectWebStatus.Completed:
                                ...
                                if (EmailAddresses.IsValidEmail(responseWebCur.EmailResponse))
                                {
