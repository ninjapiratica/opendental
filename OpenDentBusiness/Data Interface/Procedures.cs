    ...
    public class Procedures
    {
            ...
            DiscountPlanSub discountPlanSub = null, DiscountPlan discountPlan = null, double runningTotal = 0, List<Procedure> listAddHistProcs = null)
        {
            ...
            if (discountPlanSub == null)
            {
                ...
                if (discountPlanSub == null)
                { //Patient isn't a discountPlanSub
                    return 0;
                }
            }
            if (discountPlan == null)
            {
                ...
                if (discountPlan == null)
                { //Patient doesn't have a discountPlan
                    return 0;
                }
            }
            ...
            if (dictDiscountFees.Count == 0)
            {//Only do this if the patient has a discount plan.
                return 0;
            }
            ...
            if (procFee == null || procFee.Amount == -1)
            { //If there is no procfee from the fee sched, there will be no discount
                return 0;
            }
            ...
            if (estimatedDiscountAmt <= 0)
            { //ProcFeeAmount would be issuing a refund
                return 0;
            }
            if (runningTotal == 0)
            {
            ...
            else if (runningTotal + estimatedDiscountAmt > discountPlan.AnnualMax)
            ...
            , bool isProcLinkedToOrthoCase = false)
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {
                ...
                {//This conditional must match below where we call ProcMultiVisits.UpdateGroupForProc().
            ...
            if (procedure.ProcStatus == ProcStat.TP)
            {
                ...
                if (AvaTax.DoSendProcToAvalara(procedure, isSilent))
                {//checks isHQ
            ...
            if (oldProcedure.ProcStatus != ProcStat.C && procedure.ProcStatus == ProcStat.C && !isProcLinkedToOrthoCase)
            {
                ...
                if (!CompareDouble.IsZero(procedure.Discount))
            ...
            switch (PrefC.GetInt(PrefName.ProcFeeUpdatePrompt))
            {
                ...
                case 3://Prompt if procedure fee amount is different per our manual
                    for (int i = 0; i < listNewProcs.Count; i++)
                    {
                        ...
                        Procedure procOld = listOldProcs.FirstOrDefault(x => x.ProcNum == proc.ProcNum);
                        if (procOld == null)
                        { // New procedure, so fee can't be changed since it didn't exist before
                            continue;
                        }
                        procFeeHelper.FillData();
                        proc.ProcFee = GetProcFee(procFeeHelper.Pat, procFeeHelper.ListPatPlans, procFeeHelper.ListInsSubs, procFeeHelper.ListInsPlans, proc.CodeNum,
        ...
        private static void QueueDataBatches(ODThread odThread)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < _listProcNumsMaxForGroups.Count; i++)
                {
                    ...
                    if (PrefC.HasClinicsEnabled && clinicNumGlobal > -1)
            ...
            List<Procedure> listProcsForAppt, List<InsSub> subList, Userod curUser)
        {
            ...
            foreach (Procedure procCur in listProcsForAppt)
            {
                ...
                {//skip because SetCanadianLabFeesCompleteForProc() called update already
                ...
                {//If proc was linked to orthoCase, Pass ortho case objects to ComputeEstimates.
                ...
                {//Otherwise, call without orthocase objects.
                    ComputeEstimates(procCur, apt.PatNum, ref claimProcList, false, planList, patPlans, benefitList,
                        histList, loopList, true,
                        patient.Age, subList,
                        null, false, false, listSubstLinks, false,
        ...
        public static bool EntriesAreValid(string textNotes, bool isSigChangedAndNotBlank, string textTimeStart, string textTimeEnd, int unityQty, long provNumSelected,
            ...
            Procedure procedure, Procedure procedureOld, ref ProcedureCode procedureCode, string translationSource,
            ...
            ref List<ClaimProc> listClaimProcsForProc)
        {
            ...
            {//if status was changed to complete
                ...
                if (!isQuickAdd && !Security.IsAuthorized(Permissions.ProcComplCreate, textDate, actionNotAuthorized: actionOnFailure))
                {//use the new date
                    return false;
                }
            }
            ...
            {//if new procedure is complete
                if (!Security.IsAuthorized(Permissions.ProcComplCreate, textDate, procedure.CodeNum, procFee, actionNotAuthorized: actionOnFailure))
                {
                    return false;
                }
            }
            ...
            {//an old procedure
                ...
                {//that was already complete
                    if (!CheckPermissionsAndGlobalLockDate(procedureOld, procedure, textDate, procFee, actionOnFailure))
                    {
                        return false;
                    }
                }
        ...
        public static bool CheckPermissionsAndGlobalLockDate(Procedure procOld, Procedure procNew, DateTime procDate, double procFeeOverride = double.MinValue,
            Action<string> actionNotAuthorized = null)
        {
            //No need to check MiddleTierRole; no call to db.
            ...
            if (procOld.ProcStatus != procNew.ProcStatus && !Security.IsAuthorized(perm, dateToUseProcOld, actionNotAuthorized: actionNotAuthorized))
            {//block old date
                return false;
            }
            if (procNew.ProcStatus.In(ProcStat.C, ProcStat.EO, ProcStat.EC))
            {
                ...
                if (procOld.ProcStatus != procNew.ProcStatus)
                {
                    ...
                    if (!Security.IsAuthorized(perm, dateToUseProcCur, actionNotAuthorized: actionNotAuthorized))
                    {//block new date, too
                        return false;
                    }
                }
                    ...
                    || procOld.CodeNum != procNew.CodeNum) //If user changed the procedure code
                {
                    ...
                    if (!Security.IsAuthorized(perm, dateToUseProcCur, procNew.CodeNum, procFee, actionNotAuthorized: actionNotAuthorized))
                    {
                        return false;
                    }
                }
        ...
        public static void CanadianLabHelper(string textCanadaLabFee1Text, bool isCanadianLab, List<Procedure> listCanadaLabFees, Procedure procedure, string textCanadaLabFee2Text)
        {
            ...
            if (canadaLabFee1 == 0)
            {
                if (isCanadianLab && listCanadaLabFees.Count > 0)
            ...
            if (canadaLabFee2 == 0)
            {
                if (isCanadianLab && listCanadaLabFees.Count > 1)
                { //Don't worry about deleting child lab fees if we are editing a lab fee. No such concept.
        ...
        {
            if (PrefC.GetBool(PrefName.SalesTaxDoAutomate) && !PrefC.IsODHQ)
            { //HQ shouldn't be doing this, it's taken care of elsewhere by AvaTax.
                for (int i = 0; i < listProcedures.Count; i++)
                {
        ...
        private static Procedure CreateProcedureForInsHist(Patient patCur, DateTime date, PrefName prefName)
        {
            ...
            retVal.PlaceService = Clinics.GetPlaceService(retVal.ClinicNum);
            retVal.Surf = "";
        ...
        {
            //No need to check MiddleTierRole; no call to db.
            List<Procedure> listHistCodeNumProcs = listPatEoProcs.FindAll(x => listInsHistCodeNums.Contains(x.CodeNum));
