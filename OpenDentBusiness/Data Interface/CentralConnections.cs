...
namespace OpenDentBusiness
{
            ...
            if (!string.IsNullOrEmpty(centralConnection.ServiceURI))
            {
                ...
                try
                {
                    ...
                    if (useEcwAlgorithm)
                    {
                        ...
                        {//eCW gave us the password already hashed via command line arguments, simply use it.
            ...
            , bool noShowOnStartup = false, List<string> listAdminCompNames = null, bool isCommandLineArgs = false, bool useDynamicMode = false, bool allowAutoLogin = true)
        {
            try
            {
                    ...
                    && centralConnection.IsAutomaticLogin)
                {
                    ...
                    if (PasswordVaultWrapper.TryRetrieveUserName(centralConnection.ServiceURI, out string userName))
                    {//Found a saved OD MT UserName
    ...
    public class ChooseDatabaseInfo
    {
        ...
        {
            if (string.IsNullOrEmpty(RemotingClient.ServerURI))
            {//Direct connection
