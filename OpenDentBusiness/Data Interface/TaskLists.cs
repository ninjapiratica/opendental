...
namespace OpenDentBusiness
{
    ...
    public class TaskLists
    {
        ...
        public static string BuildFilterWhereClause(long currentUserNum, long filterClinicFkey, long filterRegionFkey)
        {
            ...
            if (listUnrestrictedClinicNums.Contains(filterClinicFkey))
...
//THIS WON'T WORK BECAUSE THERE ARE TOO MANY REPEATING TASKLISTS.
        ...
        public static bool IsAncestorTaskListArchived(ref Dictionary<long, TaskList> dictAllTaskLists, TaskList taskList, bool isDictionaryRefreshed = false)
        {
            ...
            {//If list has no parent return false.
                return false;
            }
            if (dictAllTaskLists.ContainsKey(taskList.Parent))
            {//If list parent is in dictionary
                if (dictAllTaskLists[taskList.Parent].TaskListStatus == TaskListStatusEnum.Archived)
                {//and parent is archived return true
                    return true;
                }
        ...
        public static bool IsAnchorTaskListArchived2(List<TaskList> listTaskListsAll, TaskList taskList)
        {
            ...
            {//If list parent is present
                if (taskListParent.TaskListStatus == TaskListStatusEnum.Archived)
                {//and parent is archived return true
                    return true;
                }
