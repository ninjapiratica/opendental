...
namespace OpenDentBusiness
{
    ...
    public class RepeatCharges
    {
            ...
            bool isAutomated = false, LogSources source = LogSources.None, Patient newPat = null)
        {
            ...
            if (isAutomated)
            {//If the change was created automatically by SignUpPortal, eRx, etc
            ...
            try
            {
                ...
                foreach (RepeatCharge repeatCharge in listRepeatingCharges)
                {
                    ...
                    foreach (DateTime billingDate in listBillingDates)
                    {
                        ...
                        try
                        {
                            procAdded = AddProcForRepeatCharge(repeatCharge, billingDate, dateRun, orthoCaseProcLinkingData);
                        }
                        ...
                        if (repeatCharge.CreatesClaim && !InsPlanPreferences.NoBillIns(ProcedureCodes.GetProcCode(repeatCharge.ProcCode), insPlan))
                        {
                            ...
                            listClaimsAdded = AddClaimsHelper(repeatCharge, procAdded, orthoCaseProcLinkingData, listPlansForPat, listInsSubs, listInsPlans);
                        }
        ...
        private static List<Claim> AddClaimsHelper(RepeatCharge repeateCharge, Procedure proc, OrthoCaseProcLinkingData orthoCaseProcLinkingData, List<PatPlan> patPlanList, List<InsSub> subList,
            ...
            , OrthoCaseProcLinkingData orthoCaseProcLinkingData, bool isNewCropInitial = false, bool isNewCropFutureDated = false)
        {
            ...
            if (ProcedureCodes.AreAnyProcCodesHidden(procCode.CodeNum) && !procCode.ProcCode.ToLower().StartsWith("z"))
            {//Procedure is in a hidden category and not a Z-code.
            ...
            {
                if (AvaTax.DoSendProcToAvalara(procedure, isSilent: true))
