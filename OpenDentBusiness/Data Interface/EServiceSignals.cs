...
namespace OpenDentBusiness
{
    ...
    public class EServiceSignals
    {
        ...
        public static eServiceSignalSeverity GetListenerServiceStatus()
        {
            ...
            if (listSignal.Count == 0)
            { //No signals means the eConnector has never run. Nothing to report.				
                return eServiceSignalSeverity.None;
            }
            if (listSignal[0].Severity == eServiceSignalSeverity.NotEnabled)
            { //NotEnabled means they don't care what the status is. Nothing to report.
