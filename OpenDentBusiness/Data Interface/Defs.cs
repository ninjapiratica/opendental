...
namespace OpenDentBusiness
{
    public class Defs
    {
        ...
        {
            //No need to check MiddleTierRole; no call to db.
            List<Def> defs = Defs.GetDefsForCategory(DefCat.AdjTypes, true).FindAll(x => x.ItemValue == "+");
            if (considerPermission)
            {
                return defs.FindAll(x => GroupPermissions.HasPermissionForAdjType(x));
            }
            return defs;
        }
        ...
        public static bool IsDefinitionInUse(Def def)
        {
            ...
            switch (def.Category)
            {
                ...
                case DefCat.BillingTypes:
                    ...
                    if (isClinicDefaultBillingType || PrefC.GetLong(PrefName.PracticeDefaultBillType) == def.DefNum)
                    { //If practice default billing type is equal to the definitions DefNum, then in use.
                        return true;
                    }
                    break;
        ...
        public static void MergeBillingTypeDefNums(long defNumFrom, long defNumTo)
        {
            ...
            long programNumExpressCollect = Programs.GetProgramNum(ProgramName.TrojanExpressCollect);
            command = $@"UPDATE programproperty 
				SET PropertyValue='{strDefNumTo}' 
				WHERE ProgramNum={POut.Long(programNumExpressCollect)} 
