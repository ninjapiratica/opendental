    ...
    public class InsEditLogs
    {
        ...
        public static List<InsEditLog> GetLogsForPlan(long planNum, long carrierNum, long employerNum)
        {
            ...
            for (int i = 0; i < listInsVerifyHists.Count; i++)
            {
                ...
                if (insVerifyOld != null)
                {
                    oldVal = insVerifyOld.DateLastVerified.ToShortDateString();
                }
        ...
        private static List<InsEditLog> GetChangedLogs(List<InsEditLog> listLogs)
        {
            ...
            for (int i = 0; i < listInsPlanChangedLogs.Count; i++)
            {
                if (listInsPlanChangedLogs[i].FieldName == "PlanNum")
                {
                    InsPlan oldPlan = InsPlans.GetPlan(PIn.Long(listInsPlanChangedLogs[i].OldValue), null);
                    if (oldPlan != null)
                    {
                        listLogs.AddRange(GetLinkedLogs(oldPlan.CarrierNum, InsEditLogType.Carrier, listInsPlanChangedLogs[i], listLogs));
                    }
                    listLogs.AddRange(GetLinkedLogs(PIn.Long(listInsPlanChangedLogs[i].OldValue), InsEditLogType.InsPlan, listInsPlanChangedLogs[i], listLogs));
                }
                if (listInsPlanChangedLogs[i].FieldName == "CarrierNum")
                {
                    listLogs.AddRange(GetLinkedLogs(PIn.Long(listInsPlanChangedLogs[i].OldValue), InsEditLogType.Carrier, listInsPlanChangedLogs[i], listLogs));
                }
            }
            return listLogs;
        }
        ...
        public static void MakeLogEntry<T>(T itemCur, T itemOld, InsEditLogType logType, long userNumCur)
        {
            ...
            {
                if (fieldInfoArray[i].IsDefined(typeof(CrudColumnAttribute))
                && (((CrudColumnAttribute)fieldInfoArray[i].GetCustomAttribute(typeof(CrudColumnAttribute))).SpecialType.HasFlag(CrudSpecialColType.DateEntry)
                 || ((CrudColumnAttribute)fieldInfoArray[i].GetCustomAttribute(typeof(CrudColumnAttribute))).SpecialType.HasFlag(CrudSpecialColType.DateTEntry)
                 || ((CrudColumnAttribute)fieldInfoArray[i].GetCustomAttribute(typeof(CrudColumnAttribute))).SpecialType.HasFlag(CrudSpecialColType.ExcludeFromUpdate)
                 || ((CrudColumnAttribute)fieldInfoArray[i].GetCustomAttribute(typeof(CrudColumnAttribute))).SpecialType.HasFlag(CrudSpecialColType.TimeStamp)))
                {
                    continue; //skip logs that are not user editable.
                }
                object valOld = fieldInfoArray[i].GetValue(itemOld) ?? "";
                if (itemCur == null)
                { //deleted, show all deleted columns
                    logCur = new InsEditLog()
                    {
                        FieldName = fieldInfoArray[i].Name,
                        UserNum = userNumCur,
                ...
                object valCur = fieldInfoArray[i].GetValue(itemCur) ?? "";
