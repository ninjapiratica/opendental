...
namespace OpenDentBusiness
{
    ...
    public class ClockEvents
    {
        ...
        public static DataTable GetTimeCardManage(DateTime startDate, DateTime stopDate, long clinicNum, bool isAll)
        {
            ...
            retVal.Columns.Add("rate3OTHours");
            retVal.Columns.Add("PTOHours");
            ...
            foreach (Employee employee in listEmployees)
            {
                ...
                List<TimeSpan> listTsRate2HoursWeekly = new List<TimeSpan>();//Not included in total hours worked.  tsRate2Hours is different than r2Hours and r2OTHours
                List<TimeSpan> listTsRate3HoursWeekly = new List<TimeSpan>();//Not included in total hours worked.  tsRate3Hours is differant than r3Hours and r3OTHours
                ...
                for (int i = 0; i < weekStartDates.Count; i++)
                {
                    ...
                    listTsRate2HoursWeekly.Add(TimeSpan.Zero);
                    listTsRate3HoursWeekly.Add(TimeSpan.Zero);
                }
                int weekCur = 0;
                for (int i = 0; i < listEmpClockEvents.Count; i++)
                {
                    ...
                    //OverTime-----
                    if (listEmpClockEvents[i].OTimeHours != TimeSpan.FromHours(-1))
                    {//Manual override
                    ...
                    //Rate2
                    if (listEmpClockEvents[i].Rate2Hours != TimeSpan.FromHours(-1))
                    {//Manual override
                        listTsRate2HoursWeekly[weekCur] += listEmpClockEvents[i].Rate2Hours;
                    }
                    else
                    {
                        listTsRate2HoursWeekly[weekCur] += listEmpClockEvents[i].Rate2Auto;
                    }
                    //Rate3
                    if (listEmpClockEvents[i].Rate3Hours != TimeSpan.FromHours(-1))
                    {//Manual override
                        listTsRate3HoursWeekly[weekCur] += listEmpClockEvents[i].Rate3Hours;
                    }
                    else
                    {
                        listTsRate3HoursWeekly[weekCur] += listEmpClockEvents[i].Rate3Auto;
                    }
                }
                //reset current week to itterate through time adjusts
                weekCur = 0;
                for (int i = 0; i < listEmpTimeAdjusts.Count; i++)
                {//list of timeAdjusts have already been filtered.
                ...
                for (int i = 0; i < weekStartDates.Count; i++)
                {
                    ...
                    totalRate2HoursWorked += listTsRate2HoursWeekly[i].TotalHours;
                    totalRate3HoursWorked += listTsRate3HoursWeekly[i].TotalHours;
                }
                double rate1ratio = 0;
                double rate2ratio = 0;
                double rate3ratio = 0;
                ...
                double ptoHours = listEmpTimeAdjusts.Where(x => x.PtoDefNum != 0).Sum(x => x.PtoHours.TotalHours);
                ...
                dataRowCur["rate3OTHours"] = TimeSpan.FromHours(r3OTHours).ToString();
                dataRowCur["PTOHours"] = TimeSpan.FromHours(ptoHours).ToString();
