...
namespace OpenDentBusiness
{
    ...
    public class Sheets
    {
        ...
        public static int CreateSheetsForCheckIn(Appointment appt)
        {
            ...
            if (!MobileAppDevices.IsClinicSignedUpForEClipboard(PrefC.HasClinicsEnabled ? appt.ClinicNum : 0))
            { //this clinic isn't signed up for this feature
                return 0;
            }
            if (!ClinicPrefs.GetBool(PrefName.EClipboardCreateMissingFormsOnCheckIn, appt.ClinicNum))
            ...
            if (listSheetsToCreate.Count == 0)
            { //There aren't any sheets to create here
                return 0;
            }
            ...
            foreach (EClipboardSheetDef sheetInsert in listSheetsToCreate.OrderBy(x => x.ItemOrder))
            {
                ...
                {
                    if (sheetInsert.ResubmitInterval.Days == 0 && lastCompleted.RevID >= sheetInsert.PrefillStatusOverride)
                    {//Once should always equal 0 but in case check both
        ...
        public static string CreatePdfForXamarin(long sheetNum)
        {
            ...
            sheetDrawingJob.CreatePdf(sheet, tempFile, null, null, null);
