...
namespace OpenDentBusiness
{
    ...
    public class Claims
    {
        ...
        public static void CalculateAndUpdate(List<Procedure> listProcedures, List<InsPlan> listInsPlans, Claim claimCur, List<PatPlan> patPlans, List<Benefit> benefitList, Patient patient, List<InsSub> subList)
        {
            ...
            bool isFeeBilledUpdateNeeded = !claimCur.ClaimStatus.In("R", "S");//If claimCur is not received/sent then we can update the feeBilled/claimFee
            ...
            for (int i = 0; i < listClaimProcsForClaim.Count; i++)
            {
                ...
                //fee:
                if (isFeeBilledUpdateNeeded && plan.ClaimsUseUCR)
                ...
                else if (isFeeBilledUpdateNeeded)
            ...
            if (isFeeBilledUpdateNeeded)
            { //if claimStatus is sent or recieved, don't update. This may be unneccessary but it doesn't hurt anything and it makes make our intentions clear
        ...
        public static bool GetAssignmentOfBenefits(Claim claim, InsSub insSub)
        {
            ...
            if (clinicPref != null)
            {//If the clinicpref exists, we know that we are always assigning to patient, which translates to false for the pref.
        ...
        public static List<string> GetAllCreditsGreaterThanProcFees(List<ClaimProc> listClaimProcsHypothetical)
        {
            ...
            for (int i = 0; i < listClaimProcsHypothetical.Count; i++)
            {
                ...
                decimal patPaySum = listPaySplitsForClaimProcsDB.Where(x => x.ProcNum == claimProc.ProcNum).Select(x => (decimal)x.SplitAmt).Sum();
        ...
        public static bool ReceiveAsNoPaymentIfNeeded(long claimNum)
        {
            ...
            SecurityLogs.MakeLogEntry(Permissions.ClaimSentEdit, claim.PatNum, logText, claim.ClaimNum, LogSources.ClaimReceiveAutomatic, claim.SecDateTEdit, userNum: 0);
