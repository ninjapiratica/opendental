...
namespace OpenDentBusiness
{
    ...
    public class HL7Msgs
    {
        ...
        public static string GetControlId(HL7Msg msg)
        {
            ...
            for (int s = 0; s < hl7defmsg.ListHL7DefSegments.Count; s++)
            {//get MSH segment
                if (hl7defmsg.ListHL7DefSegments[s].SegmentName == SegmentNameHL7.MSH)
                {
                    for (int f = 0; f < hl7defmsg.ListHL7DefSegments[s].hl7DefFields.Count; f++)
                    {//find messageControlId field in MSH segment def
                        if (hl7defmsg.ListHL7DefSegments[s].hl7DefFields[f].FieldName == "messageControlId")
                        {
                            controlIdOrder = hl7defmsg.ListHL7DefSegments[s].hl7DefFields[f].OrdinalPos;
                            break;
                        }
                    }
                    break;
