...
namespace OpenDentBusiness
{
    ...
    public class ProcedureCodes
    {
        ...
        {
            eServiceCode eservice = ProcedureCodes.GetEServiceForProcCode(procCode.ProcCode);
            return EnumTools.GetAttributeOrDefault<HostedUrlAttribute>(eservice).CanResellerOverride;
        }
        ...
        public static long GetSubstituteCodeNum(string procCode, string toothNum, long planNum, List<SubstitutionLink> listSubLinks = null)
        {
            ...
            ODException.SwallowAnyException(() =>
            {
                ...
                if (codeNum == 0)
                {//If somehow we got a bad value up above we'll revert back to the procedure code
                    codeNum = procedureCode.CodeNum;
                }
            });
            return codeNum;
        ...
        {
            List<string> listCodes = PrefC.GetString(pref).Split(',').Select(x => x.Trim()).ToList();//ProcCode aka D Code
            ...
            long feeSched2Num, long feeSched3Num)
        {
            ...
            {
                command += "CASE ";
                if (!feeSched.IsGlobal && Clinics.ClinicNum != 0)
            ...
            {
                command += "CASE ";
                if (!feeSched2.IsGlobal && Clinics.ClinicNum != 0)
                {//Use local clinic fee if there's one present
            ...
            {
                command += "CASE ";
                if (!feeSched3.IsGlobal && Clinics.ClinicNum != 0)
                {//Use local clinic fee if there's one present
        ...
        public static void ResetApptProcsQuickAddCA()
        {
            ...
            for (int i = 0; i < array.Length; i += 2)
            {
                if (!ProcedureCodes.IsValidCode(array[i + 1]))
                {//first, test all procedures for valid
                    continue;
                }
                def = new Def
        ...
        public static int ResetADAdescriptionsAndAbbrs(List<ProcedureCode> codeList)
        {
            ...
            for (int i = 0; i < codeList.Count; i++)
            {
                ...
                if (!isDescriptMatch || isDbProcAbbrDescBlank)
                {//Only increments one time for each code if there are changes necessary.
                    count++;
                }
                if (!isDescriptMatch)
                {//Update description.
