...
namespace OpenDentBusiness
{
    ...
    public class ApptReminderRules
    {
            ...
            , Action<string> logOnError = null)
        {
            ...
            else if (aptCur.AptStatus.In(ApptStatus.Broken, ApptStatus.UnschedList, ApptStatus.Complete))
            {//Broken,Unscheduled, and Complete appointments should not be confirmed.
                logError("when AptStatus is " + aptCur.AptStatus.GetDescription());
                return ConfStatusUpdate.Failure;
            }
            else if (request.ApptDateTime != aptCur.AptDateTime)
            {//Appointments moved from their confirmationrequest original time should not be confirmed.
                logError($"when AptDateTime has changed since sending Confirmation.  AptDateTimeOrig: {request.ApptDateTime}; AptDatetime: {aptCur.AptDateTime}");
                return ConfStatusUpdate.Failure;
            }
        ...
        public static ApptReminderRule CreateDefaultReminderRule(ApptReminderType ruleType, long clinicNum = 0, bool isBeforeAppointment = true)
        {
            ...
            bool canUseCalendarTag = PrefC.HasClinicsEnabled ? ClinicPrefs.GetBool(PrefName.ApptConfirmAutoEnabled, clinicNum) : PrefC.GetBool(PrefName.ApptConfirmAutoEnabled);
            string strAddToCalendar = canUseCalendarTag ? " To add this to your calendar, visit [AddToCalendar]." : "";
            string strAddToCalendarPerAppt = canUseCalendarTag ? " Add to calendar: [AddToCalendar]" : "";
            switch (ruleType)
            {
...
Click <a href=""[ConfirmURL]"">[ConfirmURL]</a> to confirm or call <a href=""tel:[OfficePhone]"">[OfficePhone]</a>.",
                        ...
                        TemplateAutoReply = "Thank you for confirming your appointment with [OfficeName].  We look forward to seeing you." + strAddToCalendar,
                        TemplateAutoReplyAgg = "Thank you for confirming your appointments with [OfficeName].  We look forward to seeing you" + strAddToCalendarPerAppt,
