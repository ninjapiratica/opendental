...
namespace OpenDentBusiness
{
    ...
    public class PatientNotes
    {
        ...
        {
            //No need to check MiddleTierRole; Private static method.
        ...
        public static void Merge(Patient patFrom, Patient patTo)
        {
            ...
            string strMergeDiv = "\r\n";
            if (!string.IsNullOrEmpty(patNoteTo.FamFinancial))
            {//FamFinancial
            ...
            if (!string.IsNullOrEmpty(patNoteTo.Medical))
            {//Medical
                patNoteTo.Medical += strMergeDiv;
            }
            patNoteTo.Medical += patNoteFrom.Medical;
            if (!string.IsNullOrEmpty(patNoteTo.Service))
            {//Service
                patNoteTo.Service += strMergeDiv;
            }
            patNoteTo.Service += patNoteFrom.Service;
            if (!string.IsNullOrEmpty(patNoteTo.MedicalComp))
            {//MedicalComp
                patNoteTo.MedicalComp += strMergeDiv;
            }
            patNoteTo.MedicalComp += patNoteFrom.MedicalComp;
            if (!string.IsNullOrEmpty(patNoteTo.Treatment))
            {//Treatment
                patNoteTo.Treatment += strMergeDiv;
            }
            patNoteTo.Treatment += patNoteFrom.Treatment;
            if (string.IsNullOrEmpty(patNoteTo.ICEName))
            {//ICEName, only change if patNotTo was not set.
                patNoteTo.ICEName += patNoteFrom.ICEName;
            }
            if (string.IsNullOrEmpty(patNoteTo.ICEPhone))
            {//ICEPhone, only change if patNotTo was not set.
                patNoteTo.ICEPhone += patNoteFrom.ICEPhone;
            }
            if (patNoteTo.OrthoMonthsTreatOverride == -1)
            {//OrthoMonthsTreatOverride, only change if patNoteTo was not set.
                patNoteTo.OrthoMonthsTreatOverride = patNoteFrom.OrthoMonthsTreatOverride;
            }
            if (patNoteTo.DateOrthoPlacementOverride != DateTime.MinValue)
            {//DateOrthoPlacementOverride, only change if patNotTo was not set.
