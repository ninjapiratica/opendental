    ...
    public class EmailHostingTemplates
    {
        ...
        private static void SyncWithHq(long clinicNum, List<EmailHostingTemplate> listDatabaseTemplates)
        {
            ...
            List<long> listNoRules = listBirthdayTemplates
                .FindAll(x => !listBirthdayRules.Select(y => y.EmailHostingTemplateNum).Contains(x.EmailHostingTemplateNum))
                .Select(z => z.EmailHostingTemplateNum)
                .ToList();
            listDatabaseTemplates.RemoveAll(x => listNoRules.Contains(x.EmailHostingTemplateNum));//Remove orphaned templates
