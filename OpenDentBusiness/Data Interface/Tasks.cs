...
namespace OpenDentBusiness
{
    ...
    public class Tasks
    {
        ...
        {
            bool isAuthorized = Security.IsAuthorized(Permissions.EditReadOnlyTasks, true);
        ...
        public static DataTable GetDataSet(long userNum, List<long> listTaskListNums, List<long> listTaskNums, string taskDateCreatedFrom, string taskDateCreatedTo,
            string taskDateCompletedFrom, string taskDateCompletedTo, string taskIncluding, string taskExcluding, long taskPriorityNum, long patNum, bool doIncludeTaskNote,
            bool doIncludeCompleted, bool doIncludeAttachments, bool limit, bool doRunOnReportServer)
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {
                return Meth.GetTable(MethodBase.GetCurrentMethod(), userNum, listTaskListNums, listTaskNums, taskDateCreatedFrom, taskDateCreatedTo,
                    taskDateCompletedFrom, taskDateCompletedTo, taskIncluding, taskExcluding, taskPriorityNum, patNum, doIncludeTaskNote, doIncludeCompleted, doIncludeAttachments,
                    limit, doRunOnReportServer);
            }
        ...
        public static List<long> GetTasksNumsForSearch(long userNum, List<long> listTaskListNums, List<long> listTaskNums, DateTime dateCreatedFrom,
            DateTime dateCreatedTo, DateTime dateCompletedFrom, DateTime dateCompletedTo, string taskIncluding, string taskExcluding, long taskPriorityNum, long patNum,
            bool doIncludeTaskNote, bool doIncludeCompleted, bool doIncludeAttachments, bool limit)
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {
                return Meth.GetObject<List<long>>(MethodBase.GetCurrentMethod(), userNum, listTaskListNums, listTaskNums, dateCreatedFrom, dateCreatedTo,
                    dateCompletedFrom, dateCompletedTo, taskIncluding, taskExcluding, taskPriorityNum, patNum, doIncludeTaskNote, doIncludeCompleted, doIncludeAttachments, limit);
            }
            ...
            if (doIncludeTaskNote)
            {
                ...
                if (doExcludeAttachments)
                {//Used only for Union column alignment.
        ...
        private static string BuildFilterWhereClause(long currentUserNum, GlobalTaskFilterType globalFilterType, List<long> listFilterFkeys)
        {
            ...
            switch (globalFilterType)
            {
                case GlobalTaskFilterType.Clinic:
                    ...
                    if (listFilterFkeys.Count == 1 && listFilterFkeys.Contains(0))
                    ...
                    {//Make sure user is not restricted for these clinics.
                        listFkeys.AddRange(listUnrestrictedClinics.Where(x => listFilterFkeys.Contains(x.ClinicNum)).Select(x => x.ClinicNum));
                    }
                    break;
                case GlobalTaskFilterType.Region:
                    listFkeys.AddRange(listUnrestrictedClinics.FindAll(x => listFilterFkeys.Contains(x.Region)).Select(x => x.ClinicNum));
                    break;
