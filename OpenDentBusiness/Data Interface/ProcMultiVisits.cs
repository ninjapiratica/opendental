...
namespace OpenDentBusiness
{
    ...
    public class ProcMultiVisits
    {
        ...
        public static void CreateGroup(List<Procedure> listProcedures)
        {
            ...
            {//No reason to make a "group" with 0 or 1 items.
                return;
            }
        ...
        public static List<long> UpdateGroupForProc(long procNum, ProcStat stat)
        {
            ...
            for (int i = 0; i < listClaims.Count; i++)
            {
                ...
                {//This is rare but still happens.  See DBM. 
                    continue;
                }
                ...
                else
                {
                    ...
                    { //If the current claim isn't primary, we need to check if we should set the status to Hold Until Pri Received instead of Waiting to Send.
                        ...
                        if (listClaimsForClaimProcs.Exists(x => x.ClaimType == "P"))
                        { //Check if a primary claim is attached to one of the same procedures that this non-primary claim is attached to
                    ...
                    foreach (ClaimProc cp in listClaimProcsForClaim)
                    {
                        ...
                        {//Procedure is part of a multi visit group.
                            ...
                            foreach (ProcMultiVisit procMultiVisit in listGroupPmvs)
                            {
                                ...
                                if (procCur != null && procCur.ProcDate > dateForGroup)
                                {//Can be null if the user deleted the proc after the multi visit group was created.
                    ...
                    if (listProcMultiVisitsForClaims.Count > 0)
                    {//A multi visit claim.
