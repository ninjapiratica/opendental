...
namespace OpenDentBusiness
{
    public class DatabaseMaintenances
    {
        ...
        public static void RepairAndOptimize()
        {
            ...
            for (int i = 0; i < tableNames.Length; i++)
            {
                if (listInnoDbTableNames.Contains(tableNames[i]))
                {//Skip innodb tables
                    continue;
                }
            ...
            switch (modeCur)
            {
                ...
                case DbmMode.Breakdown:
                    ...
                    for (int i = 0; i < dataTable.Columns.Count; i++)
                    {
                        ...
                        if (strColumnValueCur.Length < columnWidthCur)
                    ...
                    for (int i = 0; i < dataTable.Rows.Count; i++)
                    {
                        for (int j = 0; j < dataTable.Columns.Count; j++)
                        {
                            ...
                            if (strColumnValueCur.Length < dataTable.Columns[j].ColumnName.Length)
                            {//Pad with spaces if needed
                ...
                case DbmMode.Breakdown:
                    ...
                    foreach (KeyValuePair<long, List<Procedure>> keyValuePair in dictPatProcs)
                    {
                        ...
                        for (int i = 0; i < keyValuePair.Value.Count; i++)
                        {
                            ...
                            for (int j = 0; j < listAttachedAdjustments.Count; j++)
                            {//Adjustments
                            ...
                            for (int j = 0; j < listAttachedPaySplits.Count; j++)
                            {//PaySplits
                            ...
                            for (int j = 0; j < listAttachedClaimProc.Count; j++)
                            {//ClaimProcs
                            ...
                            for (int j = 0; j < listAttachedPayPlanCharges.Count; j++)
                            {//PayPlanCharges
                            ...
                            for (int j = 0; j < listAttachedPayPlanLinks.Count; j++)
                            {//PayPlanLinks
                ...
                case DbmMode.Breakdown:
                    if (table.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < table.Rows.Count; i++)
                        {
                            ...
                            if (table.Rows[i]["ToothNum"].ToString() != "")
                            {   //displays ToothNum if present
            ...
            switch (modeCur)
            {
                ...
                case DbmMode.Fix:
                    ...
                    if (listApptViewItemNumsToDelete.Count > 0)
                    {
                        ...
                        if (listOrphaned.Count > 0)
                        {//If any orphaned rows exist, then we delete the ApptView as well
...
HAVING cnt>1";
        ...
        public static string CleanUpRawEmails()
        {
            ...
            if (DataConnection.DBtype == DatabaseType.MySql && tableEmailMessageNums.Rows.Count != noChangeCount)
