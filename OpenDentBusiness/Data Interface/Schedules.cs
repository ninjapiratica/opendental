...
namespace OpenDentBusiness
{
    ...
    public class Schedules
    {
        ...
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {//Remoting role check so that the PK can be returned to the client.
        ...
        public static List<Schedule> GetProvSchedsForOp(List<Schedule> listSchedulesPeriod, Operatory op)
        {
            ...
            foreach (Schedule schedule in listSchedulesPeriod.FindAll(x => x.SchedType == ScheduleType.Provider))
            {//only schedules for provs
                if (schedule.Ops.Count(x => x != 0) > 0)
                {//leaving count only non 0's, but 0's are no longer added in ConvertTableToList with remove empty entries code
                    if (schedule.Ops.Contains(op.OperatoryNum))
                ...
                {//op is either a hygiene op with no hygienist set or not a hygiene op with no provider set
                    if (schedule.ProvNum == PrefC.GetLong(PrefName.ScheduleProvUnassigned))
            ...
            , long clinicNum, Logger.IWriteLine log = null, List<Schedule> listRestrictedToBlockouts = null, bool isNewPat = false)
        {
            ...
            if (isRecall)
            {
                ...
                { //If the recall type is associated with some Restricted-To blockout types, then remove all blockouts from the list that do not match those blockouts
            ...
            else
            {
                ...
                { //If the appointment type is associated with some Restricted-To blockout types, then remove all blockouts from the list that do not match those blockouts
            ...
            , List<long> listProvNums, List<long> listBlockoutTypeDefNums, List<int> listSchedTypes, Logger.IWriteLine log = null, bool isForMakeRecall = false)
        {
            ...
            foreach (string provColumnName in new List<string>() { "ProvDentist", "ProvHygienist" })
            {
                ...
                if (provColumnName == "ProvDentist" && !listProvNumsDent.IsNullOrEmpty())
                { //includes relevant provider parameter if possible
            ...
            long scheduleNum, int limit, int offset, List<long> listOpNums = null, string dateFormatString = "yyyy-MM-dd")
        {
            ...
            if (schedType > -1)
            { //0 is included when looking for a change in schedType, since it's an Enum that starts at 0.
            ...
            for (int i = offset; i < listRowsDistinct.Count; i++)
            {
                ...
                for (int j = 0; j < listRowsSameScheduleNum.Count; j++)
                { //If a schedule entry is in multiple operatories, scheduleop will have 1 row for each operatory a ScheduleNum is in.
