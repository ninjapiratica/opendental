...
namespace OpenDentBusiness
{
    ...
    public class Payments
    {
        ...
        public static long InsertFromXWeb(long patNum, long provNum, long clinicNum, double amount, string payNote, string receipt, CreditCardSource ccSource, string logGuid = "")
        {
            ...
            if (logGuid != "")
            {//There is no way to make an XWeb payment from the Patient Portal without making a log GUID.
        ...
        public static long InsertFromPayConnect(long patNum, long provNum, long clinicNum, double amount, string payNote, string receipt, CreditCardSource ccSource, string logGuid = "")
        {
            ...
            SecurityLogs.MakeLogEntry(Permissions.PaymentCreate, patNum, Lans.g("Payments.InsertFromPayConnect", "PayConnect payment by") + " "
                + Patients.GetLim(patNum).GetNameLF() + ", " + amount.ToString("c"), LogSources.PatientPortal);
            if (logGuid != "")
            {//There is no way to make a PayConnect payment from the Patient Portal without making a log GUID.
        ...
        public static Payment InsertReturnXWebPayment(Payment payment, string payNote, double payAmt, ProcessStat processStat = ProcessStat.OfficeProcessed)
        {
            ...
            if (paymentReturn.PayAmt > 0)
            {//User passed us original amount, negate. Voids are always negative.
        ...
        public static long ProcessPaymentForWeb(Payment odbPayment, Patient odbPatient, double payAmt, bool isPatientPreferred = false, bool isPrepayment = false, List<AccountEntry> listAccountEntries = null)
        {
