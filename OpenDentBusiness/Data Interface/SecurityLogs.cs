...
namespace OpenDentBusiness
{
    ...
    public class SecurityLogs
    {
            ...
            DateTime datePreviousFrom, DateTime datePreviousTo, int limit = 0, long userNum = -1, int logSource = -1)
        {
            ...
            if (userNum >= 0)
            { //Greater than or equal to 0, since 0 is no/unknown user, and we want to be able to filter by that option in some cases.
                command += " AND UserNum=" + POut.Long(userNum);
            }
            if (logSource >= 0)
            { //Greater than or equal to 0, since 0 is Automation/unknown, and we want to be able to filter by that option in some cases.
