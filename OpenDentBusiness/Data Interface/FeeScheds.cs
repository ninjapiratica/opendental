    ...
    public class FeeScheds
    {
        ...
        public static long GetMedFeeSched(Patient pat, List<InsPlan> planList, List<PatPlan> patPlans, List<InsSub> subList, long procProvNum)
        {
            ...
            if (PatPlans.GetInsSubNum(patPlans, 1) != 0)
            {
                ...
                if (!hasMedIns)
                { //If this patient doesn't have medical insurance (under ordinal 10)
        ...
        public static long GlobalUpdateWriteoffs(List<long> listWriteoffClinicNums, ODProgressExtended progress, bool doUpdatePrevClinicPref = false)
        {
            ...
            foreach (long clinicNumCur in listWriteoffClinicNums)
            {
                ...
                if (PrefC.HasClinicsEnabled && clinicNumCur > 0)
                {//listFeesHQ is already the fees for ClinicNum 0, only add to list if > 0
                ...
                listActions = listFamProcs.Select(x => new Action(() =>
                {
                    ...
                    foreach (PatProc patProc in x.ListPatProcs)
                    {//foreach patient in the family
                        ...
                        BlueBookEstimateData blueBookEstimateData = new BlueBookEstimateData(listInsPlansCur, listInsSubsCur, listPatPlansCur, patProc.ListProcs, listSubstitutionLinks);
                        foreach (Procedure procCur in patProc.ListProcs)
                ...
                if (listWriteoffClinicNums.Count > 1)
                {//only show if more than one clinic
                    progress.Fire(ODEventType.FeeSched,
