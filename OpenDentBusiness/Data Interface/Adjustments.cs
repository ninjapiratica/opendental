...
namespace OpenDentBusiness
{
    ...
    public class Adjustments
    {
        ...
        public static List<Adjustment> GetForProc(long procNum, Adjustment[] List)
        {
        ...
        public static List<Adjustment> GetAdjustForPatsByType(List<long> listPatNums, long adjType, DateTime maxAdjDate)
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {
                return Meth.GetObject<List<Adjustment>>(MethodBase.GetCurrentMethod(), listPatNums, adjType, maxAdjDate);
            }
        ...
        public static void CreateSalesTaxRefundIfNeeded(Procedure procedure, Adjustment adjustmentExistingLocked)
        {
            ...
            adjustmentSalesTaxReturn.AdjType = AvaTax.GetSalesTaxReturnAdjType();
        ...
        public static void CreateOrUpdateSalesTaxIfNeeded(Procedure procedure, Adjustment salesTaxAdj = null, bool doCalcTax = true, bool isRepeatCharge = false)
        {
            ...
            if (salesTaxAdj == null)
            {
                ...
                salesTaxAdj.AdjType = AvaTax.GetSalesTaxAdjType();
        ...
        {
            if (AvaTax.IsEnabled() && modifiedAdj.ProcNum > 0 && modifiedAdj.AdjType != AvaTax.GetSalesTaxAdjType() && modifiedAdj.AdjType != AvaTax.GetSalesTaxReturnAdjType())
            {
        ...
        public static ChargeUndoData UndoFinanceOrBillingCharges(DateTime dateUndo, bool isBillingCharges)
        {
            ...
            {//loops through the remaining rows and creates an audit trail entry for every row to be deleted
                ...
                listActions.Add(new Action(() =>
                {
                    ...
                    if (++loopCount % 5 == 0)
                    {//Have to use loopCount instead of i because we must increment within the action.
        ...
        public static ChargeUndoData UndoLateCharges(DateTime dateUndo)
        {
            ...
            {
                DataRow rowCur = table.Rows[i];
                if (PIn.Bool(rowCur["HasPaySplits"].ToString()) || PIn.Bool(rowCur["HasPayPlan"].ToString()))
