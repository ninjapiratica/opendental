...
namespace OpenDentBusiness
{
    ...
    public class ClaimProcs
    {
        ...
        public static List<PayAsTotal> GetOutstandingClaimPayByTotal(List<long> listFamilyPatNums, List<ClaimProc> listClaimsAsTotalForPats = null)
        {
            ...
            foreach (long key in dictClaimsAsTotalByClaim.Keys)
            {//foreach claim
                var groupedClaimProcs = dictClaimsAsTotalByClaim[key].GroupBy(x => new { x.PatNum, x.ProvNum, x.ClinicNum });//group by pat/prov/clinic
                foreach (var group in groupedClaimProcs)
                {//loop through the groups to get sum of what needs to be transferred.
        ...
        public static ClaimTransferResult TransferClaimsAsTotalToProcedures(List<long> listFamilyPatNums, List<ClaimProc> listClaimsAsTotalForPats = null)
        {
            ...
            foreach (PayAsTotal payAsTotal in listClaimsAsTotalForFamily)
            {
                ...
                foreach (ClaimProc procClaimProc in listValidClaimProcsForAsTotalPayment)
                {//pay only up to the insurance estimate for this first round through.
                    ...
                    if (claimPayForProc == null)
                    {
                        ...
                        if (procClaimProc.InsPayEst != 0 && insPayAmtToAllocate != 0 && insPayAmtSupplemental < procClaimProc.FeeBilled)
                        ...
                        if (procClaimProc.WriteOffEst != -1 && writeOffAmtToAllocate != 0)
                        {//estimated writeoff exists and there is money to allocate
                    ...
                    else
                    {
                        ...
                        if (procClaimProc.InsPayEst != 0 && insPayAmtToAllocate != 0)
                        {//estimated payment exists and there is money to allocate
                ...
                if (!CompareDouble.IsZero(insPayAmtToAllocate) || !CompareDouble.IsZero(writeOffAmtToAllocate))
                {//negatives are okay when original pay as total was negative
                    ...
                    foreach (ClaimProc claimPayForProc in listClaimProcsCreatedForClaim)
                    {
                        ...
                        if (amountRemainingOnProc != 0 && writeOffAmtToAllocate != 0)
                        {//we paid the insPayAmt but there is still room to apply a write off
            ...
            , List<OrthoProcLink> listProcLinksForOrthoCase)
        {
            ...
            foreach (ClaimProc claimProcCur in listClaimProcsToUpdate)
            {
                ...
                {//At this point, if ordinal is not 1, it must be 2.
        ...
        public static void ApplyAsTotalPayment(ref ClaimProc[] listClaimProcs, double totalPayAmt, List<Procedure> listProcs)
        {
            ...
            foreach (ClaimProc claimProcCur in sortedClaimProcs)
            {//Build the list of supplemental Write offs / ins payments and put them in a list.
                ...
                for (int i = 0; i < supplementalProcs.Count; i++)
                {//Sum the attached supplemental payments and writeoffs. 
        ...
        public static double GetWriteOffFromList(List<ClaimProc> listClaimProcs, long procNum)
        {
            ...
            {
                if (claimProc.ProcNum != procNum)
                { //Skip claimprocs that don't match
                    continue;
                }
