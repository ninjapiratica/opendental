...
namespace OpenDentBusiness
{
    ...
    public class Appointments
    {
        ...
        ///<summary></summary>
        public static List<AppointmentWithServerDT> GetAppointmentsForApi(int limit, int offset, DateTime dateStart, DateTime dateEnd, DateTime dateTStamp, long clinicNum, long patNum, int aptStatus)
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {
                return Meth.GetObject<List<AppointmentWithServerDT>>(MethodBase.GetCurrentMethod(), limit, offset, dateStart, dateEnd, dateTStamp, clinicNum, patNum, aptStatus);
            }
            ...
            List<AppointmentWithServerDT> listAppointmentForApis = new List<AppointmentWithServerDT>();
            for (int i = 0; i < listAppointments.Count; i++)
            {//list can be empty
                AppointmentWithServerDT appointmentForApi = new AppointmentWithServerDT();
        ...
        public static DataTable GetWebSchedAppointmentsForApi(int limit, int offset, DateTime dateStart, DateTime dateEnd, DateTime dateTStamp, long clinicNum, string dateTimeFormatString)
        {
            ...
            command += "WHERE appointment.AptDateTime >= " + POut.DateT(dateStart) + " "
                + "AND appointment.AptDateTime < " + POut.DateT(dateEnd) + " "
            ...
            , DateTime schedDateEvaluating, List<Schedule> listSchedules, List<Appointment> listAppointments)
        {
            ...
            List<Schedule> listSchedulesOrdered = listSchedules.OrderByDescending(x => listBlockoutTypesDoNotSchedule.Contains(x.BlockoutType))
            ...
            foreach (Schedule schedule in listSchedulesOrdered)
            {
                ...
                {//schedule applies to one of the selected providers
                    ...
                    {
                        if (schedule.BlockoutType > 0 && listBlockoutTypesDoNotSchedule.Contains(schedule.BlockoutType))
                        {
            ...
            List<long> listOpNums = null, List<long> listProvNums = null, bool doRunQueryOnNoOps = true, bool includeVerifyIns = false)
        {
            ...
            for (int j = 0; j < listRefAttaches.Count; j++)
            {
                ...
                if (!dictPatientRefInfos.ContainsKey(listRefAttaches[j].PatNum))
                {//New entry
            ...
            if (includeVerifyIns)
            {
                ...
                if (insPlanNums != "")
                {//if no insPlans, then there can't be any patPlans.
            ...
            foreach (DataRow rowRaw in tableRaw.Rows)
            {
                ...
                actions.Add(new Action(() =>
                {
                    ...
                    row["hasFamilyBalance"] = rowRaw["hasFamilyBalance"].ToString();
                    row["hasIns[I]"] = "";
                    if (rowRaw["hasIns"].ToString() != "0")
                    {
                        row["hasIns[I]"] += "I";
                    ...
                    if (dictPatientRefInfos.ContainsKey(apptPatNum))
                    {//Add this patient's referrals
        ...
        public static DataTable GetProcTable(string patNum, string aptNum, string apptStatus, string aptDateTime)
        {
            ...
            for (int i = 0; i < rawProc.Rows.Count; i++)
            {
                ...
                {
                    row["descript"] += " #" + Tooth.DisplayRange(rawProc.Rows[i]["ToothRange"].ToString());
                }
                ...
                row["toothNum"] = Tooth.Display(rawProc.Rows[i]["ToothNum"].ToString());
        ...
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {//Remoting role check in order to limit the amount of data sent back to the client.
        ...
        public static long InsertIncludeAptNum(Appointment appt, bool useExistingPK, long secUserNum = 0)
        {
            ...
            appt.SecurityHash = HashFields(appt);
        ...
        public static void SetAptStatus(Appointment appt, ApptStatus newStatus, bool suppressHistory = false)
        {
            ...
            appt.AptStatus = newStatus;
            appt.SecurityHash = HashFields(appt);
            string command = "UPDATE appointment SET AptStatus=" + POut.Long((int)newStatus);
            command += ",SecurityHash='" + POut.String(appt.SecurityHash) + "'";
        ...
        public static void SetAptStatusComplete(Appointment appt, long planNum1, long planNum2)
        {
            ...
            string command = "UPDATE appointment SET "
                ...
                + "SecurityHash='" + POut.String(appt.SecurityHash) + "' "
        ...
        public static void SetConfirmed(Appointment appt, long confirmStatus, bool createSheetsForCheckin = true)
        {
            ...
            command += " WHERE AptNum=" + POut.Long(appt.AptNum);
            Db.NonQ(command);
            if (confirmStatus != PrefC.GetLong(PrefName.ApptEConfirmStatusDeclined))
        ...
        public static bool Update(Appointment appointment, Appointment oldAppointment, bool suppressHistory = false)
        {
            ...
            if (IsAppointmentHashValid(oldAppointment))
            { //Only rehash appointments that are already valid
                appointment.SecurityHash = HashFields(appointment);
            }
        ...
        public static bool IsAppointmentHashValid(Appointment appointment)
        {
            ...
            if (appointment.AptDateTime < dateHashStart)
            { //old
            ...
            , bool hasBlockoutCheck, out bool isPatternChanged)
        {
            ...
            if (!TryAdjustAppointment_HasOverlap(apt, hasEndTimeCheck, hasBlockoutCheck, apt.Op))
        ...
        private static bool TryAdjustAppointment_HasOverlap(Appointment apt, bool hasEndTimeCheck, bool hasBlockoutCheck, long opNum)
        {
            ...
            {//Only check for another appointment overlap when the preference is turned off
                return false;
            }
        ...
        public static List<Schedule> GetBlockoutsOverlappingNoSchedule(Appointment apt, List<Schedule> listSchedulesBlockouts = null)
        {
            ...
            if (listSchedulesBlockouts is null)
            { //Get all of today's blockouts that exist in the same operatory as the appointment
            ...
            bool doAllowFreqConflicts, bool doResetConfirmationStatus, bool doUpdatePatStatus, bool provChanged, bool hygChanged, bool timeWasMoved, bool isOpChanged, bool isOpUpdate = false, LogSources secLogSource = LogSources.None, long userNum = 0)
        {
            ...
            {
                if (apt.AptStatus == ApptStatus.Complete)
                { //separate log entry for editing completed appointments
            ...
            Func<bool> funcProcsConcurrentAndNotPlanned, List<Procedure> listProceduresAll, Action actionCompletedProceduresBeingMoved)
        {
            ...
            if (PrefC.GetBool(PrefName.ApptsRequireProc) && listProceduresBeingMoved.Count > 0)
