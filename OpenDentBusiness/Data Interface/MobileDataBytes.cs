...
namespace OpenDentBusiness
{
    ...
    public class MobileDataBytes
    {
        ...
        public static long CreateAndInsertTreatmentPlan(long mobileAppDeviceNum, long patNum, out TreatPlan treatPlan)
        {
            ...
            sheetTP.Parameters.Add(new SheetParameter(true, "toothChartImg")
            {
                ParamValue = SheetFramework.ToothChartHelper.GetImage(patNum,
        ...
        public static long CreateAndInsertPerioChartPdf(Patient pat, PerioExam perioExam)
        {
            PdfDocument pdfDocument = PerioExams.CreatePerioPDF(pat, perioExam);
            MobileDataByte mobileDataByte = null;
            long mobileDataByteNum = 0;
            string errorMsg = "";
            try
            {
                TryInsertPDF(pdfDocument, pat.PatNum, null, eActionType.PerioExam, out mobileDataByteNum, out errorMsg);
