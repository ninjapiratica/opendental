...
namespace OpenDentBusiness
{
    ...
    public class PayPlans
    {
        ...
        public static bool IsPayPlanHashValid(PayPlan payPlan)
        {
            ...
            if (payPlan.SecurityHash == null)
            {//When a payplan is first created through middle tier and not yet refreshed from db, this can be null and should not show a warning triangle.
                return true;
            }
            DateTime dateHashStart = Misc.SecurityHash.DateStart;
            if (payPlan.PayPlanDate < dateHashStart)
            { //old
                return true;
            }
            if (payPlan.SecurityHash == HashFields(payPlan))
            { //Hash is not what it should be
                return true;
            }
            return false;
        }
        ...
        public static Sheet PayPlanToSheet(PayPlan payPlan)
        {
            ...
            if (payPlan.IsDynamic)
            {
                ...
                for (int i = 0; i < listChargesExpected.Count; i++)
