...
namespace OpenDentBusiness.Eclaims
{
    ...
    public partial class x837Controller
    {
        ...
        { //called from this.SendBatch. Clinic-level clearinghouse passed in.
            ...
            { //If attempting to use WebMD, exit if client program does not exist.
            ...
            if (clearinghouseClin.IsClaimExportAllowed)
            {
                ...
                if (ODBuild.IsWeb() && DoSendBatchToCloudClient(clearinghouseClin))
                {
                    ...
                    catch (ODException odEx)
                    {
                        ...
                        if (odEx.ErrorCodeAsEnum != ODException.ErrorCodes.ClaimArchiveFailed)
                        {//If archiving failed, we can continue with sending.
                            return "";
                        }
                    }
                    catch (Exception ex)
