    ...
    public class PaymentEdit
    {
            ...
            List<AccountEntry> listCharges, List<PayPlan> listPayPlans)
        {
            ...
            listPayPlanAccountEntries.RemoveAll(x => x.ChargeType == PayPlanChargeType.Debit
                && CompareDecimal.IsZero(x.AmountEnd)
                && listAllocatedDebits.Select(y => y.PayPlanChargeNum).Contains(x.PayPlanChargeNum));
        ...
        private static List<AccountEntry> ExplicitlyLinkCredits(List<AccountEntry> listAccountEntries, List<PaySplit> listSplitsCurrentAndHistoric, bool hasInsOverpay = false)
        {
            ...
            foreach (AccountEntry accountEntryInsProc in listInsProcEntries)
            {
                ...
                accountEntryInsProc.WarningMsg.AppendLine($"Procedure #{accountEntryInsProc.ProcNum} "
                    + $"for PatNum #{accountEntryInsProc.PatNum} ignored all insurance overpayment:");
                ...
                decimal amountTransferrable = Math.Min(patPay, amountOverpaid);
                //We know that AmountEnd was negative by a certain amount if we got to this point.
                //Only make it as negative as the amount of associated patient payments.
                accountEntryInsProc.AmountEnd = (amountTransferrable * -1);
            }
            #endregion
        ...
        public static ClaimTransferResult TransferClaimsPayAsTotal(long patNum, List<long> listFamPatNums, string logText)
        {
            ...
            if (claimTransferResult != null && claimTransferResult.ListInsertedClaimProcs.Count > 0)
            ...
            List<PayPlan> listPayPlans = null)
        {
            ...
            if (!listPayPlansPatient.IsNullOrEmpty())
            {
                ...
                if (listInvalidNegPayPlanNums.Count > 0)
                {
                    ...
                    List<PayPlan> listInvalidPayPlans = listPayPlansPatient.FindAll(x => listInvalidNegPayPlanNums.Contains(x.PayPlanNum));
                    incomeTransferData.StringBuilderErrors.AppendLine(GetInvalidPayPlanDescription(errorMsgStart, listInvalidPayPlans, dictPayPlanCharges));
                    return false;
                }
            }
