...
namespace OpenDentBusiness
{
    public class ChartModules
    {
        ...
        public static DataTable GetPtInfoForApi(Patient patient)
        {
            ...
            { //For each field, make a new row with its name as Field, then fill Contents on a case-by-case basis.
                ...
                switch (fieldCur.InternalName)
                {
                    ...
                    case "Allergies": //There can be multiple allergies, so we may need to make multiple rows.
                        ...
                        { //If we don't have any entries, make it known that there are none, then add this row.
                            row["Contents"] = "None";
                            tableRetVal.Rows.Add(row); //Only add the "None" row if no sub-rows will exist.
                        }
                        for (int i = 0; i < listAllergies.Count; i++)
                        { //Make a new row for each allergy in the list
                            row = tableRetVal.NewRow();
                    ...
                    case "Medications": //There can be multiple medications, so we may need to make multiple rows.
                        ...
                        { //Create a new row for each medication.
                            ...
                            { //If MedicationNum is 0, we need to use MedDescript for eRx reasons.
                                row["Field"] = "Medication - " + listMedicationPats[i].MedDescript;
                            }
                            else
                            { //Get the correct name for this medication.
                    ...
                    case "Pat Restrictions": //There can be multiple Pat Restrictions, so we may need multiple rows.
                        ...
                        { //Create a new row for each patient restriction.
                            row = tableRetVal.NewRow();
                            if (fieldCur.Description == "")
                            {
                                row["Field"] = fieldCur.InternalName;
                    ...
                    case "Premedicate":
                        if (patient.Premed)
                        { //Only create & add this row if patient.Premed is true.
                            row["Field"] = "";
                    ...
                    case "Problems":
                        ...
                        if (string.IsNullOrWhiteSpace(fieldCur.Description))
                        { //Use an alternate description if one exists.
                            row["Field"] = fieldCur.InternalName;
                        }
                        else
                        {
                        ...
                        { //Add a new row for each problem/disease.
                            row = tableRetVal.NewRow();
