            ...
            float scale = 1)
        {
            ...
            else
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    {
                        if (propertyItem.Value[0] == 6 || propertyItem.Value[0] == 8)
                        {//Image should be rotated 90 or 270 degrees
                            degreesRotated = 90;
        ...
        private static string ProcessList(string s, string prefixChars)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {//we are still hunting for the first line of a list.
                    if (lines[i].StartsWith(prefixChars) || lines[i].StartsWith("<body>" + prefixChars))
                    {//we found the first line of a list.
                        blockOriginal = lines[i];
                        if (!lines[i].EndsWith("</body>"))
                        {//not the last line of the document, a list item can be on the last line, if it is, do not add the \n
                            blockOriginal += "\n";
                        }
                        ...
                        strb.Append("</span></li>\n");
                        if (lines[i].EndsWith("</body>"))
                        {//ends with body tag, so last line is a list item, close the body and replace in original string
                            if (prefixChars.Contains("*"))
                            {
                                strb.Append("</ul>");
                            }
