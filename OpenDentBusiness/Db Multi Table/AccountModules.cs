...
namespace OpenDentBusiness
{
    public class AccountModules
    {
        ...
        {
            //No need to check MiddleTierRole; this method contains out parameters.
            if (stmt == null)
            {
            ...
            if (stmt.StatementType == StmtType.LimitedStatement && statementNum > 0)
            {
                ...
                payPlanChargeNumsForLimited = string.Join(",", stmt.ListPayPlanChargeNums.Select(x => POut.Long(x)));
            }
            #region Claimprocs
            ...
            for (int i = 0; i < rawProc.Rows.Count; i++)
            {
                ...
                if (toothRange != "")
                {
                    row["description"] += " #" + Tooth.DisplayRange(toothRange);
                }
                ...
                row["tth"] = Tooth.Display(toothNum);
                if (strProcNumLab == "0")
                {//normal proc
                    rows.Add(row);
                }
            ...
            //Payment query 2
            if (!string.IsNullOrWhiteSpace(familyPatNums) && stmt.StatementType != StmtType.LimitedStatement)
            ...
            for (int i = 0; i < rawPay.Rows.Count; i++)
            {
                ...
                if (!fam.ListPats.Select(x => x.PatNum).Concat(listSuperFamPatNums).Contains(rowPatNum))
                {
                    continue;
                }
            ...
            { //this information is only required for v2
                ...
                if (stmt.StatementType == StmtType.LimitedStatement && !string.IsNullOrEmpty(payPlanChargeNumsForLimited))
                {
                ...
                for (int i = 0; i < rawPayPlan2.Rows.Count; i++)
                {
                    ...
                    {//show principle amount as a credit if it's a credit chargeType.
                        if (stmt.StatementType == StmtType.LimitedStatement)
                        {//Not interested in credits for limited statements.
                            continue;
                        }
                        row["chargesDouble"] = 0;
                        row["charges"] = "";
            ...
            {//LimitedStatements don't have a balance forward row
                ...
                if (foundBalForward && (rows.Count > 0 || !CompareDecimal.IsZero(balanceForward)))
