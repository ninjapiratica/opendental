...
namespace OpenDentBusiness
{
    public class ApptSearch
    {
            ...
            , List<long> listClinicNums, TimeSpan beforeTime, TimeSpan afterTime, List<long> listBlockoutTypes = null, bool hasProvAndBlockout = false, int resultCount = 10, bool isForMakeRecall = false)
        {
            ...
            {//No call to db but designed to save middle tier trips.
        ...
        public static List<ApiAppointmentSlot> GetApiSlots(long provNum, DateTime dateStart, DateTime dateEnd, long lengthMinutes, long operatoryNum)
        {
            ...
            {//Loop through each day.
                ...
                {
                    if (bool5[j] && blockStart == -1)
                    {//Beginning of true blocks.
                        blockStart = j;
                    }
                    if (bool5[j])
                    {//Running endpoint of true blocks.
                        blockEnd = j;
                ...
                if (listApiAppointmentSlots.Exists(x => x.DateTimeStart.Date == date))
                {//Slots were added today.
                 //User provided an operatoryNum, use that.
                    ...
                    else
                    {
                        ...
                        else if (operatory is null)
                        { //If that fails, then look through list of scheduleops to see if one of the schedules for today is linked to an op.
