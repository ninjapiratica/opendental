    ...
    public static class PushNotificationUtils
    {
            ...
            List<long> listPrimaryKeys = null, List<string> listTags = null, string alertTitle = null, string alertMessage = null, bool runAsync = true)
        {
            void execute()
            {
                ...
                if (mobileAppDeviceNum > 0)
                { //Send to one exact device.
                ...
                {
                    if (clinicNum > -1 || pushType != PushType.ODM_LogoutODUser)
                    { //Logout is allowed to be sent to non-specific clinicNum. All others are not.
            ...
            th.Start();
            if (MockExceptionHandler != null || !runAsync)
            { //Join back to main thread to cause this to be a blocking call. Unit tests will always block.
