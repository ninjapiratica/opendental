...
namespace OpenDentBusiness.Bridges
{
    public class EHG_statements
    {
        ...
        public static void GeneratePracticeInfo(XmlWriter writer, long clinicNum)
        {
            ...
            if (eBillClinic != null && eBillClinic.ClientAcctNumber != "")
            {//clinic eBill entry exists, check the fields for overrides
        ...
        public static void GenerateOneStatement(XmlWriter writer, Statement stmt, Patient pat, Family fam, DataSet dataSet)
        {
            ...
            foreach (DataRow rowCur in tableAccount.Rows)
            {
                ...
                {
                    lineCur = lines[li];
                    if (lineCur.Length < 1 || lineCur.All(x => char.IsWhiteSpace(x)))
                    {//nothing to write
                        continue;
                    }
                    ...
                    if (lineCur.Length > lineMaxLen)
                    {
                        ...
                        for (int c = lineMaxLen - 2; c > -1; c--)
                        {//-2, 1 for length to index and 1 so we can safely check index and index+1
        ...
        public static void Send(string data, long clinicNum, out string alertMessage)
        {
            ...
            if (PrefC.HasClinicsEnabled && clinicNum != 0)
            {
                ...
                if (eBill != null)
                {//eBill entry exists, check the fields for overrides.
