    ...
    public class X837_5010 : X12object
    {
            ...
            Clearinghouse clearinghouseClin, EnumClaimMedType medType)
        {
            ...
            for (int i = 0; i < queueItems.Count; i++)
            {
                ...
                { //Use the practice PayToAddress if clinic is null or the clinic's PayToAddress is blank.
                ...
                {//Validation guarantees there is at least one (principal) diagnosis code as required
                    sw.Write("HI" + s
                        + ((listDiagnosesVersions[0] == 9) ? "BK" : "ABK") + isa16//HI01-1 1/3 Code List Qualifier Code: BK=ICD-9,ABK=ICD-10 Principal Diagnosis
                        + Sout(listDiagnoses[0].Replace(".", ""), 30));//HI01-2 1/30 Industry Code: Diagnosis code. No periods.
                    for (int j = 1; j < listDiagnoses.Count; j++)
                ...
                {//Claim procs for this claim
                    ...
                    if (medType == EnumClaimMedType.Medical)
                    {
                        ...
                        {//There is at least one diagnostic code on the proc, and also at least one proc on the claim (at least the principal diagnosis).
                            ...
                            for (int d = 0; d < listDiagnoses.Count; d++)
        ...
        {//,out string warning) {
            ...
            if (clinic == null || (clinic != null && clinic.PayToAddress == ""))
            {//No clinic Pay To address.
