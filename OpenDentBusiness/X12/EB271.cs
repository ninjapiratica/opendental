...
namespace OpenDentBusiness
{
    ...
    public class EB271
    {
        ...
        public EB271(X12Segment segment, bool isInNetwork, bool isCoinsuranceInverted, X12Segment segHsd = null)
        {
            ...
            if (segment == null)
            {//Should never happen except in rare scenarios where we want to spoof an EB271 object.
                return;
            }
            Segment = segment;
            ...
            {//if percentage
             //must have either a category or a proc code
                ...
                {//if no proc code is specified
                    if (eb03val == null || eb03val.ServiceType == EbenefitCategory.None || eb03val.ServiceType == EbenefitCategory.General)
            ...
            if (Segment.Get(8) != "")
            {
                ...
                {//Some carriers incorrectly send insurance percentage.
        ...
        public string ToString(bool hasFreeFormText = true)
        {
            ...
            if (!hasFreeFormText)
            {//Surgically clear the free form text element and reconstruct the segment string.
                string[] arraySegmentElements = retVal.Split('*');
                if (arraySegmentElements.Length >= 6)
                {//Only clear it out if the element exists.
        ...
        {
            if (listEb271s == null || listEb271s.Count == 0)
            {//Avoid queries.
                return 0;
            }
            ...
            foreach (EB271 eb in listEb271s)
            {
                ...
                if (insHistDate == DateTime.MinValue)
                {//When insHistDate is not MinValue, prefName is guarenteed to be valid.
