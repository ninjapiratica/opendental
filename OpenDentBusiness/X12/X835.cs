...
namespace OpenDentBusiness
{
    ...
    public class X835 : X12object
    {
        ...
        public DateTime DateReceived
        {
            get
            {
                if (_paymentMethodCode == "NON")
                {//No payment
                    return DateTime.Today;
                }
                if (_paymentMethodCode == "ACH" && DateEffective.Year > 1980)
                {//Electronic check
                    return DateEffective;
                }
                if (_paymentMethodCode == "BOP" && DateEffective.Year > 1980)
                ...
                if (_paymentMethodCode == "CHK")
                {//Physical check
                    return DateTime.Today;
                }
                if (_paymentMethodCode == "FWT" && DateEffective.Year > 1980)
                {//Wire transfer
                    return DateEffective;
                }
                return DateTime.Today;
        ...
        {
            if (xobj.FunctGroups == null || xobj.FunctGroups.Count != 1)
            {//Exactly 1 GS segment in each 835.
                return false;
            }
            //Check the Application Sender's Code.  Code identifying party sending transmission; codes agreed to by trading partners.
            if (xobj.FunctGroups[0].Header.Get(2).Trim().ToUpper() == "DENTICAL")
            {//GS02 (standard guide pg. 279)
                return true;
            }
            return false;
        }
        ...
        public X835(Etrans etransSource, string messageText, string tranSetId, List<Etrans835Attach> listAttached = null) : base(messageText)
        {//Parsing happens in the base class.
        ...
        public X835Status GetStatus(List<X835ClaimData> listClaimDatas, List<Hx835_ShortClaimProc> listAllClaimProcs, List<Etrans835Attach> listAllAttaches)
        {
            ...
            {
                if (claimData.IsManuallyDetached)
                {//Manually detached claim created for claim that was unattached by user.
                    countDetached++;
                    continue;
                }
            ...
            if (countProcessed == this.ListClaimsPaid.Count - countDetached)
            {//All claims found and marked recieved.
                if (countRevievedWithPay == this.ListClaimsPaid.Count - countDetached)
                {
                    if (countDetached > 0)
                    {
                        if (countDetached == this.ListClaimsPaid.Count)
                        {//All ERA claims have been manually detached.
            ...
            else if (countOther == this.ListClaimsPaid.Count - countDetached)
            {//Not recieved.
                return X835Status.Unprocessed;
            }
            else if (countProcessed > 0)
            {//Some recieved but not all.
        ...
        private List<Hx835_Adj> ProcessCAS(int segNum)
        {
            ...
            for (int k = 2; k <= 17; k += 3)
            {
                ...
                else if (adjCode != "PR" && strAdjReasonCode == "45")
        ...
        private Hx835_Claim ProcessCLP(int segNum, string npi)
        {
            ...
            while (segNum < _listSegments.Count && _listSegments[segNum].SegmentID == "REF")
            {//We clump 2 segments into a single loop, because neither segment is used, and there are multiple REF01 choices for each.
                segNum++;
            }
            ...
            if (retVal.DateServiceStart != DateTime.MinValue && retVal.DateServiceEnd == DateTime.MinValue)
            {//Start date was provided, but end date was not.
                retVal.DateServiceEnd = retVal.DateServiceStart;//The end date is the same as the start date.
            }
            else if (retVal.DateServiceStart == DateTime.MinValue && retVal.DateServiceEnd != DateTime.MinValue)
            {//Start date not provided, but end date was.  Should not happen.  Just in case.
            ...
            foreach (Hx835_Adj adj in retVal.ListClaimAdjustments)
            {//Sum claim level adjustments.
                if (adj.AdjCode == "CO")
                {//Contractual Obligations (writeoffs).  Guide page 198.
                ...
                else if (adj.AdjCode == "OA")
                {//Other Adjustments.  Guide page 198.
                ...
                {//Payor Initiated Reductions  Guide page 198.
                 //Going to display amount to the user, they must decide if they want to use it.
                ...
                else if (adj.AdjCode == "PR")
                {//Patient Responsibility.  Guide page 198.
                    if (adj.IsDeductible)
                    {
            ...
            {//Add sum of procedure level adjustments to claim level adjustments.
        ...
        private string ProcessLQ(int segNum)
        {
            ...
            {//Should not be possible, but here for future versions of X12 just in case.
        ...
        private Hx835_Proc ProcessSVC(int segNum, DateTime dateClaimServiceStart, DateTime dateClaimServiceEnd)
        {
            ...
            while (segNum < _listSegments.Count && _listSegments[segNum].SegmentID == "DTM")
            {
                ...
                if (_listSegments[segNum].Get(1) == "151")
                {//Service period end.
                    proc.DateServiceEnd = X12Parse.ToDate(dateStr);
                }
                else
                {//_listSegments[segNum].Get(1)=="150" || _listSegments[segNum].Get(1)=="472"//Service period start
                    proc.DateServiceStart = X12Parse.ToDate(dateStr);
                }
                segNum++;
            }
            if (proc.DateServiceStart != DateTime.MinValue && proc.DateServiceEnd == DateTime.MinValue)
            {//Start date provided, but end date not provided.
                proc.DateServiceEnd = proc.DateServiceStart;
            }
            else if (proc.DateServiceStart == DateTime.MinValue && proc.DateServiceEnd != DateTime.MinValue)
            {//Start date not provided, but end date provided.  Should not happen.  Just in case.
            ...
            while (segNum < _listSegments.Count && _listSegments[segNum].SegmentID == "REF")
            {//4 segment types clumped together, but we only care about REF*6R.
    ...
    public class Hx835_Claim
    {
        ...
        public static List<List<Hx835_Proc>> GetPaymentsForClaimProcs(List<ClaimProc> listClaimProcs, List<Hx835_Proc> listProcsUnassigned)
        {
            ...
            for (int i = 0; i < listClaimProcs.Count; i++)
            {
                ...
                for (int j = listProcsUnassigned.Count - 1; j >= 0; j--)
                {//We go backward, so we can remove the current item without modifying j.
    ...
    public class Hx835_Proc
    {
        ...
        public EraProcMatchingFormat MatchingVersion
        {
            get
            {
                if (ProcNum != 0 && PlanOrdinal != 0 && PlanNum != 0)
                {//Like 'xProc.ProcNum/Ordinal/InsPlan.PlanNum'.  Version 3.
                    return EraProcMatchingFormat.X;
                }
                else if (ProcNum != 0 && PlanOrdinal != 0 && PartialPlanNum != 0)
                {//Like 'yProc.ProcNum/Ordinal/partial InsPlan.PlanNum'.  Version 4.
                    return EraProcMatchingFormat.Y;
                }
                else if (ProcNum != 0)
                {//Like 'pProc.ProcNum'.  Version 2.
        ...
        public bool TryGetMatchedClaimProc(out Hx835_ShortClaimProc matchedClaimProc, List<Hx835_ShortClaimProc> listClaimProcs, bool isSupplemental)
        {
            ...
            foreach (Hx835_ShortClaimProc claimProc in listClaimProcs)
            {
                ...
                if (claimProc.ClaimNum == ClaimPaid.ClaimNum && !HasPaymentBeenEntered(claimProc))
                {//Proc was not split from original ERA and payment not entered.
                    continue;
                }
        ...
        private bool IsBasicMatch(Hx835_ShortClaimProc claimProc, bool isSupplemental)
        {
            ...
            {//Procedures only, no by total claimProcs
                return false;
            }
            if (this.ProcNum == claimProc.ProcNum)
            {//Direct match.  When PlanNum and Ordinal are set, then they have already been verified earlier.
                return true;
            }
            if (ProcCodeBilled != claimProc.CodeSent)
            {//Failed direct match and code is wrong.
                return false;
            }
            //Code matches from here on.
            if (isSupplemental)
            {//FeeBilled is not set for supplemental claimProcs.  So only care about status matching.
    ...
    public class EraAutomationResult
    {
        ...
        private string GetMessageForClaimErrors(bool isForSingleEra)
        {
            ...
            if (isForSingleEra)
            {//If any claims are unprocessed and we are dealing with a single ERA, show claim error details.
            ...
            {//If we are processing a message for multiple ERAs, show claim error counts.
                if (ListClaimErrors.Count > 0)
                {
        ...
        public static string CreateMessage(List<EraAutomationResult> listAutomationResults, bool isForSingleEra)
        {
            ...
            if (isForSingleEra && countPaymentsFinalized == 1)
            {//Don't show count if we are only dealing with one ERA and payment was finalized.
            ...
            else if (!isForSingleEra)
            {//For multiple ERAs, show the count finalized.
            ...
            for (int i = 0; i < listAutomationResults.Count; i++)
            {
                ...
                if (!isForSingleEra)
                {//Add info for each ERA if dealing with multiple ERAs.
                    ...
                    if (listAutomationResults[i].TransactionCount > 1)
                    {//Show the transaction number if Etrans has multiple transactions
                ...
                {//if we get here, we must have at least one unmatched claim or claim error, otherwise we would have hit continue above.
