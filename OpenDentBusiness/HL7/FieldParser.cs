...
namespace OpenDentBusiness.HL7
{
    ...
    public class FieldParser
    {
        ...
        public static DateTime DateTimeParse(string str)
        {
            ...
            {//PIn.Int could fail if not able to parse into an Int32
                ex.DoNothing();
                return DateTime.MinValue;
            }
            if (str.Length >= 10)
        ...
        public static long ProvParse(FieldHL7 field, SegmentNameHL7 segName, bool isVerbose, bool isV2_3 = false)
        {
            ...
            if (provIdHierarch.Length > 1)
            {//must have a root and an ID
            ...
            {
                if (strProvIdRoot == OIDInternals.GetForType(IdentifierType.Provider).IDRoot)
                {//The office's root OID for a provider object, ProvId should be the OD ProvNum
                    try
                    {
                ...
                {//there was a ProvID and a ProvID root, but the root is not the office's root OID for a provider object, check the oidexternals table
                    OIDExternal oidExtProv = OIDExternals.GetByRootAndExtension(strProvIdRoot, strProvId);
                    if (oidExtProv == null)
                    {//add to the list of oid's to add to the oidexternal table if we find a provider
            ...
            for (int i = 0; i < field.ListRepeatFields.Count; i++)
            {//could be repetitions of this field with other IDs
                strProvId = "";
                strProvIdRoot = "";
                provIdHierarch = field.ListRepeatFields[i].GetComponentVal(0).Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries);
                if (provIdHierarch.Length < 2)
                {//must be a root and an ID
                    continue;
                }
                strProvId = provIdHierarch[provIdHierarch.Length - 1];
                ...
                if (provNum == 0 && strProvIdRoot == OIDInternals.GetForType(IdentifierType.Provider).IDRoot)
                {//The office's root OID for a provider object, ProvId should be the OD ProvNum
                    try
                    {
                        if (Providers.GetProv(PIn.Long(strProvId)) != null)
                ...
                else if (strProvIdRoot != OIDInternals.GetForType(IdentifierType.Provider).IDRoot)
                {//there was a ProvID and a ProvID root, but the root is not the office's root OID for a provider object, check the oidexternals table
                    OIDExternal oidExtProv = OIDExternals.GetByRootAndExtension(strProvIdRoot, strProvId);
                    if (oidExtProv == null)
                    {//add to the list of oid's to add to the oidexternal table if we find a provider
            ...
            {//AIG is the data type CWE with format ProvNum^LName, FName^^Abbr (for isV2_3 there will be no Abbr)
            ...
            else if (segName == SegmentNameHL7.AIP || segName == SegmentNameHL7.PV1)
            {//AIP and PV1 are the data type XCN with the format ProvNum^LName^FName^^^Abbr
            ...
            {//could be repetitions of this field with other IDs
                ...
                if (!isV2_3 && string.IsNullOrEmpty(provAbbr))
                {//Abbr not required for v2_3
