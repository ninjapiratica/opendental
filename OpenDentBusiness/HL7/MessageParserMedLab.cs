...
namespace OpenDentBusiness.HL7
{
    ...
    public class MessageParserMedLab
    {
        ...
        public static List<long> Process(MessageHL7 msg, string msgArchiveFilePath, bool isVerboseLogging, Patient selectedPat = null)
        {
            ...
            if (_patCur == null)
            {//if no patient is located using PID segment or if selectedPat is not null, use selectedPat
            ...
            for (int i = 2; i < msg.Segments.Count; i++)
            {
                ...
                if (indexFirstOrc == 0 && msg.Segments[i].Name == SegmentNameHL7.NTE)
                {//if we find an ORC segment, the NTEs that follow won't be at the PID level
                ...
                {
                    if (indexFirstZps == 0)
                    {//this is the first ZPS segment we've encountered, set the index
                        indexFirstZps = i;
                    }
                    listZpsSegs.Add(msg.Segments[i]);
                    continue;
                }
                if (indexFirstOrc == 0 && msg.Segments[i].Name == SegmentNameHL7.ORC)
                {//this is the first ORC segment we've encountered, set the index
                    indexFirstOrc = i;
                }
            }
        ...
        public static Patient GetPatFromPID(HL7DefSegment pidSegDef, SegmentHL7 pidSeg)
        {
            ...
            if (patCur != null && IsMatchNameBirthdate(patCur, patLName, patFName, birthdate))
            {
                ...
                {
                    if (patNumFromAlt == 0)
                    { //if the altPatID isn't stored in the oidexternals table as IDExternal, insert
                    ...
                    else if (patCur.PatNum != patNumFromAlt)
                    { //else if patCur.PatNum is different than the IDInternal stored in the oidexternals table, update
        ...
        public static HL7DefSegment GetSegDefByName(HL7DefMessage msgDef, SegmentNameHL7 segName, SegmentNameHL7 prevSegName)
        {
            ...
            for (int i = 0; i < msgDef.ListHL7DefSegments.Count; i++)
            {//start with the second segment in the def since the first segment won't have a predecessor
        ...
        public static void ProcessSeg(HL7DefMessage msgDef, List<SegmentHL7> listSegs, MessageHL7 msg, SegmentNameHL7 prevSegName = SegmentNameHL7.Unknown)
        {
            ...
            switch (listSegs[0].Name)
            {
                ...
                case SegmentNameHL7.NK1://optional segment
                    ...
                    if (segDefCur == null)
                    {//do not process the NK1 segment if it's not defined
                        return;
                    }
                ...
                case SegmentNameHL7.NTE://optional segment
                    if (segDefCur == null)
                    {//do not process the NTE segment if it's not defined
                        return;
                    }
                ...
                case SegmentNameHL7.SPM://optional segment
                    if (segDefCur == null)
                    {//do not process the SPM segment if it's not defined
                        return;
                    }
                ...
                case SegmentNameHL7.ZEF://optional segment, LabCorp proprietary segment, not offical HL7 spec segment
                    if (segDefCur == null)
                    {//do not process the ZEF segment if it's not defined
                        return;
                    }
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (fieldDefCur.FieldName)
                {
                    ...
                    case "facilityID":
                        ...
                        if (!_dictFacilityCodeNum.ContainsKey(facIdCur))
                        {//no ZPS segments with this footnote ID
                            continue;
                        }
                    ...
                    case "obsTestID":
                        ...
                        continue;
                    case "orderingProv":
                        if (_medLabCur.OrderingProvNPI == null || _medLabCur.OrderingProvNPI == "")
                        {//may be filled from the ORC segment
                            _medLabCur.OrderingProvNPI = FieldParserMedLab.OrderingProvIDParse(fieldCur, "N");
                        }
                        if (_medLabCur.OrderingProvLocalID == null || _medLabCur.OrderingProvLocalID == "")
                        {//may be filled from the ORC segment
                            _medLabCur.OrderingProvLocalID = FieldParserMedLab.OrderingProvIDParse(fieldCur, "L");
                        }
                        int k = 0;
                        if (_medLabCur.OrderingProvLName == null || _medLabCur.OrderingProvLName == "")
                        {//may be filled from the ORC segment
                            _medLabCur.OrderingProvLName = fieldCur.GetComponentVal(1).Trim();
                            while (_medLabCur.OrderingProvLName == "" && k < fieldCur.ListRepeatFields.Count)
                            {//if LName is not present in first repetition check others
                                _medLabCur.OrderingProvLName = fieldCur.ListRepeatFields[k].GetComponentVal(1).Trim();
                                k++;
                            }
                        }
                        k = 0;
                        if (_medLabCur.OrderingProvFName == null || _medLabCur.OrderingProvFName == "")
                        {//may be filled from the ORC segment
                            _medLabCur.OrderingProvFName = fieldCur.GetComponentVal(2).Trim();
                            while (_medLabCur.OrderingProvFName == "" && k < fieldCur.ListRepeatFields.Count)
                            {//if FName is not present in first repetition check others
                                _medLabCur.OrderingProvFName = fieldCur.ListRepeatFields[k].GetComponentVal(2).Trim();
                                k++;
                            }
                        }
                        #region Locate Provider
                        if (_medLabCur.ProvNum != 0)
                        {//may have located the provider from the ORC segment, nothing left to do
                            continue;
                        }
                        ...
                        if (listProvs.Count > 0)
                        {//if a provider with either a matching NPI or Medicaid ID is found, use the first matching prov
                        ...
                        {//no provider match based on NPI or MedicaidID
                            ...
                            if (listProvs.Count > 0)
                            {//use the first provider found with matching LName and FName
                                _medLabCur.ProvNum = listProvs[0].ProvNum;
                            }
                        }
                    ...
                    case "specimenAction":
                        ...
                        continue;
                    case "specimenID":
                        if (_medLabCur.SpecimenID != null && _medLabCur.SpecimenID != "")
                        {//could be filled by value in ORC
                            continue;
                        }
                    ...
                    case "specimenIDFiller":
                        if (_medLabCur.SpecimenIDFiller != null && _medLabCur.SpecimenIDFiller != "")
                        {//could be filled by value in ORC
                            continue;
                        }
        ...
        public static void ProcessOBX(HL7DefSegment segDef, SegmentHL7 obxSeg)
        {
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (fieldDefCur.FieldName)
                {
                    ...
                    case "obsValue":
                        ...
                        continue;
                    case "obsValueType":
                        if (obxSeg.GetFieldComponent(fieldDefCur.OrdinalPos).ToLower() == "tx")
                        {//if value type is text (TX) the value will be in attached NTEs
                            _isObsValueNte = true;
                        }
                        continue;
                    case "obsUnits":
                        if (_medLabResultCur.ObsValue.ToLower() == "test not performed")
                        {//if TNP then we won't bother saving the units
                            continue;
                        }
            ...
            if (_medLabResultCur.FacilityID == "")
            {//no facility ID in the OBX segment, can't link to facilities in the ZPS segments
                return;
            }
            if (!_dictFacilityCodeNum.ContainsKey(_medLabResultCur.FacilityID))
            {//no facility in the ZPS segments with this facility ID
                return;
            }
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (fieldDefCur.FieldName)
                {
                    case "orderingProv":
                        ...
                        _medLabCur.OrderingProvFName = fieldCur.GetComponentVal(2).Trim();
                        int j = 0;
                        while (_medLabCur.OrderingProvLName == "" && j < fieldCur.ListRepeatFields.Count)
                        {//if LName is not present in first repetition, check others
                            _medLabCur.OrderingProvLName = fieldCur.ListRepeatFields[j].GetComponentVal(1).Trim();
                            j++;
                        }
                        j = 0;
                        while (_medLabCur.OrderingProvFName == "" && j < fieldCur.ListRepeatFields.Count)
                        {//if FName is not present in first repetition, check others
                    ...
                    case "specimenID":
                        if (_medLabCur.SpecimenID != null && _medLabCur.SpecimenID != "")
                        {//could be filled in by value in OBR
                            continue;
                        }
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (fieldDefCur.FieldName)
                {
                    ...
                    case "patBirthdateAge":
                        ...
                        if (fieldCur == null || fieldCur.Components.Count < 4)
                        {//if there aren't even 4 components (3 ^'s) in the field, don't set the age
                            continue;
                        }
        ...
        public static void ProcessSPM(HL7DefSegment segDef, List<SegmentHL7> listSegs)
        {
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {//get indexes for fields from the segment def
                HL7DefField fieldDefCur = segDef.hl7DefFields[i];
                switch (fieldDefCur.FieldName)
                {
                    case "specimenID":
        ...
        public static void ProcessZPS(HL7DefSegment segDef, List<SegmentHL7> listSegs, MessageHL7 msg)
        {
            ...
            for (int i = 0; i < listSegs.Count; i++)
            {
                ...
                if (!_dictFacilityCodeNum.ContainsKey(segFacId))
                {//if the footnote id doesn't exist in the dictionary, add it and the MedLabFacilityNum value
                    _dictFacilityCodeNum.Add(segFacId, new List<long>() { facilityCur.MedLabFacilityNum });
                    continue;
                }
                if (!_dictFacilityCodeNum[segFacId].Contains(facilityCur.MedLabFacilityNum))
                {//if the id is not linked to this facilitynum, add it to the list
