...
namespace OpenDentBusiness
{
    ...
    public class WebServiceTests
    {
        ...
        public static TimeSpan SendTimeSpanParam(TimeSpan timeSpan)
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {
                return Meth.GetObject<TimeSpan>(MethodBase.GetCurrentMethod(), timeSpan);
            }
            return timeSpan;
        }
        ...
        public static PatientData GetPatientData(DataTable tableProgNotes)
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {
                return Meth.GetObject<PatientData>(MethodBase.GetCurrentMethod(), tableProgNotes);
            }
            PatientData patientData = new PatientData();
            patientData.TableProgNotes = tableProgNotes;
            return patientData;
        }
        ...
        ///<summary>Sends an enum across middle tier and returns it to the client.</summary>
        public static HostedEmailStatus GetAndSendEnum(HostedEmailStatus hostedEmailStatus)
        {
            if (RemotingClient.MiddleTierRole == MiddleTierRole.ClientMT)
            {
                return Meth.GetObject<HostedEmailStatus>(MethodBase.GetCurrentMethod(), hostedEmailStatus);
            }
