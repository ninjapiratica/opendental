    ...
    public class SheetFiller
    {
            ...
            , StaticTextData staticTextData = null, long refAttachProvNum = 0)
        {
            ...
            switch (sheet.SheetType)
            {
                ...
                case SheetTypeEnum.ReferralSlip:
                    ...
                    pat = (pat == null || pat.PatNum != patNum ? Patients.GetPat(patNum) : pat);
                    Referrals.TryGetReferral((long)GetParamByName(sheet, "ReferralNum").ParamValue, out refer);
                    FillFieldsForReferralSlip(sheet, pat, refer, refAttachProvNum);
                    break;
        ...
        private static void FillFieldsInStaticText(Sheet sheet, Patient pat, Family fam = null, StaticTextData data = null)
        {
            ...
            if (pat != null)
            {
                ...
                #endregion
                PronounPreferred pronounPreferred = Patients.GetPronoun(pat.Gender, patNote.Pronoun);
                ...
                for (int i = 0; i < proceduresList.Count; i++)
                {
                    ...
                    if (listInsBenBWCodes.Contains(proc.CodeNum) && proc.ProcDate > dBW)
                    { //newest
                        dBW = proc.ProcDate;
                ...
                {
                    PatientPortalCredential patientPortalCredential = UserWebs.GetNewPatientPortalCredentials(pat);
            ...
            foreach (SheetField field in sheet.SheetFields)
            {
                ...
                if (pat != null)
                {
                    ...
                    fldval = fldval.Replace(StaticTextField.genderHeShe.ToReplacementString(), pronounHeSheThey);
                    fldval = fldval.Replace(StaticTextField.genderheshe.ToReplacementString(), pronounheshethey);
                    fldval = fldval.Replace(StaticTextField.genderHimHer.ToReplacementString(), pronounHimHerThem);
                    fldval = fldval.Replace(StaticTextField.genderhimher.ToReplacementString(), pronounhimherthem);
                    fldval = fldval.Replace(StaticTextField.genderHimselfHerself.ToReplacementString(), pronounHimselfHerselfThemself);
                    fldval = fldval.Replace(StaticTextField.genderhimselfherself.ToReplacementString(), pronounhimselfherselfthemself);
                    fldval = fldval.Replace(StaticTextField.genderHisHer.ToReplacementString(), pronounHisHerTheir);
                    fldval = fldval.Replace(StaticTextField.genderhisher.ToReplacementString(), pronounhishertheir);
                    fldval = fldval.Replace(StaticTextField.genderHisHers.ToReplacementString(), pronounHisHersTheirs);
                    fldval = fldval.Replace(StaticTextField.genderhishers.ToReplacementString(), pronounhisherstheirs);
        ...
        private static void FillFieldsForLabCase(Sheet sheet, Patient pat, LabCase labcase)
        {
            ...
            {//Only get clinic specific information if clinics are turned on.
