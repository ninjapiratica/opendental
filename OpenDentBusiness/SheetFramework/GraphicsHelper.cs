...
namespace OpenDentBusiness
{
    public class GraphicsHelper
    {
        ...
        public static RichTextBox CreateTextBoxForSheetDisplay(SheetField sheetField, bool isClipText = true, RichTextBox textbox = null)
        {
            ...
            if (isClipText && textbox.Text.Length > 0 && textbox.Multiline)
            {
                ...
                if (listLines.Count > 1)
                {
                    ...
                    while (fitLineIndex >= 0 && listLines[fitLineIndex].Top + textbox.Font.Height - 1 > textbox.Height)
                    {//if bottom of line is past bottom of textbox
                        fitLineIndex--;
                    }
                    if (fitLineIndex < 0)
                    {//Every single line of text falls below the bottom of the field (the sheet field is too small for the font).
                    ...
                    else if (fitLineIndex < listLines.Count - 1)
                    {//If at least one line was truncated from the bottom.
            ...
            if (isClipText && textbox.Text.Length > 0 && !textbox.Multiline)
            {
                ...
                if (indexRight < textbox.Text.Length - 1)
                {//If the string is too wide for the textbox.
        ...
        public static RichTextLineInfo GetOneTextLine(RichTextBox richTextbox, int lineIndex)
        {
            ...
            if (lineInfo.FirstCharIndex == -1)
            {//Return the last line's information.
            ...
            Point posNextLine;
            if (lineIndex != GetTextLineCount(richTextbox) - 1)
            ...
            { //Only do this "phony last line" logic when we are looking at the actual last line.  Trying to copy all the content/formatting of an 
        ...
        public static bool IsTextBoxMultiline(RichTextBox richTextBox, SheetField sheetField = null, SheetFieldDef sheetFieldDef = null)
        {
            ...
            if (sheetField == null)
            {
                ...
                {//InputField that is too tall to be considered a "single line" text box or not an InputField or has some GrowthBehavior set.
    ...
    public class RichTextLineInfo
    {
        ...
        {
            if (lineIndex == listLines.Count - 1)
            {//Last line
