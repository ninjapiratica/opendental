...
namespace OpenDentBusiness
{
    ...
    public class ImageStore
    {
        ...
        ///<summary>Only makes a call to the database on startup.  After that, just uses cached data.  Does not validate that the path exists except if the main one is used.  ONLY used from Client layer or S class methods that have "No need to check MiddleTierRole; no call to db" and which also make sure PrefC.AtoZfolderUsed.  Returns Cloud AtoZ path if CloudStorage.IsCloudStorage</summary>
        ...
        public static void SaveDocument(Document doc, byte[] arrayBytes, string patFolder)
        {
            ...
            {//Assume DataStorageType.InDatabase
