    ...
    public partial class ConvertDatabases
    {
        ...
        private static void To16_2_1()
        {
            if (FromVersion < new Version("16.2.1.0"))
            {
                ...
                if (DataConnection.DBtype == DatabaseType.MySql)
                {//Insert PaySplitUnearnedType definition if it doesn't already exist.
                    command = "SELECT COUNT(*) FROM definition WHERE Category=29 AND ItemName='Prepayment'";
                    if (PIn.Int(Db.GetCount(command)) == 0)
                    {//PaySplitUnearnedType definition doesn't already exist
                ...
                {//oracle (Note for reviewer: I'm not at all sure this is oracle compatible)
                    command = "SELECT COUNT(*) FROM definition WHERE Category=29 AND ItemName='Prepayment'";
                    if (PIn.Int(Db.GetCount(command)) == 0)
                    {//PaySplitUnearnedType definition doesn't already exist
                        command = "SELECT MAX(ItemOrder)+1 FROM definition WHERE Category=29";
                ...
                if (DataConnection.DBtype == DatabaseType.MySql)
                {//Insert Canceled Appointment Procedure account color
                ...
                if (DataConnection.DBtype == DatabaseType.MySql)
                { //Oracle users will have to manually close payment plans.
        ...
        private static void To16_2_10()
        {
            if (FromVersion < new Version("16.2.10.0"))
            {
                ...
                { //only run the convert script if MapAreaContainerNum does not exist.
                    if (DataConnection.DBtype == DatabaseType.MySql)
                    {
        ...
        private static void To16_3_1()
        {
            if (FromVersion < new Version("16.3.1.0"))
            {
                ...
                if (Db.GetCount(command) == "0")
                {//Definition for reminder priority does not exist yet.
        ...
        private static void To16_3_29()
        {
            if (FromVersion < new Version("16.3.29.0"))
            {
                ...
                {//The eRx bridge is currently disabled.
                    isErxEnabled = false;
                }
                else
                {//The eRx bridge is currently enabled.
                    command = "SELECT ValueString FROM preference WHERE PrefName='NewCropAccountId'";
                    if (Db.GetScalar(command) == "")
                    {//Nobody has clicked through to eRx.
                        isErxEnabled = false;
                    }
                }
                if (isErxEnabled)
        ...
        private static void To16_4_1()
        {
            if (FromVersion < new Version("16.4.1.0"))
            {
                ...
                if (defNum == 0)
                {//Sales Tax adjustment definition doesn't exist.  Create one for them.
                    if (DataConnection.DBtype == DatabaseType.MySql)
                    {
        ...
        private static void To16_4_19()
        {
            if (FromVersion < new Version("16.4.19.0"))
            {
                ...
                if (Db.GetCount(command) != "0")
                {//Turn repeating tasks back on because it may have been turned off while still in use.
