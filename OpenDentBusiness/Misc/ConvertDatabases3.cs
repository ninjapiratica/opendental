    ...
    public partial class ConvertDatabases
    {
        ...
        private static void To13_2_1()
        {
            if (FromVersion < new Version("13.2.1.0"))
            {
                ...
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    ...
                    switch (PIn.Int(table.Rows[i]["Race"].ToString()))
                    {//PatientRaceOld
        ...
        private static void To15_1_1()
        {
            if (FromVersion < new Version("15.1.1.0"))
            {
                ...
                try
                {
                    if (DataConnection.DBtype == DatabaseType.MySql)
                    {
                        if (!LargeTableHelper.IndexExists("hl7msg", "MsgText"))
                        {//only add this index if it has not already been added
        ...
        private static void To15_1_16()
        {
            if (FromVersion < new Version("15.1.16.0"))
            {
                ...
                {//The InsPPOsecWriteoffs pref does not already exist
                    if (DataConnection.DBtype == DatabaseType.MySql)
                    {
                        command = "INSERT INTO preference(PrefName,ValueString) VALUES('InsPPOsecWriteoffs','0')";
                    }
        ...
        private static void To15_1_20()
        {
            if (FromVersion < new Version("15.1.20.0"))
            {
                ...
                for (int i = 0; i < tableCommlogs.Rows.Count; i++)
                {//Make ehrmeasureevent for users who have been sending reminders from FormRecallList
        ...
        private static void To15_2_1()
        {
            if (FromVersion < new Version("15.2.1.0"))
            {
                ...
                if (codeNum != 0)
                {//If foreign, the customer will probably not have D9986.  This is for USA only.
                    if (DataConnection.DBtype == DatabaseType.MySql)
                    {
                ...
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    ...
                    if (countPatPortals > 5)
                    {//Check for more than 5 patient portal patients to avoid false positives.
        ...
        private static void To15_4_1()
        {
            if (FromVersion < new Version("15.4.1.0"))
            {
                ...
                if (tableAccountId.Rows[0]["ValueString"].ToString() != "")
                {//NewCropAccountId is not blank.
                    isLegacyErx = true;
                }
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                ...
                if (isLegacyErx)
                {
                    ...
                    for (int i = 0; i < tableProviders.Rows.Count; i++)
                    {
                        ...
                        if (!listNpis.Contains(npi))
                        {//Do not duplicate NPI in providererx table.
                            listNpis.Add(npi);
                        }
                    }
        ...
        private static void To15_4_7()
        {
            if (FromVersion < new Version("15.4.7.0"))
            {
                ...
                if (result == "0" || result == "")
                {//Set default text value for ApptReminderHourMessage
                ...
                if (result == "0" || result == "")
                {//Set default text value for ApptReminderDayMessage
                    string recallText = "[nameFL],\r\nYou have an upcoming appointment on [apptDate] at [apptTime] with [practiceName] with [provName].\r\nThanks!";
        ...
        private static void To16_1_1()
        {
            if (FromVersion < new Version("16.1.1.0"))
            {
                ...
                if (DataConnection.DBtype == DatabaseType.MySql)
                { //38 is DefCat.InsuranceVerificationStatus
                ...
                if (hasBrokenApptProcCode)
                {
                    ...
                    if (!descript.ToLower().Contains("missed appointment"))
                    {//They have D9986 that isn't "missed appointment".  Move the code to a new entry.
                ...
                table = Db.GetTable(command);
                for (int i = 0; i < table.Rows.Count; i++)
                {//Insert a UserClinic entry for each user's default clinic.
