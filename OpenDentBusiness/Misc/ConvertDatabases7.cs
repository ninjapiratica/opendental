...
namespace OpenDentBusiness
{
    public partial class ConvertDatabases
    {
        ...
        private static void RemoveDuplicateProcCodesFromFrequencyLimitationPreferences()
        {
            ...
            if (listProcCodesDuplicates.IsNullOrEmpty())
            {//No duplicates found, kick out
                return;
            }
        ...
        private static void To20_5_41()
        {
            ...
            command = "SELECT program.Path FROM program WHERE ProgName='DentalEye'";
            string programPath = Db.GetScalar(command);//only one path
            if (programPath.Contains("DentalEye.exe\\CmdLink.exe"))
            {//this isn't a valid path, so fix it
        ...
        private static void To21_2_1()
        {
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                if (!abbrDesc.IsNullOrEmpty())
                {//Customers can add their own abbreviations, so skip abbreviation if not blank.
                    continue;
                }
            ...
            Db.NonQ(command);
            if (!IndexExists("fee", "FeeSched,CodeNum,ClinicNum,ProvNum"))
            {//may have been added manually
            ...
            {//If the database has no allowed addresses then we don't need to insert any into the new table.
        ...
        private static void To22_1_1()
        {
            ...
            for (int i = 0; i < listUserGroupNums.Count; i++)
            {
                ...
                if (listIntersectingDisplayReportNums.Count == listDisplayReportNums.Count)
                {//Has all permissions.
                ...
                {//Has some or no permissions, remove FKey of 0 regardless.
            ...
            for (int i = 0; i < listUserGroupNums.Count; i++)
            {
                ...
                if (listIntersectingDashboardWidgetNums.Count == listDashboardWidgetNums.Count)
                {//Has all permissions.
            ...
            for (int i = 0; i < listUserGroupNums.Count; i++)
            {
                ...
                if (listIntersectingAdjustmentTypeNums.Count == listAdjustmentTypeNums.Count)
                {//Has all permissions denied.
        ...
        private static void To22_2_56()
        {
            ...
            DataTable table = Db.GetTable(command);
            for (int i = 0; i < table.Rows.Count; i++)
            { //Can have multiple duplicate Mobile Provider View Items so need to clean each one individually
            ...
            table = Db.GetTable(command);
            for (int i = 0; i < table.Rows.Count; i++)
            { //Can have multiple duplicate Mobile Operatory View Items so need to clean each one individually
                long apptViewNum = PIn.Long(table.Rows[i]["ApptViewNum"].ToString());
        ...
        private static void To22_3_22()
        {
            ...
            if (table.Rows.Count > 0 && !string.IsNullOrWhiteSpace(PIn.String(table.Rows[0]["RefreshToken"].ToString())))
            {
                ...
                if (string.IsNullOrWhiteSpace(decryptedCache))
                { //old encryption is in place, clear the information out
                    command = "UPDATE emailaddress SET AccessToken='' WHERE AuthenticationType=2";//AuthenticationType.Microsoft=2
                    Db.NonQ(command);
                    command = "UPDATE emailaddress SET RefreshToken='' WHERE AuthenticationType=2";//AuthenticationType.Microsoft=2
                    Db.NonQ(command);
        ...
        private static void To22_4_1()
        {
            ...
            if (valueString == 0)
            {//If Pref "RecurringChargesInactivateDeclinedCards" is 0 (unknown), it will be updated to T below.  Per Nathan, any customer that receives this change should also have the pref "RecurringChargesShowInactive" updated to true.
