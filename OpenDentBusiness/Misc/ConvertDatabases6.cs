...
Please call our office at your earliest convenience to schedule an appointment.";
        ...
        private static void To18_3_1()
        {
            ...
            if (defNum == 0)
            {//We didn't find a definition named 'Sales Tax'.
        ...
        private static void To19_1_1()
        {
            ...
            foreach (KeyValuePair<long, DataRow> kvp in dictProvs)
            {
                ...
                if (listDefaultProvClinicsProvNums.Contains(kvp.Key))
                {
        ...
        private static void To19_2_1()
        {
            ...
            table = Db.GetTable(command);
            if (table.Rows.Count == 0)
            { //The cash payment type does not exist, add it
            ...
            { //The cash payment type already exists, get the existing DefNum
            ...
            {
                if (dictClinExclAdjTypes.ContainsKey(0))
                {//if HQ clinic has props set, use them for any clinic with no props
                    command = "SELECT ClinicNum FROM clinic";
                ...
                command = @"SELECT tsitranslog.TsiTransLogNum
                    ...
                    + $@"AND ({string.Join(" OR ", dictClinExclAdjTypes
        ...
        private static void To19_2_2()
        {
            ...
            {//if 1 or 2, prepayments to providers were always allowed. 
        ...
        {
            if (!LargeTableHelper.ColumnExists(LargeTableHelper.GetCurrentDatabase(), "claimproc", "IsTransfer"))
            {
        ...
        {//This method did not truly get added until 19.3.52.
            string command;
        ...
        private static void To20_3_9()
        {
            ...
            foreach (string internalAlertCategory in new string[] { "OdAllTypes", "eServices" })
            {
                command = @"SELECT alertcategory.AlertCategoryNum,alertcategorylink.AlertType 
					...
					WHERE InternalName='" + POut.String(internalAlertCategory) + "' AND IsHQCategory=1";
                table = Db.GetTable(command);
                if (table.Rows.Count > 0 && !table.Rows.AsEnumerable<DataRow>().Any(x => PIn.Int(x["AlertType"].ToString()) == 24))
                {//AlertType.EConnectorMySqlTime=24
...
If you are not currently utilizing Web Forms, you can disregard this message.";
        ...
        private static void To20_4_1()
        {
            ...
            foreach (string internalAlertCategory in new string[] { "OdAllTypes", "eServices" })
            {
                command = @"SELECT alertcategory.AlertCategoryNum,alertcategorylink.AlertType 
					...
					WHERE InternalName='" + POut.String(internalAlertCategory) + "' AND IsHQCategory=1";
                table = Db.GetTable(command);
                if (table.Rows.Count > 0 && !table.Rows.AsEnumerable<DataRow>().Any(x => PIn.Int(x["AlertType"].ToString()) == 27))
                {//AlertType.EmailSecure=27
                 //Alert category exists and there aren't already any links to the EConnectorMySqlTime AlertType
                    long alertCategoryNum = PIn.Long(table.Rows[0]["AlertCategoryNum"].ToString());
        ...
        {
            string command;
            if (!LargeTableHelper.IndexExists("phonenumber", "PhoneNumberDigits"))
            {//index may have been added in version 20.3.44
                command = "ALTER TABLE phonenumber ADD INDEX (PhoneNumberDigits)";
                Db.NonQ(command);
            }
        }
        ...
        private static void To20_4_28()
        {
            ...
            foreach (string internalAlertCategory in new string[] { "OdAllTypes", "eServices" })
            {
                command = @"SELECT alertcategory.AlertCategoryNum,alertcategorylink.AlertType 
					...
					WHERE InternalName='" + POut.String(internalAlertCategory) + "' AND IsHQCategory=1";
                table = Db.GetTable(command);
                if (table.Rows.Count > 0 && !table.Rows.AsEnumerable<DataRow>().Any(x => PIn.Int(x["AlertType"].ToString()) == 28))
                {//AlertType.WebSchedExistingPatApptCreated=28
