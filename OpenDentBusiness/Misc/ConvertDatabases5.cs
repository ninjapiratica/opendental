...
namespace OpenDentBusiness
{
    public partial class ConvertDatabases
    {
        ...
        {
            if (DataConnection.DBtype != DatabaseType.MySql || !CultureInfo.CurrentCulture.Name.EndsWith("CA"))
            {//Canada only.
                return;
            }
...
Go to eServices -> Web Sched to activate.";
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    if (!LargeTableHelper.IndexExists("statement", "SuperFamily,Mode_,DateSent"))
                    {//add composite index if it doesn't already exist
                        ...
                        Db.NonQ(command);
                        if (LargeTableHelper.IndexExists("statement", "SuperFamily"))
                        {//drop redundant index once composite index is successfully added and only if it exists
        ...
        private static void To17_1_8()
        {
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    if (!LargeTableHelper.IndexExists("procedurelog", "PatNum,ProcStatus,CodeNum,ProcDate"))
                    {//add composite index if it doesn't already exist
                        command = "ALTER TABLE procedurelog ADD INDEX PatStatusCodeDate (PatNum,ProcStatus,CodeNum,ProcDate)";
                        Db.NonQ(command);
                        if (LargeTableHelper.IndexExists("procedurelog", "PatNum"))
                        {//drop redundant index once composite index is successfully added and only if it exists
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    ...
                    {//add composite index if it doesn't already exist
                        ...
                        Db.NonQ(command);
                        if (LargeTableHelper.IndexExists("appointment", "AptStatus"))
                        {//drop redundant index once composite index is successfully added and only if it exists
        ...
        private static void To17_1_10()
        {
            ...
            {//They have not set the status yet.
                long defNum = 0;
                ...
                if (defNum == 0)
                {//We didn't find a definition named 'Texted'.
        ...
        private static void To17_1_16()
        {
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    if (!LargeTableHelper.IndexExists("patient", "FeeSched"))
        ...
        private static void To17_2_1()
        {
            ...
            if (DataConnection.DBtype == DatabaseType.MySql)
            { //allow future debits, default to current behavior. 0 by default
        ...
        private static void To17_2_13()
        {
            ...
            if (Db.GetScalar(command) == "0")
            {//Was suppose to be in 17.2.12 but was put in 17.2.8, so fixing in 17.2.13
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    command = "INSERT INTO preference (PrefName,ValueString) VALUES('PlannedApptDaysFuture','0')";
                    Db.NonQ(command);
...
No Action Required in many cases, check your new patient Web Sched on your web site to see behavior. See online manual for details.";
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    if (!LargeTableHelper.IndexExists("fee", "ClinicNum"))
                    {//add index if it doesn't already exist
        ...
        private static void To17_3_8()
        {
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    if (!LargeTableHelper.IndexExists("claimproc", "DateSuppReceived"))
                    {//add index if it doesn't already exist
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    if (!LargeTableHelper.IndexExists("procedurelog", "DateComplete"))
                    {//add index if it doesn't already exist
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    if (!LargeTableHelper.IndexExists("patient", "SecDateEntry"))
                    {//add index if it doesn't already exist
        ...
        private static void To17_3_32()
        {
            ...
            try
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    if (!LargeTableHelper.IndexExists("procedurelog", "AptNum,CodeNum,ProcStatus,IsCpoe,ProvNum"))
                    {//add composite index if it doesn't already exist
                        command = "ALTER TABLE procedurelog ADD INDEX RadiologyProcs (AptNum,CodeNum,ProcStatus,IsCpoe,ProvNum)";
                        Db.NonQ(command);
                        if (LargeTableHelper.IndexExists("procedurelog", "AptNum"))
                        {//drop redundant index once composite index is successfully added and only if it exists
        ...
        private static void To17_4_12()
        {
            ...
            if (newCropAcctID == "")
            {//If not using NewCrop, set the eRx option to DoseSpot.
        ...
        private static void To17_4_21()
        {
            ...
            if (Db.GetTable(command).Rows.Count == 0)
            {//This preference may have been inserted in 17.3.63.
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    command = "INSERT INTO preference (PrefName,ValueString) VALUES('ApptConfirmAutoSignedUp','0')";//false by default, eConnector will change it if needed
                    Db.NonQ(command);
        ...
        private static void To17_4_30()
        {
            ...
            if (Db.GetCount(command) == "0")
            {//could've been added in 17.3.59
             //Add AgingReportShowAgePatPayplanPayments preference to make the age patient payments to payment plans checkbox visible.
             //For a specific customer, no UI, defaults to false, enable via query only.
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
        ...
        private static void To17_4_36()
        {
            ...
            foreach (DataRow row in tableWikiListAll.Rows)
            {
                ...
                foreach (Match m in matchCollection)
                {
                    ...
                    {//page title exist, replace with WikiPageNum instead of PageTitle
                    ...
                    {//page title does not exist. Replace with [[0]]
        ...
        private static void To17_4_39()
        {
            ...
            if (dateSecurityLock != DateTime.MinValue || securityLockDays != 0)
            {//No DB changes needed if we'd just set the grouppermission to default value.
             //Check to see if the security lock doesn't include admins.
                ...
                if (listSecurityAdminUserGroupNums.Count > 0)
                {//Exclude admins
        ...
        private static void To17_4_52()
        {
            ...
            {//oracle datatype is already clob which goes up to 4GB
            }
        }
        ...
        private static void To18_1_1()
        {
            ...
            if (methodName == "")
            {//didn't find row in table, insert
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
            ...
            if (DataConnection.DBtype == DatabaseType.MySql)
            {
                if (!LargeTableHelper.IndexExists("clockevent", "EmployeeNum"))
                {//add index if it doesn't already exist
        ...
        private static void To18_1_56()
        {
            ...
            table = Db.GetTable(command);
            if (table.Rows.Count == 0)
            {//DateTimeOriginal column was not correctly added.
                command = "ALTER TABLE task ADD DateTimeOriginal datetime NOT NULL DEFAULT '0001-01-01 00:00:00'";
                Db.NonQ(command);
            }
            #endregion
            ...
            table = Db.GetTable(command);
            if (table.Rows.Count == 0)
            {//DocNum column was not correctly added.
                command = "ALTER TABLE sheet ADD DocNum bigint NOT NULL";
                Db.NonQ(command);
                command = "ALTER TABLE sheet ADD INDEX (DocNum)";
                Db.NonQ(command);
...
@@@body@@@
        ...
        private static void To18_2_4()
        {
            ...
            if (propertyNum == 0)
            {//ProgramProperty does not exist
                command = "INSERT INTO programproperty (ProgramNum,PropertyDesc,PropertyValue"
                     + ") VALUES("
                     + "'" + POut.Long(programNum) + "', "
                     + "'Birthdate format (default MM/dd/yyyy)', "
        ...
        private static void To18_2_38()
        {
            ...
            if (table.Rows.Count > 0 && PIn.Bool(table.Rows[0]["Enabled"].ToString()))
            {
                ...
                if (isNewCrop)
                {//Only update rows if the office has eRx enabled and is using NewCrop.
