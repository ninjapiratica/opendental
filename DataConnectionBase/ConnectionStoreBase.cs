...
namespace DataConnectionBase
{
    ...
    public class ConnectionStoreBase
    {
        ...
        private static Dictionary<ConnectionNames, CentralConnectionBase> GetDictCentConnSafe()
        {
            ...
            actionTryInit(new Func<Dictionary<ConnectionNames, CentralConnectionBase>>(() =>
            ...
            actionTryInit(new Func<Dictionary<ConnectionNames, CentralConnectionBase>>(() =>
            { //Dental Office Read-Only Server (via prefs).
                if (HasSingleEntry(ConnectionNames.DentalOfficeReadOnlyServer))
                { //Only try to add this value if it doesn't already exist.
                    return null;
                }
            ...
            actionTryInit(new Func<Dictionary<ConnectionNames, CentralConnectionBase>>(() =>
            { //TriageHQ (if implemented).
                if (HasSingleEntry(ConnectionNames.TriageHQ))
                { //Only try to add this value if it doesn't already exist.
                    return null;
                }
