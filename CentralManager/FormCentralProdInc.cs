    ...
    public partial class FormCentralProdInc : Form
    {
        ...
        private void RunDaily()
        {
            ...
            {//Per connection
                ...
                for (int j = 0; j < tableDailyProdForConn.Rows.Count; j++)
                {//Go through the rows
                    DataRow connRow = tableDailyProdForConn.Rows[j];
                    bool isFound = false;
                    for (int k = 0; k < connectionTotals.Rows.Count; k++)
                    {//Attempt to find an existing totals row that matches.
                        ...
                        if (hasClinicMatch && reportDataCur.ConnectionString == connTotalsRow["Connection"].ToString())
                        {//Totals row already exists. Add to it.
                    ...
                    if (!isFound)
                    {//Totals row for this connection and clinic combination doesn't exist yet.
                        DataRow totalRow = connectionTotals.NewRow();
                        totalRow[0] = reportDataCur.ConnectionString;
                        if (hasClinicBreakdown)
                        { //Only add clinic if clinicbreakdown is enabled for this connection
            ...
            if (listReportData.Any(x => x.HasClinicBreakdown))
            { //Only add clinic column if at least one connection has HasClinicBreakdown
        ...
        private void RunMonthly()
        {
            ...
            for (int i = 0; i < listReportData.Count; i++)
            {
                ...
                if (dt.Rows.Count > 0 && dtTot.Rows.Count > 0)
                {
                    ...
                    if (listReportData[i].HasClinicBreakdown)
                    { //Connection has clinic breakdown
