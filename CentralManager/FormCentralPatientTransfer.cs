...
namespace CentralManager
{
    public partial class FormCentralPatientTransfer : Form
    {
        ...
        private List<DataRow> _listPatientDataRows = new List<DataRow>();
        private List<CentralConnection> _listConnectionsToTransferTo = new List<CentralConnection>();
        private List<Sheet> _listSheetsForSelectedPats;

        public FormCentralPatientTransfer(CentralConnection sourceConn)
        {
        ...
        private void FillDatabases()
        {
            ...
            gridDatabasesTo.Columns.Add(new GridColumn(Lan.g("TableDatabases", "Databases"), 150));
            gridDatabasesTo.Columns.Add(new GridColumn(Lan.g("TableDatabases", "Note"), 200));
            gridDatabasesTo.Columns.Add(new GridColumn("Status", 30, HorizontalAlignment.Center));
        ...
        private void FillPatients()
        {
            ...
            gridPatients.Columns.Add(new GridColumn(Lan.g("TablePatients", "PatNum"), 140));
            gridPatients.Columns.Add(new GridColumn(Lan.g("TablePatients", "LName"), 140));
            gridPatients.Columns.Add(new GridColumn(Lan.g("TablePatients", "FName"), 140, true));
        ...
        private void butAddDatabases_Click(object sender, EventArgs e)
        {
            ...
            {
                List<CentralConnection> listConnsSelected = FormCC.ListConnsSelected;
                listConnsSelected.RemoveAll(x => x.CentralConnectionNum == _sourceConnection.CentralConnectionNum);
                _listConnectionsToTransferTo.AddRange(listConnsSelected.FindAll(x => !_listConnectionsToTransferTo.Select(y => y.CentralConnectionNum).Contains(x.CentralConnectionNum)));
                FillDatabases();
            }
        }
        ...
        private void RunTransfer()
        {
            ...
            odThread.AddExceptionHandler((ex) => stringFailedConns = ex.Message);
            odThread.GroupName = "FetchSheets";
            odThread.Start();
            ODThread.JoinThreadsByGroupName(Timeout.Infinite, "FetchSheets");
            if (stringFailedConns != "")
            {
                MsgBox.Show($"Unexpected error when getting patient transfer sheets for connection '{CentralConnections.GetConnectionString(_sourceConnection)}':\r\n{stringFailedConns}");
                return;
            }
            ...
            else
            {
                ...
                ODThread.RunParallel(listActions, onException: (ex) => stringFailedConns += $"Critical Error: {ex.Message}");
            }
            if (stringFailedConns != "")
            {
